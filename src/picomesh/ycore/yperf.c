 ...
 *
 * yconfig_get peels path segments on a miss, so a deep dotted lookup can
...
struct perf_child_lookup {
    const char *key;
    const struct yconfig_node *found;
};

static int perf_child_lookup_cb(const char *key, const struct yconfig_node *val, void *ud)
{
...
static const struct yconfig_node *perf_node_child(const struct yconfig_node *map, const char *key)
{
    if (!map || yconfig_node_kind(map) != YCONFIG_MAP) return NULL;
    struct perf_child_lookup look = {.key = key, .found = NULL};
    yconfig_node_for_each(map, perf_child_lookup_cb, &look);
    return look.found;
}

static int perf_config_enabled(const struct yconfig_node *perf_node)
{
    return perf_node && yconfig_node_as_bool(perf_node_child(perf_node, "enabled"), 0);
}
...
struct yperf {
    ...
    int log;
    struct yloop *loop;
    struct yloop_timer *timer; /* NULL when log == 0 */
};
...
struct yperf_ptr_result yperf_create(const struct yconfig_node *perf_node,
                                     struct yloop *loop, const char *label)
{
    if (!perf_config_enabled(perf_node)) return PICOMESH_OK(yperf_ptr, NULL);

    const char *tag = (label && *label) ? label : "process";

    const char *mode = yconfig_node_as_string(perf_node_child(perf_node, "mode"), "counters");
    ...
    const struct yconfig_node *events = perf_node_child(perf_node, "events");
    if (!events || yconfig_node_kind(events) != YCONFIG_LIST || yconfig_node_size(events) == 0)
    ...
    int interval_ms = (int)yconfig_node_as_int(perf_node_child(perf_node, "interval_ms"), 1000);
    if (interval_ms < 10) interval_ms = 10;
    int do_log = yconfig_node_as_bool(perf_node_child(perf_node, "log"), 1);
    int exclude_kernel = yconfig_node_as_bool(perf_node_child(perf_node, "exclude_kernel"), 0);
    ...
    size_t want = yconfig_node_size(events);
    for (size_t i = 0; i < want; ++i) {
        ...
        const char *ename = yconfig_node_as_string(yconfig_node_at(events, i), NULL);
    ...
    if (perf->log) {
        struct yloop_timer_ptr_result tr =
            yloop_timer_start(loop, perf->interval_ms, yperf_report_tick, perf);
