...
struct exec *exec_create(int n, void *(*shard_init)(void *),
                         void (*shard_free)(void *), void *ud) {
  if (n < 1)
    n = 1;
...
void exec_submit(struct exec *e, struct loop *loop, uint32_t key,
  ...
  struct exec_work w = {
      .fn = fn, .arg = arg, .loop = loop, .coro = self, .next = NULL};
  uv_mutex_lock(&s->mu);
  if (s->tail)
    s->tail->next = &w;
  else
...
int exec_shard_count(const struct exec *e) { return e ? e->n : 0; }
