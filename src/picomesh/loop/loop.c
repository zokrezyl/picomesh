...
#ifndef SO_REUSEPORT
#define SO_REUSEPORT                                                           \
  15 /* Linux value; header may hide it without _GNU_SOURCE */
#endif
...
static void ring_append(struct loop_stream *stream, const uint8_t *data,
                        size_t len) {
  if (!stream->rbuf) {
...
static void on_read(uv_stream_t *uv_stream, ssize_t nread,
                    const uv_buf_t *buf) {
...
struct picomesh_size_result loop_read_some(struct loop_stream *stream,
                                           void *buf, size_t cap) {
  if (!stream || !buf)
    return PICOMESH_ERR(picomesh_size, "loop_read_some: bad args");
  if (cap == 0)
    return PICOMESH_OK(picomesh_size, 0); /* zero-length read is a no-op */
...
struct picomesh_size_result loop_read(struct loop_stream *stream, void *buf,
                                      size_t n) {
  if (!stream || !buf)
    return PICOMESH_ERR(picomesh_size, "loop_read: bad args");
  if (n == 0)
    return PICOMESH_OK(picomesh_size, 0); /* zero-length read is a no-op */
  stream->coro = picomesh_coro_current(); /* resume the current caller (see
                                             loop_read_some) */
...
struct picomesh_size_result loop_write(struct loop_stream *stream,
                                       const void *buf, size_t n) {
  if (!stream || !buf)
    return PICOMESH_ERR(picomesh_size, "loop_write: bad args");
  if (n == 0)
    return PICOMESH_OK(picomesh_size, 0); /* zero-length write is a no-op */
  struct write_req *write_request = calloc(1, sizeof(*write_request));
  if (!write_request)
    return PICOMESH_ERR(picomesh_size, "loop_write: write_req alloc failed");
  ...
  if (!write_request->data) {
    free(write_request);
    return PICOMESH_ERR(picomesh_size, "loop_write: payload alloc failed");
  }
  memcpy(write_request->data, buf, n);
  uv_buf_t uv_buf = uv_buf_init(write_request->data, (unsigned)n);
  int rc = uv_write(&write_request->req, (uv_stream_t *)&stream->tcp, &uv_buf,
                    1, on_write);
  if (rc < 0) {
    free(write_request->data);
    free(write_request);
  ...
  picomesh_coro_yield();
  if (status != 0)
    return PICOMESH_ERR(picomesh_size, uv_strerror(status));
...
static void on_handle_close(uv_handle_t *handle) {
  ...
  if (stream->owns_coro && stream->coro &&
      picomesh_coro_is_finished(stream->coro)) {
...
static void serve_trampoline(void *arg) {
  ...
  struct serve_closure *closure =
      (struct serve_closure *)((char *)stream + sizeof(*stream));
...
static void on_connection(uv_stream_t *server, int status) {
  ...
  struct loop_stream *stream =
      calloc(1, sizeof(*stream) + sizeof(struct serve_closure));
  if (!stream)
    return;
  ...
  if (PICOMESH_IS_ERR(spawn_res)) {
    picomesh_error_print(stderr, "loop: serve coro spawn failed",
                         spawn_res.error);
...
void loop_post_resume(struct loop *loop, struct picomesh_coro *coro) {
  if (!loop || !loop->resume_ready || !coro)
    return;
  struct resume_node *node = calloc(1, sizeof(*node));
  if (!node)
    return; /* OOM: the coro will not be resumed — fatal-ish, but we
             * cannot recover here; the offloading path treats a
             * never-resumed coro as a hung request. */
  node->coro = coro;
  uv_mutex_lock(&loop->resume_mu);
  if (loop->resume_tail)
    loop->resume_tail->next = node;
  else
    loop->resume_head = node;
...
struct loop_ptr_result loop_create(void) {
  ...
  if (!loop)
    return PICOMESH_ERR(loop_ptr, "loop_create: calloc failed");
...
struct picomesh_void_result loop_run(struct loop *loop) {
  if (!loop)
    return PICOMESH_ERR(picomesh_void, "loop_run: NULL loop");
...
static void on_sleep_closed(uv_handle_t *handle) {
  free(handle->data); /* the heap sleep_state, once uv is fully done with the
                         timer */
}
...
struct loop_timer_ptr_result loop_timer_start(struct loop *loop,
                                              unsigned int interval_ms,
                                              loop_timer_cb cb, void *ud) {
  if (!loop)
    return PICOMESH_ERR(loop_timer_ptr, "loop_timer_start: NULL loop");
  if (!cb)
    return PICOMESH_ERR(loop_timer_ptr, "loop_timer_start: NULL callback");
  struct loop_timer *timer = calloc(1, sizeof(*timer));
  if (!timer)
    return PICOMESH_ERR(loop_timer_ptr, "loop_timer_start: calloc failed");
  timer->cb = cb;
  timer->ud = ud;
...
static void on_timer_closed(uv_handle_t *handle) { free(handle->data); }

void loop_timer_stop(struct loop_timer *timer) {
  if (!timer)
    return;
  timer->cb =
      NULL; /* belt-and-braces: no callback even if a tick is mid-flight */
...
static void on_proc_exit(uv_process_t *process, int64_t exit_status,
                         int term_signal) {
...
struct picomesh_int_result loop_spawn(struct loop *loop, const char *file,
                                      char *const argv[],
                                      loop_process_exit_cb on_exit, void *ud) {
  if (!loop || !file || !argv)
    return PICOMESH_ERR(picomesh_int, "loop_spawn: bad args");

  struct loop_process *self = calloc(1, sizeof(*self));
  if (!self)
    return PICOMESH_ERR(picomesh_int, "loop_spawn: calloc failed");
  self->cb = on_exit;
  self->ud = ud;
  self->proc.data = self;
  ...
  if (rc < 0) {
    char msg[256];
    snprintf(msg, sizeof(msg), "loop_spawn: uv_spawn failed: %s",
             uv_strerror(rc));
    free(self);
...
struct picomesh_void_result loop_kill(struct loop *loop, int pid, int signum) {
  if (!loop || pid <= 0)
    return PICOMESH_ERR(picomesh_void, "loop_kill: bad args");
  int rc = uv_kill(pid, signum);
  if (rc < 0) {
    char msg[256];
    snprintf(msg, sizeof(msg), "loop_kill: uv_kill(pid=%d, sig=%d) failed: %s",
             pid, signum, uv_strerror(rc));
  ...
  if (status == 0 && res) {
    ...
    for (struct addrinfo *addr = res; addr; addr = addr->ai_next) {
      if (addr->ai_family == AF_INET &&
          addr->ai_addrlen >= sizeof(resolve->addr)) {
...
struct loop_stream_ptr_result loop_connect_tcp(struct loop *loop,
  ...
  if (!stream)
    return PICOMESH_ERR(loop_stream_ptr, "loop_connect_tcp: calloc failed");
  stream->owner = loop;
  stream->coro = self;
  ...
  if (rc < 0) {
    free(stream);
    return PICOMESH_ERR(loop_stream_ptr,
                        "loop_connect_tcp: uv_tcp_init failed");
  }
  stream->tcp.data = stream;
  ...
  rc = uv_tcp_connect(&connect.req, &stream->tcp,
                      (const struct sockaddr *)&addr, on_connect);
  if (rc < 0) {
    uv_close((uv_handle_t *)&stream->tcp, on_handle_close);
    return PICOMESH_ERR(loop_stream_ptr,
                        "loop_connect_tcp: uv_tcp_connect dispatch failed");
  }
  picomesh_coro_yield();
  if (connect.status < 0) {
    uv_close((uv_handle_t *)&stream->tcp, on_handle_close);
  ...
  if (rc < 0) {
    uv_close((uv_handle_t *)&stream->tcp, on_handle_close);
    return PICOMESH_ERR(loop_stream_ptr,
                        "loop_connect_tcp: uv_read_start failed");
  }
...
struct picomesh_void_result loop_run_blocking(struct loop *loop,
  ...
  blocking.req.data =
      &blocking; /* lives on the suspended coro's stack until resume */
  int rc = uv_queue_work(&loop->loop, &blocking.req, on_blocking_work,
                         on_blocking_done);
  if (rc < 0) {
    ...
    return PICOMESH_ERR(
        picomesh_void,
        "loop_run_blocking: uv_queue_work failed (worker pool dispatch)");
  }
  picomesh_coro_yield();
  return PICOMESH_OK_VOID();
}

struct picomesh_void_result loop_listen_tcp(struct loop *loop, const char *host,
