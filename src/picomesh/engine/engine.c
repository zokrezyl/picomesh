...
void picomesh_engine_set_resolved_remote(struct picomesh_engine *engine,
  ...
  for (size_t i = 0; i < engine->resolved_count; ++i) {
    if (engine->resolved[i].used &&
        strcmp(engine->resolved[i].name, name) == 0) {
      snprintf(engine->resolved[i].host, sizeof(engine->resolved[i].host), "%s",
               host);
...
int picomesh_engine_get_resolved_remote(struct picomesh_engine *engine,
  ...
  for (size_t i = 0; i < engine->resolved_count; ++i) {
    if (engine->resolved[i].used &&
      ...
      if (host_out && host_cap)
        snprintf(host_out, host_cap, "%s", engine->resolved[i].host);
      if (port_out)
        *port_out = engine->resolved[i].port;
      return 1;
    }
  }
  return 0;
...
static struct picomesh_worker *
engine_current_worker(struct picomesh_engine *engine) {
...
void picomesh_engine_worker_set_security(struct picomesh_engine *engine,
                                         void *ptr, void (*free_fn)(void *)) {
  if (!engine)
    return;
  struct picomesh_worker *worker = engine_current_worker(engine);
  if (!worker)
    return;
  if (worker->security_free)
    worker->security_free(worker->security);
  worker->security = ptr;
...
static void apply_cli_env(const struct argv_chain *cli) {
  ...
  const char *envs[64];
  size_t count =
      argv_get_kv_list(cli, "env", envs, sizeof(envs) / sizeof(envs[0]));
...
picomesh_engine_create(const struct picomesh_engine_args *args) {
  ...
  if (!engine)
    return PICOMESH_ERR(picomesh_engine_ptr,
                        "picomesh_engine_create: calloc failed");
  ...
  if (engine->cli) {
    cli_override_count =
        argv_get_kv_list(engine->cli, "config", cli_overrides,
  ...
  if (PICOMESH_IS_ERR(config_res)) {
    free(engine);
    return PICOMESH_ERR(picomesh_engine_ptr,
                        "picomesh_engine_create: config_create failed",
                        config_res);
  }
  ...
  if (engine->cli)
    self_name = argv_get_string(engine->cli, "name", NULL);
  if (self_name && *self_name) {
    char path[256];
    snprintf(path, sizeof(path), "mesh.services.%s.config", self_name);
    struct picomesh_void_result promote_res =
        config_promote_subtree(engine->config, path);
  ...
  if (!engine->workers) {
    ...
    free(engine);
    return PICOMESH_ERR(picomesh_engine_ptr,
                        "picomesh_engine_create: calloc(workers) failed");
  }
  ...
  if (PICOMESH_IS_ERR(loop_res)) {
    ...
    free(engine);
    return PICOMESH_ERR(picomesh_engine_ptr,
                        "picomesh_engine_create: loop_create failed", loop_res);
  }
...
void picomesh_engine_destroy(struct picomesh_engine *engine) {
  ...
  for (size_t i = 0; i < engine->worker_count; ++i) {
    if (engine->workers[i].security_free)
      engine->workers[i].security_free(engine->workers[i].security);
    ...
    if (engine->workers[i].flush_timer)
      loop_timer_stop(engine->workers[i].flush_timer);
...
struct rpc_async_client {
  ...
  struct peer_channel
      *owner; /* channel whose proxy cache we flush on reconnect */
};
...
static int rpc_async_drain(struct loop_stream *stream, uint32_t remaining) {
  ...
  while (remaining) {
    ...
    if (PICOMESH_IS_ERR(drain_read)) {
      picomesh_error_destroy(drain_read.error);
      return 0;
    }
    if (drain_read.value != chunk)
      return 0;
...
static void rpc_async_deliver(struct rpc_async_client *client,
                              struct pending_call *call, size_t resp_len) {
  call->resp_len = resp_len;
  call->done = 1;
  HASH_DEL(client->pending, call);
  if (call->waiting)
    picomesh_coro_resume(call->coro);
}
...
static void rpc_async_fail_all(struct rpc_async_client *client) {
  ...
  while (dead) {
    struct pending_call *next =
        dead->next_dead; /* capture before resume frees it */
...
static void rpc_async_reader_entry(void *arg) {
  ...
  for (;;) {
    ...
    if (PICOMESH_IS_ERR(reqid_read)) {
      picomesh_error_destroy(reqid_read.error);
      break;
    }
    if (reqid_read.value != 4)
      break;
    struct picomesh_size_result resplen_read = loop_read(stream, &resp_len, 4);
    if (PICOMESH_IS_ERR(resplen_read)) {
      picomesh_error_destroy(resplen_read.error);
      break;
    }
    if (resplen_read.value != 4)
      break;
    ...
    if (!call) {
      ...
      if (req_id != 0)
        ywarn("yrpc-reader: response for unknown req_id=%u, draining %u",
              req_id, resp_len);
      if (!rpc_async_drain(stream, resp_len))
        break;
      continue;
    }
    ...
    if (resp_len) {
      struct picomesh_size_result resp_read =
          loop_read(stream, call->resp, resp_len);
      if (PICOMESH_IS_ERR(resp_read)) {
        picomesh_error_destroy(resp_read.error);
        break;
      }
      if (resp_read.value != resp_len)
        break;
    }
...
rpc_async_ensure_connected(struct rpc_async_client *client) {
  if (client->stream)
    return PICOMESH_OK(picomesh_int, 1);

  if (client->connecting) {
    struct coro_waiter waiter = {.coro = picomesh_coro_current(), .next = NULL};
    if (client->connect_wq_tail)
      client->connect_wq_tail->next = &waiter;
    else
      client->connect_wq_head = &waiter;
  ...
  int ok = 0;
  struct loop_stream_ptr_result stream_res =
  ...
  if (PICOMESH_IS_ERR(stream_res)) {
    ...
    if (PICOMESH_IS_ERR(reader_res)) {
      picomesh_error_print(stderr, "rpc async: reader coro spawn failed",
                           reader_res.error);
  ...
  if (ok && client->owner)
    peer_channel_flush_proxy_cache(client->owner);

  /* Start the reader (it runs to its first loop_read and parks). */
  if (ok)
    picomesh_coro_resume(client->reader);
...
static size_t rpc_async_client_call(void *vclient, enum rpc_op op, uint32_t id,
  ...
  if (PICOMESH_IS_ERR(connected)) {
    picomesh_error_destroy(connected.error);
    return 0;
  }
  if (!connected.value)
    return 0;
  ...
  if (req_id == 0)
    req_id = ++client->next_req_id; /* keep 0 reserved */
  ...
  struct picomesh_size_result write_res =
      loop_write(client->stream, frame, frame_len);
  free(frame);
...
static struct rpc_async_client *
rpc_async_client_create(struct loop *loop, const char *host, int port) {
...
static struct peer_channel *open_async_session(struct loop *loop,
  ...
  struct peer_channel *channel =
      peer_channel_create(-1); /* no fd; async carries IO */
  if (!channel) {
    rpc_async_client_destroy(client);
    return NULL;
  }
  client->owner =
      channel; /* so reconnects can flush the channel's proxy cache */
  peer_channel_set_async(channel, client, rpc_async_client_call,
                         rpc_async_client_destroy);
...
struct msgpack_async_client {
  ...
  struct coro_waiter *wq_head,
      *wq_tail; /* callers parked behind the in-flight one */
};
...
static void msgpack_async_gate_enter(struct msgpack_async_client *client) {
  ...
  struct coro_waiter waiter = {.coro = picomesh_coro_current(), .next = NULL};
  if (client->wq_tail)
    client->wq_tail->next = &waiter;
  else
    client->wq_head = &waiter;
 ...
 */
static struct picomesh_int_result
msgpack_async_roundtrip(struct msgpack_async_client *client, const uint8_t *env,
                        size_t env_len, uint8_t **resp_out,
                        size_t *resp_len_out, char *err, size_t err_cap) {
  if (!client->stream) {
    struct loop_stream_ptr_result connect_res =
        loop_connect_tcp(client->loop, client->host, client->port);
    if (PICOMESH_IS_ERR(connect_res)) {
      snprintf(err, err_cap, "msgpack async: connect %s:%d failed",
               client->host, client->port);
      picomesh_error_print(stderr, "msgpack async: connect failed",
                           connect_res.error);
  ...
  struct picomesh_size_result lenbuf_write =
      loop_write(client->stream, lenbuf, 4);
  ...
  if (PICOMESH_IS_ERR(lenbuf_write) || PICOMESH_IS_ERR(env_write)) {
    if (PICOMESH_IS_ERR(lenbuf_write))
      picomesh_error_destroy(lenbuf_write.error);
    if (PICOMESH_IS_ERR(env_write))
      picomesh_error_destroy(env_write.error);
  ...
  struct picomesh_size_result resplen_read =
      loop_read(client->stream, resp_lenbuf, 4);
  ...
  struct picomesh_size_result resp_read =
      loop_read(client->stream, resp, resp_len);
 ...
 * chain rendered. */
PICOMESH_EXTERNAL_CALLBACK
static int msgpack_async_io(void *ctx, const uint8_t *env, size_t env_len,
                            uint8_t **resp_out, size_t *resp_len_out, char *err,
                            size_t err_cap) {
  struct msgpack_async_client *client = ctx;
  *resp_out = NULL;
  *resp_len_out = 0;
  msgpack_async_gate_enter(client);
  struct picomesh_int_result roundtrip_res = msgpack_async_roundtrip(
      client, env, env_len, resp_out, resp_len_out, err, err_cap);
  msgpack_async_gate_leave(client);
  if (PICOMESH_IS_ERR(roundtrip_res)) {
    picomesh_error_print(stderr, "msgpack async io: roundtrip",
                         roundtrip_res.error);
...
static struct peer_channel *
open_async_msgpack_session(struct loop *loop, const char *host, int port) {
  ...
  struct peer_channel *channel =
      peer_channel_create_msgpack(-1); /* fd-less; async carries IO */
  if (!channel) {
    free(client->host);
    free(client);
    return NULL;
  }
  peer_channel_set_msgpack_async(channel, client, msgpack_async_io,
                                 msgpack_async_client_destroy);
  return channel;
}

/* Pick the outbound transport for a config remote. */
static struct peer_channel *open_remote_session(struct loop *loop,
                                                const char *host, int port,
...
picomesh_engine_add_remote_transport(struct picomesh_engine *engine,
                                     const char *name, const char *host,
                                     int port, const char *transport) {
  if (!engine || !name)
    return PICOMESH_ERR(picomesh_void, "add_remote: bad args");
  if (!host)
    host = "127.0.0.1";
  const char *transport_label =
      (transport && strcmp(transport, "msgpack") == 0) ? "msgpack" : "yrpc";
  ...
  for (struct remote_entry *remote = worker->remotes; remote;
       remote = remote->next) {
    if (strcmp(remote->name, name) == 0) {
      peer_channel_destroy(remote->peer);
      remote->peer = open_remote_session(worker->loop, host, port, transport);
      if (!remote->peer)
        return PICOMESH_ERR(picomesh_void, "add_remote: session_create");
  ...
  struct peer_channel *channel =
      open_remote_session(worker->loop, host, port, transport);
  if (!channel)
    return PICOMESH_ERR(picomesh_void, "add_remote: session_create");
  ...
  if (!node) {
    ...
    return PICOMESH_ERR(picomesh_void, "add_remote: calloc");
  }
  node->name = strdup(name);
  if (!node->name) {
    peer_channel_destroy(channel);
    free(node);
    return PICOMESH_ERR(picomesh_void, "add_remote: strdup");
  }
  node->peer = channel;
  node->next = worker->remotes;
  worker->remotes = node;
  yinfo("engine[w%zu]: opened remote '%s' → %s:%d (%s)", worker->index, name,
        host, port, transport_label);
  return PICOMESH_OK_VOID();
}

struct picomesh_void_result
picomesh_engine_add_remote(struct picomesh_engine *engine, const char *name,
                           const char *host, int port) {
  return picomesh_engine_add_remote_transport(engine, name, host, port, NULL);
}

struct peer_channel *picomesh_engine_remote(struct picomesh_engine *engine,
                                            const char *name) {
  if (!engine || !name)
    return NULL;
  struct picomesh_worker *worker = engine_current_worker(engine);
...
struct ctx picomesh_engine_service_ctx(struct picomesh_engine *engine,
                                       const char *service) {
  ...
  if (!ctx.peer) {
    ...
    struct picomesh_worker *worker = engine_current_worker(engine);
    if (worker)
      ctx.local_cache = &worker->local_instances;
  }
  return ctx;
}
 ...
 * success. */
static struct picomesh_int_result
resolve_service_bind(struct picomesh_engine *engine, const char *service,
                     char *host_out, size_t host_cap, int *port_out) {
  char path[256];
  snprintf(path, sizeof(path), "mesh.services.%s.host", service);
  struct config_node_ptr_result host_res =
      config_get(picomesh_engine_config(engine), path);
  PICOMESH_RETURN_IF_ERR(picomesh_int, host_res,
                         "resolve_service_bind: host config read failed");
  const char *host =
      host_res.value ? config_node_as_string(host_res.value, NULL) : NULL;
  snprintf(path, sizeof(path), "mesh.services.%s.port", service);
  struct config_node_ptr_result port_res =
      config_get(picomesh_engine_config(engine), path);
  PICOMESH_RETURN_IF_ERR(picomesh_int, port_res,
                         "resolve_service_bind: port config read failed");
...
picomesh_engine_service_addr(struct picomesh_engine *engine,
                             const char *service, char *host_out,
                             size_t host_cap, int *port_out) {
  if (!engine || !service || !host_out || !port_out)
    return PICOMESH_OK(picomesh_int, 0);
...
struct remote_entry_fields {
  ...
  int port;
  const char
      *transport; /* NULL/"yrpc" = native binary; "msgpack" = issue #22 */
};
...
picomesh_engine_open_remotes(struct picomesh_engine *engine,
                             const char *plugin) {
  if (!engine || !plugin)
    return PICOMESH_OK(picomesh_size, 0);

  char path[256];
  snprintf(path, sizeof(path), "mesh.services.%s.config.remotes", plugin);
  struct config_node_ptr_result list_res =
      config_get(picomesh_engine_config(engine), path);
  PICOMESH_RETURN_IF_ERR(picomesh_size, list_res,
                         "open_remotes: remotes config read failed");
  if (!list_res.value)
    return PICOMESH_OK(picomesh_size, 0);
  const struct config_node *list = list_res.value;
  if (config_node_kind(list) != CONFIG_LIST)
    return PICOMESH_OK(picomesh_size, 0);
  ...
  for (size_t i = 0; i < count; ++i) {
    ...
    if (port <= 0) {
      char bind_host[128];
      int bind_port = 0;
      struct picomesh_int_result bind_res = resolve_service_bind(
          engine, fields.svc, bind_host, sizeof(bind_host), &bind_port);
      PICOMESH_RETURN_IF_ERR(picomesh_size, bind_res,
                             "open_remotes: service bind lookup failed");
      if (bind_res.value) {
        if (!host)
          host = bind_host;
        port = bind_port;
    ...
    if (port <= 0) {
      ywarn("engine: remote '%s' for plugin '%s' — no port (skip)", fields.svc,
            plugin);
      continue;
    }
    struct picomesh_void_result add_res = picomesh_engine_add_remote_transport(
        engine, fields.svc, host, port, fields.transport);
    if (PICOMESH_IS_OK(add_res)) {
      opened++;
    } else {
      ywarn("engine: failed to open remote '%s' for plugin '%s'", fields.svc,
            plugin);
...
worker_start_flush_timer(struct picomesh_worker *w);

struct picomesh_void_result
picomesh_engine_run(struct picomesh_engine *engine) {
  if (!engine)
    return PICOMESH_ERR(picomesh_void, "picomesh_engine_run: NULL engine");
  *current_worker_slot() = &engine->workers[0];
  struct picomesh_void_result flush_res =
      worker_start_flush_timer(&engine->workers[0]);
  PICOMESH_RETURN_IF_ERR(picomesh_void, flush_res,
                         "picomesh_engine_run: start flush timer");
  struct picomesh_void_result run_res = loop_run(engine->workers[0].loop);
  PICOMESH_RETURN_IF_ERR(picomesh_void, run_res,
                         "picomesh_engine_run: loop_run failed");
  return PICOMESH_OK_VOID();
}
...
static void telemetry_flush_loop(void *arg) {
  ...
  for (;;) {
    ...
    if (ytelemetry_pending_local() > 0)
      ytelemetry_flush_local(); /* sender batch (yields) */
  }
}

/* Spawn the flush coroutine on a worker's loop. */
static struct picomesh_void_result
worker_start_flush_timer(struct picomesh_worker *worker) {
  struct picomesh_coro_ptr_result spawn_res =
      picomesh_coro_spawn(telemetry_flush_loop, worker->loop, 0, "ytel-flush");
  PICOMESH_RETURN_IF_ERR(picomesh_void, spawn_res,
                         "worker_start_flush_timer: spawn ytel-flush");
  picomesh_coro_resume(
      spawn_res.value); /* runs to its first loop_sleep_ms yield */
  return PICOMESH_OK_VOID();
}
...
static void *worker_thread_main(void *arg) {
  ...
  *current_worker_slot() = worker;
  ...
  struct picomesh_void_result setup_res =
      worker->setup(worker->engine, (int)worker->index, worker->setup_ud);
  if (PICOMESH_IS_ERR(setup_res)) {
    picomesh_error_print(stderr, "picomesh_engine_run_workers: worker setup",
                         setup_res.error);
  ...
  if (PICOMESH_IS_ERR(flush_res)) {
    picomesh_error_print(stderr, "worker_thread_main: flush timer",
                         flush_res.error);
...
struct picomesh_void_result
picomesh_engine_run_workers(struct picomesh_engine *engine, size_t workers,
                            picomesh_worker_setup_fn setup, void *ud) {
  if (!engine)
    return PICOMESH_ERR(picomesh_void, "run_workers: NULL engine");
  if (!setup)
    return PICOMESH_ERR(picomesh_void, "run_workers: NULL setup");
  if (workers < 1)
    workers = 1;
  ...
  if (workers > engine->worker_count) {
    ...
    if (!grown)
      return PICOMESH_ERR(picomesh_void,
                          "run_workers: realloc(workers) failed");
  ...
  *current_worker_slot() = &engine->workers[0];
  ...
  PICOMESH_RETURN_IF_ERR(picomesh_void, setup_res0,
                         "run_workers: worker 0 setup failed");
  ...
  for (size_t i = 1; i < engine->worker_count; ++i) {
    if (engine->workers[i].started && engine->workers[i].loop)
      loop_stop(engine->workers[i].loop);
  }
  for (size_t i = 1; i < engine->worker_count; ++i) {
    if (engine->workers[i].started)
      pthread_join(engine->workers[i].thread, NULL);
  }

  PICOMESH_RETURN_IF_ERR(picomesh_void, run_res,
                         "run_workers: worker 0 loop failed");
  return PICOMESH_OK_VOID();
}
...
void picomesh_engine_stop(struct picomesh_engine *engine) {
  if (!engine)
    return;
  for (size_t i = 0; i < engine->worker_count; ++i) {
    if (engine->workers[i].loop)
      loop_stop(engine->workers[i].loop);
  }
}

struct picomesh_void_result
picomesh_engine_perf_start(struct picomesh_engine *engine,
                           const char *service) {
  if (!engine)
    return PICOMESH_ERR(picomesh_void,
                        "picomesh_engine_perf_start: NULL engine");
  struct picomesh_worker *worker = engine_current_worker(engine);
  ...
  struct yperf_ptr_result perf_res =
      yperf_create(perf_node, worker->loop, label);
  if (PICOMESH_IS_ERR(perf_res))
    return PICOMESH_ERR(picomesh_void,
                        "picomesh_engine_perf_start: yperf_create failed",
                        perf_res);
...
picomesh_engine_plugin_config(struct picomesh_engine *engine,
                              const char *plugin) {
...
struct picomesh_void_result
picomesh_engine_for_each_plugin(struct picomesh_engine *engine,
