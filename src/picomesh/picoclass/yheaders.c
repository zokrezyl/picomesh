...
static struct yheaders_pair *find(const struct yheaders *headers,
                                  const char *key) {
  if (!headers || !key)
    return NULL;
  for (size_t i = 0; i < headers->len; ++i)
...
int yheaders_set(struct yheaders *headers, const char *key, const char *value) {
  ...
  if (headers->len == headers->cap) {
    ...
    struct yheaders_pair *new_items =
        realloc(headers->items, new_cap * sizeof(*new_items));
    if (!new_items)
      return -1;
...
int yheaders_set_u32(struct yheaders *headers, const char *key,
                     uint32_t value) {
  char buf[16];
...
uint32_t yheaders_get_u32(const struct yheaders *headers, const char *key,
                          uint32_t fallback) {
...
int yheaders_set_u64(struct yheaders *headers, const char *key,
                     uint64_t value) {
  char buf[24];
...
uint64_t yheaders_get_u64(const struct yheaders *headers, const char *key,
                          uint64_t fallback) {
  const char *value = yheaders_get(headers, key);
  if (!value || !*value)
    return fallback;
...
struct yheaders *yheaders_copy(const struct yheaders *headers) {
  ...
  if (!out || !headers)
    return out;
  for (size_t i = 0; i < headers->len; ++i) {
    if (yheaders_set(out, headers->items[i].key, headers->items[i].value) !=
        0) {
      yheaders_free(out);
      return NULL;
    }
...
size_t yheaders_serialize(const struct yheaders *headers, void *buf,
                          size_t cap) {
  ...
  for (size_t i = 0; i < headers->len; ++i) {
    ...
    memcpy(bytes + offset, &klen, 2);
    offset += 2;
    memcpy(bytes + offset, headers->items[i].key, klen);
    offset += klen;
    memcpy(bytes + offset, &vlen, 4);
    offset += 4;
    memcpy(bytes + offset, headers->items[i].value, vlen);
    offset += vlen;
  }
  return offset;
}
