...
struct slot_table_ptr_result slot_table_get(const char *domain) {
  ...
  if (!domain)
    return PICOMESH_ERR(slot_table_ptr, "slot_table_get: NULL domain");
  ...
  if (reg->next_id >= METHOD_SLOT_MAX_DOMAINS) {
    return PICOMESH_ERR(slot_table_ptr,
  ...
  HASH_ADD_KEYPTR(hh_dom, reg->by_name, table->domain, strlen(table->domain),
                  table);
  return PICOMESH_OK(slot_table_ptr, table);
}
...
method_slot_register(const char *domain, const char *name, method_id_t id) {
  ydebug("domain=%s name=%s id=%p", domain ? domain : "(null)",
         name ? name : "(null)", (void *)id);
  if (!domain || !name) {
    return PICOMESH_ERR(method_slot,
                        "method_slot_register: NULL domain or name");
  }
  struct slot_table_ptr_result table_res = slot_table_get(domain);
  PICOMESH_RETURN_IF_ERR(method_slot, table_res,
  ...
  if (table->count >= table->cap) {
    ...
    while (ncap <= table->count)
      ncap *= 2;
    struct slot_entry **new_index =
        realloc(table->by_index, ncap * sizeof(*new_index));
    if (!new_index) {
      free(entry->qname);
      free(entry);
      return PICOMESH_ERR(method_slot,
  ...
  HASH_ADD_KEYPTR(hh_lname, table->by_local_name, entry->local_name,
                  strlen(entry->local_name), entry);
  HASH_ADD(hh_id, table->by_local_id, local_id, sizeof(method_id_t), entry);
  HASH_ADD_KEYPTR(hh_qname, *global_qname_root(), entry->qname,
...
struct method_slot_result method_slot_get(const char *domain, method_id_t id) {
  ...
  if (!domain || !id)
    return PICOMESH_ERR(method_slot, "method_slot_get: NULL domain or id");
  struct domain_registry *reg = dreg();
  struct slot_table *table = NULL;
  HASH_FIND(hh_dom, reg->by_name, domain, strlen(domain), table);
  if (!table)
    return PICOMESH_ERR(method_slot, "method_slot_get: domain not registered");
  struct slot_entry *entry = NULL;
...
struct method_slot_result method_slot_by_name(const char *domain,
                                              const char *name) {
  ydebug("domain=%s name=%s", domain ? domain : "(null)",
         name ? name : "(null)");
  if (!domain || !name) {
    return PICOMESH_ERR(method_slot,
  ...
  if (!entry)
    return PICOMESH_ERR(method_slot,
                        "method_slot_by_name: name not in slot table");
  return PICOMESH_OK(method_slot, entry->slot_index);
}
...
struct method_slot_result method_slot_by_qname(const char *qname) {
  ...
  if (!qname)
    return PICOMESH_ERR(method_slot, "method_slot_by_qname: NULL qname");
  struct slot_entry *entry = NULL;
...
struct const_char_ptr_result method_slot_name(method_slot slot) {
  ...
  if (slot == METHOD_SLOT_UNDEFINED) {
    return PICOMESH_ERR(const_char_ptr,
...
static struct picomesh_void_result class_registry_add(struct class *cls) {
  ...
  if (reg->count == reg->cap) {
    ...
    struct class **new_index =
        realloc(reg->by_index, ncap * sizeof(*new_index));
    if (!new_index)
      return PICOMESH_ERR(picomesh_void, "class_registry_add: realloc failed");
    reg->by_index = new_index;
    reg->cap = ncap;
  }
  reg->by_index[reg->count++] = cls;
  HASH_ADD_KEYPTR(hh, reg->by_name, cls->desc->name, strlen(cls->desc->name),
                  cls);
  return PICOMESH_OK_VOID();
}

static struct picomesh_void_result
dispatch_slice_grow(struct dispatch_slice *slice, size_t needed) {
  if (needed <= slice->count)
    return PICOMESH_OK_VOID();
  impl_t *new_impls = realloc(slice->impls, needed * sizeof(*new_impls));
  if (!new_impls)
    return PICOMESH_ERR(picomesh_void, "dispatch_slice_grow: realloc failed");
  memset(new_impls + slice->count, 0,
         (needed - slice->count) * sizeof(*new_impls));
...
static struct picomesh_void_result
class_inherit_dispatch(struct class *cls, const struct class *src) {
  for (uint8_t domain_id = 0; domain_id < METHOD_SLOT_MAX_DOMAINS;
       ++domain_id) {
    const struct dispatch_slice *src_slice =
        &src->dispatch_by_domain[domain_id];
    if (src_slice->count == 0)
      continue;
    struct dispatch_slice *dest_slice = &cls->dispatch_by_domain[domain_id];
    struct picomesh_void_result grow_res =
        dispatch_slice_grow(dest_slice, src_slice->count);
    PICOMESH_RETURN_IF_ERR(picomesh_void, grow_res,
                           "class_inherit_dispatch: grow failed");
    for (size_t i = 0; i < src_slice->count; ++i) {
      if (src_slice->impls[i])
        dest_slice->impls[i] = src_slice->impls[i];
    }
  }
  return PICOMESH_OK_VOID();
}
...
static void class_destroy(struct class *cls) {
  ...
  for (uint8_t domain_id = 0; domain_id < METHOD_SLOT_MAX_DOMAINS; ++domain_id)
    free(cls->dispatch_by_domain[domain_id].impls);
...
struct class_ptr_result class_register(const struct class_descriptor *desc,
  ...
  if (!desc)
    return PICOMESH_ERR(class_ptr, "class_register: NULL descriptor");
  struct class *cls = calloc(1, sizeof(*cls));
  if (!cls)
    return PICOMESH_ERR(class_ptr, "class_register: calloc(class) failed");
  cls->desc = desc;
  cls->parent = parent;
  ...
  for (const struct class *ancestor = parent; ancestor != NULL;
  ...
  if (parent) {
    struct picomesh_void_result inherit_res =
        class_inherit_dispatch(cls, parent);
    if (PICOMESH_IS_ERR(inherit_res)) {
      class_destroy(cls);
      return PICOMESH_ERR(class_ptr,
                          "class_register: inherit parent dispatch failed",
                          inherit_res);
    }
  }
  for (size_t m = 0; m < mixin_count; ++m) {
    struct picomesh_void_result inherit_res =
        class_inherit_dispatch(cls, mixins[m]);
    if (PICOMESH_IS_ERR(inherit_res)) {
      class_destroy(cls);
      return PICOMESH_ERR(class_ptr,
                          "class_register: inherit mixin dispatch failed",
                          inherit_res);
    }
  }
  ...
  for (size_t i = 0; i < ops_count; ++i) {
    ...
    if (PICOMESH_IS_ERR(slot_res)) {
      class_destroy(cls);
      return PICOMESH_ERR(
          class_ptr, "class_register: method_slot_register failed", slot_res);
    }
    ...
    struct picomesh_void_result grow_res =
        dispatch_slice_grow(slice, (size_t)index + 1);
    if (PICOMESH_IS_ERR(grow_res)) {
      class_destroy(cls);
      return PICOMESH_ERR(
          class_ptr, "class_register: dispatch slice grow failed", grow_res);
    }
  ...
  if (PICOMESH_IS_ERR(add_res)) {
    class_destroy(cls);
    return PICOMESH_ERR(class_ptr, "class_register: class_registry_add failed",
                        add_res);
  }
...
struct picomesh_void_result class_add_accessor_lookup(accessor_lookup_fn fn) {
  ...
  if (!fn)
    return PICOMESH_ERR(picomesh_void, "class_add_accessor_lookup: NULL fn");
  struct accessor_node *node = calloc(1, sizeof(*node));
  if (!node)
    return PICOMESH_ERR(picomesh_void,
                        "class_add_accessor_lookup: calloc failed");
  node->fn = fn;
...
struct class_ptr_result class_by_name(const char *name) {
  ...
  if (!name)
    return PICOMESH_ERR(class_ptr, "class_by_name: NULL name");
  struct class_registry *reg = class_registry_get();
  struct class *cls = NULL;
  HASH_FIND_STR(reg->by_name, name, cls);
  if (cls)
    return PICOMESH_OK(class_ptr, cls);
  for (struct accessor_node *node = *accessor_chain_head(); node;
       node = node->next) {
    struct class_ptr_result accessor_res = node->fn(name);
    if (PICOMESH_IS_ERR(accessor_res)) {
      return PICOMESH_ERR(class_ptr, "class_by_name: accessor hook failed",
                          accessor_res);
    }
    if (accessor_res.value)
      return accessor_res;
  }
  return PICOMESH_ERR(class_ptr,
  ...
  for (uint8_t domain_id = 0; domain_id < METHOD_SLOT_MAX_DOMAINS;
    ...
    for (size_t i = 0; i < slice->count; ++i) {
      ...
      PICOMESH_RETURN_IF_ERR(
          picomesh_void, name_res,
          "class_for_each_slot: method slot name lookup failed");
      if (name_res.value)
...
void class_for_each(void (*cb)(const struct class *cls, const char *name,
  ...
  for (size_t i = 0; i < reg->count; ++i) {
    ...
    if (cls && cls->desc && cls->desc->name)
      cb(cls, cls->desc->name, userdata);
  }
}
...
struct object_ptr_result object_alloc(const struct class *cls) {
  ...
  if (!cls)
    return PICOMESH_ERR(object_ptr, "object_alloc: NULL class");
  struct object *obj = calloc(1, cls->instance_size);
  if (!obj)
    return PICOMESH_ERR(object_ptr, "object_alloc: calloc failed");
  obj->klass = cls;
