...
picomesh_jwt_verifier_create(struct picomesh_engine *engine) {
  ...
  if (!verifier)
    return PICOMESH_ERR(picomesh_void_ptr,
                        "jwt_verifier_create: out of memory");
...
picomesh_jwt_verifier_verify(struct picomesh_jwt_verifier *verifier,
                             const char *jwt) {
  if (!verifier)
    return PICOMESH_ERR(picomesh_string, "jwt_verifier_verify: NULL verifier");
  if (!jwt || !*jwt)
    return PICOMESH_ERR(picomesh_string, "jwt_verifier_verify: empty token");

  if (!verifier->secret) {
    struct picomesh_string_result secret =
        picomesh_security_jwt_secret(verifier->engine);
    if (PICOMESH_IS_ERR(secret))
      return PICOMESH_ERR(picomesh_string,
                          "jwt_verifier_verify: signing key unavailable",
                          secret);
