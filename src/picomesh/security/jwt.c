...
static const char *jwt_header_b64(void) {
  ...
  if (!ready) {
    ...
    picomesh_base64url_encode(header, sizeof(header) - 1, encoded,
                              sizeof(encoded));
    ready = 1;
  }
  return encoded;
...
char *picomesh_jwt_build_claims(const char *issuer, uint32_t sub,
  ...
  json_writer_key(writer, "iss");
  json_writer_string(writer, issuer ? issuer : "picomesh");
  json_writer_key(writer, "sub");
  json_writer_int(writer, (int64_t)sub);
  ...
  json_writer_key(writer, "iat");
  json_writer_int(writer, issued_at);
  json_writer_key(writer, "exp");
  json_writer_int(writer, expires_at);
  ...
  json_writer_key(writer, "jti");
  json_writer_string(writer, jti);
...
char *picomesh_jwt_encode(const char *claims_json, const char *secret) {
  ...
  if (!payload_segment)
    return NULL;
  size_t payload_len = picomesh_base64url_encode(claims_json, claims_len,
                                                 payload_segment, payload_cap);
  if (payload_len == 0) {
  ...
  size_t signature_len = picomesh_base64url_encode(
      mac, sizeof(mac), signature_segment, sizeof(signature_segment));
  if (signature_len == 0) {
    free(signing_input);
    return NULL;
  }
...
static int constant_time_equal(const char *a, size_t a_len, const char *b,
                               size_t b_len) {
  if (a_len != b_len)
    return 0;
...
picomesh_jwt_verify(const char *jwt, const char *secret, int64_t now) {
  ...
  if (!first_dot)
    return PICOMESH_ERR(picomesh_string,
                        "jwt_verify: malformed (no header.payload)");
  const char *second_dot = strchr(first_dot + 1, '.');
  if (!second_dot)
    return PICOMESH_ERR(picomesh_string,
                        "jwt_verify: malformed (no signature)");
  ...
  size_t expected_len = picomesh_base64url_encode(
      mac, sizeof(mac), expected_signature, sizeof(expected_signature));
  if (expected_len == 0)
    return PICOMESH_ERR(picomesh_string, "jwt_verify: signature encode failed");
  if (!constant_time_equal(signature_segment, signature_len, expected_signature,
                           expected_len))
  ...
  char header_text[128];
  {
    ...
    size_t decoded = picomesh_base64url_decode(
        header_copy, (uint8_t *)header_text, sizeof(header_text) - 1);
    if (decoded == (size_t)-1)
  ...
  if (!header_doc)
    return PICOMESH_ERR(picomesh_string, "jwt_verify: header not JSON");
  const char *alg =
      json_as_string(json_object_get(json_doc_root(header_doc), "alg"), "");
  int alg_ok = strcmp(alg, "HS256") == 0;
  json_doc_free(header_doc);
  if (!alg_ok)
    return PICOMESH_ERR(picomesh_string, "jwt_verify: unsupported alg");

  /* Decode the payload. */
  char *payload_copy = malloc(payload_seg_len + 1);
  if (!payload_copy)
    return PICOMESH_ERR(picomesh_string, "jwt_verify: out of memory");
  memcpy(payload_copy, payload_segment, payload_seg_len);
  payload_copy[payload_seg_len] = 0;
  size_t payload_cap =
      payload_seg_len; /* decoded is always shorter than encoded */
  char *payload_json = malloc(payload_cap + 1);
  if (!payload_json) {
    free(payload_copy);
    return PICOMESH_ERR(picomesh_string, "jwt_verify: out of memory");
  }
  size_t payload_len = picomesh_base64url_decode(
      payload_copy, (uint8_t *)payload_json, payload_cap);
  free(payload_copy);
  if (payload_len == (size_t)-1) {
    free(payload_json);
    return PICOMESH_ERR(picomesh_string, "jwt_verify: payload decode failed");
  }
  ...
  if (!payload_doc) {
    free(payload_json);
    return PICOMESH_ERR(picomesh_string, "jwt_verify: payload not JSON");
  }
  ...
  int64_t exp =
      json_as_int(json_object_get(json_doc_root(payload_doc), "exp"), 0);
  json_doc_free(payload_doc);
  if (exp <= 0) {
    free(payload_json);
    return PICOMESH_ERR(picomesh_string, "jwt_verify: missing exp claim");
  }
  if (now > exp) {
    free(payload_json);
    return PICOMESH_ERR(picomesh_string, "jwt_verify: expired");
  }
...
int picomesh_groups_contains(const char *groups_csv, const char *group) {
  ...
  while (*cursor) {
    const char *comma = strchr(cursor, ',');
    size_t span = comma ? (size_t)(comma - cursor) : strlen(cursor);
    if (span == glen && memcmp(cursor, group, glen) == 0)
      return 1;
    if (!comma)
      break;
    cursor = comma + 1;
...
int picomesh_groups_max_role(const char *groups_csv, const char *account) {
  ...
  while (*cursor) {
    ...
    if (colon) {
      ...
      if (slug_len == account_len &&
          memcmp(cursor, account, account_len) == 0) {
        char role[32];
...
int picomesh_groups_effective_role(const char *groups_csv,
  ...
  if (!groups_csv || !namespace_path || !*namespace_path)
    return -1;
  ...
  char prefix[256];
  int best = -1;
  for (const char *end = namespace_path + strlen(namespace_path);
       end > namespace_path;) {
...
picomesh_authctx_from_jwt(const char *jwt, const char *secret,
  ...
  if (!jwt || !*jwt)
    return PICOMESH_OK_VOID(); /* anonymous, not an error */

  struct picomesh_string_result verified =
      picomesh_jwt_verify(jwt, secret, picomesh_security_now());
  if (PICOMESH_IS_ERR(verified))
    return PICOMESH_ERR(picomesh_void, "authctx: jwt verification failed",
                        verified);

  struct json_doc *doc = json_parse(verified.value, strlen(verified.value));
  if (!doc) {
    free(verified.value);
    return PICOMESH_ERR(picomesh_void, "authctx: claims not JSON");
  }
  ...
  for (size_t i = 0; i < group_count; ++i) {
    ...
    int written = snprintf(out->groups_csv + pos, sizeof(out->groups_csv) - pos,
                           "%s%s", pos ? "," : "", group);
    if (written < 0 || (size_t)written >= sizeof(out->groups_csv) - pos)
      break;
    pos += (size_t)written;
  }
  json_doc_free(doc);
