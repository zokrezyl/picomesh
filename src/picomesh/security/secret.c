...
picomesh_security_jwt_secret(struct picomesh_engine *engine) {
  ...
  if (engine) {
    ...
    if (config) {
      struct config_node_ptr_result node =
          config_get(config, "security.jwt_secret");
      if (PICOMESH_IS_OK(node) && node.value)
        secret = config_node_as_string(node.value, NULL);
      else if (PICOMESH_IS_ERR(node))
        picomesh_error_destroy(node.error);
    }
  }
  ...
  if (!secret || !*secret)
    secret = getenv("PICOMESH_JWT_SECRET");
  if (!secret || !*secret)
    return PICOMESH_ERR(
        picomesh_string,
  ...
  if (!copy)
    return PICOMESH_ERR(picomesh_string,
                        "security: out of memory copying jwt secret");
...
picomesh_authctx_from_headers(struct yheaders *hdrs,
  ...
  if (!jwt || !*jwt)
    return PICOMESH_OK_VOID(); /* anonymous */
  ...
  struct picomesh_void_result verified =
      picomesh_authctx_from_jwt(jwt, secret.value, out);
  free(secret.value);
  if (PICOMESH_IS_ERR(verified)) {
    picomesh_error_destroy(verified.error);
    memset(out, 0,
           sizeof(*out)); /* invalid JWT → unauthenticated, fail closed */
  }
...
picomesh_security_access_ttl(struct picomesh_engine *engine) {
  ...
  if (!engine)
    return PICOMESH_OK(picomesh_int64, default_ttl);
  const struct config *config = picomesh_engine_config(engine);
  if (!config)
    return PICOMESH_OK(picomesh_int64, default_ttl);
  struct config_node_ptr_result node =
      config_get(config, "security.access_ttl_seconds");
