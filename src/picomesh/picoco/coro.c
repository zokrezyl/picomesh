...
struct picomesh_coro_ptr_result picomesh_coro_spawn(picomesh_coro_entry entry,
                                                    ...
                                                    const char *name) {
  if (!entry)
    return PICOMESH_ERR(picomesh_coro_ptr, "picomesh_coro_spawn: NULL entry");
  struct picomesh_coro *coro = calloc(1, sizeof(*coro));
  if (!coro)
    return PICOMESH_ERR(picomesh_coro_ptr,
                        "picomesh_coro_spawn: calloc failed");
  coro->entry = entry;
  coro->arg = arg;
  coro->id = next_id();
  ...
  if (!coro->thread) {
    ...
    free(coro);
    return PICOMESH_ERR(picomesh_coro_ptr,
                        "picomesh_coro_spawn: co_create failed");
  }
...
struct picomesh_coro *picomesh_coro_current(void) { return *current_slot(); }
