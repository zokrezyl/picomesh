...
struct picomesh_json_result git_pipeline_git_pipeline_lease_job(struct ctx * ctx, struct object * obj, struct yheaders * hdrs, uint32_t runner_id, const char * labels, uint32_t wait_ms)
{
    static method_slot _slot = METHOD_SLOT_UNDEFINED;
    if (_slot == METHOD_SLOT_UNDEFINED) {
        struct method_slot_result _sr =
    ...
    if (_s && _s->peer) {
        if (peer_channel_is_msgpack(_s->peer)) {
            ...
            cmp_write_uinteger(&_maw, (uint64_t)wait_ms);
            size_t _mrlen = 0;
            char _merr[8192] = {0};
        ...
        if (_off + sizeof(wait_ms) > _acap)
            { ytelemetry_span_end(&_tsp, 0, "git_pipeline_git_pipeline_lease_job: pack overflow"); _ret = PICOMESH_ERR(picomesh_json, "git_pipeline_git_pipeline_lease_job: pack overflow"); goto _rpc_done; }
        memcpy(_a + _off, &wait_ms, sizeof(wait_ms)); _off += sizeof(wait_ms);
        size_t _wn = rpc_call(_s->peer, RPC_OP_CALL, _rid, _a, _off,
                              _wbuf, 65536);
        ytelemetry_span_end(&_tsp, _wn >= 1 && _wbuf[0] == 0, NULL);
    ...
    _rpc_done:
        ...
        return ((git_pipeline_git_pipeline_lease_job_fn)fn)(ctx, obj, hdrs, runner_id, labels, wait_ms);
    }
}
