...
struct PICOMESH_CLASS_ANNOTATE("class@portalloc:portalloc")
    portalloc_portalloc_data {
  int placeholder;
};
...
static struct picomesh_void_result portalloc_range(uint32_t *lo, uint32_t *hi) {
  ...
  *hi = PORTALLOC_DEFAULT_HI;
  ...
  PICOMESH_RETURN_IF_ERR(picomesh_void, config_res,
                         "portalloc_range: config read failed");
  if (config_res.value) {
    const char *range_str = config_node_as_string(config_res.value, NULL);
    unsigned range_lo = 0, range_hi = 0;
    if (range_str && sscanf(range_str, "%u-%u", &range_lo, &range_hi) == 2 &&
...
portalloc_portalloc_allocate_impl(struct ctx *ctx, struct object *obj,
  ...
  if (!service_name || !*service_name)
    return PICOMESH_ERR(picomesh_uint32,
                        "portalloc_allocate: empty service name");
  if (!host || !*host)
    host = "127.0.0.1";
  struct portalloc_state *state = portalloc_state();
  uint32_t lo, hi;
  struct picomesh_void_result range_res = portalloc_range(&lo, &hi);
  PICOMESH_RETURN_IF_ERR(picomesh_uint32, range_res,
                         "portalloc_allocate: port range failed");
  ...
  for (size_t i = 0; i < PORTALLOC_MAX_ENTRIES; ++i) {
    if (state->entries[i].used &&
        strcmp(state->entries[i].service_name, service_name) == 0) {
  ...
  for (uint32_t candidate_port = lo; candidate_port <= hi; ++candidate_port) {
    int taken = 0;
    for (size_t i = 0; i < PORTALLOC_MAX_ENTRIES; ++i) {
      if (state->entries[i].used && state->entries[i].port == candidate_port) {
        taken = 1;
        break;
      }
    }
    if (taken)
      continue;
    if (!portalloc_port_free(host, candidate_port))
      continue;
    for (size_t i = 0; i < PORTALLOC_MAX_ENTRIES; ++i) {
      if (!state->entries[i].used) {
        snprintf(state->entries[i].service_name,
                 sizeof(state->entries[i].service_name), "%s", service_name);
        state->entries[i].port = candidate_port;
        state->entries[i].used = 1;
        state->count++;
        pthread_mutex_unlock(&state->mu);
        yinfo("portalloc: '%s' → port %u (host %s)", service_name,
              candidate_port, host);
    ...
    pthread_mutex_unlock(&state->mu);
    return PICOMESH_ERR(picomesh_uint32,
                        "portalloc_allocate: lease table full");
  }
  pthread_mutex_unlock(&state->mu);
  return PICOMESH_ERR(picomesh_uint32,
                      "portalloc_allocate: no free port in range");
}

PICOMESH_CLASS_ANNOTATE("override@portalloc:portalloc:portalloc_release")
struct picomesh_int_result
portalloc_portalloc_release_impl(struct ctx *ctx, struct object *obj,
                                 struct yheaders *hdrs, uint32_t port) {
  (void)ctx;
  (void)obj;
  (void)hdrs;
  struct portalloc_state *state = portalloc_state();
...
portalloc_portalloc_count_used_impl(struct ctx *ctx, struct object *obj,
                                    struct yheaders *hdrs) {
  (void)ctx;
  (void)obj;
  (void)hdrs;
...
portalloc_list_window(struct portalloc_state *state, int64_t offset,
                      int64_t limit) {
  struct json_writer *writer = json_writer_new();
  if (!writer)
    return PICOMESH_ERR(picomesh_json, "portalloc_list: writer alloc failed");
  json_writer_begin_array(writer);
  int64_t skip = offset > 0 ? offset : 0, emitted = 0;
  for (size_t i = 0;
       i < PORTALLOC_MAX_ENTRIES && (limit < 0 || emitted < limit); ++i) {
    if (!state->entries[i].used)
      continue;
    if (skip > 0) {
      --skip;
    ...
    json_writer_string(writer, state->entries[i].service_name);
  ...
  if (!out)
    return PICOMESH_ERR(picomesh_json, "portalloc_list: strdup failed");
...
struct picomesh_json_result portalloc_portalloc_list_impl(struct ctx *ctx,
                                                          struct object *obj,
                                                          struct yheaders *hdrs,
                                                          int64_t offset,
                                                          int64_t limit) {
  ...
  struct portalloc_state *state = portalloc_state();
  pthread_mutex_lock(&state->mu);
  struct picomesh_json_result list_res =
      portalloc_list_window(state, offset, limit);
  pthread_mutex_unlock(&state->mu);
  return list_res;
}
...
portalloc_portalloc_list_all_impl(struct ctx *ctx, struct object *obj,
                                  struct yheaders *hdrs) {
  (void)ctx;
  (void)obj;
  (void)hdrs;
