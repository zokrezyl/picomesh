...
struct PICOMESH_CLASS_ANNOTATE("class@portalloc:store") portalloc_store_data {
...
PICOMESH_CLASS_ANNOTATE("override@portalloc:store:store_allocate")
struct picomesh_uint32_result portalloc_store_allocate_impl(struct ctx *ctx, struct object *obj, struct yheaders *hdrs,
                                                         uint32_t service_id)
{
    (void)ctx;
...
PICOMESH_CLASS_ANNOTATE("override@portalloc:store:store_release")
struct picomesh_int_result portalloc_store_release_impl(struct ctx *ctx, struct object *obj, struct yheaders *hdrs,
                                                     uint32_t port)
{
    (void)ctx;
    struct portalloc_store_data *d = pa(obj);
...
PICOMESH_CLASS_ANNOTATE("override@portalloc:store:store_count_used")
struct picomesh_size_result portalloc_store_count_used_impl(struct ctx *ctx, struct object *obj, struct yheaders *hdrs)
{
    (void)ctx;
