 ...
 *   register(runner_id, name, labels, version, host) → runner_id
 * (status→online) heartbeat(runner_id, status)→ 1 (last_seen bumped)
 ...
 *   runner:<id>          → JSON
 * {name,labels,version,host,status,last_seen,tok_hash} tok:<sha256hex>      →
 * "<runner_id>"   (token at rest is only its hash)
 *
...
struct PICOMESH_CLASS_ANNOTATE("class@runner_agent:runner_agent")
    runner_agent_runner_agent_data {
  char _unused;
};
...
static struct ra_storage_result ra_open(void) {
  ...
  if (!engine)
    return PICOMESH_ERR(ra_storage, "runner_agent: no active engine");
  struct ra_storage storage = {
      .c = picomesh_engine_service_ctx(engine, "sharded_storage")};
  struct object_ptr_result create_res = sharded_storage_db_create(&storage.c);
  if (PICOMESH_IS_ERR(create_res))
    return PICOMESH_ERR(ra_storage, "runner_agent: storage_db_create failed",
                        create_res);
...
static struct picomesh_int64_result ra_incr(struct ra_storage *storage,
                                            struct yheaders *hdrs,
                                            const char *key, int64_t delta) {
  struct picomesh_int64_result incr_res = sharded_storage_db_incr(
      &storage->c, storage->obj, hdrs, RA_CTX, key, delta);
  if (PICOMESH_IS_ERR(incr_res))
    return PICOMESH_ERR(picomesh_int64, "runner_agent: counter update failed",
                        incr_res);
  return incr_res;
}

static struct picomesh_int64_result ra_get_int(struct ra_storage *storage,
                                               struct yheaders *hdrs,
                                               const char *key,
                                               int64_t fallback) {
  struct picomesh_string_result get_res =
      sharded_storage_db_get(&storage->c, storage->obj, hdrs, RA_CTX, key);
  if (PICOMESH_IS_ERR(get_res))
    return PICOMESH_ERR(picomesh_int64, "runner_agent: storage read failed",
                        get_res);
  int64_t value = (get_res.value && get_res.value[0])
                      ? strtoll(get_res.value, NULL, 10)
                      : fallback;
  free(get_res.value);
...
static char *ra_record_to_json(const struct ra_record *rec) {
  ...
  json_writer_string(writer, rec->name);
  json_writer_key(writer, "labels");
  json_writer_string(writer, rec->labels);
  json_writer_key(writer, "version");
  json_writer_string(writer, rec->version);
  json_writer_key(writer, "host");
  json_writer_string(writer, rec->host);
  json_writer_key(writer, "status");
  json_writer_string(writer, rec->status);
  json_writer_key(writer, "last_seen");
  json_writer_int(writer, rec->last_seen);
  json_writer_key(writer, "tok_hash");
  json_writer_string(writer, rec->tok_hash);
...
static void ra_copy_field(char *dst, size_t cap, const struct json_value *obj,
                          const char *key) {
...
static struct picomesh_int_result ra_record_load(struct ra_storage *storage,
                                                 struct yheaders *hdrs,
                                                 uint32_t runner_id,
  ...
  struct picomesh_string_result get_res =
      sharded_storage_db_get(&storage->c, storage->obj, hdrs, RA_CTX, key);
  if (PICOMESH_IS_ERR(get_res))
    return PICOMESH_ERR(picomesh_int, "runner_agent: record read failed",
                        get_res);
  if (!get_res.value || !get_res.value[0]) {
    free(get_res.value);
  ...
  free(get_res.value);
  if (!doc)
    return PICOMESH_ERR(picomesh_int, "runner_agent: record parse failed");
...
ra_record_store(struct ra_storage *storage, struct yheaders *hdrs,
                uint32_t runner_id, const struct ra_record *rec) {
  char *json = ra_record_to_json(rec);
  if (!json)
    return PICOMESH_ERR(picomesh_void, "runner_agent: record encode failed");
  char key[48];
  snprintf(key, sizeof(key), "runner:%u", runner_id);
  struct picomesh_int_result set_res = sharded_storage_db_set(
      &storage->c, storage->obj, hdrs, RA_CTX, key, json);
  free(json);
  if (PICOMESH_IS_ERR(set_res))
    return PICOMESH_ERR(picomesh_void, "runner_agent: record write failed",
                        set_res);
...
PICOMESH_CLASS_ANNOTATE(
    "override@runner_agent:runner_agent:runner_agent_create_token")
struct picomesh_json_result runner_agent_runner_agent_create_token_impl(
  ...
  if (PICOMESH_IS_ERR(open_res))
    return PICOMESH_ERR(picomesh_json,
                        "runner_agent_create_token: storage open failed",
                        open_res);
  ...
  if (!ra_alloc_token(token, sizeof(token)))
    return PICOMESH_ERR(picomesh_json,
                        "runner_agent_create_token: secure random unavailable");

  struct picomesh_int64_result id_res = ra_incr(&storage, hdrs, "next_id", 1);
  if (PICOMESH_IS_ERR(id_res))
    return PICOMESH_ERR(
        picomesh_json, "runner_agent_create_token: allocate id failed", id_res);
  ...
  struct picomesh_void_result store_res =
      ra_record_store(&storage, hdrs, runner_id, &rec);
  if (PICOMESH_IS_ERR(store_res))
    return PICOMESH_ERR(picomesh_json,
                        "runner_agent_create_token: persist record failed",
                        store_res);
  ...
  if (!writer)
    return PICOMESH_ERR(picomesh_json,
                        "runner_agent_create_token: writer alloc failed");
  json_writer_begin_object(writer);
  json_writer_key(writer, "runner_id");
  json_writer_int(writer, (int64_t)runner_id);
  json_writer_key(writer, "token");
  json_writer_string(writer, token);
  json_writer_end_object(writer);
  size_t len = 0;
  const char *data = json_writer_data(writer, &len);
  char *out = data ? strdup(data) : NULL;
  ...
  if (!out)
    return PICOMESH_ERR(picomesh_json,
                        "runner_agent_create_token: encode failed");

  char tok_key[80];
  snprintf(tok_key, sizeof(tok_key), "tok:%s", rec.tok_hash);
  char id_str[16];
  snprintf(id_str, sizeof(id_str), "%u", runner_id);
  struct picomesh_int_result tok_set_res = sharded_storage_db_set(
      &storage.c, storage.obj, hdrs, RA_CTX, tok_key, id_str);
  if (PICOMESH_IS_ERR(tok_set_res)) {
    free(out);
    return PICOMESH_ERR(picomesh_json,
                        "runner_agent_create_token: persist token failed",
                        tok_set_res);
  }
  ...
  if (PICOMESH_IS_ERR(count_res)) {
    picomesh_error_print(
        stderr,
        "runner_agent_create_token: active-count bump (count may drift low)",
        count_res.error);
...
PICOMESH_CLASS_ANNOTATE(
    "override@runner_agent:runner_agent:runner_agent_lookup_token")
struct picomesh_uint32_result
runner_agent_runner_agent_lookup_token_impl(struct ctx *ctx, struct object *obj,
                                            struct yheaders *hdrs,
                                            const char *token) {
  (void)ctx;
  (void)obj;
  ...
  if (PICOMESH_IS_ERR(open_res))
    return PICOMESH_ERR(picomesh_uint32,
                        "runner_agent_lookup_token: storage open failed",
                        open_res);
  struct ra_storage storage = open_res.value;
  ...
  struct picomesh_string_result get_res =
      sharded_storage_db_get(&storage.c, storage.obj, hdrs, RA_CTX, tok_key);
  if (PICOMESH_IS_ERR(get_res))
    return PICOMESH_ERR(picomesh_uint32,
                        "runner_agent_lookup_token: read failed", get_res);
  if (!get_res.value || !get_res.value[0]) {
    free(get_res.value);
    return PICOMESH_OK(picomesh_uint32, 0);
  }
  ...
  struct picomesh_int_result load_res =
      ra_record_load(&storage, hdrs, runner_id, &rec);
  if (PICOMESH_IS_ERR(load_res))
    return PICOMESH_ERR(picomesh_uint32,
                        "runner_agent_lookup_token: record load failed",
                        load_res);
  if (load_res.value == 0)
    return PICOMESH_OK(picomesh_uint32, 0);
 ...
 * exchange. */
PICOMESH_CLASS_ANNOTATE(
    "override@runner_agent:runner_agent:runner_agent_exchange")
struct picomesh_string_result
runner_agent_runner_agent_exchange_impl(struct ctx *ctx, struct object *obj,
                                        struct yheaders *hdrs,
                                        const char *token) {
  struct picomesh_uint32_result lookup_res =
      runner_agent_runner_agent_lookup_token_impl(ctx, obj, hdrs, token);
  if (PICOMESH_IS_ERR(lookup_res))
    return PICOMESH_ERR(picomesh_string, "runner_agent_exchange: lookup failed",
                        lookup_res);
  uint32_t runner_id = lookup_res.value;
  if (runner_id == 0)
    return PICOMESH_ERR(
        picomesh_string,
        "runner_agent_exchange: unknown or revoked runner token");
  ...
  if (!engine)
    return PICOMESH_ERR(picomesh_string,
                        "runner_agent_exchange: no active engine");
  ...
  if (PICOMESH_IS_ERR(ti_obj))
    return PICOMESH_ERR(picomesh_string,
                        "runner_agent_exchange: token_issuer unreachable",
                        ti_obj);
  struct picomesh_string_result jwt = token_issuer_token_issuer_mint(
      &ti_ctx, ti_obj.value, hdrs, runner_id, username, groups, 0);
  if (PICOMESH_IS_ERR(jwt))
    return PICOMESH_ERR(picomesh_string, "runner_agent_exchange: mint failed",
                        jwt);
  return jwt;
}

PICOMESH_CLASS_ANNOTATE(
    "override@runner_agent:runner_agent:runner_agent_revoke_token")
struct picomesh_int_result
runner_agent_runner_agent_revoke_token_impl(struct ctx *ctx, struct object *obj,
                                            struct yheaders *hdrs,
                                            uint32_t runner_id) {
  (void)ctx;
  (void)obj;
  ...
  if (PICOMESH_IS_ERR(open_res))
    return PICOMESH_ERR(picomesh_int,
                        "runner_agent_revoke_token: storage open failed",
                        open_res);
  struct ra_storage storage = open_res.value;
  ...
  if (PICOMESH_IS_ERR(load_res))
    return PICOMESH_ERR(picomesh_int,
                        "runner_agent_revoke_token: record load failed",
                        load_res);
  if (load_res.value == 0)
    return PICOMESH_OK(picomesh_int, 0);
  if (strcmp(rec.status, "disabled") == 0)
  ...
  if (rec.tok_hash[0]) {
    char tok_key[80];
    snprintf(tok_key, sizeof(tok_key), "tok:%s", rec.tok_hash);
    struct picomesh_int_result del_res =
        sharded_storage_db_del(&storage.c, storage.obj, hdrs, RA_CTX, tok_key);
    if (PICOMESH_IS_ERR(del_res))
      return PICOMESH_ERR(picomesh_int,
                          "runner_agent_revoke_token: token delete failed",
                          del_res);
  }
  ...
  if (PICOMESH_IS_ERR(store_res))
    return PICOMESH_ERR(picomesh_int,
                        "runner_agent_revoke_token: record write failed",
                        store_res);
  ...
  struct picomesh_int64_result count_res = ra_incr(&storage, hdrs, "count", -1);
  if (PICOMESH_IS_ERR(count_res)) {
    picomesh_error_print(stderr,
                         "runner_agent_revoke_token: active-count decrement "
                         "(count may drift high)",
                         count_res.error);
    picomesh_error_destroy(count_res.error);
  }
...
PICOMESH_CLASS_ANNOTATE(
    "override@runner_agent:runner_agent:runner_agent_register")
struct picomesh_uint32_result runner_agent_runner_agent_register_impl(
    struct ctx *ctx, struct object *obj, struct yheaders *hdrs,
    uint32_t runner_id, const char *name, const char *labels,
    const char *version, const char *host) {
  (void)ctx;
  (void)obj;
  if (!ra_caller_is(hdrs, runner_id))
    return PICOMESH_ERR(picomesh_uint32,
                        "runner_agent_register: caller is not this runner");
  struct ra_storage_result open_res = ra_open();
  if (PICOMESH_IS_ERR(open_res))
    return PICOMESH_ERR(picomesh_uint32,
                        "runner_agent_register: storage open failed", open_res);
  struct ra_storage storage = open_res.value;
  ...
  if (PICOMESH_IS_ERR(load_res))
    return PICOMESH_ERR(picomesh_uint32,
                        "runner_agent_register: record load failed", load_res);
  if (load_res.value == 0)
    return PICOMESH_ERR(picomesh_uint32,
                        "runner_agent_register: unknown runner");
  if (strcmp(rec.status, "disabled") == 0)
    return PICOMESH_ERR(picomesh_uint32,
                        "runner_agent_register: runner is disabled");

  if (name && *name)
    snprintf(rec.name, sizeof(rec.name), "%s", name);
  if (labels)
    snprintf(rec.labels, sizeof(rec.labels), "%s", labels);
  ...
  if (PICOMESH_IS_ERR(store_res))
    return PICOMESH_ERR(picomesh_uint32,
                        "runner_agent_register: record write failed",
                        store_res);
...
PICOMESH_CLASS_ANNOTATE(
    "override@runner_agent:runner_agent:runner_agent_heartbeat")
struct picomesh_int_result runner_agent_runner_agent_heartbeat_impl(
    struct ctx *ctx, struct object *obj, struct yheaders *hdrs,
  ...
  if (!ra_caller_is(hdrs, runner_id))
    return PICOMESH_ERR(picomesh_int,
                        "runner_agent_heartbeat: caller is not this runner");
  struct ra_storage_result open_res = ra_open();
  if (PICOMESH_IS_ERR(open_res))
    return PICOMESH_ERR(
        picomesh_int, "runner_agent_heartbeat: storage open failed", open_res);
  struct ra_storage storage = open_res.value;
  ...
  if (PICOMESH_IS_ERR(load_res))
    return PICOMESH_ERR(picomesh_int,
                        "runner_agent_heartbeat: record load failed", load_res);
  if (load_res.value == 0)
    return PICOMESH_OK(picomesh_int, 0);
  if (strcmp(rec.status, "disabled") == 0)
    return PICOMESH_ERR(picomesh_int,
                        "runner_agent_heartbeat: runner is disabled");

  snprintf(rec.status, sizeof(rec.status), "%s",
  ...
  if (PICOMESH_IS_ERR(store_res))
    return PICOMESH_ERR(
        picomesh_int, "runner_agent_heartbeat: record write failed", store_res);
  return PICOMESH_OK(picomesh_int, 1);
}

PICOMESH_CLASS_ANNOTATE("override@runner_agent:runner_agent:runner_agent_get")
struct picomesh_json_result
runner_agent_runner_agent_get_impl(struct ctx *ctx, struct object *obj,
                                   struct yheaders *hdrs, uint32_t runner_id) {
  (void)ctx;
  (void)obj;
  struct ra_storage_result open_res = ra_open();
  if (PICOMESH_IS_ERR(open_res))
    return PICOMESH_ERR(picomesh_json, "runner_agent_get: storage open failed",
                        open_res);
  struct ra_storage storage = open_res.value;
  struct ra_record rec;
  struct picomesh_int_result load_res =
      ra_record_load(&storage, hdrs, runner_id, &rec);
  if (PICOMESH_IS_ERR(load_res))
    return PICOMESH_ERR(picomesh_json, "runner_agent_get: record load failed",
                        load_res);
  if (load_res.value == 0) {
    char *empty = strdup("{}");
    return empty ? PICOMESH_OK(picomesh_json, empty)
  ...
  struct json_writer *writer = json_writer_new();
  if (!writer)
    return PICOMESH_ERR(picomesh_json, "runner_agent_get: writer alloc failed");
  json_writer_begin_object(writer);
  json_writer_key(writer, "runner_id");
  json_writer_int(writer, (int64_t)runner_id);
  json_writer_key(writer, "name");
  ...
  json_writer_free(writer);
  if (!out)
    return PICOMESH_ERR(picomesh_json, "runner_agent_get: encode failed");
  return PICOMESH_OK(picomesh_json, out);
}

PICOMESH_CLASS_ANNOTATE("override@runner_agent:runner_agent:runner_agent_list")
struct picomesh_json_result
runner_agent_runner_agent_list_impl(struct ctx *ctx, struct object *obj,
  ...
  struct ra_storage_result open_res = ra_open();
  if (PICOMESH_IS_ERR(open_res))
    return PICOMESH_ERR(picomesh_json, "runner_agent_list: storage open failed",
                        open_res);
  struct ra_storage storage = open_res.value;
  return sharded_storage_db_list(&storage.c, storage.obj, hdrs, RA_CTX,
                                 "runner:", offset, limit);
}

PICOMESH_CLASS_ANNOTATE(
    "override@runner_agent:runner_agent:runner_agent_list_all")
struct picomesh_json_result
runner_agent_runner_agent_list_all_impl(struct ctx *ctx, struct object *obj,
                                        struct yheaders *hdrs) {
  (void)ctx;
  (void)obj;
  struct ra_storage_result open_res = ra_open();
  if (PICOMESH_IS_ERR(open_res))
    return PICOMESH_ERR(picomesh_json,
                        "runner_agent_list_all: storage open failed", open_res);
  struct ra_storage storage = open_res.value;
  return sharded_storage_db_list_all(&storage.c, storage.obj, hdrs, RA_CTX,
                                     "runner:");
}

PICOMESH_CLASS_ANNOTATE(
    "override@runner_agent:runner_agent:runner_agent_count_active")
struct picomesh_size_result
runner_agent_runner_agent_count_active_impl(struct ctx *ctx, struct object *obj,
                                            struct yheaders *hdrs) {
  (void)ctx;
  (void)obj;
  struct ra_storage_result open_res = ra_open();
  if (PICOMESH_IS_ERR(open_res))
    return PICOMESH_ERR(picomesh_size,
                        "runner_agent_count_active: storage open failed",
                        open_res);
  struct ra_storage storage = open_res.value;
  struct picomesh_int64_result count_res =
      ra_get_int(&storage, hdrs, "count", 0);
  if (PICOMESH_IS_ERR(count_res))
    return PICOMESH_ERR(picomesh_size, "runner_agent_count_active: read failed",
                        count_res);
