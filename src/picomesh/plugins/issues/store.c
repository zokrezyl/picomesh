...
static struct picomesh_void_result issues_open(struct rel_handle *handle) {
  struct picomesh_void_result open_res =
      rel_open(handle, ISSUES_STORE, ISSUES_DB);
  if (PICOMESH_IS_ERR(open_res))
    return PICOMESH_ERR(picomesh_void, "issues: open rstore_uid failed",
                        open_res);
...
is_caller_can(struct yheaders *hdrs, uint32_t repo_id, const char *min_role) {
  ...
  PICOMESH_RETURN_IF_ERR(picomesh_int, create_res,
                         "is_caller_can: repo create failed");
  ...
  PICOMESH_RETURN_IF_ERR(picomesh_int, namespace_res,
                         "is_caller_can: namespace_of failed");
  int allowed =
      namespace_res.value && namespace_res.value[0] &&
 ...
 */
static struct picomesh_int_result
issues_caller_site_admin(struct yheaders *hdrs) {
  struct picomesh_authctx caller;
  struct picomesh_void_result authctx_res =
      picomesh_authctx_from_headers(hdrs, picomesh_active_engine(), &caller);
  PICOMESH_RETURN_IF_ERR(picomesh_int, authctx_res,
                         "issues_caller_site_admin: authctx failed");
  if (!caller.authenticated)
    return PICOMESH_OK(picomesh_int, 0);
  if (picomesh_groups_contains(caller.groups_csv, PICOMESH_GROUP_SYSTEM))
    return PICOMESH_OK(picomesh_int, 1);
...
struct picomesh_uint32_result issues_issues_open_impl(struct ctx *ctx,
                                                      struct object *obj,
  ...
  struct picomesh_int_result open_authz =
      is_caller_can(hdrs, repo_id, "reporter");
  PICOMESH_RETURN_IF_ERR(picomesh_uint32, open_authz,
                         "issues_open: authz check failed");
  if (!open_authz.value)
    return PICOMESH_ERR(picomesh_uint32,
                        "issues_open: forbidden (insufficient namespace role)");
  ...
  if (PICOMESH_IS_ERR(open_res))
    return PICOMESH_ERR(picomesh_uint32, "issues_open: storage open failed",
                        open_res);
  ...
  if (!args_writer)
    return PICOMESH_ERR(picomesh_uint32, "issues_open: out of memory");
  ...
  struct picomesh_json_result insert_res = rel_exec(
      &handle, hdrs,
      "INSERT INTO issues(repo_id, number, status, author_uid, created_at) "
      "VALUES(?, (SELECT COALESCE(MAX(number),0)+1 FROM issues WHERE "
      "repo_id=?), 'open', ?, ?)",
      args);
  free(args);
  if (PICOMESH_IS_ERR(insert_res))
    return PICOMESH_ERR(picomesh_uint32, "issues_open: persist issue failed",
                        insert_res);
  struct json_doc *doc =
      json_parse(insert_res.value ? insert_res.value : "{}",
  ...
  if (issue_id <= 0)
    return PICOMESH_ERR(picomesh_uint32, "issues_open: no insert id");
  yinfo("issues: open id=%lld repo=%u by=%u", (long long)issue_id, repo_id,
        author_id);
  return PICOMESH_OK(picomesh_uint32, (uint32_t)issue_id);
}

PICOMESH_CLASS_ANNOTATE("override@issues:issues:issues_close")
struct picomesh_int_result issues_issues_close_impl(struct ctx *ctx,
                                                    struct object *obj,
                                                    struct yheaders *hdrs,
                                                    uint32_t issue_id) {
  (void)ctx;
  ...
  struct picomesh_void_result open_res = issues_open(&handle);
  if (PICOMESH_IS_ERR(open_res))
    return PICOMESH_ERR(picomesh_int, "issues_close: storage open failed",
                        open_res);
  ...
  struct picomesh_int64_result repo_res = rel_query_int_result(
      &handle, hdrs, "SELECT repo_id FROM issues WHERE id=?", load_args,
      "repo_id", 0);
  if (PICOMESH_IS_ERR(repo_res)) {
    free(load_args);
    return PICOMESH_ERR(picomesh_int, "issues_close: load failed", repo_res);
  }
  ...
  struct picomesh_int_result close_authz =
      is_caller_can(hdrs, repo_id, "reporter");
  PICOMESH_RETURN_IF_ERR(picomesh_int, close_authz,
                         "issues_close: authz check failed");
  if (!close_authz.value)
    return PICOMESH_ERR(
        picomesh_int, "issues_close: forbidden (insufficient namespace role)");
  ...
  struct picomesh_json_result update_res =
      rel_exec(&handle, hdrs,
               "UPDATE issues SET status='closed' WHERE id=? AND status='open'",
               close_args);
  free(close_args);
  if (PICOMESH_IS_ERR(update_res))
    return PICOMESH_ERR(picomesh_int, "issues_close: close update failed",
                        update_res);
  struct json_doc *doc =
      json_parse(update_res.value ? update_res.value : "{}",
                 update_res.value ? strlen(update_res.value) : 2);
  int64_t changes =
      doc ? json_as_int(json_object_get(json_doc_root(doc), "changes"), 0) : 0;
  if (doc)
    json_doc_free(doc);
...
struct picomesh_int_result issues_issues_status_impl(struct ctx *ctx,
                                                     struct object *obj,
                                                     struct yheaders *hdrs,
                                                     uint32_t issue_id) {
  (void)ctx;
  ...
  struct picomesh_void_result open_res = issues_open(&handle);
  if (PICOMESH_IS_ERR(open_res))
    return PICOMESH_ERR(picomesh_int, "issues_status: storage open failed",
                        open_res);

  char *args = rel_args1i((int64_t)issue_id);
  struct picomesh_json_result row_res = rel_query(
      &handle, hdrs, "SELECT repo_id, status FROM issues WHERE id=?", args);
  free(args);
  if (PICOMESH_IS_ERR(row_res))
    return PICOMESH_ERR(picomesh_int, "issues_status: load failed", row_res);
  ...
  struct picomesh_int_result status_authz =
      is_caller_can(hdrs, repo_id, "reporter");
  PICOMESH_RETURN_IF_ERR(picomesh_int, status_authz,
                         "issues_status: authz check failed");
  if (!status_authz.value)
    return PICOMESH_ERR(
        picomesh_int, "issues_status: forbidden (insufficient namespace role)");
...
issues_issues_count_open_in_repo_impl(struct ctx *ctx, struct object *obj,
                                      struct yheaders *hdrs, uint32_t repo_id) {
  (void)ctx;
  (void)obj;
  /* The open-issue count is repo-scoped: reporter+ on the repo's namespace. */
  struct picomesh_int_result count_authz =
      is_caller_can(hdrs, repo_id, "reporter");
  PICOMESH_RETURN_IF_ERR(picomesh_size, count_authz,
                         "issues_count_open: authz check failed");
  if (!count_authz.value)
    return PICOMESH_ERR(
        picomesh_size,
        "issues_count_open: forbidden (insufficient namespace role)");
  struct rel_handle handle;
  struct picomesh_void_result open_res = issues_open(&handle);
  if (PICOMESH_IS_ERR(open_res))
    return PICOMESH_ERR(picomesh_size, "issues_count_open: storage open failed",
                        open_res);

  char *args = rel_args1i((int64_t)repo_id);
  struct picomesh_int64_result count_res = rel_query_int_result(
      &handle, hdrs,
      "SELECT COUNT(*) AS n FROM issues WHERE repo_id=? AND status='open'",
      args, "n", 0);
  free(args);
  if (PICOMESH_IS_ERR(count_res))
    return PICOMESH_ERR(picomesh_size, "issues_count_open: read failed",
                        count_res);
...
struct picomesh_size_result
issues_issues_count_total_impl(struct ctx *ctx, struct object *obj,
                               struct yheaders *hdrs) {
  (void)ctx;
  (void)obj;
  struct rel_handle handle;
  ...
  if (PICOMESH_IS_ERR(open_res))
    return PICOMESH_ERR(picomesh_size,
                        "issues_count_total: storage open failed", open_res);
  struct picomesh_int64_result count_res = rel_query_int_result(
      &handle, hdrs, "SELECT COUNT(*) AS n FROM issues", "[]", "n", 0);
  if (PICOMESH_IS_ERR(count_res))
    return PICOMESH_ERR(picomesh_size, "issues_count_total: read failed",
                        count_res);
  return PICOMESH_OK(picomesh_size,
                     (size_t)(count_res.value < 0 ? 0 : count_res.value));
}
...
issues_issues_list_impl(struct ctx *ctx, struct object *obj,
  ...
  PICOMESH_RETURN_IF_ERR(picomesh_json, list_admin,
                         "issues_list: authz check failed");
  if (!list_admin.value)
    return PICOMESH_ERR(picomesh_json,
                        "issues_list: forbidden (site admin only)");
  struct rel_handle handle;
  struct picomesh_void_result open_res = issues_open(&handle);
  if (PICOMESH_IS_ERR(open_res))
    return PICOMESH_ERR(picomesh_json, "issues_list: storage open failed",
                        open_res);
  return rel_query_page(
      &handle, hdrs,
      "SELECT id, repo_id, number, status, author_uid, created_at FROM issues",
      "[]", "id", 0, offset, limit);
}
...
struct picomesh_json_result issues_issues_list_all_impl(struct ctx *ctx,
                                                        struct object *obj,
                                                        struct yheaders *hdrs) {
  (void)ctx;
  (void)obj;
  ...
  PICOMESH_RETURN_IF_ERR(picomesh_json, list_all_admin,
                         "issues_list_all: authz check failed");
  if (!list_all_admin.value)
    return PICOMESH_ERR(picomesh_json,
                        "issues_list_all: forbidden (site admin only)");
  struct rel_handle handle;
  struct picomesh_void_result open_res = issues_open(&handle);
  if (PICOMESH_IS_ERR(open_res))
    return PICOMESH_ERR(picomesh_json, "issues_list_all: storage open failed",
                        open_res);
  return rel_query_page(
      &handle, hdrs,
      "SELECT id, repo_id, number, status, author_uid, created_at FROM issues",
...
struct picomesh_uint32_result issues_issues_repo_of_impl(struct ctx *ctx,
                                                         struct object *obj,
                                                         struct yheaders *hdrs,
                                                         uint32_t issue_id) {
  (void)ctx;
  ...
  struct picomesh_void_result open_res = issues_open(&handle);
  if (PICOMESH_IS_ERR(open_res))
    return PICOMESH_ERR(picomesh_uint32, "issues_repo_of: storage open failed",
                        open_res);
  char *args = rel_args1i((int64_t)issue_id);
  struct picomesh_int64_result repo_res = rel_query_int_result(
      &handle, hdrs, "SELECT repo_id FROM issues WHERE id=?", args, "repo_id",
      0);
  free(args);
  if (PICOMESH_IS_ERR(repo_res))
    return PICOMESH_ERR(picomesh_uint32, "issues_repo_of: read failed",
                        repo_res);
