    ...
    if (_s && _s->peer) {
         ...
         * this client span's id as parent_span_id across the serialize. */
        {
            ...
            if (_hn == 0) {
                ...
                return PICOMESH_ERR(picomesh_uint32, "issues_issues_repo_of: header serialize overflow");
            }
            _off = _hn;
        }
        {
            uint64_t _h = *(uint64_t *)((char *)obj + sizeof(*obj));
            if (_off + 8 > sizeof(_a))
                { ytelemetry_span_end(&_tsp, 0, "issues_issues_repo_of: pack overflow"); return PICOMESH_ERR(picomesh_uint32, "issues_issues_repo_of: pack overflow"); }
            memcpy(_a + _off, &_h, 8); _off += 8;
        }
        if (_off + sizeof(issue_id) > sizeof(_a))
            { ytelemetry_span_end(&_tsp, 0, "issues_issues_repo_of: pack overflow"); return PICOMESH_ERR(picomesh_uint32, "issues_issues_repo_of: pack overflow"); }
        ...
        size_t _wn = rpc_call(_s->peer, RPC_OP_CALL, _rid, _a, _off,
                              _wbuf, sizeof(_wbuf));
        ytelemetry_span_end(&_tsp, _wn >= 1 && _wbuf[0] == 0, NULL);
        if (_wn < 1) return PICOMESH_ERR(picomesh_uint32, "issues_issues_repo_of: short RPC response");
        if (_wbuf[0] != 0) {
            uint32_t _msg_len = 0;
            if (_wn >= 5) memcpy(&_msg_len, _wbuf + 1, 4);
            char _msg[8193];
            ...
            if (_wn >= 5 + _copy) memcpy(_msg, _wbuf + 5, _copy);
            _msg[_copy] = 0;
            return PICOMESH_ERR(picomesh_uint32, _msg[0] ? strdup(_msg) : "issues_issues_repo_of: remote error (no msg)");
        }
        if (_wn != 1 + sizeof(uint32_t)) return PICOMESH_ERR(picomesh_uint32, "issues_issues_repo_of: truncated RPC payload");
        uint32_t _v;
        memcpy(&_v, _wbuf + 1, sizeof(_v));
        return PICOMESH_OK(picomesh_uint32, _v);
    } else {
