...
struct picomesh_uint32_result issues_store_open(struct ctx * ctx, struct object * obj, struct yheaders * hdrs, uint32_t repo_id, uint32_t author_id)
{
    ...
    if (_slot == METHOD_SLOT_UNDEFINED) {
        ...
        if (PICOMESH_IS_ERR(_sr))
            return PICOMESH_ERR(picomesh_uint32, "issues_store_open: method_slot_get failed", _sr);
        _slot = _sr.value;
    }

    if (!obj) return PICOMESH_ERR(picomesh_uint32, "issues_store_open: NULL object");
    ...
    if (_s && _s->peer) {
        ...
        if (_wbuf[0] != 0) {
            ...
            _msg[_copy] = 0;
            return PICOMESH_ERR(picomesh_uint32, _msg[0] ? strdup(_msg) : "issues_store_open: remote error (no msg)");
        }
        if (_wn != 1 + sizeof(uint32_t)) return PICOMESH_ERR(picomesh_uint32, "issues_store_open: truncated RPC payload");
        uint32_t _v;
        memcpy(&_v, _wbuf + 1, sizeof(_v));
        return PICOMESH_OK(picomesh_uint32, _v);
    } else {
        impl_t fn = class_dispatch_lookup(object_class(obj), _slot);
        if (!fn) return PICOMESH_ERR(picomesh_uint32, "issues_store_open: no impl on this class");
...
struct picomesh_int_result issues_store_close(struct ctx * ctx, struct object * obj, struct yheaders * hdrs, uint32_t issue_id)
{
    ...
    if (_slot == METHOD_SLOT_UNDEFINED) {
        ...
        if (PICOMESH_IS_ERR(_sr))
            return PICOMESH_ERR(picomesh_int, "issues_store_close: method_slot_get failed", _sr);
        _slot = _sr.value;
    }

    if (!obj) return PICOMESH_ERR(picomesh_int, "issues_store_close: NULL object");
    ...
    if (_s && _s->peer) {
        ...
        if (_wbuf[0] != 0) {
            ...
            if (_wn >= 5 + _copy) memcpy(_msg, _wbuf + 5, _copy);
            _msg[_copy] = 0;
            return PICOMESH_ERR(picomesh_int, _msg[0] ? strdup(_msg) : "issues_store_close: remote error (no msg)");
        }
        if (_wn != 1 + sizeof(int)) return PICOMESH_ERR(picomesh_int, "issues_store_close: truncated RPC payload");
        int _v;
        memcpy(&_v, _wbuf + 1, sizeof(_v));
        return PICOMESH_OK(picomesh_int, _v);
    } else {
        impl_t fn = class_dispatch_lookup(object_class(obj), _slot);
        if (!fn) return PICOMESH_ERR(picomesh_int, "issues_store_close: no impl on this class");
...
struct picomesh_int_result issues_store_status(struct ctx * ctx, struct object * obj, struct yheaders * hdrs, uint32_t issue_id)
{
    ...
    if (_slot == METHOD_SLOT_UNDEFINED) {
        ...
        if (PICOMESH_IS_ERR(_sr))
            return PICOMESH_ERR(picomesh_int, "issues_store_status: method_slot_get failed", _sr);
        _slot = _sr.value;
    }

    if (!obj) return PICOMESH_ERR(picomesh_int, "issues_store_status: NULL object");
    ...
    if (_s && _s->peer) {
        ...
        if (_wbuf[0] != 0) {
            ...
            if (_wn >= 5 + _copy) memcpy(_msg, _wbuf + 5, _copy);
            _msg[_copy] = 0;
            return PICOMESH_ERR(picomesh_int, _msg[0] ? strdup(_msg) : "issues_store_status: remote error (no msg)");
        }
        if (_wn != 1 + sizeof(int)) return PICOMESH_ERR(picomesh_int, "issues_store_status: truncated RPC payload");
        int _v;
        memcpy(&_v, _wbuf + 1, sizeof(_v));
        return PICOMESH_OK(picomesh_int, _v);
    } else {
        impl_t fn = class_dispatch_lookup(object_class(obj), _slot);
        if (!fn) return PICOMESH_ERR(picomesh_int, "issues_store_status: no impl on this class");
...
struct picomesh_size_result issues_store_count_open_in_repo(struct ctx * ctx, struct object * obj, struct yheaders * hdrs, uint32_t repo_id)
{
    ...
    if (_slot == METHOD_SLOT_UNDEFINED) {
        ...
        if (PICOMESH_IS_ERR(_sr))
            return PICOMESH_ERR(picomesh_size, "issues_store_count_open_in_repo: method_slot_get failed", _sr);
        _slot = _sr.value;
    }

    if (!obj) return PICOMESH_ERR(picomesh_size, "issues_store_count_open_in_repo: NULL object");
    ...
    if (_s && _s->peer) {
        ...
        if (_wbuf[0] != 0) {
            ...
            if (_wn >= 5 + _copy) memcpy(_msg, _wbuf + 5, _copy);
            _msg[_copy] = 0;
            return PICOMESH_ERR(picomesh_size, _msg[0] ? strdup(_msg) : "issues_store_count_open_in_repo: remote error (no msg)");
        }
        if (_wn != 1 + sizeof(size_t)) return PICOMESH_ERR(picomesh_size, "issues_store_count_open_in_repo: truncated RPC payload");
        size_t _v;
        memcpy(&_v, _wbuf + 1, sizeof(_v));
        return PICOMESH_OK(picomesh_size, _v);
    } else {
        impl_t fn = class_dispatch_lookup(object_class(obj), _slot);
        if (!fn) return PICOMESH_ERR(picomesh_size, "issues_store_count_open_in_repo: no impl on this class");
...
struct picomesh_size_result issues_store_count_total(struct ctx * ctx, struct object * obj, struct yheaders * hdrs)
{
    ...
    if (_slot == METHOD_SLOT_UNDEFINED) {
        ...
        if (PICOMESH_IS_ERR(_sr))
            return PICOMESH_ERR(picomesh_size, "issues_store_count_total: method_slot_get failed", _sr);
        _slot = _sr.value;
    }

    if (!obj) return PICOMESH_ERR(picomesh_size, "issues_store_count_total: NULL object");
    ...
    if (_s && _s->peer) {
        ...
        if (_wbuf[0] != 0) {
            ...
            if (_wn >= 5 + _copy) memcpy(_msg, _wbuf + 5, _copy);
            _msg[_copy] = 0;
            return PICOMESH_ERR(picomesh_size, _msg[0] ? strdup(_msg) : "issues_store_count_total: remote error (no msg)");
        }
        if (_wn != 1 + sizeof(size_t)) return PICOMESH_ERR(picomesh_size, "issues_store_count_total: truncated RPC payload");
        size_t _v;
        memcpy(&_v, _wbuf + 1, sizeof(_v));
        return PICOMESH_OK(picomesh_size, _v);
    } else {
        impl_t fn = class_dispatch_lookup(object_class(obj), _slot);
        if (!fn) return PICOMESH_ERR(picomesh_size, "issues_store_count_total: no impl on this class");
