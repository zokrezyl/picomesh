...
struct PICOMESH_CLASS_ANNOTATE("class@github_authn:github_authn")
    github_authn_github_authn_data {
  uint32_t client_id;
...
static struct github_authn_github_authn_data *gh(struct object *obj) {
  return (struct github_authn_github_authn_data *)((char *)obj +
                                                   sizeof(struct object));
}

static struct const_char_ptr_result cfg_string(const char *path,
                                               const char *fallback) {
  struct picomesh_engine *engine = picomesh_active_engine();
  if (!engine)
    return PICOMESH_OK(const_char_ptr, fallback);
  ...
  PICOMESH_RETURN_IF_ERR(const_char_ptr, config_res,
                         "cfg_string: config read failed");
  const char *out = config_res.value
                        ? config_node_as_string(config_res.value, fallback)
                        : fallback;
...
static size_t gh_http_write(char *ptr, size_t size, size_t nmemb,
                            void *userdata) {
  struct gh_http_buf *buf = userdata;
  size_t add = size * nmemb;
  if (buf->len + add + 1 < buf->len)
    return 0; /* size_t wrap */
  if (buf->len + add > 4u * 1024 * 1024)
    return 0; /* 4 MiB ceiling */
...
github_token_exchange(const char *code, const char *redirect_uri) {
  struct const_char_ptr_result client_res =
      cfg_string("github_authn.client_id", getenv("PICOFORGE_GITHUB_CLIENT"));
  PICOMESH_RETURN_IF_ERR(picomesh_string, client_res,
                         "github_token_exchange: client_id config");
  struct const_char_ptr_result secret_res = cfg_string(
      "github_authn.client_secret", getenv("PICOFORGE_GITHUB_SECRET"));
  PICOMESH_RETURN_IF_ERR(picomesh_string, secret_res,
                         "github_token_exchange: client_secret config");
  struct const_char_ptr_result url_res = cfg_string(
      "github_authn.oauth_url", "https://github.com/login/oauth/access_token");
  PICOMESH_RETURN_IF_ERR(picomesh_string, url_res,
                         "github_token_exchange: oauth_url config");
  const char *client = client_res.value, *secret = secret_res.value,
             *url = url_res.value;
  ...
  if (!enc_client || !enc_secret || !enc_code || !enc_redirect)
    goto done;
  size_t need = strlen(enc_client) + strlen(enc_secret) + strlen(enc_code) +
                strlen(enc_redirect) + 64;
  body = malloc(need);
  if (!body)
    goto done;
...
static struct picomesh_string_result
github_user_json(const char *access_token) {
  struct const_char_ptr_result api_res =
      cfg_string("github_authn.api_url", "https://api.github.com");
  PICOMESH_RETURN_IF_ERR(picomesh_string, api_res,
                         "github_user_json: api_url config");
 ...
 * the reverse index `uid:<uid>:github_id` — the row the caller's collision
 * guard reads — is claimed FIRST via put_if_absent (an atomic winner-election).
 * This closes the partial-failure window that a plain ordered set sequence had:
 * if a prior attempt wrote the forward `github:<id>:*` rows but died before the
 ...
 *   - a different identity is rejected here, not just at the caller's read
 * guard. The forward rows are then (re)written unconditionally — safe because
 * the uid is already pinned to this github_id, so a retry merely refreshes
 * them. */
static struct picomesh_void_result
store_mapping(struct yheaders *hdrs, uint32_t uid, int64_t github_id,
  ...
  if (!engine || !access_token || !*access_token || github_id <= 0)
    return PICOMESH_ERR(picomesh_void,
                        "store_mapping: bad args or no active engine");
  struct ctx storage_ctx =
      picomesh_engine_service_ctx(engine, "sharded_storage");
  struct object_ptr_result db_handle_res =
      sharded_storage_db_create(&storage_ctx);
  if (PICOMESH_IS_ERR(db_handle_res))
    return PICOMESH_ERR(picomesh_void,
                        "store_mapping: storage handle create failed",
                        db_handle_res);
  ...
  struct picomesh_int_result claim_res = sharded_storage_db_put_if_absent(
      &storage_ctx, db_handle, hdrs, "github_authn", key, value);
  if (PICOMESH_IS_ERR(claim_res)) {
    result = PICOMESH_ERR(
        picomesh_void, "store_mapping: reverse-index claim failed", claim_res);
    goto done;
  }
  if (claim_res.value == 0) {
    ...
    struct picomesh_string_result cur = sharded_storage_db_get(
        &storage_ctx, db_handle, hdrs, "github_authn", key);
    if (PICOMESH_IS_ERR(cur)) {
      result = PICOMESH_ERR(
          picomesh_void, "store_mapping: reverse-index read-back failed", cur);
      goto done;
    }
    int64_t bound =
        (cur.value && *cur.value) ? strtoll(cur.value, NULL, 10) : 0;
    free(cur.value);
    if (bound != github_id) {
      result = PICOMESH_ERR(
          picomesh_void,
  ...
  set_res = sharded_storage_db_set(&storage_ctx, db_handle, hdrs,
                                   "github_authn", key, access_token);
  if (PICOMESH_IS_ERR(set_res)) {
    result = PICOMESH_ERR(picomesh_void,
                          "store_mapping: github token write failed", set_res);
    goto done;
  }
  snprintf(key, sizeof(key), "github:%lld:uid", (long long)github_id);
  snprintf(value, sizeof(value), "%u", uid);
  set_res = sharded_storage_db_set(&storage_ctx, db_handle, hdrs,
                                   "github_authn", key, value);
  if (PICOMESH_IS_ERR(set_res)) {
    result = PICOMESH_ERR(
        picomesh_void, "store_mapping: github_id->uid write failed", set_res);
    goto done;
  }
  snprintf(key, sizeof(key), "uid:%u:username", uid);
  set_res =
      sharded_storage_db_set(&storage_ctx, db_handle, hdrs, "github_authn", key,
                             username ? username : "");
  if (PICOMESH_IS_ERR(set_res)) {
    result = PICOMESH_ERR(picomesh_void,
                          "store_mapping: uid->username write failed", set_res);
    goto done;
  }
...
gh_lookup_uid_by_github_id(struct yheaders *hdrs, int64_t github_id) {
  struct picomesh_engine *engine = picomesh_active_engine();
  if (!engine)
    return PICOMESH_ERR(picomesh_int64,
                        "gh_lookup_uid_by_github_id: no active engine");
  struct ctx service_ctx =
      picomesh_engine_service_ctx(engine, "sharded_storage");
  struct object_ptr_result object_res = sharded_storage_db_create(&service_ctx);
  PICOMESH_RETURN_IF_ERR(picomesh_int64, object_res,
                         "gh_lookup_uid_by_github_id: storage create failed");
  char key[96];
  snprintf(key, sizeof(key), "github:%lld:uid", (long long)github_id);
  struct picomesh_string_result get_res = sharded_storage_db_get(
      &service_ctx, object_res.value, hdrs, "github_authn", key);
  object_release_in_ctx(&service_ctx, object_res.value);
  PICOMESH_RETURN_IF_ERR(picomesh_int64, get_res,
                         "gh_lookup_uid_by_github_id: storage get failed");
  int64_t uid =
      (get_res.value && *get_res.value) ? strtoll(get_res.value, NULL, 10) : 0;
  free(get_res.value);
...
static struct picomesh_int64_result gh_uid_for_username(const char *username) {
  ...
  if (!engine)
    return PICOMESH_ERR(picomesh_int64,
                        "gh_uid_for_username: no active engine");
  ...
  PICOMESH_RETURN_IF_ERR(picomesh_int64, object_res,
                         "gh_uid_for_username: accounts create failed");
  struct picomesh_int64_result uid_res = accounts_accounts_uid_for_username(
      &service_ctx, object_res.value, NULL, username);
  object_release_in_ctx(&service_ctx, object_res.value);
  return uid_res;
}
...
static struct picomesh_int64_result gh_allocate_uid(void) {
  struct picomesh_engine *engine = picomesh_active_engine();
  if (!engine)
    return PICOMESH_ERR(picomesh_int64, "gh_allocate_uid: no active engine");
  struct ctx service_ctx = picomesh_engine_service_ctx(engine, "accounts");
  struct object_ptr_result object_res = accounts_accounts_create(&service_ctx);
  PICOMESH_RETURN_IF_ERR(picomesh_int64, object_res,
                         "gh_allocate_uid: accounts create failed");
  struct picomesh_int64_result uid_res =
      accounts_accounts_allocate_uid(&service_ctx, object_res.value, NULL);
  object_release_in_ctx(&service_ctx, object_res.value);
  return uid_res;
}

PICOMESH_CLASS_ANNOTATE(
    "override@github_authn:github_authn:github_authn_exchange_code")
struct picomesh_json_result github_authn_github_authn_exchange_code_impl(
  ...
  if (!code || !*code || !redirect_uri || !*redirect_uri)
    return PICOMESH_ERR(
        picomesh_json,
        "github_authn_exchange_code: missing code or redirect_uri");
  struct picomesh_string_result token_res =
      github_token_exchange(code, redirect_uri);
  PICOMESH_RETURN_IF_ERR(picomesh_json, token_res,
                         "github_authn_exchange_code: token exchange failed");
  char *token = token_res.value;
  if (!token)
    return PICOMESH_ERR(picomesh_json,
                        "github_authn_exchange_code: token exchange failed");
  struct picomesh_string_result user_res = github_user_json(token);
  if (PICOMESH_IS_ERR(user_res)) {
    free(token);
    return PICOMESH_ERR(picomesh_json,
                        "github_authn_exchange_code: /user request failed",
                        user_res);
  }
  ...
  if (!doc) {
    free(token);
    return PICOMESH_ERR(picomesh_json,
                        "github_authn_exchange_code: bad /user JSON");
  }
  ...
  json_doc_free(doc);
  if (github_id <= 0 || !username[0]) {
    free(token);
    return PICOMESH_ERR(picomesh_json,
                        "github_authn_exchange_code: missing GitHub id/login");
  }
  ...
  uint32_t uid = 0;
  struct picomesh_int64_result bound_uid_res =
      gh_lookup_uid_by_github_id(hdrs, github_id);
  if (PICOMESH_IS_ERR(bound_uid_res)) {
    free(token);
    return PICOMESH_ERR(picomesh_json,
                        "github_authn_exchange_code: identity lookup failed",
                        bound_uid_res);
  }
  if (bound_uid_res.value > 0) {
    ...
    if (PICOMESH_IS_ERR(named_res)) {
      free(token);
      return PICOMESH_ERR(picomesh_json,
                          "github_authn_exchange_code: account lookup failed",
                          named_res);
    }
    if (named_res.value > 0) {
      ywarn("github_authn: refusing to bind GitHub id=%lld — username '%s' "
            "already held by a non-GitHub account (uid=%lld)",
            (long long)github_id, username, (long long)named_res.value);
      free(token);
      return PICOMESH_ERR(picomesh_json,
                          "github_authn_exchange_code: an account with this "
                          "name already exists");
    }
    ...
    if (PICOMESH_IS_ERR(alloc_res)) {
      free(token);
      return PICOMESH_ERR(picomesh_json,
                          "github_authn_exchange_code: uid allocation failed",
                          alloc_res);
    }
  ...
  struct picomesh_void_result map_res =
  ...
  if (!w)
    return PICOMESH_ERR(picomesh_json,
                        "github_authn_exchange_code: writer alloc failed");
  json_writer_begin_object(w);
  json_writer_key(w, "uid");
  json_writer_int(w, (int64_t)uid);
  json_writer_key(w, "username");
  json_writer_string(w, username);
  json_writer_key(w, "github_id");
  json_writer_int(w, github_id);
  ...
  if (!out)
    return PICOMESH_ERR(picomesh_json,
                        "github_authn_exchange_code: strdup failed");
  yinfo("github_authn: exchanged GitHub OAuth code for user=%s id=%lld",
        username, (long long)github_id);
  return PICOMESH_OK(picomesh_json, out);
}

PICOMESH_CLASS_ANNOTATE(
    "override@github_authn:github_authn:github_authn_set_credentials")
struct picomesh_int_result github_authn_github_authn_set_credentials_impl(
    struct ctx *ctx, struct object *obj, struct yheaders *hdrs,
...
PICOMESH_CLASS_ANNOTATE(
    "override@github_authn:github_authn:github_authn_register_code")
struct picomesh_int_result github_authn_github_authn_register_code_impl(
    struct ctx *ctx, struct object *obj, struct yheaders *hdrs, uint32_t code,
    uint32_t user_id) {
  (void)ctx;
  struct github_authn_github_authn_data *d = gh(obj);
...
PICOMESH_CLASS_ANNOTATE(
    "override@github_authn:github_authn:github_authn_resolve")
struct picomesh_uint32_result
github_authn_github_authn_resolve_impl(struct ctx *ctx, struct object *obj,
...
PICOMESH_CLASS_ANNOTATE(
    "override@github_authn:github_authn:github_authn_count_codes")
struct picomesh_size_result
github_authn_github_authn_count_codes_impl(struct ctx *ctx, struct object *obj,
...
github_authn_list_window(struct object *obj, int64_t offset, int64_t limit) {
  struct github_authn_github_authn_data *d = gh(obj);
  struct json_writer *w = json_writer_new();
  if (!w)
    return PICOMESH_ERR(picomesh_json,
                        "github_authn_list: writer alloc failed");
  ...
  for (size_t i = 0; i < GH_MAX_CODES && (limit < 0 || emitted < limit); ++i) {
    ...
    json_writer_begin_object(w);
    json_writer_key(w, "code");
    json_writer_int(w, (int64_t)d->codes[i].code);
    json_writer_key(w, "user_id");
    json_writer_int(w, (int64_t)d->codes[i].user_id);
    json_writer_end_object(w);
    ++emitted;
  }
  ...
  json_writer_free(w);
  if (!out)
    return PICOMESH_ERR(picomesh_json, "github_authn_list: strdup failed");
  return PICOMESH_OK(picomesh_json, out);
}
...
github_authn_github_authn_list_impl(struct ctx *ctx, struct object *obj,
...
PICOMESH_CLASS_ANNOTATE(
    "override@github_authn:github_authn:github_authn_list_all")
struct picomesh_json_result
github_authn_github_authn_list_all_impl(struct ctx *ctx, struct object *obj,
                                        struct yheaders *hdrs) {
  (void)ctx;
  (void)hdrs;
