/* GENERATED — do not edit. */
#include "git_repo.internal.h"

__attribute__((unused))
static git_repo_store_make_fn _git_repo_store_git_repo_store_make_check = git_repo_store_make_impl;
__attribute__((unused))
static git_repo_store_delete_fn _git_repo_store_git_repo_store_delete_check = git_repo_store_delete_impl;
__attribute__((unused))
static git_repo_store_owner_of_fn _git_repo_store_git_repo_store_owner_of_check = git_repo_store_owner_of_impl;
__attribute__((unused))
static git_repo_store_count_for_owner_fn _git_repo_store_git_repo_store_count_for_owner_check = git_repo_store_count_for_owner_impl;
__attribute__((unused))
static git_repo_store_count_total_fn _git_repo_store_git_repo_store_count_total_check = git_repo_store_count_total_impl;
__attribute__((unused))
static git_repo_store_list_for_owner_fn _git_repo_store_git_repo_store_list_for_owner_check = git_repo_store_list_for_owner_impl;
__attribute__((unused))
static git_repo_store_read_tree_fn _git_repo_store_git_repo_store_read_tree_check = git_repo_store_read_tree_impl;
__attribute__((unused))
static git_repo_store_read_file_fn _git_repo_store_git_repo_store_read_file_check = git_repo_store_read_file_impl;
__attribute__((unused))
static git_repo_store_put_file_fn _git_repo_store_git_repo_store_put_file_check = git_repo_store_put_file_impl;
__attribute__((unused))
static git_repo_store_is_public_fn _git_repo_store_git_repo_store_is_public_check = git_repo_store_is_public_impl;
__attribute__((unused))
static git_repo_store_set_public_fn _git_repo_store_git_repo_store_set_public_check = git_repo_store_set_public_impl;

struct class_ptr_result git_repo_store_class_get(void)
{
    static const struct class *cls = NULL;
    if (cls) return PICOMESH_OK(class_ptr, cls);
    ydebug("registering class=git_repo_store");

    static const struct class_descriptor desc = {
        .name = "git_repo_store",
        .type = CLASS_TYPE_REGULAR,
        .data_size = sizeof(struct git_repo_store_data),
    };
    static const struct op ops[] = {
        {"git_repo", "store_make", (method_id_t)git_repo_store_make, (impl_t)git_repo_store_make_impl},
        {"git_repo", "store_delete", (method_id_t)git_repo_store_delete, (impl_t)git_repo_store_delete_impl},
        {"git_repo", "store_owner_of", (method_id_t)git_repo_store_owner_of, (impl_t)git_repo_store_owner_of_impl},
        {"git_repo", "store_count_for_owner", (method_id_t)git_repo_store_count_for_owner, (impl_t)git_repo_store_count_for_owner_impl},
        {"git_repo", "store_count_total", (method_id_t)git_repo_store_count_total, (impl_t)git_repo_store_count_total_impl},
        {"git_repo", "store_list_for_owner", (method_id_t)git_repo_store_list_for_owner, (impl_t)git_repo_store_list_for_owner_impl},
        {"git_repo", "store_read_tree", (method_id_t)git_repo_store_read_tree, (impl_t)git_repo_store_read_tree_impl},
        {"git_repo", "store_read_file", (method_id_t)git_repo_store_read_file, (impl_t)git_repo_store_read_file_impl},
        {"git_repo", "store_put_file", (method_id_t)git_repo_store_put_file, (impl_t)git_repo_store_put_file_impl},
        {"git_repo", "store_is_public", (method_id_t)git_repo_store_is_public, (impl_t)git_repo_store_is_public_impl},
        {"git_repo", "store_set_public", (method_id_t)git_repo_store_set_public, (impl_t)git_repo_store_set_public_impl},
    };
    struct class_ptr_result _r =
        class_register(&desc, ops, sizeof(ops) / sizeof(ops[0]),
                       NULL, NULL, 0);
    if (PICOMESH_IS_ERR(_r))
        return PICOMESH_ERR(class_ptr, "git_repo_store_class_get: class_register failed", _r);
    cls = _r.value;
    return _r;
}
