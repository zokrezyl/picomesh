 ...
 *   repo:<rid>        →
 * "<owner_id>\t<owner_name>\t<repo_name>\t<is_public>\t<namespace_id>" count →
 * total live repos (decimal) owner:<owner_id>  → newline-separated repo names
 * that uid owns
 *
...
struct PICOMESH_CLASS_ANNOTATE("class@git_repo:git_repo")
    git_repo_git_repo_data {
  char _unused;
};
...
static struct gr_storage_result gr_open(void) {
  ...
  struct picomesh_void_result open_res =
      rel_open(&storage.h, GIT_REPO_STORE, GIT_REPO_DB);
  if (PICOMESH_IS_ERR(open_res))
    return PICOMESH_ERR(gr_storage, "git_repo: relational open failed",
                        open_res);
  return PICOMESH_OK(gr_storage, storage);
}

/* Resolve the owning namespace PATH to its canonical id via the accounts
 * service (issue #30). The namespaces table is the authority: a repo can only
 * be created under a namespace that already exists. Returns the id (>0) on
 * success, 0 when no such namespace exists; a backend error propagates. */
static struct picomesh_int64_result gr_resolve_namespace(struct yheaders *hdrs,
                                                         const char *path) {
  struct picomesh_engine *engine = picomesh_active_engine();
  if (!engine)
    return PICOMESH_ERR(picomesh_int64,
                        "git_repo: no active engine for namespace resolve");
  ...
  if (PICOMESH_IS_ERR(create_res))
    return PICOMESH_ERR(picomesh_int64, "git_repo: accounts unreachable",
                        create_res);
  return accounts_accounts_ns_resolve(&accounts_ctx, create_res.value, hdrs,
                                      path);
}
 ...
 * such repo". */
static struct picomesh_int_result repo_load(struct gr_storage *storage,
                                            struct yheaders *hdrs, uint32_t rid,
                                            struct repo_rec *out) {
  storage->h.shard = rid;
  char *qa = rel_args1i((int64_t)rid);
  struct picomesh_json_result q =
      rel_query(&storage->h, hdrs,
                "SELECT namespace_id, namespace_path, name, owner_uid, "
                "visibility FROM repos WHERE id=?",
                qa);
  free(qa);
  if (PICOMESH_IS_ERR(q))
    return PICOMESH_ERR(picomesh_int, "git_repo: repo read failed", q);
  struct json_doc *doc = q.value ? json_parse(q.value, strlen(q.value)) : NULL;
  free(q.value);
  if (!doc)
    return PICOMESH_ERR(picomesh_int, "git_repo: repo row parse failed");
  ...
  out->namespace_id =
      (uint32_t)json_as_int(json_object_get(row, "namespace_id"), 0);
  snprintf(out->owner_name, sizeof(out->owner_name), "%s",
           json_as_string(json_object_get(row, "namespace_path"), ""));
  snprintf(out->repo_name, sizeof(out->repo_name), "%s",
           json_as_string(json_object_get(row, "name"), ""));
  out->owner_id = (uint32_t)json_as_int(json_object_get(row, "owner_uid"), 0);
  {
    const char *vis =
        json_as_string(json_object_get(row, "visibility"), "private");
    out->is_public = (vis && strcmp(vis, "public") == 0) ? 1 : 0;
  }
  if (!out->namespace_id)
    out->namespace_id = picomesh_fnv1a32(out->owner_name);
  json_doc_free(doc);
...
repo_caller_has_role(struct yheaders *hdrs, const char *ns_path,
  ...
  PICOMESH_RETURN_IF_ERR(picomesh_int, authctx_res,
                         "repo_caller_has_role: authctx failed");
  if (!caller.authenticated)
    return PICOMESH_OK(picomesh_int, 0); /* fail closed */
  if (picomesh_groups_contains(caller.groups_csv, PICOMESH_GROUP_SYSTEM))
    return PICOMESH_OK(picomesh_int, 1); /* trusted internal */
  if (picomesh_groups_max_role(caller.groups_csv, "site") >=
      picomesh_role_rank("maintainer"))
    return PICOMESH_OK(picomesh_int, 1);
  return PICOMESH_OK(picomesh_int, picomesh_groups_effective_role(
                                       caller.groups_csv, ns_path) >=
                                           picomesh_role_rank(required_role)
                                       ? 1
                                       : 0);
}
...
static struct picomesh_int_result repo_caller_may_list(struct yheaders *hdrs,
                                                       uint32_t owner_id) {
  struct picomesh_authctx caller;
  struct picomesh_void_result authctx_res =
      picomesh_authctx_from_headers(hdrs, picomesh_active_engine(), &caller);
  PICOMESH_RETURN_IF_ERR(picomesh_int, authctx_res,
                         "repo_caller_may_list: authctx failed");
  if (!caller.authenticated)
    return PICOMESH_OK(picomesh_int, 0);
  if (caller.uid == owner_id)
    return PICOMESH_OK(picomesh_int, 1);
  if (picomesh_groups_contains(caller.groups_csv, PICOMESH_GROUP_SYSTEM))
    return PICOMESH_OK(picomesh_int, 1);
...
repo_set_visibility(struct gr_storage *storage, struct yheaders *hdrs,
                    uint32_t rid, int is_public) {
  storage->h.shard = rid;
  ...
  struct picomesh_int_result up = rel_exec_changes(
      &storage->h, hdrs, "UPDATE repos SET visibility=? WHERE id=?", args);
  free(args);
  if (PICOMESH_IS_ERR(up))
    return PICOMESH_ERR(picomesh_void, "git_repo: visibility update failed",
                        up);
...
static struct picomesh_int64_result repo_count_where(struct gr_storage *storage,
...
repo_list_join(struct gr_storage *storage, struct yheaders *hdrs,
               const char *base_sql, const char *args_json, int full_path) {
  struct picomesh_json_result page = rel_query_page(
      &storage->h, hdrs, base_sql, args_json, "created_at", 0, 0, 0);
  if (PICOMESH_IS_ERR(page))
    return PICOMESH_ERR(picomesh_string, "git_repo: list query failed", page);
  struct json_doc *doc =
      page.value ? json_parse(page.value, strlen(page.value)) : NULL;
  free(page.value);
  if (!doc)
    return PICOMESH_ERR(picomesh_string, "git_repo: list parse failed");
  ...
  if (!out) {
    json_doc_free(doc);
    return PICOMESH_ERR(picomesh_string, "git_repo: list out of memory");
  }
  size_t len = 0;
  out[0] = 0;
...
static struct const_char_ptr_result resolve_repos_dir(void) {
  struct picomesh_engine *engine = picomesh_active_engine();
  if (!engine) {
    ywarn("git_repo: no active engine — cannot resolve required config "
          "'git_repo.repos_dir'");
  ...
  PICOMESH_RETURN_IF_ERR(const_char_ptr, config_res,
                         "resolve_repos_dir: config read failed");
  const char *repos_dir =
      config_res.value ? config_node_as_string(config_res.value, NULL) : NULL;
...
static struct picomesh_int_result repo_disk_init_enabled(void) {
  ...
  if (engine) {
    ...
    PICOMESH_RETURN_IF_ERR(picomesh_int, config_res,
                           "repo_disk_init_enabled: config read failed");
    if (config_res.value)
      return PICOMESH_OK(picomesh_int,
                         config_node_as_int(config_res.value, 1) != 0 ? 1 : 0);
  }
  return PICOMESH_OK(picomesh_int, 1);
}
...
static int path_ok(const char *path) {
  ...
  if (path[0] == '/' || path[len - 1] == '/' || strstr(path, "//"))
    return 0;
  return segments > 0;
}
...
static struct picomesh_void_result repo_dir_build(const char *owner_name,
                                                  const char *repo_name,
                                                  char *out, size_t cap) {
  struct const_char_ptr_result root_res = resolve_repos_dir();
  PICOMESH_RETURN_IF_ERR(picomesh_void, root_res,
                         "repo_dir_build: repos_dir config failed");
  if (!root_res.value)
    return PICOMESH_ERR(picomesh_void,
                        "repo_dir_build: repos_dir not configured");
  int written =
      snprintf(out, cap, "%s/%s/%s.git", root_res.value, owner_name, repo_name);
  if (!(written > 0 && (size_t)written < cap))
    return PICOMESH_ERR(picomesh_void, "repo_dir_build: path too long");
  return PICOMESH_OK_VOID();
}
...
static int gr_repo_acquire(struct gr_repo_cache *cache, const char *path,
                           git_repository **out) {
  *out = NULL;
  struct gr_cached *entry = NULL;
  if (cache)
    HASH_FIND_STR(cache->by_path, path, entry);
  if (entry) {
    *out = entry->repo;
    return 0;
  }
  ...
  *out = repo;
  if (!cache)
    return 0; /* defensive: no cache ⇒ uncached (shouldn't happen) */
  ...
  if (entry) {
    ...
    cache->count++;
    if (cache->max &&
        cache->count > cache->max) { /* over budget → evict the oldest */
...
static void gr_run(const char *key_path,
                   void (*fn)(void *shard_state, void *arg), void *work) {
  struct picomesh_engine *engine = picomesh_active_engine();
...
static int resolve_tree(git_repository *repo, const char *ref,
                        git_tree **out_tree) {
...
static void git_read_tree_work_fn(void *shard_state, void *ud) {
  ...
  git_repository *repo = NULL;
  if (gr_repo_acquire(cache, work->repo_path, &repo) != 0) {
    work->rc = -1;
    return;
  }
  ...
  if (work->path[0]) {
    ...
    int lookup_rc =
        git_tree_lookup(&subtree, repo, git_tree_entry_id(tree_entry));
    git_tree_entry_free(tree_entry);
    if (lookup_rc != 0) {
      git_tree_free(tree);
      gr_repo_release(cache, repo);
  ...
  if (buf) {
    ...
    for (size_t i = 0; i < entry_count; i++) {
      ...
      const char *type =
          git_tree_entry_type(entry) == GIT_OBJECT_TREE ? "tree" : "blob";
...
static int put_insert(git_repository *repo, const git_tree *base, char **comps,
                      int idx, int comp_count, const git_oid *blob_oid,
                      git_oid *out) {
  git_treebuilder *builder = NULL;
  if (git_treebuilder_new(&builder, repo, base) != 0)
    return -1;

  int rc;
  if (idx == comp_count - 1) {
    rc = git_treebuilder_insert(NULL, builder, comps[idx], blob_oid,
                                GIT_FILEMODE_BLOB);
  } else {
    git_tree *subtree = NULL;
    if (base) {
    ...
    git_oid sub_oid;
    rc = put_insert(repo, subtree, comps, idx + 1, comp_count, blob_oid,
                    &sub_oid);
    if (subtree)
      git_tree_free(subtree);
    if (rc == 0)
      rc = git_treebuilder_insert(NULL, builder, comps[idx], &sub_oid,
                                  GIT_FILEMODE_TREE);
  }
  if (rc == 0)
...
static void git_put_file_work_fn(void *shard_state, void *ud) {
  ...
  /* 1) content → blob */
  git_oid blob_oid;
  if (git_blob_create_from_buffer(&blob_oid, repo, work->content,
                                  work->content_len) != 0) {
    gr_repo_release(cache, repo);
    work->rc = -2;
    return;
  ...
  if (git_reference_lookup(&head, repo, "HEAD") == 0) {
    if (git_reference_type(head) == GIT_REFERENCE_SYMBOLIC) {
      ...
      if (target)
        snprintf(branch_ref, sizeof(branch_ref), "%s", target);
    }
  ...
  char *comps[64];
  int comp_count = 0;
  for (char *token = strtok(pathbuf, "/"); token && comp_count < 64;
       token = strtok(NULL, "/"))
  ...
  const char *email =
      work->author_email[0] ? work->author_email : "picoforge@localhost";
  ...
  if (rc == 0)
    git_oid_tostr(work->out_oid, sizeof(work->out_oid), &commit_oid);
  else
    work->rc = -7;
...
git_repo_git_repo_make_impl(struct ctx *ctx, struct object *obj,
  ...
  if (!path_ok(owner_name) || !path_segment_ok(repo_name))
    return PICOMESH_ERR(picomesh_uint32,
                        "git_repo_make: invalid owner_name/repo_name");
  ...
  {
    static const char *const reserved[] = {"issues", "runs", "edit", "new",
                                           "settings"};
    for (size_t i = 0; i < sizeof(reserved) / sizeof(reserved[0]); ++i)
      if (strcmp(repo_name, reserved[i]) == 0)
        return PICOMESH_ERR(picomesh_uint32,
                            "git_repo_make: reserved repo name");
  }
   ...
   * nothing. */
  struct picomesh_int64_result namespace_res =
      gr_resolve_namespace(hdrs, owner_name);
  if (PICOMESH_IS_ERR(namespace_res))
    return PICOMESH_ERR(picomesh_uint32,
                        "git_repo_make: namespace resolve failed",
                        namespace_res);
  if (namespace_res.value <= 0)
    return PICOMESH_ERR(picomesh_uint32,
                        "git_repo_make: owning namespace does not exist");
  ...
  struct picomesh_int_result make_role_res =
      repo_caller_has_role(hdrs, owner_name, "developer");
  PICOMESH_RETURN_IF_ERR(picomesh_uint32, make_role_res,
                         "git_repo_make: authz check failed");
  if (!make_role_res.value)
    return PICOMESH_ERR(
        picomesh_uint32,
        "git_repo_make: forbidden (insufficient namespace role)");
   ...
   * token carrying the real creator's uid). */
  {
    ...
    if (!creator.authenticated)
      return PICOMESH_ERR(picomesh_uint32,
                          "git_repo_make: authentication required");
  ...
  if (PICOMESH_IS_ERR(storage_res))
    return PICOMESH_ERR(picomesh_uint32, "git_repo_make: storage open failed",
                        storage_res);
  ...
  struct picomesh_int_result dup_res =
      repo_load(&storage, hdrs, repo_id, &existing);
  if (PICOMESH_IS_ERR(dup_res))
    return PICOMESH_ERR(picomesh_uint32,
                        "git_repo_make: duplicate check failed", dup_res);
  if (dup_res.value)
  ...
  PICOMESH_RETURN_IF_ERR(picomesh_uint32, disk_init_res,
                         "git_repo_make: disk_init config failed");
  if (disk_init_res.value) {
    if (!ensure_libgit2())
      return PICOMESH_ERR(picomesh_uint32,
                          "git_repo_make: libgit2 init failed");
    struct git_init_work work = {0};
    struct picomesh_void_result dir_res =
        repo_dir_build(owner_name, repo_name, work.path, sizeof(work.path));
    PICOMESH_RETURN_IF_ERR(
        picomesh_uint32, dir_res,
        "git_repo_make: repos_dir not configured or path too long");
    struct const_char_ptr_result root_res = resolve_repos_dir();
    PICOMESH_RETURN_IF_ERR(picomesh_uint32, root_res,
                           "git_repo_make: repos_dir config failed");
    const char *root = root_res.value;
    if (!root)
      return PICOMESH_ERR(
          picomesh_uint32,
          "git_repo_make: required config 'git_repo.repos_dir' missing");
    int written =
        snprintf(work.parent, sizeof(work.parent), "%s/%s", root, owner_name);
    ...
    if (work.mkdir_errno != 0) {
      ywarn("git_repo: cannot create per-user parent %s: %s", work.parent,
            strerror(work.mkdir_errno));
      return PICOMESH_ERR(picomesh_uint32,
                          "git_repo_make: mkdir parent failed");
    }
    if (work.git_rc < 0) {
    ...
    yinfo("git_repo: created repo=%u %s/%s at %s", repo_id, owner_name,
          repo_name, work.path);
  } else {
    yinfo("git_repo: recorded repo=%u %s/%s (disk_init off)", repo_id,
  ...
  struct json_writer *writer = json_writer_new();
  if (!writer)
    return PICOMESH_ERR(picomesh_uint32, "git_repo_make: writer alloc failed");
  json_writer_begin_array(writer);
  ...
  char *args = rel_args_take(writer);
  struct picomesh_int_result insert_res = rel_exec_changes(
      &storage.h, hdrs,
      "INSERT OR IGNORE INTO repos(id, namespace_id, namespace_path, name, "
      "owner_uid, visibility, created_at) "
      "VALUES(?, ?, ?, ?, ?, 'private', ?)",
      args);
  free(args);
  if (PICOMESH_IS_ERR(insert_res))
    return PICOMESH_ERR(picomesh_uint32,
                        "git_repo_make: repo row insert failed", insert_res);
...
struct picomesh_int_result git_repo_git_repo_delete_impl(struct ctx *ctx,
                                                         struct object *obj,
                                                         struct yheaders *hdrs,
                                                         uint32_t repo_id) {
  (void)ctx;
  ...
  struct gr_storage_result storage_res = gr_open();
  if (PICOMESH_IS_ERR(storage_res))
    return PICOMESH_ERR(picomesh_int, "git_repo_delete: storage open failed",
                        storage_res);
  struct gr_storage storage = storage_res.value;

  struct repo_rec rec;
  struct picomesh_int_result load_res =
      repo_load(&storage, hdrs, repo_id, &rec);
  if (PICOMESH_IS_ERR(load_res))
    return PICOMESH_ERR(picomesh_int, "git_repo_delete: load failed", load_res);
  if (load_res.value == 0)
    return PICOMESH_OK(picomesh_int, 0); /* no such repo */
  ...
  struct picomesh_int_result delete_role_res =
      repo_caller_has_role(hdrs, rec.owner_name, "maintainer");
  PICOMESH_RETURN_IF_ERR(picomesh_int, delete_role_res,
                         "git_repo_delete: authz check failed");
  if (!delete_role_res.value)
    return PICOMESH_ERR(
        picomesh_int,
        "git_repo_delete: forbidden (insufficient namespace role)");
  ...
  struct picomesh_int_result del_res = rel_exec_changes(
      &storage.h, hdrs, "DELETE FROM repos WHERE id=?", del_args);
  free(del_args);
  if (PICOMESH_IS_ERR(del_res))
    return PICOMESH_ERR(picomesh_int, "git_repo_delete: row delete failed",
                        del_res);
  if (del_res.value == 0)
    return PICOMESH_OK(picomesh_int, 0); /* another caller removed it first */

  /* On-disk dir LAST. */
  char path[1024];
  struct picomesh_void_result delete_dir =
      repo_dir_build(rec.owner_name, rec.repo_name, path, sizeof(path));
  PICOMESH_RETURN_IF_ERR(picomesh_int, delete_dir,
                         "git_repo_delete: repo dir resolution failed");
...
struct picomesh_uint32_result
git_repo_git_repo_owner_of_impl(struct ctx *ctx, struct object *obj,
  ...
  if (PICOMESH_IS_ERR(storage_res))
    return PICOMESH_ERR(picomesh_uint32,
                        "git_repo_owner_of: storage open failed", storage_res);
  struct gr_storage storage = storage_res.value;
  struct repo_rec rec;
  struct picomesh_int_result load_res =
      repo_load(&storage, hdrs, repo_id, &rec);
  if (PICOMESH_IS_ERR(load_res))
    return PICOMESH_ERR(picomesh_uint32, "git_repo_owner_of: load failed",
                        load_res);
...
git_repo_git_repo_namespace_of_impl(struct ctx *ctx, struct object *obj,
                                    struct yheaders *hdrs, uint32_t repo_id) {
  (void)ctx;
  (void)obj;
  struct gr_storage_result storage_res = gr_open();
  if (PICOMESH_IS_ERR(storage_res))
    return PICOMESH_ERR(picomesh_string,
                        "git_repo_namespace_of: storage open failed",
                        storage_res);
  struct gr_storage storage = storage_res.value;
  struct repo_rec rec;
  struct picomesh_int_result load_res =
      repo_load(&storage, hdrs, repo_id, &rec);
  if (PICOMESH_IS_ERR(load_res))
    return PICOMESH_ERR(picomesh_string, "git_repo_namespace_of: load failed",
                        load_res);
  char *out = strdup(load_res.value ? rec.owner_name : "");
  if (!out)
    return PICOMESH_ERR(picomesh_string,
                        "git_repo_namespace_of: out of memory");
  return PICOMESH_OK(picomesh_string, out);
}

PICOMESH_CLASS_ANNOTATE("override@git_repo:git_repo:git_repo_count_for_owner")
struct picomesh_size_result
git_repo_git_repo_count_for_owner_impl(struct ctx *ctx, struct object *obj,
                                       struct yheaders *hdrs,
                                       uint32_t owner_id) {
  (void)ctx;
  (void)obj;
  struct picomesh_int_result count_list_res =
      repo_caller_may_list(hdrs, owner_id);
  PICOMESH_RETURN_IF_ERR(picomesh_size, count_list_res,
                         "git_repo_count_for_owner: authz check failed");
  if (!count_list_res.value)
    return PICOMESH_ERR(
        picomesh_size,
        "git_repo_count_for_owner: forbidden (cannot list another namespace)");
  struct gr_storage_result storage_res = gr_open();
  if (PICOMESH_IS_ERR(storage_res))
    return PICOMESH_ERR(picomesh_size,
                        "git_repo_count_for_owner: storage open failed",
                        storage_res);
  struct gr_storage storage = storage_res.value;
  char *qa = rel_args1i((int64_t)owner_id);
  struct picomesh_int64_result count_res = repo_count_where(
      &storage, hdrs, "SELECT COUNT(*) AS n FROM repos WHERE owner_uid=?", qa);
  free(qa);
  if (PICOMESH_IS_ERR(count_res))
    return PICOMESH_ERR(picomesh_size, "git_repo_count_for_owner: read failed",
                        count_res);
...
struct picomesh_size_result
git_repo_git_repo_count_total_impl(struct ctx *ctx, struct object *obj,
                                   struct yheaders *hdrs) {
  (void)ctx;
  (void)obj;
  struct gr_storage_result storage_res = gr_open();
  if (PICOMESH_IS_ERR(storage_res))
    return PICOMESH_ERR(picomesh_size,
                        "git_repo_count_total: storage open failed",
                        storage_res);
  struct gr_storage storage = storage_res.value;
  struct picomesh_int64_result count_res =
      repo_count_where(&storage, hdrs, "SELECT COUNT(*) AS n FROM repos", "[]");
  if (PICOMESH_IS_ERR(count_res))
    return PICOMESH_ERR(picomesh_size, "git_repo_count_total: read failed",
                        count_res);
  return PICOMESH_OK(picomesh_size, (size_t)count_res.value);
}
...
struct picomesh_string_result
git_repo_git_repo_list_for_owner_impl(struct ctx *ctx, struct object *obj,
                                      struct yheaders *hdrs,
                                      uint32_t owner_id) {
  (void)ctx;
  (void)obj;
  struct picomesh_int_result list_owner_res =
      repo_caller_may_list(hdrs, owner_id);
  PICOMESH_RETURN_IF_ERR(picomesh_string, list_owner_res,
                         "git_repo_list_for_owner: authz check failed");
  if (!list_owner_res.value)
    return PICOMESH_ERR(
        picomesh_string,
        "git_repo_list_for_owner: forbidden (cannot list another namespace)");
  struct gr_storage_result storage_res = gr_open();
  if (PICOMESH_IS_ERR(storage_res))
    return PICOMESH_ERR(picomesh_string,
                        "git_repo_list_for_owner: storage open failed",
                        storage_res);
  struct gr_storage storage = storage_res.value;
  ...
  char *qa = rel_args1i((int64_t)owner_id);
  struct picomesh_string_result list_res = repo_list_join(
      &storage, hdrs,
      "SELECT name, namespace_path, created_at FROM repos WHERE owner_uid=?",
      qa, 1);
  free(qa);
  if (PICOMESH_IS_ERR(list_res))
    return PICOMESH_ERR(picomesh_string, "git_repo_list_for_owner: read failed",
                        list_res);
  return list_res;
}
...
PICOMESH_CLASS_ANNOTATE(
    "override@git_repo:git_repo:git_repo_list_for_namespace")
struct picomesh_string_result
git_repo_git_repo_list_for_namespace_impl(struct ctx *ctx, struct object *obj,
                                          struct yheaders *hdrs,
                                          const char *path) {
  (void)ctx;
  (void)obj;
  if (!path_ok(path))
    return PICOMESH_ERR(picomesh_string,
                        "git_repo_list_for_namespace: invalid namespace path");
  struct picomesh_int_result list_ns_res =
      repo_caller_has_role(hdrs, path, "reporter");
  PICOMESH_RETURN_IF_ERR(picomesh_string, list_ns_res,
                         "git_repo_list_for_namespace: authz check failed");
  if (!list_ns_res.value)
    return PICOMESH_ERR(
        picomesh_string,
        "git_repo_list_for_namespace: forbidden (insufficient namespace role)");
  struct gr_storage_result storage_res = gr_open();
  if (PICOMESH_IS_ERR(storage_res))
    return PICOMESH_ERR(picomesh_string,
                        "git_repo_list_for_namespace: storage open failed",
                        storage_res);
  struct gr_storage storage = storage_res.value;
  char *qa = rel_args1s(path);
  struct picomesh_string_result list_res =
      repo_list_join(&storage, hdrs,
                     "SELECT name, namespace_path, created_at FROM repos WHERE "
                     "namespace_path=?",
                     qa, 0);
  free(qa);
  if (PICOMESH_IS_ERR(list_res))
    return PICOMESH_ERR(picomesh_string,
                        "git_repo_list_for_namespace: read failed", list_res);
  return list_res;
}

/* Count of repos owned by NAMESPACE PATH `path` (same authz as the listing). */
PICOMESH_CLASS_ANNOTATE(
    "override@git_repo:git_repo:git_repo_count_for_namespace")
struct picomesh_size_result
git_repo_git_repo_count_for_namespace_impl(struct ctx *ctx, struct object *obj,
                                           struct yheaders *hdrs,
                                           const char *path) {
  (void)ctx;
  (void)obj;
  if (!path_ok(path))
    return PICOMESH_ERR(picomesh_size,
                        "git_repo_count_for_namespace: invalid namespace path");
  struct picomesh_int_result count_ns_res =
      repo_caller_has_role(hdrs, path, "reporter");
  PICOMESH_RETURN_IF_ERR(picomesh_size, count_ns_res,
                         "git_repo_count_for_namespace: authz check failed");
  if (!count_ns_res.value)
    return PICOMESH_ERR(picomesh_size,
                        "git_repo_count_for_namespace: forbidden (insufficient "
                        "namespace role)");
  struct gr_storage_result storage_res = gr_open();
  if (PICOMESH_IS_ERR(storage_res))
    return PICOMESH_ERR(picomesh_size,
                        "git_repo_count_for_namespace: storage open failed",
                        storage_res);
  struct gr_storage storage = storage_res.value;
  char *qa = rel_args1s(path);
  struct picomesh_int64_result count_res = repo_count_where(
      &storage, hdrs, "SELECT COUNT(*) AS n FROM repos WHERE namespace_path=?",
      qa);
  free(qa);
  if (PICOMESH_IS_ERR(count_res))
    return PICOMESH_ERR(picomesh_size,
                        "git_repo_count_for_namespace: read failed", count_res);
  return PICOMESH_OK(picomesh_size, (size_t)count_res.value);
}
...
struct picomesh_string_result
git_repo_git_repo_read_tree_impl(struct ctx *ctx, struct object *obj,
                                 struct yheaders *hdrs, uint32_t repo_id,
  ...
  if (PICOMESH_IS_ERR(storage_res))
    return PICOMESH_ERR(picomesh_string,
                        "git_repo_read_tree: storage open failed", storage_res);
  struct gr_storage storage = storage_res.value;
  struct repo_rec rec;
  struct picomesh_int_result load_res =
      repo_load(&storage, hdrs, repo_id, &rec);
  if (PICOMESH_IS_ERR(load_res))
    return PICOMESH_ERR(picomesh_string, "git_repo_read_tree: load failed",
                        load_res);
  if (load_res.value == 0)
    return PICOMESH_ERR(picomesh_string, "git_repo_read_tree: no such repo");
  ...
  if (!rec.is_public) {
    struct picomesh_int_result read_tree_role =
        repo_caller_has_role(hdrs, rec.owner_name, "reporter");
    PICOMESH_RETURN_IF_ERR(picomesh_string, read_tree_role,
                           "git_repo_read_tree: authz check failed");
    if (!read_tree_role.value)
      return PICOMESH_ERR(
          picomesh_string,
          "git_repo_read_tree: forbidden (insufficient namespace role)");
  }
  if (!ensure_libgit2())
    return PICOMESH_ERR(picomesh_string,
                        "git_repo_read_tree: libgit2 init failed");

  struct git_read_work work;
  memset(&work, 0, sizeof(work));
  struct picomesh_void_result read_tree_dir = repo_dir_build(
      rec.owner_name, rec.repo_name, work.repo_path, sizeof(work.repo_path));
  PICOMESH_RETURN_IF_ERR(picomesh_string, read_tree_dir,
                         "git_repo_read_tree: path too long");
  ...
  if (work.rc != 0 || !work.out) {
    free(work.out);
    return PICOMESH_ERR(picomesh_string,
                        "git_repo_read_tree: not a directory or git error");
  }
...
struct picomesh_string_result
git_repo_git_repo_read_file_impl(struct ctx *ctx, struct object *obj,
                                 struct yheaders *hdrs, uint32_t repo_id,
                                 const char *ref, const char *path) {
  (void)ctx;
  (void)obj;
  if (!path || !*path)
    return PICOMESH_ERR(picomesh_string, "git_repo_read_file: path required");
  struct gr_storage_result storage_res = gr_open();
  if (PICOMESH_IS_ERR(storage_res))
    return PICOMESH_ERR(picomesh_string,
                        "git_repo_read_file: storage open failed", storage_res);
  struct gr_storage storage = storage_res.value;
  struct repo_rec rec;
  struct picomesh_int_result load_res =
      repo_load(&storage, hdrs, repo_id, &rec);
  if (PICOMESH_IS_ERR(load_res))
    return PICOMESH_ERR(picomesh_string, "git_repo_read_file: load failed",
                        load_res);
  if (load_res.value == 0)
    return PICOMESH_ERR(picomesh_string, "git_repo_read_file: no such repo");
  /* Same read authz as read_tree: public → anyone, private → reporter+ on the
   * repo's namespace (inherited) or a site admin. */
  if (!rec.is_public) {
    struct picomesh_int_result read_file_role =
        repo_caller_has_role(hdrs, rec.owner_name, "reporter");
    PICOMESH_RETURN_IF_ERR(picomesh_string, read_file_role,
                           "git_repo_read_file: authz check failed");
    if (!read_file_role.value)
      return PICOMESH_ERR(
          picomesh_string,
          "git_repo_read_file: forbidden (insufficient namespace role)");
  }
  if (!ensure_libgit2())
    return PICOMESH_ERR(picomesh_string,
                        "git_repo_read_file: libgit2 init failed");

  struct git_read_work work;
  memset(&work, 0, sizeof(work));
  struct picomesh_void_result read_file_dir = repo_dir_build(
      rec.owner_name, rec.repo_name, work.repo_path, sizeof(work.repo_path));
  PICOMESH_RETURN_IF_ERR(picomesh_string, read_file_dir,
                         "git_repo_read_file: path too long");
  snprintf(work.ref, sizeof(work.ref), "%s", ref ? ref : "");
  ...
  if (work.rc != 0 || !work.out) {
    free(work.out);
    return PICOMESH_ERR(picomesh_string,
                        "git_repo_read_file: not a file or git error");
  }
  return PICOMESH_OK(picomesh_string, work.out);
}
...
struct picomesh_string_result git_repo_git_repo_put_file_impl(
  ...
  (void)obj;
  if (!path || !*path)
    return PICOMESH_ERR(picomesh_string, "git_repo_put_file: path required");
  struct gr_storage_result storage_res = gr_open();
  if (PICOMESH_IS_ERR(storage_res))
    return PICOMESH_ERR(picomesh_string,
                        "git_repo_put_file: storage open failed", storage_res);
  struct gr_storage storage = storage_res.value;
  struct repo_rec rec;
  struct picomesh_int_result load_res =
      repo_load(&storage, hdrs, repo_id, &rec);
  if (PICOMESH_IS_ERR(load_res))
    return PICOMESH_ERR(picomesh_string, "git_repo_put_file: load failed",
                        load_res);
  if (load_res.value == 0)
    return PICOMESH_ERR(picomesh_string, "git_repo_put_file: no such repo");
  ...
  struct picomesh_int_result put_role_res =
      repo_caller_has_role(hdrs, rec.owner_name, "developer");
  PICOMESH_RETURN_IF_ERR(picomesh_string, put_role_res,
                         "git_repo_put_file: authz check failed");
  if (!put_role_res.value)
    return PICOMESH_ERR(
        picomesh_string,
        "git_repo_put_file: forbidden (insufficient namespace role)");
  if (!ensure_libgit2())
    return PICOMESH_ERR(picomesh_string,
                        "git_repo_put_file: libgit2 init failed");

  struct git_put_work work;
  memset(&work, 0, sizeof(work));
  struct picomesh_void_result put_dir = repo_dir_build(
      rec.owner_name, rec.repo_name, work.repo_path, sizeof(work.repo_path));
  PICOMESH_RETURN_IF_ERR(picomesh_string, put_dir,
                         "git_repo_put_file: path too long");
  snprintf(work.path, sizeof(work.path), "%s", path);
  ...
  snprintf(work.author_name, sizeof(work.author_name), "%s",
           author_name ? author_name : "");
  snprintf(work.author_email, sizeof(work.author_email), "%s",
           author_email ? author_email : "");

  gr_run(work.repo_path, git_put_file_work_fn, &work);
  if (work.rc != 0 || !work.out_oid[0])
    return PICOMESH_ERR(picomesh_string, "git_repo_put_file: git error");
  char *oid = strdup(work.out_oid);
  if (!oid)
    return PICOMESH_ERR(picomesh_string, "git_repo_put_file: out of memory");
...
git_repo_git_repo_is_public_impl(struct ctx *ctx, struct object *obj,
                                 struct yheaders *hdrs, uint32_t repo_id) {
  (void)ctx;
  (void)obj;
  struct gr_storage_result storage_res = gr_open();
  if (PICOMESH_IS_ERR(storage_res))
    return PICOMESH_ERR(picomesh_int, "git_repo_is_public: storage open failed",
                        storage_res);
  struct gr_storage storage = storage_res.value;
  struct repo_rec rec;
  struct picomesh_int_result load_res =
      repo_load(&storage, hdrs, repo_id, &rec);
  if (PICOMESH_IS_ERR(load_res))
    return PICOMESH_ERR(picomesh_int, "git_repo_is_public: load failed",
                        load_res);
...
struct picomesh_int_result
git_repo_git_repo_set_public_impl(struct ctx *ctx, struct object *obj,
                                  struct yheaders *hdrs, uint32_t repo_id,
                                  int is_public) {
  (void)ctx;
  (void)obj;
  ...
  if (PICOMESH_IS_ERR(storage_res))
    return PICOMESH_ERR(
        picomesh_int, "git_repo_set_public: storage open failed", storage_res);
  struct gr_storage storage = storage_res.value;
  struct repo_rec rec;
  struct picomesh_int_result load_res =
      repo_load(&storage, hdrs, repo_id, &rec);
  if (PICOMESH_IS_ERR(load_res))
    return PICOMESH_ERR(picomesh_int, "git_repo_set_public: load failed",
                        load_res);
  if (load_res.value == 0)
    return PICOMESH_ERR(picomesh_int, "git_repo_set_public: no such repo");
  /* Changing visibility is a maintainer-grade operation on the repo's
   * namespace (inherited), or a site admin (issue #30). */
  struct picomesh_int_result setpub_role_res =
      repo_caller_has_role(hdrs, rec.owner_name, "maintainer");
  PICOMESH_RETURN_IF_ERR(picomesh_int, setpub_role_res,
                         "git_repo_set_public: authz check failed");
  if (!setpub_role_res.value)
    return PICOMESH_ERR(
        picomesh_int,
        "git_repo_set_public: forbidden (insufficient namespace role)");
  struct picomesh_void_result store_res =
      repo_set_visibility(&storage, hdrs, repo_id, is_public ? 1 : 0);
  if (PICOMESH_IS_ERR(store_res))
    return PICOMESH_ERR(picomesh_int, "git_repo_set_public: write failed",
                        store_res);
  yinfo("git_repo: repo=%u visibility -> %s", repo_id,
...
struct picomesh_json_result git_repo_git_repo_list_impl(struct ctx *ctx,
                                                        struct object *obj,
                                                        struct yheaders *hdrs,
                                                        int64_t offset,
                                                        int64_t limit) {
  ...
  struct gr_storage_result storage_res = gr_open();
  if (PICOMESH_IS_ERR(storage_res))
    return PICOMESH_ERR(picomesh_json, "git_repo_list: storage open failed",
                        storage_res);
  struct gr_storage storage = storage_res.value;
  return rel_query_page(&storage.h, hdrs,
                        "SELECT id, namespace_id, namespace_path, name, "
                        "owner_uid, visibility, created_at FROM repos",
                        "[]", "id", 0, offset, limit);
}
...
git_repo_git_repo_list_all_impl(struct ctx *ctx, struct object *obj,
                                struct yheaders *hdrs) {
  (void)ctx;
  (void)obj;
  struct gr_storage_result storage_res = gr_open();
  if (PICOMESH_IS_ERR(storage_res))
    return PICOMESH_ERR(picomesh_json, "git_repo_list_all: storage open failed",
                        storage_res);
  struct gr_storage storage = storage_res.value;
  return rel_query_page(&storage.h, hdrs,
                        "SELECT id, namespace_id, namespace_path, name, "
