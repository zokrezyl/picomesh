 ...
 *   resolve(name)                                   → "host:port" ("" if
 * unknown) discover_service(name)                          → JSON
 * {service_name, instances:[…]} list_services() → JSON [{service_name,
 * instances:[…]}] count()                                         → number of
 * live instances */
...
struct PICOMESH_CLASS_ANNOTATE("class@registry:registry")
    registry_registry_data {
  int placeholder;
};
...
registry_registry_register_service_impl(struct ctx *ctx, struct object *obj,
  ...
  (void)hdrs;
  if (!name || !*name)
    return PICOMESH_ERR(picomesh_int, "registry_register: empty service name");
  ...
  snprintf(state->entries[slot].name, sizeof(state->entries[slot].name), "%s",
           name);
  snprintf(state->entries[slot].instance_id,
           sizeof(state->entries[slot].instance_id), "%s", instance_id);
  snprintf(state->entries[slot].host, sizeof(state->entries[slot].host), "%s",
           host);
...
PICOMESH_CLASS_ANNOTATE(
    "override@registry:registry:registry_deregister_service")
struct picomesh_int_result registry_registry_deregister_service_impl(
...
registry_registry_resolve_impl(struct ctx *ctx, struct object *obj,
                               struct yheaders *hdrs, const char *name) {
  (void)ctx;
  (void)obj;
  (void)hdrs;
  ...
  if (name && *name) {
    for (size_t i = 0; i < REGISTRY_MAX_ENTRIES; ++i) {
      if (state->entries[i].used && strcmp(state->entries[i].name, name) == 0) {
        snprintf(buf, sizeof(buf), "%s:%u", state->entries[i].host,
                 state->entries[i].port);
  ...
  char *out = strdup(buf);
  if (!out)
    return PICOMESH_ERR(picomesh_string, "registry_resolve: out of memory");
...
static void registry_write_instances(struct json_writer *writer,
  ...
  for (size_t i = 0; i < REGISTRY_MAX_ENTRIES; ++i) {
    ...
    json_writer_key(writer, "instance_id");
    json_writer_string(writer, state->entries[i].instance_id);
    json_writer_key(writer, "host");
    json_writer_string(writer, state->entries[i].host);
    json_writer_key(writer, "port");
    json_writer_int(writer, (int64_t)state->entries[i].port);
...
registry_registry_discover_service_impl(struct ctx *ctx, struct object *obj,
  ...
  if (!writer)
    return PICOMESH_ERR(picomesh_json,
                        "registry_discover: writer alloc failed");
  pthread_mutex_lock(&state->mu);
  size_t total = 0;
  for (size_t i = 0; i < REGISTRY_MAX_ENTRIES; ++i) {
    if (state->entries[i].used && strcmp(state->entries[i].name, name) == 0)
  ...
  json_writer_string(writer, name);
  json_writer_key(writer, "instances");
  registry_write_instances(writer, state, name);
  json_writer_key(writer, "total_instances");
  json_writer_int(writer, (int64_t)total);
  json_writer_end_object(writer);
  pthread_mutex_unlock(&state->mu);
  size_t len = 0;
  ...
  if (!out)
    return PICOMESH_ERR(picomesh_json, "registry_discover: out of memory");
  return PICOMESH_OK(picomesh_json, out);
}

PICOMESH_CLASS_ANNOTATE("override@registry:registry:registry_list_services")
struct picomesh_json_result
registry_registry_list_services_impl(struct ctx *ctx, struct object *obj,
                                     struct yheaders *hdrs) {
  (void)ctx;
  (void)obj;
  (void)hdrs;
  ...
  struct json_writer *writer = json_writer_new();
  if (!writer)
    return PICOMESH_ERR(picomesh_json, "registry_list: writer alloc failed");
  pthread_mutex_lock(&state->mu);
  json_writer_begin_array(writer);
  ...
  for (size_t i = 0; i < REGISTRY_MAX_ENTRIES; ++i) {
    ...
    for (size_t j = 0; j < i; ++j) {
      if (state->entries[j].used &&
          strcmp(state->entries[j].name, state->entries[i].name) == 0) {
        first = 0;
        break;
      }
    }
    ...
    json_writer_begin_object(writer);
    json_writer_key(writer, "service_name");
    json_writer_string(writer, state->entries[i].name);
    json_writer_key(writer, "instances");
    registry_write_instances(writer, state, state->entries[i].name);
    json_writer_end_object(writer);
  }
  json_writer_end_array(writer);
  pthread_mutex_unlock(&state->mu);
  ...
  json_writer_free(writer);
  if (!out)
    return PICOMESH_ERR(picomesh_json, "registry_list: out of memory");
  return PICOMESH_OK(picomesh_json, out);
}

PICOMESH_CLASS_ANNOTATE("override@registry:registry:registry_count")
struct picomesh_size_result
registry_registry_count_impl(struct ctx *ctx, struct object *obj,
                             struct yheaders *hdrs) {
  (void)ctx;
  (void)obj;
  (void)hdrs;
