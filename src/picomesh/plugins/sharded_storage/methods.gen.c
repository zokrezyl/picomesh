...
struct picomesh_int64_result sharded_storage_db_incr(struct ctx * ctx, struct object * obj, struct yheaders * hdrs, const char * context, const char * key, int64_t delta)
{
    ...
    if (_s && _s->peer) {
        ...
        size_t _wn = rpc_call(_s->peer, RPC_OP_CALL, _rid, _a, _off,
                              _wbuf, sizeof(_wbuf));
        double span_us = (picomesh_ytime_monotonic_sec() - span_start) * 1e6;
        ydebug("span trace=%s op=rpc.sharded_storage_db_incr dur_us=%.0f", span_trace, span_us);
        yspan_record("rpc.sharded_storage_db_incr", span_us);
...
struct picomesh_int_result sharded_storage_db_put_if_absent(struct ctx * ctx, struct object * obj, struct yheaders * hdrs, const char * context, const char * key, const char * value)
{
    ...
    if (_s && _s->peer) {
        ...
        size_t _wn = rpc_call(_s->peer, RPC_OP_CALL, _rid, _a, _off,
                              _wbuf, sizeof(_wbuf));
        double span_us = (picomesh_ytime_monotonic_sec() - span_start) * 1e6;
        ydebug("span trace=%s op=rpc.sharded_storage_db_put_if_absent dur_us=%.0f", span_trace, span_us);
        yspan_record("rpc.sharded_storage_db_put_if_absent", span_us);
...
struct picomesh_int_result sharded_storage_db_compare_and_set(struct ctx * ctx, struct object * obj, struct yheaders * hdrs, const char * context, const char * key, const char * expected, const char * replacement)
{
    ...
    if (_s && _s->peer) {
        ...
        size_t _wn = rpc_call(_s->peer, RPC_OP_CALL, _rid, _a, _off,
                              _wbuf, sizeof(_wbuf));
        double span_us = (picomesh_ytime_monotonic_sec() - span_start) * 1e6;
        ydebug("span trace=%s op=rpc.sharded_storage_db_compare_and_set dur_us=%.0f", span_trace, span_us);
        yspan_record("rpc.sharded_storage_db_compare_and_set", span_us);
