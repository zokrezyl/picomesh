...
struct PICOMESH_CLASS_ANNOTATE("class@sharded_storage:db")
    sharded_storage_data {
...
static struct shard_set_ptr_result shard_set(void) {
  ...
  if (tried)
    return PICOMESH_ERR(
        shard_set_ptr,
        "sharded_storage: shard set previously failed to initialize");
  ...
  if (!e) {
    ...
    return PICOMESH_ERR(shard_set_ptr,
                        "sharded_storage: no active engine — cannot resolve "
                        "required config 'sharded_storage.path'");
  }
  ...
  if (PICOMESH_IS_ERR(path_res)) {
    ...
    return PICOMESH_ERR(
        shard_set_ptr,
        "sharded_storage: required config 'sharded_storage.path' is missing — "
  ...
  if (!base || !*base) {
    ...
    return PICOMESH_ERR(
        shard_set_ptr,
        "sharded_storage: required config 'sharded_storage.path' is empty");
  }
  ...
  int n = (int)config_get_int(picomesh_engine_config(e),
                              "sharded_storage.shards", 8);
  if (n < 1 || n > SHARDED_MAX_SHARDS)
    n = 8;
  ...
  if (mkdir(base, 0700) != 0 && errno != EEXIST) {
    ...
    return PICOMESH_ERR(
        shard_set_ptr,
        "sharded_storage: mkdir of the base data directory failed");
  }
  for (int i = 0; i < n; ++i) {
    char path[512];
    ...
    if (mkdir(path, 0700) != 0 && errno != EEXIST) {
      ...
      pthread_mutex_unlock(&mu);
      return PICOMESH_ERR(shard_set_ptr,
                          "sharded_storage: mkdir of a shard directory failed");
    }
    MDBX_env *env = NULL;
    if (mdbx_env_create(&env) != MDBX_SUCCESS) {
      ...
      pthread_mutex_unlock(&mu);
      return PICOMESH_ERR(shard_set_ptr,
                          "sharded_storage: mdbx_env_create failed");
    }
    ...
    if (mdbx_env_open(env, path, MDBX_WRITEMAP | MDBX_NOMETASYNC, 0644) !=
        MDBX_SUCCESS) {
      ...
      pthread_mutex_unlock(&mu);
      return PICOMESH_ERR(shard_set_ptr,
                          "sharded_storage: mdbx_env_open of a shard failed");
    }
    s.shards[i].env = env;
...
static int shard_for(const struct shard_set *s, const char *ns,
                     const char *key) {
  uint32_t h = 2166136261u;
  for (const unsigned char *p = (const unsigned char *)ns; *p; ++p) {
    h ^= *p;
    h *= 16777619u;
  }
  h ^= 0u;
...
static void shard_work_fn(void *arg) {
  ...
  if (w->op == OP_LIST_ALL_NS) {
    ...
    for (int i = 0; i < s->n && (limit < 0 || emitted < limit); ++i) {
      MDBX_txn *txn = NULL;
      if (mdbx_txn_begin(s->shards[i].env, NULL, MDBX_TXN_RDONLY, &txn) !=
          MDBX_SUCCESS)
        continue;
      ...
      if (mdbx_dbi_open(txn, NULL, 0, &main_dbi) == MDBX_SUCCESS &&
        ...
        while (ncr == MDBX_SUCCESS && ns_count < SHARDED_DBI_PER_SHARD) {
          size_t nlen = nk.iov_len < sizeof(namespaces[0]) - 1
                            ? nk.iov_len
                            : sizeof(namespaces[0]) - 1;
      ...
      for (int n = 0; n < ns_count && (limit < 0 || emitted < limit); ++n) {
        ...
        while (cr == MDBX_SUCCESS && (limit < 0 || emitted < limit)) {
          if (plen &&
              (k.iov_len < plen || memcmp(k.iov_base, prefix, plen) != 0)) {
            cr = mdbx_cursor_get(cur, &k, &v, MDBX_NEXT);
            continue;
          }
          char kbuf[256];
          size_t klen =
              k.iov_len < sizeof(kbuf) - 1 ? k.iov_len : sizeof(kbuf) - 1;
          memcpy(kbuf, k.iov_base, klen);
          kbuf[klen] = 0;
          char *vbuf = malloc(v.iov_len + 1);
          if (vbuf) {
            memcpy(vbuf, v.iov_base, v.iov_len);
            vbuf[v.iov_len] = 0;
          }
          json_writer_begin_object(jw);
          json_writer_key(jw, "namespace");
          json_writer_string(jw, namespaces[n]);
          json_writer_key(jw, "key");
          json_writer_string(jw, kbuf);
          json_writer_key(jw, "value");
          json_writer_string(jw, vbuf ? vbuf : "");
  ...
  if (w->op == OP_COUNT) {
    ...
    for (int i = 0; i < s->n; ++i) {
      ...
      if (dbi_open_rc == MDBX_SUCCESS) {
        MDBX_stat dbi_stat;
        if (mdbx_dbi_stat(txn, dbi, &dbi_stat, sizeof(dbi_stat)) !=
            MDBX_SUCCESS) {
          mdbx_txn_abort(txn);
          w->rc = SHARD_INTERNAL;
          return;
  ...
  if (w->op == OP_LIST) {
    ...
    size_t plen = strlen(prefix);
    int64_t skip =
        w->in_offset > 0 ? w->in_offset : 0; /* matches still to skip */
    int64_t limit = w->in_limit;             /* < 0 == unbounded */
    int64_t emitted = 0;
    struct json_writer *jw = json_writer_new();
    if (!jw) {
  ...
  MDBX_txn *txn = NULL;
  if (mdbx_txn_begin(sh->env, NULL,
                     rdonly ? MDBX_TXN_RDONLY : MDBX_TXN_READWRITE,
                     &txn) != MDBX_SUCCESS) {
    w->rc = SHARD_INTERNAL;
    return;
  }
  ...
  case OP_GET: {
    ...
    if (r == MDBX_SUCCESS) {
      ...
      if (!out) {
        ...
        if (v.iov_len)
          memcpy(out, v.iov_base, v.iov_len);
        out[v.iov_len] = 0;
        w->out_str = out;
        w->rc = SHARD_OK;
      }
...
static struct picomesh_void_result shard_run(struct shard_work *w) {
  ...
  if (PICOMESH_IS_ERR(r)) {
    ...
    picomesh_error_print(
        stderr, "sharded_storage: worker-pool offload failed — running inline",
        r.error);
...
static const char *shard_rc_msg(enum shard_rc rc) {
  ...
  case SHARD_BAD_CONTEXT:
    return "invalid namespace";
  case SHARD_OPEN_FAILED:
    return "shard open failed";
  case SHARD_INTERNAL:
    return "shard internal error";
  }
  return "unknown";
}
 ...
 * cooperative yields. */
static const char *shard_err_key(enum shard_rc rc, const char *ns,
                                 const char *key) {
  static _Thread_local char buf[320];
  snprintf(buf, sizeof(buf), "%s [%s/%s]", shard_rc_msg(rc), ns ? ns : "?",
           key ? key : "?");
  return buf;
}
...
sharded_storage_db_set_impl(struct ctx *ctx, struct object *obj,
  ...
  struct shard_work w = {
      .op = OP_SET, .ns = context, .key = key, .value = value};
  double span_start = picomesh_ytime_monotonic_sec();
  PICOMESH_RETURN_IF_ERR(picomesh_int, shard_run(&w),
                         "sharded_storage: dispatch failed");
  double span_us = (picomesh_ytime_monotonic_sec() - span_start) * 1e6;
  ydebug("span trace=%s op=shard.set.%s dur_us=%.0f", trace ? trace : "-",
         context, span_us);
  yspan_record("shard.set", span_us);
  if (w.rc != SHARD_OK)
    return PICOMESH_ERR(picomesh_int, shard_err_key(w.rc, context, key));
  return PICOMESH_OK(picomesh_int, 1);
}

PICOMESH_CLASS_ANNOTATE("override@sharded_storage:db:db_get")
struct picomesh_string_result sharded_storage_db_get_impl(struct ctx *ctx,
                                                          struct object *obj,
  ...
  double span_start = picomesh_ytime_monotonic_sec();
  PICOMESH_RETURN_IF_ERR(picomesh_string, shard_run(&w),
                         "sharded_storage: dispatch failed");
  double span_us = (picomesh_ytime_monotonic_sec() - span_start) * 1e6;
  ydebug("span trace=%s op=shard.get.%s dur_us=%.0f", trace ? trace : "-",
         context, span_us);
  ...
  if (w.rc == SHARD_NOT_FOUND) {
    ...
    if (!empty)
      return PICOMESH_ERR(picomesh_string, "db_get: out of memory");
    return PICOMESH_OK(picomesh_string, empty);
  }
  if (w.rc != SHARD_OK)
    return PICOMESH_ERR(picomesh_string, shard_err_key(w.rc, context, key));
  return PICOMESH_OK(picomesh_string, w.out_str);
}

PICOMESH_CLASS_ANNOTATE("override@sharded_storage:db:db_exists")
struct picomesh_int_result sharded_storage_db_exists_impl(struct ctx *ctx,
                                                          struct object *obj,
                                                          struct yheaders *hdrs,
                                                          const char *context,
                                                          const char *key) {
  ...
  PICOMESH_RETURN_IF_ERR(picomesh_int, shard_run(&w),
                         "sharded_storage: dispatch failed");
  if (w.rc != SHARD_OK)
    return PICOMESH_ERR(picomesh_int, shard_rc_msg(w.rc));
  return PICOMESH_OK(picomesh_int, w.out_i);
}

PICOMESH_CLASS_ANNOTATE("override@sharded_storage:db:db_del")
struct picomesh_int_result sharded_storage_db_del_impl(struct ctx *ctx,
                                                       struct object *obj,
                                                       struct yheaders *hdrs,
                                                       const char *context,
                                                       const char *key) {
...
struct picomesh_size_result sharded_storage_db_count_impl(struct ctx *ctx,
                                                          struct object *obj,
                                                          struct yheaders *hdrs,
                                                          const char *context) {
  (void)ctx;
  ...
  PICOMESH_RETURN_IF_ERR(picomesh_size, shard_run(&w),
                         "sharded_storage: dispatch failed");
  if (w.rc != SHARD_OK)
    return PICOMESH_ERR(picomesh_size, shard_rc_msg(w.rc));
...
struct picomesh_json_result sharded_storage_db_list_impl(
    struct ctx *ctx, struct object *obj, struct yheaders *hdrs,
    const char *context, const char *prefix, int64_t offset, int64_t limit) {
  (void)ctx;
  (void)obj;
  (void)hdrs;
  if (limit <= 0)
    limit = SHARDED_LIST_DEFAULT_LIMIT;
  struct shard_work w = {.op = OP_LIST,
                         .ns = context,
                         .key = prefix ? prefix : "",
                         .in_offset = offset,
                         .in_limit = limit};
  PICOMESH_RETURN_IF_ERR(picomesh_json, shard_run(&w),
                         "sharded_storage: dispatch failed");
  if (w.rc != SHARD_OK)
    return PICOMESH_ERR(picomesh_json, shard_rc_msg(w.rc));
 ...
 * EVERY namespace, across all shards (`[{"namespace","key","value"}]`) — this
 * is what a generic console's "list all" means. Use with care: returns the
 * whole set in one response. */
PICOMESH_CLASS_ANNOTATE("override@sharded_storage:db:db_list_all")
struct picomesh_json_result
sharded_storage_db_list_all_impl(struct ctx *ctx, struct object *obj,
                                 struct yheaders *hdrs, const char *context,
                                 const char *prefix) {
  (void)ctx;
  (void)obj;
...
sharded_storage_db_incr_impl(struct ctx *ctx, struct object *obj,
                             struct yheaders *hdrs, const char *context,
  ...
  const char *trace = hdrs ? yheaders_get(hdrs, "trace_id") : NULL;
  struct shard_work w = {
      .op = OP_INCR, .ns = context, .key = key, .in_delta = delta};
  double span_start = picomesh_ytime_monotonic_sec();
  PICOMESH_RETURN_IF_ERR(picomesh_int64, shard_run(&w),
                         "sharded_storage: dispatch failed");
  double span_us = (picomesh_ytime_monotonic_sec() - span_start) * 1e6;
  ydebug("span trace=%s op=shard.incr.%s dur_us=%.0f", trace ? trace : "-",
         context, span_us);
  yspan_record("shard.incr", span_us);
  if (w.rc != SHARD_OK)
    return PICOMESH_ERR(picomesh_int64, shard_err_key(w.rc, context, key));
...
struct picomesh_int_result sharded_storage_db_put_if_absent_impl(
    struct ctx *ctx, struct object *obj, struct yheaders *hdrs,
    const char *context, const char *key, const char *value) {
  (void)ctx;
  (void)obj;
  (void)hdrs;
  struct shard_work w = {
      .op = OP_PUT_IF_ABSENT, .ns = context, .key = key, .value = value};
  PICOMESH_RETURN_IF_ERR(picomesh_int, shard_run(&w),
                         "sharded_storage: dispatch failed");
  if (w.rc != SHARD_OK)
    return PICOMESH_ERR(picomesh_int, shard_err_key(w.rc, context, key));
...
struct picomesh_int_result sharded_storage_db_compare_and_set_impl(
    struct ctx *ctx, struct object *obj, struct yheaders *hdrs,
    const char *context, const char *key, const char *expected,
    const char *replacement) {
  (void)ctx;
  (void)obj;
  (void)hdrs;
  struct shard_work w = {.op = OP_CAS,
                         .ns = context,
                         .key = key,
                         .expected = expected,
                         .value = replacement};
