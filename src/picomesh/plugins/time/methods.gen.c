...
time_clock_now_ms(struct ctx *ctx, struct object *obj, struct yheaders *hdrs) {
  ...
  if (_slot == METHOD_SLOT_UNDEFINED) {
    ...
    if (PICOMESH_IS_ERR(_sr))
      return PICOMESH_ERR(picomesh_int64,
                          "time_clock_now_ms: method_slot_get failed", _sr);
    _slot = _sr.value;
  }

  if (!obj)
    return PICOMESH_ERR(picomesh_int64, "time_clock_now_ms: NULL object");
  ...
  if (_s && _s->peer) {
      ...
      if (!peer_channel_msgpack_call(_s->peer, "time.clock.now_ms", hdrs,
        ...
        _mret = _merr[0]
                    ...
                    : PICOMESH_ERR(picomesh_int64,
                                   "time_clock_now_ms: msgpack call failed");
      } else {
        ...
        if (!cmp_read_integer(&_mrr, &_mv))
          _mret = PICOMESH_ERR(picomesh_int64,
                               "time_clock_now_ms: bad msgpack int result");
        else
    ...
    if (PICOMESH_IS_ERR(_rid_res))
      return PICOMESH_ERR(picomesh_int64,
                          "time_clock_now_ms: ensure remote id failed",
                          _rid_res);
    uint32_t _rid = _rid_res.value;
    if (_rid == RPC_REMOTE_ID_UNRESOLVED)
      return PICOMESH_ERR(picomesh_int64,
                          "time_clock_now_ms: remote id unresolved");
    ...
    if (!_a || !_wbuf) {
      ...
      return PICOMESH_ERR(picomesh_int64,
                          "time_clock_now_ms: wire scratch alloc failed");
    }
    size_t _off = 0;
     ...
     * this client span's id as parent_span_id across the serialize. */
    {
      ...
      if (_hn == 0) {
        ytelemetry_span_end(&_tsp, 0,
                            "time_clock_now_ms: header serialize overflow");
    ...
    }
    {
      ...
      if (_off + 8 > _acap) {
        ytelemetry_span_end(&_tsp, 0, "time_clock_now_ms: pack overflow");
        _ret = PICOMESH_ERR(picomesh_int64, "time_clock_now_ms: pack overflow");
        goto _rpc_done;
      }
      memcpy(_a + _off, &_h, 8);
      _off += 8;
    ...
    if (_wn < 1) {
      _ret =
          PICOMESH_ERR(picomesh_int64, "time_clock_now_ms: short RPC response");
      goto _rpc_done;
    }
    if (_wbuf[0] != 0) {
      uint32_t _msg_len = 0;
      ...
      _ret = _msg[0] ? PICOMESH_ERR_OWNED(picomesh_int64, strdup(_msg))
                     : PICOMESH_ERR(picomesh_int64,
                                    "time_clock_now_ms: remote error (no msg)");
      goto _rpc_done;
    }
    if (_wn != 1 + sizeof(int64_t)) {
      _ret = PICOMESH_ERR(picomesh_int64,
                          "time_clock_now_ms: truncated RPC payload");
      goto _rpc_done;
    }
    int64_t _v;
  ...
  _rpc_done:
    ...
    if (!fn)
      return PICOMESH_ERR(picomesh_int64,
                          "time_clock_now_ms: no impl on this class");
...
struct picomesh_int64_result time_clock_sleep_ms(struct ctx *ctx,
  ...
  if (_slot == METHOD_SLOT_UNDEFINED) {
    ...
    if (PICOMESH_IS_ERR(_sr))
      return PICOMESH_ERR(picomesh_int64,
                          "time_clock_sleep_ms: method_slot_get failed", _sr);
    _slot = _sr.value;
  }

  if (!obj)
    return PICOMESH_ERR(picomesh_int64, "time_clock_sleep_ms: NULL object");
  ...
  if (_s && _s->peer) {
    if (peer_channel_is_msgpack(_s->peer)) {
      ...
      if (!_margs || !_mresp) {
        ...
        free(_mresp);
        return PICOMESH_ERR(picomesh_int64,
                            "time_clock_sleep_ms: out of memory");
      }
      struct picomesh_msgpack_buffer _mab;
      cmp_ctx_t _maw;
      picomesh_msgpack_writer_init(&_maw, &_mab, _margs, 16384);
      ...
      if (!peer_channel_msgpack_call(_s->peer, "time.clock.sleep_ms", hdrs,
        ...
        _mret = _merr[0]
                    ? PICOMESH_ERR_OWNED(picomesh_int64, strdup(_merr))
                    : PICOMESH_ERR(picomesh_int64,
                                   "time_clock_sleep_ms: msgpack call failed");
      } else {
        struct picomesh_msgpack_buffer _mrb;
        cmp_ctx_t _mrr;
        picomesh_msgpack_reader_init(&_mrr, &_mrb, _mresp, _mrlen);
        ...
        if (!cmp_read_integer(&_mrr, &_mv))
          _mret = PICOMESH_ERR(picomesh_int64,
                               "time_clock_sleep_ms: bad msgpack int result");
        else
          _mret = PICOMESH_OK(picomesh_int64, (int64_t)_mv);
      }
      free(_margs);
    ...
    if (PICOMESH_IS_ERR(_rid_res))
      return PICOMESH_ERR(picomesh_int64,
                          "time_clock_sleep_ms: ensure remote id failed",
                          _rid_res);
    uint32_t _rid = _rid_res.value;
    if (_rid == RPC_REMOTE_ID_UNRESOLVED)
      return PICOMESH_ERR(picomesh_int64,
                          "time_clock_sleep_ms: remote id unresolved");
    /* Wire scratch comes from THIS THREAD's pool, not the stack: the arg
     * buffer (_a, _acap bytes) and response buffer (_wbuf) are large
     * (~16 KiB + up to 64 KiB), and a nested chain of in-process hops would
     * overflow the fixed-size coroutine stack if these were locals. Every
    ...
    if (!_a || !_wbuf) {
      ...
      picomesh_allocator_free(_pool, _wbuf);
      return PICOMESH_ERR(picomesh_int64,
                          "time_clock_sleep_ms: wire scratch alloc failed");
    }
    size_t _off = 0;
    /* Client span for this downstream call. Minted BEFORE the header
     * bag is serialized so the wire carries this span as the remote
     ...
     * this client span's id as parent_span_id across the serialize. */
    {
      ...
      if (_hn == 0) {
        ytelemetry_span_end(&_tsp, 0,
                            "time_clock_sleep_ms: header serialize overflow");
        _ret = PICOMESH_ERR(picomesh_int64,
                            "time_clock_sleep_ms: header serialize overflow");
        goto _rpc_done;
      }
    ...
    }
    {
      uint64_t _h = *(uint64_t *)((char *)obj + sizeof(*obj));
      if (_off + 8 > _acap) {
        ytelemetry_span_end(&_tsp, 0, "time_clock_sleep_ms: pack overflow");
        _ret =
            PICOMESH_ERR(picomesh_int64, "time_clock_sleep_ms: pack overflow");
        goto _rpc_done;
      }
      memcpy(_a + _off, &_h, 8);
      _off += 8;
    ...
    if (_off + sizeof(ms) > _acap) {
      ytelemetry_span_end(&_tsp, 0, "time_clock_sleep_ms: pack overflow");
      _ret = PICOMESH_ERR(picomesh_int64, "time_clock_sleep_ms: pack overflow");
      goto _rpc_done;
    }
    memcpy(_a + _off, &ms, sizeof(ms));
    _off += sizeof(ms);
    size_t _wn = rpc_call(_s->peer, RPC_OP_CALL, _rid, _a, _off, _wbuf, 8197);
    ytelemetry_span_end(&_tsp, _wn >= 1 && _wbuf[0] == 0, NULL);
    if (_wn < 1) {
      _ret = PICOMESH_ERR(picomesh_int64,
                          "time_clock_sleep_ms: short RPC response");
      goto _rpc_done;
    }
    if (_wbuf[0] != 0) {
      uint32_t _msg_len = 0;
      ...
      _ret = _msg[0]
                 ...
                 : PICOMESH_ERR(picomesh_int64,
                                "time_clock_sleep_ms: remote error (no msg)");
      goto _rpc_done;
    }
    if (_wn != 1 + sizeof(int64_t)) {
      _ret = PICOMESH_ERR(picomesh_int64,
                          "time_clock_sleep_ms: truncated RPC payload");
      goto _rpc_done;
    }
    int64_t _v;
    memcpy(&_v, _wbuf + 1, sizeof(_v));
  ...
  _rpc_done:
    ...
    if (!fn)
      return PICOMESH_ERR(picomesh_int64,
                          "time_clock_sleep_ms: no impl on this class");
