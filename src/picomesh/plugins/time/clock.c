...
struct picomesh_int64_result time_clock_now_ms_impl(struct ctx *ctx,
                                                    struct object *obj,
...
struct picomesh_int64_result time_clock_sleep_ms_impl(struct ctx *ctx,
                                                      struct object *obj,
                                                      struct yheaders *hdrs,
                                                      uint32_t ms) {
  (void)ctx;
  ...
  if (!engine)
    return PICOMESH_ERR(picomesh_int64, "clock_sleep_ms: no active engine");
  struct loop *loop = picomesh_engine_loop(engine);
  if (!loop)
    return PICOMESH_ERR(picomesh_int64, "clock_sleep_ms: engine has no loop");
