...
static struct picomesh_size_result time_clock_now_ms_skel(const void *_body,
                                                          size_t _body_len,
                                                          void *_resp,
                                                          size_t _resp_max) {
  size_t _off = 0;
  ...
  if (_resp_max < 1)
    return PICOMESH_ERR(picomesh_size,
                        "time_clock_now_ms_skel: response buffer too small");
...
static struct picomesh_size_result time_clock_sleep_ms_skel(const void *_body,
                                                            size_t _body_len,
                                                            void *_resp,
                                                            size_t _resp_max) {
  size_t _off = 0;
  ...
  struct picomesh_int64_result _r =
      time_clock_sleep_ms(&_local, _obj, _hdrs, _v1);
  ytelemetry_span_end(&_tsp, !PICOMESH_IS_ERR(_r),
                      PICOMESH_IS_ERR(_r) ? _r.error.msg : NULL);
  yheaders_free(_hdrs);
  _hdrs = NULL;
  if (_resp_max < 1)
    return PICOMESH_ERR(picomesh_size,
                        "time_clock_sleep_ms_skel: response buffer too small");
  if (PICOMESH_IS_ERR(_r)) {
    char _errbuf[8192] = {0};
    picomesh_error_snprint(_errbuf, sizeof(_errbuf), _r.error);
    const char *_msg =
...
time_clock_now_ms_jinvoke(struct ctx *ctx, struct object *obj,
  ...
  struct picomesh_int64_result call_result =
      time_clock_now_ms(call_ctx, obj, hdrs);
...
static struct picomesh_void_result time_clock_sleep_ms_jinvoke(
  ...
  struct ctx *call_ctx = ctx ? ctx : &local_ctx;
  struct picomesh_int64_result call_result =
      time_clock_sleep_ms(call_ctx, obj, hdrs, arg0);
  if (PICOMESH_IS_ERR(call_result)) {
    char chain[8192] = {0};
    picomesh_error_snprint(chain, sizeof(chain), call_result.error);
...
static struct picomesh_void_result
time_clock_now_ms_minvoke(struct ctx *ctx, struct object *obj,
                          struct yheaders *hdrs, cmp_ctx_t *_mr, uint32_t _argc,
                          cmp_ctx_t *_mw, char *_err, size_t _err_cap) {
  (void)_mr;
  if (_argc != 0u) {
    snprintf(_err, _err_cap, "time_clock_now_ms: expected 0 arg(s), got %u",
             _argc);
    return PICOMESH_ERR(picomesh_void,
                        "time_clock_now_ms: wrong argument count");
  }
  struct ctx local_ctx = {0};
  struct ctx *call_ctx = ctx ? ctx : &local_ctx;
  struct picomesh_int64_result call_result =
...
static struct picomesh_void_result time_clock_sleep_ms_minvoke(
  ...
  (void)_mr;
  if (_argc != 1u) {
    snprintf(_err, _err_cap, "time_clock_sleep_ms: expected 1 arg(s), got %u",
             _argc);
    return PICOMESH_ERR(picomesh_void,
                        "time_clock_sleep_ms: wrong argument count");
  }
  uint32_t _v0;
  {
    uint64_t _u;
    if (!cmp_read_uinteger(_mr, &_u)) {
      snprintf(_err, _err_cap, "ms: expected unsigned int (%s)",
               cmp_strerror(_mr));
      return PICOMESH_ERR(picomesh_void, "minvoke: bad argument");
    }
    if (_u > UINT32_MAX) {
      snprintf(_err, _err_cap, "ms: value %llu out of range for uint32_t",
  ...
  struct ctx *call_ctx = ctx ? ctx : &local_ctx;
  struct picomesh_int64_result call_result =
      time_clock_sleep_ms(call_ctx, obj, hdrs, _v0);
  if (PICOMESH_IS_ERR(call_result)) {
    char chain[8192] = {0};
    picomesh_error_snprint(chain, sizeof(chain), call_result.error);
...
struct object_ptr_result time_clock_create(struct ctx *ctx) {
  ...
  if (PICOMESH_IS_ERR(_kr))
    return PICOMESH_ERR(object_ptr, "time_clock_create: class accessor failed",
                        _kr);
...
static const struct jinvoke_params *time_params_lookup(const char *qname) {
  for (size_t i = 0; i < sizeof(time_params_rows) / sizeof(time_params_rows[0]);
       ++i)
...
static struct class_ptr_result time_accessor_lookup(const char *name) {
  if (strcmp(name, "time_clock") == 0)
    return time_clock_class_get();
  return PICOMESH_OK(class_ptr, NULL);
}

/* ---- time: slot → skel, name-keyed static data --------------- */

struct time_skel_row {
  const char *name;
  rpc_skel_fn fn;
};
...
static rpc_skel_fn time_skel_lookup(const char *name) {
  ...
  for (size_t i = 0; i < sizeof(time_skel_rows) / sizeof(time_skel_rows[0]);
       ++i)
...
struct picomesh_void_result picomesh_plugin_time_register(void) {
  struct picomesh_void_result _ar =
      class_add_accessor_lookup(time_accessor_lookup);
  ...
  jinvoke_params_add_lookup(time_params_lookup);
  {
    ...
    PICOMESH_RETURN_IF_ERR(picomesh_void, reg,
                           "time register: prewarm time_clock_class_get");
  }
  return PICOMESH_OK_VOID();
}
