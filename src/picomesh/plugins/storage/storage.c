...
static struct picomesh_int_result resolve_configured_backend(void) {
  ...
  PICOMESH_RETURN_IF_ERR(picomesh_int, backend_node,
                         "storage: reading 'storage.backend' failed");
...
static struct picomesh_void_result ensure_backend(struct storage_data *data) {
  if (data->backend != STORAGE_BACKEND_UNSET && data->vt)
    return PICOMESH_OK_VOID();
  struct picomesh_int_result backend_res = resolve_configured_backend();
  PICOMESH_RETURN_IF_ERR(picomesh_void, backend_res,
                         "storage: resolve backend failed");
  ...
  PICOMESH_RETURN_IF_ERR(picomesh_void, path_node,
                         "storage: reading data-path config failed");
  data->data_path =
      path_node.value ? config_node_as_string(path_node.value, NULL) : NULL;
  data->backend = backend;
  ydebug("storage: backend=%s",
         backend == STORAGE_BACKEND_MDBX ? "mdbx" : "sqlite");
  return PICOMESH_OK_VOID();
}
...
static const char *rc_msg(enum storage_rc rc) {
  ...
  case STORAGE_RC_OK:
    return "ok";
  case STORAGE_RC_NOT_FOUND:
    return "key not found";
  case STORAGE_RC_BAD_CONTEXT:
    return "invalid context name";
  case STORAGE_RC_OPEN_FAILED:
    return "backend open failed";
  case STORAGE_RC_INTERNAL:
...
static void storage_work_fn(void *arg) {
  ...
  switch (work->op) {
  case STORAGE_OP_SET:
    work->rc = vt->set(work->data, work->context, work->key, work->value);
    break;
  case STORAGE_OP_GET:
    work->rc = vt->get(work->data, work->context, work->key, &work->out_str);
    break;
  case STORAGE_OP_EXISTS:
    work->rc = vt->exists(work->data, work->context, work->key, &work->out_i);
    break;
  case STORAGE_OP_DEL:
    work->rc = vt->del(work->data, work->context, work->key, &work->out_i);
    break;
  case STORAGE_OP_COUNT:
    work->rc = vt->count(work->data, work->context, &work->out_sz);
    break;
  }
}
...
static struct picomesh_void_result storage_run(struct storage_work *work) {
  ...
  struct picomesh_void_result offload_res =
      loop_run_blocking(loop, storage_work_fn, work);
...
storage_set_impl(struct ctx *ctx, struct object *obj, struct yheaders *hdrs,
  ...
  if (PICOMESH_IS_ERR(backend_init))
    return PICOMESH_ERR(picomesh_int, "storage_set: backend init failed",
                        backend_init);
  struct storage_work work = {.data = data,
                              .op = STORAGE_OP_SET,
                              .context = context,
                              .key = key,
                              .value = value};
  ...
  ydebug("span trace=%s op=db.set.%s dur_us=%.0f", trace ? trace : "-", context,
         span_us);
  yspan_record("db.set", span_us);
  if (work.rc != STORAGE_RC_OK)
    return PICOMESH_ERR(picomesh_int, rc_msg(work.rc));
...
storage_get_impl(struct ctx *ctx, struct object *obj, struct yheaders *hdrs,
  ...
  struct picomesh_void_result backend_init = ensure_backend(data);
  if (PICOMESH_IS_ERR(backend_init))
    return PICOMESH_ERR(picomesh_string, "storage_get: backend init failed",
                        backend_init);
  struct storage_work work = {
      .data = data, .op = STORAGE_OP_GET, .context = context, .key = key};
  double span_start = picomesh_ytime_monotonic_sec();
  storage_run(&work);
  double span_us = (picomesh_ytime_monotonic_sec() - span_start) * 1e6;
  ydebug("span trace=%s op=db.get.%s dur_us=%.0f", trace ? trace : "-", context,
         span_us);
  yspan_record("db.get", span_us);
  if (work.rc != STORAGE_RC_OK)
    return PICOMESH_ERR(picomesh_string, rc_msg(work.rc));
  return PICOMESH_OK(picomesh_string, work.out_str);
}

PICOMESH_CLASS_ANNOTATE("override@storage:db:exists")
struct picomesh_int_result
storage_exists_impl(struct ctx *ctx, struct object *obj, struct yheaders *hdrs,
                    const char *context, const char *key) {
  (void)ctx;
  struct storage_data *data = sd(obj);
  struct picomesh_void_result backend_init = ensure_backend(data);
  if (PICOMESH_IS_ERR(backend_init))
    return PICOMESH_ERR(picomesh_int, "storage_exists: backend init failed",
                        backend_init);
  struct storage_work work = {
      .data = data, .op = STORAGE_OP_EXISTS, .context = context, .key = key};
  storage_run(&work);
  if (work.rc != STORAGE_RC_OK)
    return PICOMESH_ERR(picomesh_int, rc_msg(work.rc));
...
struct picomesh_int_result storage_del_impl(struct ctx *ctx, struct object *obj,
  ...
  struct picomesh_void_result backend_init = ensure_backend(data);
  if (PICOMESH_IS_ERR(backend_init))
    return PICOMESH_ERR(picomesh_int, "storage_del: backend init failed",
                        backend_init);
  struct storage_work work = {
      .data = data, .op = STORAGE_OP_DEL, .context = context, .key = key};
  storage_run(&work);
  if (work.rc != STORAGE_RC_OK)
    return PICOMESH_ERR(picomesh_int, rc_msg(work.rc));
  return PICOMESH_OK(picomesh_int, work.out_i);
...
struct picomesh_size_result storage_count_impl(struct ctx *ctx,
                                               struct object *obj,
                                               struct yheaders *hdrs,
                                               const char *context) {
  (void)ctx;
  ...
  struct picomesh_void_result backend_init = ensure_backend(data);
  if (PICOMESH_IS_ERR(backend_init))
    return PICOMESH_ERR(picomesh_size, "storage_count: backend init failed",
                        backend_init);
  struct storage_work work = {
      .data = data, .op = STORAGE_OP_COUNT, .context = context};
  storage_run(&work);
  if (work.rc != STORAGE_RC_OK)
    return PICOMESH_ERR(picomesh_size, rc_msg(work.rc));
