...
static MDBX_env *mdbx_shared_env(const char *path) {
  ...
  rc = mdbx_env_open(new_env, path, MDBX_WRITEMAP | MDBX_NOMETASYNC, 0644);
  if (rc != MDBX_SUCCESS) {
...
static MDBX_dbi mdbx_resolve_dbi(MDBX_env *env, const char *context,
  ...
  if (rc != MDBX_SUCCESS) {
    ywarn("storage[mdbx]: txn_begin (dbi-open) failed: %s", mdbx_strerror(rc));
    pthread_mutex_unlock(&cache_mu);
  ...
  if (rc != MDBX_SUCCESS) {
    ywarn("storage[mdbx]: dbi_open(%s) failed: %s", context, mdbx_strerror(rc));
    mdbx_txn_abort(txn);
    pthread_mutex_unlock(&cache_mu);
    *out_rc = STORAGE_RC_INTERNAL;
    return 0;
  ...
  if (rc != MDBX_SUCCESS) {
    ywarn("storage[mdbx]: txn_commit (dbi-open) failed: %s", mdbx_strerror(rc));
    pthread_mutex_unlock(&cache_mu);
    *out_rc = STORAGE_RC_INTERNAL;
    return 0;
  }
...
static enum storage_rc bmdbx_setup(struct storage_data *d, const char *context,
  ...
  if (!storage_context_is_valid(context))
    return STORAGE_RC_BAD_CONTEXT;
...
static enum storage_rc bmdbx_set(struct storage_data *d, const char *context,
  ...
  if (r != MDBX_SUCCESS) {
    mdbx_txn_abort(txn);
    ywarn("storage[mdbx]: put(%s/%s) failed: %s", context, key,
          mdbx_strerror(r));
    return STORAGE_RC_INTERNAL;
  }
...
static enum storage_rc bmdbx_get(struct storage_data *d, const char *context,
  ...
  if (r == MDBX_SUCCESS) {
    ...
    if (!copy) {
      ...
      if (v.iov_len)
        memcpy(copy, v.iov_base, v.iov_len);
      copy[v.iov_len] = 0;
      *out = copy;
    ...
    ywarn("storage[mdbx]: get(%s/%s) failed: %s", context, key,
          mdbx_strerror(r));
    result = STORAGE_RC_INTERNAL;
  }
  mdbx_txn_abort(txn);
...
static enum storage_rc bmdbx_del(struct storage_data *d, const char *context,
  ...
  ywarn("storage[mdbx]: del(%s/%s) failed: %s", context, key, mdbx_strerror(r));
  return STORAGE_RC_INTERNAL;
}
...
static enum storage_rc bmdbx_count(struct storage_data *d, const char *context,
  ...
  mdbx_txn_abort(txn);
  if (r != MDBX_SUCCESS) {
    ywarn("storage[mdbx]: dbi_stat(%s) failed: %s", context, mdbx_strerror(r));
    return STORAGE_RC_INTERNAL;
  }
