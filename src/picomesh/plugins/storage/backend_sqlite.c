...
static enum storage_rc sqlite_ensure_table(struct storage_data *d,
  ...
  if (rc != SQLITE_OK) {
    ywarn("storage[sqlite]: DDL '%s' failed: %s", ddl, err ? err : "(no msg)");
    sqlite3_free(err);
...
static enum storage_rc sqlite_set(struct storage_data *d, const char *context,
  ...
  char sql[128];
  snprintf(sql, sizeof(sql), "INSERT OR REPLACE INTO kv_%s(k, v) VALUES(?, ?);",
           context);
...
static enum storage_rc sqlite_get(struct storage_data *d, const char *context,
  ...
  char sql[128];
  snprintf(sql, sizeof(sql), "SELECT v FROM kv_%s WHERE k=?;", context);
  sqlite3_stmt *stmt = NULL;
  if (sqlite3_prepare_v2(d->be.sqlite.db, sql, -1, &stmt, NULL) != SQLITE_OK)
    return STORAGE_RC_INTERNAL;
  sqlite3_bind_text(stmt, 1, key, -1, SQLITE_TRANSIENT);
...
static enum storage_rc sqlite_exists(struct storage_data *d,
  ...
  char sql[128];
  snprintf(sql, sizeof(sql), "SELECT 1 FROM kv_%s WHERE k=?;", context);
  sqlite3_stmt *stmt = NULL;
  if (sqlite3_prepare_v2(d->be.sqlite.db, sql, -1, &stmt, NULL) != SQLITE_OK)
    return STORAGE_RC_INTERNAL;
  sqlite3_bind_text(stmt, 1, key, -1, SQLITE_TRANSIENT);
...
static enum storage_rc sqlite_del(struct storage_data *d, const char *context,
  ...
  char sql[128];
  snprintf(sql, sizeof(sql), "DELETE FROM kv_%s WHERE k=?;", context);
  sqlite3_stmt *stmt = NULL;
  if (sqlite3_prepare_v2(d->be.sqlite.db, sql, -1, &stmt, NULL) != SQLITE_OK)
    return STORAGE_RC_INTERNAL;
  sqlite3_bind_text(stmt, 1, key, -1, SQLITE_TRANSIENT);
