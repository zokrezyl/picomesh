...
static void mesh_reap_signal_handler(int sig) {
  for (int i = 0; i < MESH_MAX_CHILDREN; ++i) {
    ...
    if (p > 0)
      kill(p, SIGTERM); /* async-signal-safe */
  }
  signal(sig, SIG_DFL);
  raise(sig);
}
...
static struct object_ptr_result mesh_storage_db(void) {
  ...
  if (!engine)
    return PICOMESH_ERR(object_ptr, "mesh_storage_db: no active engine");
  ...
  PICOMESH_RETURN_IF_ERR(object_ptr, create_res,
                         "mesh_storage_db: storage create failed");
 ...
 * pretend success. */
static struct picomesh_void_result mesh_storage_set(const char *key,
                                                    const char *val) {
  ...
  if (!db || !e)
    return PICOMESH_ERR(picomesh_void, "mesh: storage unavailable");
  ...
  if (PICOMESH_IS_ERR(r))
    return PICOMESH_ERR(picomesh_void, "mesh: lifecycle write failed", r);
...
static struct picomesh_void_result mesh_storage_del(const char *key) {
  ...
  if (PICOMESH_IS_ERR(r))
    return PICOMESH_ERR(picomesh_void, "mesh: lifecycle delete failed", r);
  return PICOMESH_OK_VOID();
}
...
static struct picomesh_void_result mesh_child_track(int slot, int pid,
                                                    const char *name) {
  if (slot >= 0 && slot < MESH_MAX_CHILDREN)
    g_mesh_reap_pids[slot] = pid;
  char key[32], val[160];
...
static struct picomesh_void_result mesh_reap_previous_run(void) {
  struct object_ptr_result db_res = mesh_storage_db();
  PICOMESH_RETURN_IF_ERR(picomesh_void, db_res,
                         "mesh_reap: storage unavailable");
  struct object *db = db_res.value;
  struct picomesh_engine *e = picomesh_active_engine();
  if (!db || !e)
    return PICOMESH_OK_VOID();
  ...
  for (int slot = 0; slot < MESH_MAX_CHILDREN; ++slot) {
    ...
    if (PICOMESH_IS_OK(g))
      ...
      picomesh_error_destroy(g.error);
    ...
    if (PICOMESH_IS_ERR(d)) {
      ywarn("mesh: failed to clear stale child record slot=%d (%s)", slot,
...
mesh_mesh_register_service_impl(struct ctx *ctx, struct object *obj,
                                struct yheaders *hdrs, uint32_t service_id,
                                uint32_t port) {
  (void)ctx;
...
struct picomesh_uint32_result mesh_mesh_resolve_impl(struct ctx *ctx,
                                                     struct object *obj,
...
struct picomesh_int_result mesh_mesh_forget_impl(struct ctx *ctx,
                                                 struct object *obj,
                                                 struct yheaders *hdrs,
                                                 uint32_t service_id) {
  (void)ctx;
...
mesh_mesh_count_services_impl(struct ctx *ctx, struct object *obj,
                              struct yheaders *hdrs) {
  (void)ctx;
...
static int find_child_slot(struct mesh_mesh_data *d, int32_t pid) {
  for (int i = 0; i < MESH_MAX_CHILDREN; ++i) {
    if (!d->children[i].exited && d->children[i].pid == pid)
      return i;
  }
  return -1;
}
...
static void mesh_child_exit_cb_real(struct loop_process *p, int64_t exit_status,
  ...
  if (slot >= 0) {
    ...
    if (PICOMESH_IS_ERR(untrack_res))
      picomesh_error_destroy(untrack_res.error);
    yinfo("mesh: child pid=%d exited (status=%d, term_signal=%d)", c->pid,
...
struct picomesh_int_result mesh_mesh_spawn_picomesh_impl(struct ctx *ctx,
                                                         struct object *obj,
                                                         struct yheaders *hdrs,
                                                         uint32_t port) {
  (void)ctx;
  ...
  struct picomesh_engine *e = picomesh_active_engine();
  if (!e)
    return PICOMESH_ERR(picomesh_int, "spawn_picomesh: no active engine");

  int slot = -1;
  for (int i = 0; i < MESH_MAX_CHILDREN; ++i) {
    if (d->children[i].exited || d->children[i].pid == 0) {
      slot = i;
      break;
    }
  }
  if (slot < 0)
    return PICOMESH_ERR(picomesh_int, "spawn_picomesh: child table full");
  ...
  if (n <= 0)
    return PICOMESH_ERR(picomesh_int,
                        "spawn_picomesh: readlink(/proc/self/exe) failed");
  self_exe[n] = 0;
  ...
  char *argv[] = {
      self_exe,         (char *)"--frontend", (char *)"yhttp",
      (char *)"--host", (char *)"127.0.0.1",  (char *)"--port",
      port_str,         (char *)"serve",      NULL,
  };

  struct mesh_child_cookie *c = calloc(1, sizeof(*c));
  if (!c)
    return PICOMESH_ERR(picomesh_int, "spawn_picomesh: calloc(cookie) failed");
  c->obj = obj;

  struct picomesh_int_result spawn_res = loop_spawn(
      picomesh_engine_loop(e), self_exe, argv, mesh_child_exit_cb_real, c);
  if (PICOMESH_IS_ERR(spawn_res)) {
    free(c);
    return PICOMESH_ERR(picomesh_int, "spawn_picomesh: loop_spawn failed",
                        spawn_res);
  }
  ...
  if (PICOMESH_IS_ERR(tr)) {
    ywarn("mesh: spawned pid=%d but its cross-run reap record FAILED to "
          "persist (%s) — "
          "the in-memory reaper still covers this run's shutdown",
          pid, tr.error.msg ? tr.error.msg : "?");
...
struct picomesh_int_result mesh_mesh_kill_pid_impl(struct ctx *ctx,
                                                   struct object *obj,
                                                   struct yheaders *hdrs,
                                                   int32_t pid) {
  (void)ctx;
  struct picomesh_engine *e = picomesh_active_engine();
  if (!e)
    return PICOMESH_ERR(picomesh_int, "kill_pid: no active engine");
  struct mesh_mesh_data *d = ms(obj);
  if (find_child_slot(d, pid) < 0) {
    return PICOMESH_OK(picomesh_int, 0);
  }
  struct picomesh_void_result kill_res =
      loop_kill(picomesh_engine_loop(e), pid, SIGTERM);
...
struct picomesh_size_result
mesh_mesh_count_children_impl(struct ctx *ctx, struct object *obj,
                              struct yheaders *hdrs) {
  (void)ctx;
...
static int split_config_kv_cb(const char *key, const struct config_node *val,
                              void *ud) {
  struct split_emit *se = ud;
  int kn = snprintf(se->buf + se->off, se->cap - se->off, "%s: ", key);
  if (kn < 0 || (size_t)kn >= se->cap - se->off) {
    se->overflow = 1;
    return 1;
  }
  se->off += (size_t)kn;
...
mesh_write_node_config(struct picomesh_engine *e, const char *name, char *out,
                       size_t out_cap) {
  char base[256];
  ...
  struct config_node_ptr_result cfgr =
      config_get(picomesh_engine_config(e), path);
  PICOMESH_RETURN_IF_ERR(picomesh_void, cfgr,
                         "mesh_write_node_config: config block read failed");
  const struct config_node *cfg_node = cfgr.value ? cfgr.value : NULL;

  snprintf(path, sizeof(path), "%s.port", base);
  struct config_node_ptr_result pr =
      config_get(picomesh_engine_config(e), path);
  PICOMESH_RETURN_IF_ERR(picomesh_void, pr,
                         "mesh_write_node_config: port read failed");
  ...
  struct config_node_ptr_result hr =
      config_get(picomesh_engine_config(e), path);
  PICOMESH_RETURN_IF_ERR(picomesh_void, hr,
                         "mesh_write_node_config: host read failed");
  const char *host = hr.value ? config_node_as_string(hr.value, NULL) : NULL;

  snprintf(path, sizeof(path), "%s.frontend", base);
  struct config_node_ptr_result fr =
      config_get(picomesh_engine_config(e), path);
  PICOMESH_RETURN_IF_ERR(picomesh_void, fr,
                         "mesh_write_node_config: frontend read failed");
  const char *frontend =
      fr.value ? config_node_as_string(fr.value, NULL) : NULL;

  snprintf(path, sizeof(path), "%s.plugins", base);
  struct config_node_ptr_result plr =
      config_get(picomesh_engine_config(e), path);
  PICOMESH_RETURN_IF_ERR(picomesh_void, plr,
                         "mesh_write_node_config: plugins read failed");
  const struct config_node *plugins_node = plr.value ? plr.value : NULL;

  snprintf(path, sizeof(path), "%s.workers", base);
  struct config_node_ptr_result wr =
      config_get(picomesh_engine_config(e), path);
  PICOMESH_RETURN_IF_ERR(picomesh_void, wr,
                         "mesh_write_node_config: workers read failed");
  ...
  size_t off = 0;
  off +=
      (size_t)snprintf(body + off, sizeof(body) - off, "name: \"%s\"\n", name);
  if (host)
    off += (size_t)snprintf(body + off, sizeof(body) - off, "host: \"%s\"\n",
                            host);
  ...
  if (port_auto)
    off += (size_t)snprintf(body + off, sizeof(body) - off, "port: auto\n");
  else if (port > 0)
    off += (size_t)snprintf(body + off, sizeof(body) - off, "port: %lld\n",
                            (long long)port);
  if (frontend)
    off += (size_t)snprintf(body + off, sizeof(body) - off,
                            "frontend: \"%s\"\n", frontend);
  if (workers > 0)
    off += (size_t)snprintf(body + off, sizeof(body) - off, "workers: %lld\n",
                            (long long)workers);
  ...
  PICOMESH_RETURN_IF_ERR(picomesh_void, reg_hr,
                         "mesh_write_node_config: registry host read failed");
  struct config_node_ptr_result reg_pr =
      config_get(picomesh_engine_config(e), "mesh.services.registry.port");
  PICOMESH_RETURN_IF_ERR(picomesh_void, reg_pr,
                         "mesh_write_node_config: registry port read failed");
  int64_t reg_port = reg_pr.value ? config_node_as_int(reg_pr.value, -1) : -1;
  if (reg_port > 0) {
    const char *reg_host =
        reg_hr.value ? config_node_as_string(reg_hr.value, NULL) : NULL;
    off += (size_t)snprintf(body + off, sizeof(body) - off,
                            "registry:\n  host: \"%s\"\n  port: %lld\n",
  ...
  if (cfg_node && config_node_kind(cfg_node) == CONFIG_MAP) {
    ...
    if (se.overflow)
      return PICOMESH_ERR(picomesh_void,
                          "mesh_write_node_config: config body overflow");
  }

  if (off >= sizeof(body))
    return PICOMESH_ERR(picomesh_void,
                        "mesh_write_node_config: body buffer overflow");
  ...
  PICOMESH_RETURN_IF_ERR(picomesh_void, ndr,
                         "mesh_write_node_config: nodes_dir read failed");
  const char *nodes_dir =
      ndr.value ? config_node_as_string(ndr.value, NULL) : NULL;
  ...
  if (snprintf(dir, sizeof(dir), "%s", nodes_dir) >= (int)sizeof(dir)) {
    ...
    return PICOMESH_ERR(picomesh_void,
                        "mesh_write_node_config: nodes_dir too long");
  }
  char acc[256] = {0};
  ...
  FILE *f = fopen(out, "w");
  if (!f) {
    ywarn("mesh.split: cannot write %s: %s", out, strerror(errno));
    return PICOMESH_ERR(picomesh_void,
                        "mesh_write_node_config: cannot open output file");
  }
  fwrite(body, 1, off, f);
  fclose(f);
  return PICOMESH_OK_VOID();
...
static struct picomesh_int_result mesh_internal_spawn(struct object *obj,
                                                      const char *name) {
  if (!name || !*name)
    return PICOMESH_OK(picomesh_int, 0);
  struct mesh_mesh_data *d = ms(obj);
  ...
  struct config_node_ptr_result fr =
      config_get(picomesh_engine_config(e), fe_path);
  PICOMESH_RETURN_IF_ERR(picomesh_int, fr,
                         "mesh_internal_spawn: frontend config read failed");
  if (fr.value) {
  ...
  struct picomesh_void_result write_res =
      mesh_write_node_config(e, name, node_cfg[slot], sizeof(node_cfg[slot]));
  ...
  char *argv[] = {
      self_exe,     (char *)"--config-file", (char *)cfg,      (char *)"--name",
      (char *)name, (char *)"--frontend",    (char *)frontend, (char *)"serve",
      NULL,
  };
  ...
  if (!c)
    return PICOMESH_ERR(picomesh_int,
                        "mesh_internal_spawn: child cookie alloc failed");
  c->obj = obj;
  struct picomesh_int_result spawn_res = loop_spawn(
      picomesh_engine_loop(e), self_exe, argv, mesh_child_exit_cb_real, c);
  if (PICOMESH_IS_ERR(spawn_res)) {
    free(c);
    return PICOMESH_ERR(picomesh_int, "mesh_internal_spawn: loop_spawn failed",
                        spawn_res);
  }
  int pid = spawn_res.value;
  c->pid = pid;
  ...
  if (PICOMESH_IS_ERR(tr)) {
    ywarn("mesh: spawned service='%s' pid=%d but its cross-run reap record "
          "FAILED to persist (%s) — "
          "the in-memory reaper still covers this run's shutdown",
          name, pid, tr.error.msg ? tr.error.msg : "?");
    picomesh_error_destroy(tr.error);
  }
...
static struct picomesh_int_result mesh_registry_addr(struct picomesh_engine *e,
                                                     char *host_out, size_t cap,
                                                     int *port_out) {
  struct config_node_ptr_result hr =
      config_get(picomesh_engine_config(e), "mesh.services.registry.host");
  PICOMESH_RETURN_IF_ERR(
      picomesh_int, hr, "mesh_registry_addr: registry host config read failed");
  struct config_node_ptr_result pr =
      config_get(picomesh_engine_config(e), "mesh.services.registry.port");
  PICOMESH_RETURN_IF_ERR(
      picomesh_int, pr, "mesh_registry_addr: registry port config read failed");
...
mesh_reg_session_open(struct picomesh_engine *e,
                      struct mesh_reg_session *session) {
  char host[64];
  int port = 0;
  struct picomesh_int_result addr_res =
      mesh_registry_addr(e, host, sizeof(host), &port);
  PICOMESH_RETURN_IF_ERR(picomesh_int, addr_res,
                         "mesh_reg_session_open: registry addr lookup failed");
  if (!addr_res.value)
    return PICOMESH_OK(picomesh_int, 0);
  int fd = -1;
  ...
  if (PICOMESH_IS_ERR(obj_r)) {
    ...
    return PICOMESH_ERR(picomesh_int,
                        "mesh_reg_session_open: registry create failed", obj_r);
  }
...
static struct picomesh_int_result
mesh_reg_resolve(struct mesh_reg_session *session, const char *service,
                 int wait, char *host_out, size_t cap, int *port_out) {
...
static struct picomesh_int_result
mesh_allocate_webapp_port(struct picomesh_engine *e, const char *name,
                          const char *host) {
  struct mesh_reg_session reg = {0};
  struct picomesh_int_result session_res = mesh_reg_session_open(e, &reg);
  PICOMESH_RETURN_IF_ERR(picomesh_int, session_res,
                         "mesh_allocate_webapp_port: registry session failed");
  if (!session_res.value)
    return PICOMESH_ERR(picomesh_int,
                        "mesh_allocate_webapp_port: registry unreachable");
  char ph[64];
  int pp = 0;
  struct picomesh_int_result resolve_res =
      mesh_reg_resolve(&reg, "portalloc", 1, ph, sizeof(ph), &pp);
  mesh_reg_session_close(&reg);
  PICOMESH_RETURN_IF_ERR(picomesh_int, resolve_res,
                         "mesh_allocate_webapp_port: portalloc resolve failed");
  if (!resolve_res.value)
    return PICOMESH_ERR(
        picomesh_int, "mesh_allocate_webapp_port: portalloc not discoverable");
  ...
  if (fd < 0)
    return PICOMESH_ERR(picomesh_int,
                        "mesh_allocate_webapp_port: portalloc unreachable");
  struct peer_channel *channel = peer_channel_create(fd);
  if (!channel) {
    close(fd);
    return PICOMESH_ERR(picomesh_int,
                        "mesh_allocate_webapp_port: peer channel alloc failed");
  }
  ...
  if (PICOMESH_IS_ERR(obj_r)) {
    ...
    return PICOMESH_ERR(picomesh_int,
                        "mesh_allocate_webapp_port: portalloc create failed",
                        obj_r);
  }
  ...
  for (int attempt = 0; attempt < 64; ++attempt) {
    ...
    if (mesh_probe_bind(host, (int)candidate)) {
      port = (int)candidate;
      break;
    }
    struct picomesh_int_result rel =
        portalloc_portalloc_release(&pctx, pobj, NULL, candidate);
    if (PICOMESH_IS_ERR(rel))
      picomesh_error_destroy(rel.error);
    mesh_nap_ms(20);
  }
  ...
  if (port <= 0)
    return PICOMESH_ERR(
        picomesh_int,
        "mesh_allocate_webapp_port: no bindable port from portalloc");
  return PICOMESH_OK(picomesh_int, port);
}

static struct picomesh_int_result
mesh_register_webapp(struct picomesh_engine *e, const char *name,
                     const char *host, int port) {
  struct mesh_reg_session reg = {0};
  struct picomesh_int_result session_res = mesh_reg_session_open(e, &reg);
  PICOMESH_RETURN_IF_ERR(picomesh_int, session_res,
                         "mesh_register_webapp: registry session failed");
  if (!session_res.value)
    return PICOMESH_OK(picomesh_int, 0);
  const char *reg_host = (strcmp(host, "0.0.0.0") == 0) ? "127.0.0.1" : host;
  struct picomesh_int_result r = registry_registry_register_service(
      &reg.ctx, reg.obj, NULL, name, name, reg_host, (uint32_t)port);
...
static int mesh_sibling_exe(const char *app, char *out, size_t cap) {
  if (!app || !*app)
    return 0;
  if (strchr(app, '/'))
    return snprintf(out, cap, "%s", app) < (int)cap;
  char self_exe[4096];
  ssize_t n = readlink("/proc/self/exe", self_exe, sizeof(self_exe) - 1);
  if (n <= 0)
    return 0;
...
static struct picomesh_int_result mesh_spawn_webapp(struct object *obj,
                                                    const char *name) {
  if (!name || !*name)
    return PICOMESH_OK(picomesh_int, 0);
  struct mesh_mesh_data *d = ms(obj);
  ...
  struct config_node_ptr_result ar =
      config_get(picomesh_engine_config(e), path);
  PICOMESH_RETURN_IF_ERR(picomesh_int, ar,
                         "mesh_spawn_webapp: app config read failed");
  ...
  struct config_node_ptr_result hr =
      config_get(picomesh_engine_config(e), path);
  PICOMESH_RETURN_IF_ERR(picomesh_int, hr,
                         "mesh_spawn_webapp: host config read failed");
  const char *host = hr.value ? config_node_as_string(hr.value, NULL) : NULL;
  if (!host || !*host)
    host = "127.0.0.1";
  ...
  struct config_node_ptr_result pr =
      config_get(picomesh_engine_config(e), path);
  PICOMESH_RETURN_IF_ERR(picomesh_int, pr,
                         "mesh_spawn_webapp: port config read failed");
  const struct config_node *port_node = pr.value ? pr.value : NULL;
  int port = -1;
  int port_auto = 0;
  ...
  if (port_auto) {
    struct picomesh_int_result alloc_res =
        mesh_allocate_webapp_port(e, name, host);
    PICOMESH_RETURN_IF_ERR(picomesh_int, alloc_res,
                           "mesh_spawn_webapp: webapp port allocation failed");
  ...
  struct config_node_ptr_result ur =
      config_get(picomesh_engine_config(e), path);
  PICOMESH_RETURN_IF_ERR(picomesh_int, ur,
                         "mesh_spawn_webapp: upstream config read failed");
  const char *upstream =
      ur.value ? config_node_as_string(ur.value, NULL) : NULL;
  ...
  struct picomesh_int_result session_res = mesh_reg_session_open(e, &reg);
  PICOMESH_RETURN_IF_ERR(picomesh_int, session_res,
                         "mesh_spawn_webapp: registry session failed");
  if (!session_res.value)
    return PICOMESH_OK(picomesh_int, 0);
  char up_host[64];
  int up_port = 0;
  struct picomesh_int_result resolve_res =
      mesh_reg_resolve(&reg, upstream, 1, up_host, sizeof(up_host), &up_port);
  mesh_reg_session_close(&reg);
  PICOMESH_RETURN_IF_ERR(picomesh_int, resolve_res,
                         "mesh_spawn_webapp: upstream resolve failed");
  if (!resolve_res.value)
    return PICOMESH_OK(picomesh_int, 0);

  char exe[4096];
  if (!mesh_sibling_exe(app, exe, sizeof(exe)))
    return PICOMESH_OK(picomesh_int, 0);
  ...
  if (!c)
    return PICOMESH_ERR(picomesh_int,
                        "mesh_spawn_webapp: child cookie alloc failed");
  c->obj = obj;
  struct picomesh_int_result spawn_res = loop_spawn(
      picomesh_engine_loop(e), exe, argv, mesh_child_exit_cb_real, c);
  if (PICOMESH_IS_ERR(spawn_res)) {
    free(c);
    return PICOMESH_ERR(picomesh_int, "mesh_spawn_webapp: loop_spawn failed",
                        spawn_res);
  }
  int pid = spawn_res.value;
  c->pid = pid;
  ...
  struct picomesh_int_result register_res =
      mesh_register_webapp(e, name, host, port);
  if (PICOMESH_IS_ERR(register_res)) {
    ywarn("mesh.webapps: spawned '%s' pid=%d but registry registration failed",
          name, pid);
...
static int reconcile_is_discovery_plane(const char *service_name) {
  return strcmp(service_name, "registry") == 0 ||
         strcmp(service_name, "portalloc") == 0;
}
...
static int reconcile_walk_cb(const char *service_name,
  ...
  if (config_node_kind(val) != CONFIG_MAP)
    return 0;
  if (reconcile_is_discovery_plane(service_name))
    return 0; /* already spawned */

  struct picomesh_int_result spawn_res =
      mesh_internal_spawn(rc->obj, service_name);
  if (PICOMESH_IS_ERR(spawn_res)) {
...
static int reconcile_webapp_walk_cb(const char *webapp_name,
  ...
  if (config_node_kind(val) != CONFIG_MAP)
    return 0;
  struct picomesh_int_result spawn_res =
      mesh_spawn_webapp(rc->obj, webapp_name);
  if (PICOMESH_IS_ERR(spawn_res)) {
  ...
  if (spawn_res.value > 0) {
    rc->spawned++;
    yinfo("mesh.reconcile: webapp '%s' -> pid=%d", webapp_name,
          spawn_res.value);
  } else {
...
static struct picomesh_void_result
reconcile_spawn_named(struct reconcile_ctx *rc, struct picomesh_engine *e,
                      const char *name) {
  char path[160];
  snprintf(path, sizeof(path), "mesh.services.%s", name);
  struct config_node_ptr_result r = config_get(picomesh_engine_config(e), path);
  PICOMESH_RETURN_IF_ERR(picomesh_void, r,
                         "reconcile_spawn_named: service config read failed");
  if (!r.value || config_node_kind(r.value) != CONFIG_MAP)
    return PICOMESH_OK_VOID();
  struct picomesh_int_result spawn_res = mesh_internal_spawn(rc->obj, name);
  if (PICOMESH_IS_ERR(spawn_res)) {
    ywarn("mesh.reconcile: failed to spawn discovery-plane service '%s' (%s)",
          name, spawn_res.error.msg ? spawn_res.error.msg : "?");
    picomesh_error_destroy(spawn_res.error);
    return PICOMESH_OK_VOID();
  }
  if (spawn_res.value > 0) {
    rc->spawned++;
    yinfo("mesh.reconcile: '%s' (discovery plane) → pid=%d", name,
          spawn_res.value);
  } else {
...
struct picomesh_int_result
mesh_mesh_reconcile_from_config_impl(struct ctx *ctx, struct object *obj,
                                     struct yheaders *hdrs) {
  (void)ctx;
  struct picomesh_engine *e = picomesh_active_engine();
  if (!e)
    return PICOMESH_ERR(picomesh_int,
                        "reconcile_from_config: no active engine");
  ...
  PICOMESH_RETURN_IF_ERR(picomesh_int, reap_res,
                         "reconcile_from_config: reap previous run failed");
  char parent_pid[16];
  ...
  if (PICOMESH_IS_ERR(pp))
    return PICOMESH_ERR(picomesh_int,
                        "reconcile_from_config: cannot persist parent pid", pp);
  ...
  if (PICOMESH_IS_ERR(r) || !r.value) {
    return PICOMESH_ERR(picomesh_int,
                        "reconcile_from_config: mesh.services missing");
  }
  if (config_node_kind(r.value) != CONFIG_MAP) {
    return PICOMESH_ERR(picomesh_int,
                        "reconcile_from_config: mesh.services not a map");
  }
  ...
  struct picomesh_void_result registry_spawn =
      reconcile_spawn_named(&rc, e, "registry");
  PICOMESH_RETURN_IF_ERR(picomesh_int, registry_spawn,
                         "reconcile_from_config: spawn registry");
  struct picomesh_void_result portalloc_spawn =
      reconcile_spawn_named(&rc, e, "portalloc");
  PICOMESH_RETURN_IF_ERR(picomesh_int, portalloc_spawn,
                         "reconcile_from_config: spawn portalloc");
  config_node_for_each(r.value, reconcile_walk_cb, &rc);

  struct config_node_ptr_result wr =
      config_get(picomesh_engine_config(e), "mesh.webapps");
  if (PICOMESH_IS_OK(wr) && wr.value) {
    if (config_node_kind(wr.value) != CONFIG_MAP) {
      return PICOMESH_ERR(picomesh_int,
                          "reconcile_from_config: mesh.webapps not a map");
    }
...
struct picomesh_int_result mesh_mesh_reconcile_impl(struct ctx *ctx,
                                                    struct object *obj,
                                                    struct yheaders *hdrs) {
  (void)ctx;
