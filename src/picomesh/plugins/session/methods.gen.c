...
struct picomesh_uint32_result session_store_start(struct ctx * ctx, struct object * obj, struct yheaders * hdrs, uint32_t user_id, uint32_t provider_id)
{
    ...
    if (_slot == METHOD_SLOT_UNDEFINED) {
        ...
        if (PICOMESH_IS_ERR(_sr))
            return PICOMESH_ERR(picomesh_uint32, "session_store_start: method_slot_get failed", _sr);
        _slot = _sr.value;
    }

    if (!obj) return PICOMESH_ERR(picomesh_uint32, "session_store_start: NULL object");

    struct ctx *_s = ctx;
    if (_s && _s->peer) {
        uint32_t _rid = peer_channel_ensure_remote_id(_s->peer, _slot);
        if (_rid == RPC_REMOTE_ID_UNRESOLVED)
            return PICOMESH_ERR(picomesh_uint32, "session_store_start: remote id unresolved");
        uint8_t _a[16384];
        size_t _off = 0;
         ...
         * contents — it just lets the framework (de)serialize the bag. */
        {
            ...
            if (_hn == 0)
                return PICOMESH_ERR(picomesh_uint32, "session_store_start: header serialize overflow");
            _off = _hn;
        }
        {
            uint64_t _h = *(uint64_t *)((char *)obj + sizeof(*obj));
            if (_off + 8 > sizeof(_a))
                return PICOMESH_ERR(picomesh_uint32, "session_store_start: pack overflow");
        ...
        if (_wn < 1) return PICOMESH_ERR(picomesh_uint32, "session_store_start: short RPC response");
        if (_wbuf[0] != 0) {
            uint32_t _msg_len = 0;
            ...
            _msg[_copy] = 0;
            return PICOMESH_ERR(picomesh_uint32, _msg[0] ? strdup(_msg) : "session_store_start: remote error (no msg)");
        }
        if (_wn != 1 + sizeof(uint32_t)) return PICOMESH_ERR(picomesh_uint32, "session_store_start: truncated RPC payload");
        uint32_t _v;
        memcpy(&_v, _wbuf + 1, sizeof(_v));
        return PICOMESH_OK(picomesh_uint32, _v);
    } else {
        impl_t fn = class_dispatch_lookup(object_class(obj), _slot);
        if (!fn) return PICOMESH_ERR(picomesh_uint32, "session_store_start: no impl on this class");
