 ...
 *   lookup(sid)                           → uid (0 if absent)        —
 * back-compat destroy(sid)                          → 1 if removed, 0 if
 * unknown count_active                          → live session count list /
 * list_all                       → [{"uid":…,"created_at":…}, …]
 *
...
static struct picomesh_void_result session_open(struct rel_handle *rel_handle,
                                                struct yheaders *hdrs,
                                                struct object *obj) {
  struct picomesh_void_result open_res =
      rel_open(rel_handle, "rstore_session", "session");
  if (PICOMESH_IS_ERR(open_res))
    return PICOMESH_ERR(picomesh_void,
                        "session: open relational_storage failed", open_res);
  return rel_ensure_schema(rel_handle, hdrs, &sess(obj)->schema_ensured,
                           SESSION_DDL);
}

PICOMESH_CLASS_ANNOTATE("override@session:session:session_start")
struct picomesh_string_result
session_session_start_impl(struct ctx *ctx, struct object *obj,
  ...
  if (!alloc_token(sid, sizeof(sid)))
    return PICOMESH_ERR(picomesh_string,
                        "session_start: secure random unavailable");

  struct rel_handle rel_handle;
  struct picomesh_void_result open_res = session_open(&rel_handle, hdrs, obj);
  if (PICOMESH_IS_ERR(open_res))
    return PICOMESH_ERR(picomesh_string, "session_start: open failed",
                        open_res);
  rel_handle.shard =
      picomesh_fnv1a32(sid); /* lookup cluster: shard by the sid we issue */
  ...
  struct picomesh_int_result changes_res = rel_exec_changes(
      &rel_handle, hdrs,
      "INSERT INTO sessions(sid,uid,access_jwt,refresh_token,created_at) "
      "VALUES(?,?,?,?,?)",
      args);
  free(args);
  PICOMESH_RETURN_IF_ERR(picomesh_string, changes_res,
                         "session_start: insert failed");
  if (changes_res.value < 1)
    return PICOMESH_ERR(picomesh_string, "session_start: insert failed");

  yinfo("session: started user=%u", user_id); /* never log the sid */
  char *out = strdup(sid);
  return out ? PICOMESH_OK(picomesh_string, out)
             : PICOMESH_ERR(picomesh_string, "session_start: out of memory");
}
...
struct picomesh_string_result session_session_jwt_impl(struct ctx *ctx,
                                                       struct object *obj,
                                                       struct yheaders *hdrs,
                                                       const char *sid) {
  (void)ctx;
  ...
  struct picomesh_void_result open_res = session_open(&rel_handle, hdrs, obj);
  if (PICOMESH_IS_ERR(open_res))
    return PICOMESH_ERR(picomesh_string, "session_jwt: open failed", open_res);
  rel_handle.shard = picomesh_fnv1a32(sid ? sid : "");
  char *args = rel_args1s(sid ? sid : "");
  struct picomesh_string_result jwt_res = rel_query_str(
      &rel_handle, hdrs, "SELECT access_jwt FROM sessions WHERE sid=?", args,
      "access_jwt");
  free(args);
  ...
  if (!jwt) {
    ...
    return empty ? PICOMESH_OK(picomesh_string, empty)
                 : PICOMESH_ERR(picomesh_string, "session_jwt: out of memory");
  }
  return PICOMESH_OK(picomesh_string, jwt);
}

PICOMESH_CLASS_ANNOTATE("override@session:session:session_lookup")
struct picomesh_uint32_result session_session_lookup_impl(struct ctx *ctx,
                                                          struct object *obj,
                                                          struct yheaders *hdrs,
                                                          const char *sid) {
  (void)ctx;
  ...
  struct picomesh_void_result open_res = session_open(&rel_handle, hdrs, obj);
  if (PICOMESH_IS_ERR(open_res))
    return PICOMESH_ERR(picomesh_uint32, "session_lookup: open failed",
                        open_res);
  rel_handle.shard = picomesh_fnv1a32(sid);
  char *args = rel_args1s(sid);
  struct picomesh_int64_result uid_res =
      rel_query_int(&rel_handle, hdrs, "SELECT uid FROM sessions WHERE sid=?",
                    args, "uid", 0, NULL);
  free(args);
  PICOMESH_RETURN_IF_ERR(picomesh_uint32, uid_res,
                         "session_lookup: query failed");
...
struct picomesh_int_result session_session_destroy_impl(struct ctx *ctx,
                                                        struct object *obj,
                                                        struct yheaders *hdrs,
                                                        const char *sid) {
  (void)ctx;
  ...
  struct picomesh_void_result open_res = session_open(&rel_handle, hdrs, obj);
  if (PICOMESH_IS_ERR(open_res))
    return PICOMESH_ERR(picomesh_int, "session_destroy: open failed", open_res);
  rel_handle.shard = picomesh_fnv1a32(sid);
  char *args = rel_args1s(sid);
  struct picomesh_int_result changes_res = rel_exec_changes(
      &rel_handle, hdrs, "DELETE FROM sessions WHERE sid=?", args);
  free(args);
  PICOMESH_RETURN_IF_ERR(picomesh_int, changes_res,
                         "session_destroy: delete failed");
...
session_session_count_active_impl(struct ctx *ctx, struct object *obj,
                                  struct yheaders *hdrs) {
  (void)ctx;
  struct rel_handle rel_handle;
  struct picomesh_void_result open_res = session_open(&rel_handle, hdrs, obj);
  if (PICOMESH_IS_ERR(open_res))
    return PICOMESH_ERR(picomesh_size, "session_count: open failed", open_res);
  struct picomesh_int64_result count_res = rel_query_int_all(
      &rel_handle, hdrs, "SELECT COUNT(*) AS n FROM sessions", "[]", "n");
  if (PICOMESH_IS_ERR(count_res))
    return PICOMESH_ERR(picomesh_size, "session_count: aggregate failed",
                        count_res);
...
struct picomesh_json_result session_session_list_impl(struct ctx *ctx,
                                                      struct object *obj,
                                                      struct yheaders *hdrs,
                                                      int64_t offset,
                                                      int64_t limit) {
  ...
  struct picomesh_void_result open_res = session_open(&rel_handle, hdrs, obj);
  if (PICOMESH_IS_ERR(open_res))
    return PICOMESH_ERR(picomesh_json, "session_list: open failed", open_res);
  /* Globally ordered + paginated across shards by created_at. */
  return rel_query_page(&rel_handle, hdrs,
                        "SELECT uid,created_at FROM sessions", "[]",
                        "created_at", 0, offset, limit);
}

PICOMESH_CLASS_ANNOTATE("override@session:session:session_list_all")
struct picomesh_json_result
session_session_list_all_impl(struct ctx *ctx, struct object *obj,
                              struct yheaders *hdrs) {
  (void)ctx;
  struct rel_handle rel_handle;
  struct picomesh_void_result open_res = session_open(&rel_handle, hdrs, obj);
  if (PICOMESH_IS_ERR(open_res))
    return PICOMESH_ERR(picomesh_json, "session_list_all: open failed",
                        open_res);
