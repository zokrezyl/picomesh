...
struct picomesh_uint32_result session_store_start_impl(struct ctx *ctx, struct object *obj, struct yheaders *hdrs,
                                                    uint32_t user_id, uint32_t provider_id)
{
    (void)ctx; (void)obj;
    struct session_storage_handle_result sr = open_storage();
    if (PICOMESH_IS_ERR(sr)) return PICOMESH_ERR(picomesh_uint32, "session_start: open_storage failed", sr);
    ...
    return PICOMESH_OK(picomesh_uint32, sid);
}
                                                   ...
                                                   uint32_t sid)
{
    ...
    char k[64];
    snprintf(k, sizeof(k), "uid:%u", sid);
    struct picomesh_int_result ex = sharded_storage_db_exists(&h.c, h.obj, hdrs, SESSION_CTX, k);
    int present = PICOMESH_IS_OK(ex) && ex.value;
    if (PICOMESH_IS_ERR(ex)) picomesh_error_destroy(ex.error);
    if (!present) { close_storage(&h); return PICOMESH_OK(picomesh_int, 0); }
...
struct picomesh_size_result session_store_count_active_impl(struct ctx *ctx, struct object *obj, struct yheaders *hdrs)
{
    ...
    int64_t count = kv_get_or_zero(&h, hdrs, "count");
    close_storage(&h);
    return PICOMESH_OK(picomesh_size, (size_t)(count < 0 ? 0 : count));
}
