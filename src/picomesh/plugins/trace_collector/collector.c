...
struct PICOMESH_CLASS_ANNOTATE("class@trace_collector:trace_collector")
    trace_collector_trace_collector_data {
  char placeholder;
};
...
static struct picomesh_int64_result collector_cfg_int(const struct config *cfg,
                                                      const char *key) {
  struct config_node_ptr_result config_res = config_get(cfg, key);
  PICOMESH_RETURN_IF_ERR(picomesh_int64, config_res,
                         "collector_cfg_int: config read failed");
  return PICOMESH_OK(picomesh_int64,
                     config_res.value ? config_node_as_int(config_res.value, 0)
                                      : 0);
}
...
static struct picomesh_void_result collector_ensure_store(void) {
  ...
  if (engine) {
    ...
    PICOMESH_RETURN_IF_ERR(picomesh_void, cfg_value,
                           "collector: telemetry.max_spans");
    if (cfg_value.value > 0)
      store_cfg.max_spans = (size_t)cfg_value.value;
    cfg_value = collector_cfg_int(cfg, "telemetry.max_age_seconds");
    PICOMESH_RETURN_IF_ERR(picomesh_void, cfg_value,
                           "collector: telemetry.max_age_seconds");
    if (cfg_value.value > 0)
      store_cfg.max_age_seconds = (uint64_t)cfg_value.value;
    cfg_value = collector_cfg_int(cfg, "telemetry.shards");
    PICOMESH_RETURN_IF_ERR(picomesh_void, cfg_value,
                           "collector: telemetry.shards");
    if (cfg_value.value > 0)
      store_cfg.shards = (unsigned)cfg_value.value;
    cfg_value = collector_cfg_int(cfg, "telemetry.bucket_spans");
    PICOMESH_RETURN_IF_ERR(picomesh_void, cfg_value,
                           "collector: telemetry.bucket_spans");
    if (cfg_value.value > 0)
      store_cfg.bucket_spans = (size_t)cfg_value.value;
    cfg_value = collector_cfg_int(cfg, "telemetry.flush_ms");
    PICOMESH_RETURN_IF_ERR(picomesh_void, cfg_value,
                           "collector: telemetry.flush_ms");
    if (cfg_value.value > 0)
      store_cfg.flush_ms = (uint64_t)cfg_value.value;
  }
  ytelemetry_store_init_config(
      &store_cfg); /* any field left 0 takes its built-in default */
  done = 1;
  return PICOMESH_OK_VOID();
}
...
render(void (*emit)(struct json_writer *, void *), void *user_data) {
  ...
  if (!out)
    return PICOMESH_ERR(picomesh_string, "trace_collector: out of memory");
...
PICOMESH_CLASS_ANNOTATE(
    "override@trace_collector:trace_collector:trace_collector_ingest")
struct picomesh_void_result
trace_collector_trace_collector_ingest_impl(struct ctx *ctx, struct object *obj,
  ...
  (void)hdrs;
  struct picomesh_void_result store_res = collector_ensure_store();
  PICOMESH_RETURN_IF_ERR(picomesh_void, store_res,
                         "trace_collector_ingest: store init failed");
...
PICOMESH_CLASS_ANNOTATE(
    "override@trace_collector:trace_collector:trace_collector_get_trace")
...
static void emit_traces(struct json_writer *writer, void *user_data) {
  ...
  ytelemetry_store_write_traces(writer, (arg->a && *arg->a) ? arg->a : NULL,
                                arg->n, (arg->b && *arg->b) ? arg->b : NULL);
}

PICOMESH_CLASS_ANNOTATE(
    "override@trace_collector:trace_collector:trace_collector_traces")
...
PICOMESH_CLASS_ANNOTATE(
    "override@trace_collector:trace_collector:trace_collector_services")
...
static void emit_operations(struct json_writer *writer, void *user_data) {
  ...
  ytelemetry_store_write_operations(writer,
                                    (service && *service) ? service : NULL);
}

PICOMESH_CLASS_ANNOTATE(
    "override@trace_collector:trace_collector:trace_collector_operations")
...
PICOMESH_CLASS_ANNOTATE(
    "override@trace_collector:trace_collector:trace_collector_latency")
struct picomesh_string_result trace_collector_trace_collector_latency_impl(
  ...
  struct trace_arg arg = {
      .a = service, .b = operation, .n = (uint64_t)window_secs * 1000000000ull};
...
PICOMESH_CLASS_ANNOTATE(
    "override@trace_collector:trace_collector:trace_collector_stats")
struct picomesh_string_result
trace_collector_trace_collector_stats_impl(struct ctx *ctx, struct object *obj,
                                           struct yheaders *hdrs) {
  (void)ctx;
  (void)obj;
  (void)hdrs;
  struct picomesh_void_result store_res = collector_ensure_store();
  PICOMESH_RETURN_IF_ERR(picomesh_string, store_res,
                         "trace_collector_stats: store init failed");
...
PICOMESH_CLASS_ANNOTATE(
    "override@trace_collector:trace_collector:trace_collector_errors")
struct picomesh_string_result
trace_collector_trace_collector_errors_impl(struct ctx *ctx, struct object *obj,
                                            struct yheaders *hdrs,
                                            uint32_t since_secs) {
  (void)ctx;
  (void)obj;
