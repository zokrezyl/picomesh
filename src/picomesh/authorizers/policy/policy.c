...
policy_create(struct picomesh_engine *engine,
              const struct config_node *config) {
  struct policy_state *state = calloc(1, sizeof(*state));
  if (!state)
    return PICOMESH_ERR(picomesh_void_ptr, "policy: out of memory");
  state->engine = engine;
  state->policy =
      config_node_get(config, "policy"); /* may be NULL → all default-deny */
  struct picomesh_void_ptr_result verifier =
      picomesh_jwt_verifier_create(engine);
  if (PICOMESH_IS_ERR(verifier)) {
    free(state);
    return PICOMESH_ERR(picomesh_void_ptr, "policy: verifier create failed",
                        verifier);
  }
...
static struct picomesh_authz_decision decide(int allowed, int status,
                                             const char *reason) {
  struct picomesh_authz_decision decision = {.allowed = allowed,
                                             .status = status};
...
static int groups_best_rank(const struct json_value *groups,
                            const char *account) {
  if (!groups || !json_is_array(groups) || !account)
    return -1;
...
static int groups_effective_rank(const struct json_value *groups,
                                 const char *path) {
  if (!groups || !path || !*path)
    return -1;
  char prefix[256];
...
static int resolve_account(const char *account_from, const char *endpoint,
                           const struct json_value *args, char *out,
                           size_t cap) {
  if (!account_from || !*account_from)
    return 0;
  if (strcmp(account_from, "site") == 0) {
    snprintf(out, cap, "site");
    return 1;
  }
  ...
  if (strncmp(account_from, "{kwargs.", 8) == 0) {
    ...
    size_t field_len =
        colon ? (size_t)(colon - field) : (size_t)(close - field);
    if (colon && strncmp(colon + 1, "account}", 8) == 0)
      account_part = 1;
    ...
    for (size_t i = 0; i < params->count; ++i) {
      if (strlen(params->items[i].name) == field_len &&
          strncmp(params->items[i].name, field, field_len) == 0) {
        index = (long)i;
        break;
      }
    }
  ...
  if (index < 0 || !args || !json_is_array(args))
    return 0;
...
resolve_repo_namespace(struct policy_state *state, uint32_t repo_id, char *out,
                       size_t cap) {
  ...
  PICOMESH_RETURN_IF_ERR(picomesh_int, create_res,
                         "resolve_repo_namespace: repo create failed");
  struct picomesh_string_result namespace_res =
      git_repo_git_repo_namespace_of(&ctx, create_res.value, NULL, repo_id);
  PICOMESH_RETURN_IF_ERR(picomesh_int, namespace_res,
                         "resolve_repo_namespace: namespace_of failed");
  int ok = (namespace_res.value && namespace_res.value[0]) ? 1 : 0;
  if (ok)
    snprintf(out, cap, "%s", namespace_res.value);
...
static struct picomesh_int_result
resolve_repo_public(struct policy_state *state, uint32_t repo_id) {
  struct ctx ctx = picomesh_engine_service_ctx(state->engine, "git_repo");
  struct object_ptr_result create_res = git_repo_git_repo_create(&ctx);
  PICOMESH_RETURN_IF_ERR(picomesh_int, create_res,
                         "resolve_repo_public: repo create failed");
  struct picomesh_int_result public_res =
      git_repo_git_repo_is_public(&ctx, create_res.value, NULL, repo_id);
  PICOMESH_RETURN_IF_ERR(picomesh_int, public_res,
                         "resolve_repo_public: is_public failed");
...
static struct picomesh_int_result
resolve_issue_namespace(struct policy_state *state, uint32_t issue_id,
                        char *out, size_t cap) {
  ...
  PICOMESH_RETURN_IF_ERR(picomesh_int, create_res,
                         "resolve_issue_namespace: issues create failed");
  struct picomesh_uint32_result repo_of_res =
      issues_issues_repo_of(&ctx, create_res.value, NULL, issue_id);
  PICOMESH_RETURN_IF_ERR(picomesh_int, repo_of_res,
                         "resolve_issue_namespace: repo_of failed");
  if (repo_of_res.value == 0)
    return PICOMESH_OK(picomesh_int, 0);
  return resolve_repo_namespace(state, repo_of_res.value, out, cap);
}

static struct picomesh_authz_decision_result
policy_authorize(void *state_ptr, const char *endpoint,
  ...
  if (strcmp(endpoint, "_describe") == 0 ||
      (endpoint_len >= 10 &&
       strcmp(endpoint + endpoint_len - 10, "._describe") == 0))
    lookup_key = "_describe";
  else if (strcmp(endpoint, "_describe_tree") == 0 ||
           (endpoint_len >= 15 &&
            strcmp(endpoint + endpoint_len - 15, "._describe_tree") == 0))
    lookup_key = "_describe_tree";

  const struct config_node *rule =
      state->policy ? config_node_get(state->policy, lookup_key) : NULL;
  if (!rule)
    return PICOMESH_OK(
        picomesh_authz_decision,
        decide(0, 403, "no policy entry for endpoint (default deny)"));
  ...
  if (config_node_kind(rule) == CONFIG_MAP) {
    required_role =
        config_node_as_string(config_node_get(rule, "required_role"), NULL);
    account_from =
        config_node_as_string(config_node_get(rule, "account_from"), "site");
    required_group =
        config_node_as_string(config_node_get(rule, "required_group"), NULL);
    role_scope =
        config_node_as_string(config_node_get(rule, "role_scope"), NULL);
    resource_from =
        config_node_as_string(config_node_get(rule, "resource_from"), NULL);
    site_bypass =
        config_node_as_int(config_node_get(rule, "site_bypass"), 0) != 0;
    allow_public =
        config_node_as_int(config_node_get(rule, "allow_public"), 0) != 0;
  } else {
  ...
  if (!has_role && !has_group)
    return PICOMESH_OK(
        picomesh_authz_decision,
        decide(0, 403,
               "policy entry has neither required_role nor required_group"));
   ...
   * allow_public. */
  if (has_role && allow_public && role_scope &&
      strcmp(role_scope, "repo_namespace") == 0) {
    char id_str[64];
    if (resolve_account(resource_from ? resource_from : account_from, endpoint,
                        args, id_str, sizeof(id_str))) {
      uint32_t repo_id = (uint32_t)strtoul(id_str, NULL, 10);
      struct picomesh_int_result public_res =
          resolve_repo_public(state, repo_id);
      if (PICOMESH_IS_ERR(public_res)) {
        picomesh_error_print(stderr, "policy: resolve_repo_public",
                             public_res.error);
        picomesh_error_destroy(public_res.error);
      } else if (public_res.value == 1)
        return PICOMESH_OK(picomesh_authz_decision,
                           decide(1, 0, "public repo readable by anyone"));
    }
  ...
  if (!jwt || !*jwt)
    return PICOMESH_OK(picomesh_authz_decision,
                       decide(0, 401, "authentication required"));

  struct picomesh_string_result claims_r =
      picomesh_jwt_verifier_verify(state->verifier, jwt);
  if (PICOMESH_IS_ERR(claims_r)) {
    ...
    return PICOMESH_OK(picomesh_authz_decision,
                       decide(0, 401, "invalid or expired token"));
  }
  struct json_doc *claims_doc =
      json_parse(claims_r.value, strlen(claims_r.value));
  free(claims_r.value);
  if (!claims_doc)
    return PICOMESH_OK(picomesh_authz_decision,
                       decide(0, 401, "token claims not parseable"));
  const struct json_value *groups =
      json_object_get(json_doc_root(claims_doc), "groups");
  ...
  if (has_role) {
    ...
    if (need < 0) {
      result = decide(0, 403, "policy error: unknown required_role");
      goto done;
    }
    ...
    if (site_bypass &&
        groups_best_rank(groups, "site") >= picomesh_role_rank("maintainer")) {
    ...
    if (role_scope && strcmp(role_scope, "repo_namespace") == 0) {
      char id_str[64];
      if (!resolve_account(resource_from ? resource_from : account_from,
                           endpoint, args, id_str, sizeof(id_str))) {
        result = decide(0, 403, "could not resolve repo_id from call args");
        goto done;
      }
      uint32_t repo_id = (uint32_t)strtoul(id_str, NULL, 10);
      char ns_path[256];
      struct picomesh_int_result ns_res =
          resolve_repo_namespace(state, repo_id, ns_path, sizeof(ns_path));
      if (PICOMESH_IS_ERR(ns_res) || !ns_res.value) {
        if (PICOMESH_IS_ERR(ns_res)) {
          picomesh_error_print(stderr, "policy: resolve namespace",
                               ns_res.error);
    ...
    if (role_scope && strcmp(role_scope, "namespace_path") == 0) {
      char ns_path[256];
      if (!resolve_account(resource_from ? resource_from : account_from,
                           endpoint, args, ns_path, sizeof(ns_path))) {
        result =
            decide(0, 403, "could not resolve namespace path from call args");
        goto done;
      }
      result = groups_effective_rank(groups, ns_path) >= need
                   ? decide(1, 0, "namespace role satisfies requirement")
    ...
    if (role_scope && strcmp(role_scope, "issue_namespace") == 0) {
      ...
      char ns_path[256];
      struct picomesh_int_result ns_res =
          resolve_issue_namespace(state, issue_id, ns_path, sizeof(ns_path));
      if (PICOMESH_IS_ERR(ns_res) || !ns_res.value) {
        if (PICOMESH_IS_ERR(ns_res)) {
          picomesh_error_print(stderr, "policy: resolve namespace",
                               ns_res.error);
    ...
    char account[160];
    if (!resolve_account(account_from, endpoint, args, account,
                         sizeof(account))) {
