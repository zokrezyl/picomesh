...
picomesh_authorizer_build(struct picomesh_engine *engine,
  ...
  if (!config || config_node_kind(config) != CONFIG_MAP)
    return PICOMESH_ERR(
        picomesh_void_ptr,
        "authorizer_build: `authorizer:` must be a map with a `type`");
  const char *type_name =
      config_node_as_string(config_node_get(config, "type"), NULL);
  if (!type_name)
    return PICOMESH_ERR(picomesh_void_ptr,
                        "authorizer_build: `authorizer.type` is required");
  const struct picomesh_authorizer_ops *ops = find_ops(type_name);
  if (!ops)
    return PICOMESH_ERR(picomesh_void_ptr,
                        "authorizer_build: unknown authorizer type");

  struct picomesh_authorizer *authorizer = calloc(1, sizeof(*authorizer));
  if (!authorizer)
    return PICOMESH_ERR(picomesh_void_ptr, "authorizer_build: out of memory");
  authorizer->ops = ops;
  struct picomesh_void_ptr_result state = ops->create(engine, config);
  if (PICOMESH_IS_ERR(state)) {
    free(authorizer);
    return PICOMESH_ERR(picomesh_void_ptr, "authorizer_build: create failed",
                        state);
  }
