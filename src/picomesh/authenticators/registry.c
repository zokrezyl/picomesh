...
static const struct picomesh_authenticator_ops *
find_ops(const char *type_name) {
  ...
  for (size_t i = 0; i < sizeof(getters) / sizeof(getters[0]); ++i) {
    ...
    if (ops && ops->type_name && strcmp(ops->type_name, type_name) == 0)
      return ops;
  }
  return NULL;
}
...
picomesh_authn_chain_build(struct picomesh_engine *engine,
  ...
  if (!list || config_node_kind(list) != CONFIG_LIST)
    return PICOMESH_OK(picomesh_void_ptr,
                       chain); /* empty chain → always anonymous */
  ...
  if (count) {
    ...
    if (!chain->entries) {
      ...
      return PICOMESH_ERR(picomesh_void_ptr,
                          "authn_chain_build: out of memory");
    }
  }
  for (size_t i = 0; i < count; ++i) {
    ...
    if (!entry || config_node_kind(entry) != CONFIG_MAP) {
      ...
      return PICOMESH_ERR(
          picomesh_void_ptr,
          "authn_chain_build: authenticator entry must be a map");
    }
    const struct config_node *type_node = config_node_get(entry, "type");
    const char *type_name =
        type_node ? config_node_as_string(type_node, NULL) : NULL;
    if (!type_name) {
      picomesh_authn_chain_free(chain);
      return PICOMESH_ERR(
          picomesh_void_ptr,
          "authn_chain_build: authenticator entry missing `type`");
    }
    const struct picomesh_authenticator_ops *ops = find_ops(type_name);
    if (!ops) {
      picomesh_authn_chain_free(chain);
      return PICOMESH_ERR(picomesh_void_ptr,
                          "authn_chain_build: unknown authenticator type");
    }
    struct picomesh_void_ptr_result state = ops->create(engine, entry);
    if (PICOMESH_IS_ERR(state)) {
      picomesh_authn_chain_free(chain);
      return PICOMESH_ERR(picomesh_void_ptr,
                          "authn_chain_build: authenticator create failed",
                          state);
    }
...
picomesh_authn_chain_run(struct picomesh_authn_chain *chain,
  ...
  for (size_t i = 0; i < chain->count; ++i) {
    ...
    PICOMESH_RETURN_IF_ERR(picomesh_authn_outcome, one,
                           "authn chain: authenticator failed");
    outcome = one.value;
    if (picomesh_authn_outcome_matched(&outcome) ||
        picomesh_authn_outcome_failed(&outcome))
      return PICOMESH_OK(
          picomesh_authn_outcome,
          outcome); /* first match wins; a failure short-circuits */
