...
static inline char *authn_extract_jwt(const char *envelope) {
  ...
  const struct json_value *result =
      json_object_get(json_doc_root(doc), "result");
  const char *jwt = NULL;
  if (result && json_is_string(result)) {
    ...
    if (!jwt)
      jwt = json_as_string(json_object_get(result, "access_jwt"), NULL);
    if (!jwt)
      jwt = json_as_string(json_object_get(result, "access_token"), NULL);
  }
