 ...
 */
static inline int authn_header_value(const char *raw, size_t raw_len,
  ...
  while (cursor < end) {
    ...
    if (line_len > name_len + 1 && cursor[name_len] == ':' &&
      ...
      while (value < cursor + line_len && (*value == ' ' || *value == '\t'))
        ++value;
...
static inline int authn_cookie_value(const char *raw, size_t raw_len,
                                     const char *name, char *out, size_t cap) {
  out[0] = 0;
  size_t name_len = strlen(name);
  const char *cursor = raw;
  ...
  while (cursor < end) {
    ...
    if (line_end - cursor > 7 && strncasecmp(cursor, "Cookie:", 7) == 0) {
      ...
      while (scan < line_end) {
        ...
        if (!eq)
          break;
        if ((size_t)(eq - scan) == name_len &&
            memcmp(scan, name, name_len) == 0) {
...
static inline int authn_bearer_token(const char *raw, size_t raw_len, char *out,
                                     size_t cap) {
  char header[1100];
  if (!authn_header_value(raw, raw_len, "authorization", header,
                          sizeof(header))) {
    out[0] = 0;
    return 0;
  }
...
static inline int authn_build_string_args(char *out, size_t cap,
                                          const char *value) {
  size_t pos = 0;
  if (cap < 4)
    return 0;
