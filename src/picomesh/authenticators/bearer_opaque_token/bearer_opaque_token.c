 ...
 *     prefix: "pat_"                                          # which tokens
 * are ours lookup: personal_access_tokens.personal_access_tokens.lookup
 *   - type: bearer_opaque_token
 *     prefix: "rnr_"
...
bearer_opaque_create(struct picomesh_engine *engine,
                     const struct config_node *config) {
  const char *prefix =
      config_node_as_string(config_node_get(config, "prefix"), NULL);
  const char *lookup =
      config_node_as_string(config_node_get(config, "lookup"), NULL);
  if (!prefix || !*prefix)
    return PICOMESH_ERR(
        picomesh_void_ptr,
        "bearer_opaque_token: `prefix` is required (e.g. \"pat_\")");
  if (!lookup || !*lookup)
    return PICOMESH_ERR(
        picomesh_void_ptr,
        "bearer_opaque_token: `lookup` is required (opaque token -> JWT)");

  struct bearer_opaque_state *state = calloc(1, sizeof(*state));
  if (!state)
    return PICOMESH_ERR(picomesh_void_ptr,
                        "bearer_opaque_token: out of memory");
  state->engine = engine;
  state->prefix = prefix;
  state->lookup = lookup;
  ...
  if (PICOMESH_IS_ERR(verifier)) {
    free(state);
    return PICOMESH_ERR(picomesh_void_ptr,
                        "bearer_opaque_token: verifier create failed",
                        verifier);
  }
...
bearer_opaque_authenticate(void *state_ptr,
                           const struct picomesh_authn_request *request) {
  struct bearer_opaque_state *state = state_ptr;
  struct picomesh_authn_outcome outcome = {0};

  char token[512];
  if (!authn_bearer_token(request->headers_raw, request->headers_raw_len, token,
                          sizeof(token)) ||
      !token[0])
    return PICOMESH_OK(picomesh_authn_outcome,
                       outcome); /* no bearer → no match */
  if (strncmp(token, state->prefix, strlen(state->prefix)) != 0)
    return PICOMESH_OK(
        picomesh_authn_outcome,
        outcome); /* not our prefix → let another opaque entry try */
  ...
  if (PICOMESH_IS_ERR(lookup))
    return PICOMESH_ERR(picomesh_authn_outcome,
                        "bearer_opaque_token: lookup RPC failed", lookup);

  char *jwt = authn_extract_jwt(lookup.value);
  free(lookup.value);
  if (!jwt)
    return PICOMESH_OK(picomesh_authn_outcome,
                       fail("opaque token did not resolve to a JWT"));
  ...
  if (PICOMESH_IS_ERR(claims)) {
    ...
    picomesh_error_print(stderr, "bearer_opaque_token: JWT verification",
                         claims.error);
