...
bearer_jwt_create(struct picomesh_engine *engine,
                  const struct config_node *config) {
  struct bearer_jwt_state *state = calloc(1, sizeof(*state));
  if (!state)
    return PICOMESH_ERR(picomesh_void_ptr, "bearer_jwt_token: out of memory");
  state->engine = engine;

  const struct config_node *rejects =
      config_node_get(config, "reject_prefixes");
  if (rejects && config_node_kind(rejects) == CONFIG_LIST) {
    size_t count = config_node_size(rejects);
    for (size_t i = 0;
         i < count && state->reject_count < BEARER_JWT_MAX_PREFIXES; ++i) {
      const char *prefix =
          config_node_as_string(config_node_at(rejects, i), NULL);
      if (prefix && *prefix)
        state->reject_prefixes[state->reject_count++] = prefix;
    }
  }
  ...
  if (PICOMESH_IS_ERR(verifier)) {
    ...
    return PICOMESH_ERR(picomesh_void_ptr,
                        "bearer_jwt_token: verifier create failed", verifier);
  }
...
bearer_jwt_authenticate(void *state_ptr,
  ...
  for (size_t i = 0; i < state->reject_count; ++i) {
    ...
    if (strncmp(token, state->reject_prefixes[i], prefix_len) == 0)
      return PICOMESH_OK(
          ...
          outcome); /* belongs to an opaque scheme → not our shape */
  }

  struct picomesh_string_result claims =
      picomesh_jwt_verifier_verify(state->verifier, token);
  if (PICOMESH_IS_ERR(claims)) {
    ...
    picomesh_error_print(stderr, "bearer_jwt_token: JWT verification",
                         claims.error);
