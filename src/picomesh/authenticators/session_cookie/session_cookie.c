...
struct session_cookie_state {
  ...
  struct sid_cache_entry
      *cache; /* uthash root, keyed by sid; NULL when empty */
};

static struct picomesh_void_ptr_result
session_cookie_create(struct picomesh_engine *engine,
                      const struct config_node *config) {
  const char *cookie =
      config_node_as_string(config_node_get(config, "cookie"), "picomesh-sid");
  const char *header =
      config_node_as_string(config_node_get(config, "header"), cookie);
  const char *lookup =
      config_node_as_string(config_node_get(config, "lookup"), NULL);
  if (!lookup || !*lookup)
    return PICOMESH_ERR(picomesh_void_ptr,
                        "session_cookie: `lookup` is required (e.g. "
                        "session.session.session_jwt)");

  struct session_cookie_state *state = calloc(1, sizeof(*state));
  if (!state)
    return PICOMESH_ERR(picomesh_void_ptr, "session_cookie: out of memory");
  state->engine = engine;
  state->cookie = cookie;
  state->header = header;
  state->lookup = lookup;
  state->cache_ttl = (int64_t)config_node_as_int(
      config_node_get(config, "cache_ttl_seconds"), 5);
  state->cache_max = (size_t)config_node_as_int(
      config_node_get(config, "cache_max"), SESSION_CACHE_MAX_DEFAULT);
  ...
  if (PICOMESH_IS_ERR(verifier)) {
    free(state);
    return PICOMESH_ERR(picomesh_void_ptr,
                        "session_cookie: verifier create failed", verifier);
  }
...
static struct picomesh_authn_outcome_result
session_cookie_authenticate(void *state_ptr,
                            const struct picomesh_authn_request *request) {
  struct session_cookie_state *state = state_ptr;
  struct picomesh_authn_outcome outcome = {0};

  char sid[160];
  if (!authn_cookie_value(request->headers_raw, request->headers_raw_len,
                          state->cookie, sid, sizeof(sid)) ||
      !sid[0]) {
    if (!authn_header_value(request->headers_raw, request->headers_raw_len,
                            state->header, sid, sizeof(sid)) ||
        !sid[0])
      return PICOMESH_OK(picomesh_authn_outcome,
                         outcome); /* no cookie/header → no match, try next */
  }
  ...
  struct picomesh_string_result lookup =
      picomesh_engine_invoke_json(state->engine, state->lookup, args, NULL);
  ...
  if (PICOMESH_IS_ERR(lookup))
    return PICOMESH_ERR(picomesh_authn_outcome,
                        "session_cookie: lookup RPC failed", lookup);
  ...
  struct picomesh_string_result claims =
  ...
  if (PICOMESH_IS_ERR(claims)) {
    ...
    picomesh_error_print(stderr, "session_cookie: JWT verification",
                         claims.error);
  ...
  if (state->cache_ttl > 0 &&
      strlen(sid) < sizeof(((struct sid_cache_entry *)0)->sid)) {
