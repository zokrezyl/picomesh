...
static int read_frame(struct loop_stream *stream, char **body_out) {
  ...
  while (header_len + 1 < sizeof(header)) {
    ...
    if (read_res.value != 1)
      return header_len == 0 ? 0 : -1; /* clean EOF (idle) vs partial frame */
  ...
  if (!found)
    return -1;
  if (content_len == 0 || content_len > (1u << 20))
    return -1;
  ...
  if (PICOMESH_IS_ERR(body_read)) {
    picomesh_error_destroy(body_read.error);
    free(body);
    return -1;
  }
...
static int write_frame(struct loop_stream *stream, const char *body,
                       size_t len) {
  char header[64];
  int header_len =
...
static void write_error_detail(struct json_writer *writer,
  ...
  char first[512];
  size_t copy_len =
      first_len < sizeof(first) - 1 ? first_len : sizeof(first) - 1;
...
static void write_jsonrpc_error(struct loop_stream *stream,
                                const struct json_value *id, int code,
                                const char *message) {
  if (code <= -32000)
    yerror("yttp request failed: %s", message ? message : "");
  struct json_writer *writer = json_writer_new();
  json_writer_begin_object(writer);
  json_writer_key(writer, "jsonrpc");
  json_writer_string(writer, "2.0");
  json_writer_key(writer, "id");
  if (json_is_int(id))
    json_writer_int(writer, json_as_int(id, 0));
  else if (json_is_string(id))
    json_writer_string(writer, json_as_string(id, ""));
  else
...
handle_create(struct loop_stream *stream, const struct json_value *id,
              const struct json_value *params) {
  const char *class_name =
      json_as_string(json_object_get(params, "class"), NULL);
  if (!class_name) {
  ...
  if (PICOMESH_IS_ERR(class_res)) {
    ...
    picomesh_error_print(stderr, "yttp handle_create: class_by_name",
                         class_res.error);
  ...
  if (PICOMESH_IS_ERR(object_res)) {
    ...
    picomesh_error_print(stderr, "yttp handle_create: object_alloc",
                         object_res.error);
...
static void handle_invoke(struct loop_stream *stream,
  ...
  if (PICOMESH_IS_ERR(invoke_res)) {
    ...
    json_writer_free(writer);
    write_jsonrpc_error(stream, id, -32000,
                        err[0] ? err : "invoke: impl failed");
    return;
  }
  json_writer_end_object(writer);
...
static struct picomesh_void_result
handle_describe(struct loop_stream *stream, const struct json_value *id,
                const struct json_value *params) {
  const char *class_name =
      json_as_string(json_object_get(params, "class"), NULL);
  if (!class_name) {
  ...
  struct class_ptr_result class_res = class_by_name(class_name);
  if (PICOMESH_IS_ERR(class_res)) {
    picomesh_error_print(stderr, "yttp handle_describe: class_by_name",
                         class_res.error);
    picomesh_error_destroy(class_res.error);
    char msg[256];
  ...
  json_writer_string(writer, class_name);
  ...
  if (PICOMESH_IS_ERR(slots_res)) {
    picomesh_error_print(stderr, "yttp handle_describe: class_for_each_slot",
                         slots_res.error);
...
static void serve_one(struct loop *loop, struct loop_stream *stream, void *ud) {
  ...
  for (;;) {
    ...
    if (!method) {
      ...
      struct picomesh_void_result handler_res =
      ...
      if (PICOMESH_IS_ERR(handler_res)) {
        picomesh_error_print(stderr, "yttp: handle_describe",
                             handler_res.error);
        picomesh_error_destroy(handler_res.error);
      }
    } else {
...
struct yttp_frontend_ptr_result yttp_start(struct picomesh_engine *engine,
  ...
  if (!engine)
    return PICOMESH_ERR(yttp_frontend_ptr, "yttp_start: NULL engine");
  ...
  if (!loop)
    return PICOMESH_ERR(yttp_frontend_ptr, "yttp_start: engine has no loop");

  struct picomesh_void_result listen_res =
      loop_listen_tcp(loop, host, port, serve_one, NULL);
  if (PICOMESH_IS_ERR(listen_res)) {
    return PICOMESH_ERR(yttp_frontend_ptr, "yttp_start: loop_listen_tcp failed",
                        listen_res);
  }
  struct yttp_frontend *frontend = calloc(1, sizeof(*frontend));
  if (!frontend)
    return PICOMESH_ERR(yttp_frontend_ptr, "yttp_start: calloc failed");
  frontend->engine = engine;
  yinfo("yttp: listening on %s:%d", host, port);
  return PICOMESH_OK(yttp_frontend_ptr, frontend);
}

void yttp_stop(struct yttp_frontend *frontend) { free(frontend); }
