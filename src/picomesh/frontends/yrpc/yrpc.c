...
static void yrpc_handler_entry(void *arg) {
  ...
  size_t resp_len = 0;
  if (resp) {
    struct picomesh_size_result disp =
        rpc_dispatch_one(request->header, request->body, request->body_len,
                         resp, YRPC_FRAME_MAX);
  ...
  if (frame) {
    ...
    if (resp_len)
      memcpy(frame + 8, resp, resp_len);
    struct picomesh_size_result write_res =
        loop_write(conn->stream, frame, frame_len);
    if (PICOMESH_IS_ERR(write_res)) {
      yerror("yrpc: response write failed: %s",
  ...
  conn->inflight--;
  int wake_reader =
      conn->draining && conn->reader_parked && conn->inflight == 0;
...
static void serve_one(struct loop *loop, struct loop_stream *stream, void *ud) {
  ...
  for (;;) {
    ...
    if (PICOMESH_IS_ERR(header_read)) {
      picomesh_error_destroy(header_read.error);
      break;
    }
    if (header_read.value != 4)
      break; /* clean EOF / short frame */
    ...
    if (PICOMESH_IS_ERR(bodylen_read)) {
      picomesh_error_destroy(bodylen_read.error);
      break;
    }
    if (bodylen_read.value != 4)
      break;
    if (body_len > YRPC_FRAME_MAX)
      break;
    ...
    if (PICOMESH_IS_ERR(handler_res)) {
      ...
      picomesh_error_print(
          stderr,
          "yrpc: failed to spawn handler coroutine — dropping connection",
          handler_res.error);
    ...
    picomesh_coro_resume(
        handler_res.value); /* runs until its first yield (or completion) */
  }
...
struct yrpc_frontend_ptr_result yrpc_start(struct picomesh_engine *e,
  ...
  if (!e)
    return PICOMESH_ERR(yrpc_frontend_ptr, "yrpc_start: NULL engine");
  ...
  if (!loop)
    return PICOMESH_ERR(yrpc_frontend_ptr, "yrpc_start: engine has no loop");

  struct picomesh_void_result listen_res =
      loop_listen_tcp(loop, host, port, serve_one, NULL);
  if (PICOMESH_IS_ERR(listen_res)) {
    return PICOMESH_ERR(yrpc_frontend_ptr, "yrpc_start: loop_listen_tcp failed",
                        listen_res);
  }

  struct yrpc_frontend *frontend = calloc(1, sizeof(*frontend));
  if (!frontend)
    return PICOMESH_ERR(yrpc_frontend_ptr, "yrpc_start: calloc failed");
  frontend->engine = e;
