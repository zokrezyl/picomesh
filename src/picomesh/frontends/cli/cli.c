  ...
  for (int i = 0; i < argc; ++i) {
    ...
    if (strcmp(arg, "true") == 0) {
      ...
      if (is_numeric && has_digit) {
        if (has_dot)
          json_writer_float(writer, strtod(arg, NULL));
        else
          json_writer_int(writer, (int64_t)strtoll(arg, NULL, 10));
      } else {
...
guess_class_qname(const char *method_qname) {
  char buf[256];
...
picomesh_cli_dispatch(struct picomesh_engine *engine) {
  ...
  struct picomesh_void_result invoke_res = invoke_fn(
      NULL, obj, NULL, json_doc_root(args_doc), writer, err, sizeof(err));
