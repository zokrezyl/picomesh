 ...
 * connection). */
static size_t pack_error(uint8_t *resp, size_t resp_cap, const char *code,
                         const char *message) {
  cmp_ctx_t writer;
...
static void decode_headers(cmp_ctx_t *reader, uint32_t count,
  ...
  for (uint32_t i = 0; i < count; ++i) {
    ...
    } else if (strcmp(key, "trace_id") == 0 ||
               strcmp(key, "traceparent") == 0) {
      char val[160];
...
static size_t pack_success(uint8_t *resp, size_t resp_cap, const uint8_t *value,
                           size_t value_len) {
  cmp_ctx_t writer;
  struct picomesh_msgpack_buffer writer_buf;
  picomesh_msgpack_writer_init(&writer, &writer_buf, resp, resp_cap);
...
handle_invoke(struct picomesh_engine *engine, const char *path,
              const uint8_t *frame, size_t frame_len, int args_present,
              size_t args_offset, struct yheaders *hdrs, uint8_t *value_buf,
              uint8_t *resp, size_t resp_cap) {
  if (!path || !*path)
    return PICOMESH_OK(picomesh_size, pack_error(resp, resp_cap, "bad_path",
                                                 "missing 'path'"));
  ...
  if (PICOMESH_IS_ERR(call_res)) {
    ...
    picomesh_error_print(stderr, "msgpack handle_invoke: resolve_service_call",
                         call_res.error);
  ...
  if (!method_fn) {
    ...
    return PICOMESH_OK(picomesh_size,
                       pack_error(resp, resp_cap, "no_such_method", msg));
  }
  ...
  uint32_t argc = 0;
  if (args_present) {
    picomesh_msgpack_reader_init(&args_reader, &args_reader_buf, frame,
                                 frame_len);
    ...
    if (!cmp_read_array(&args_reader, &argc)) {
      ...
      return PICOMESH_OK(
          picomesh_size,
          pack_error(resp, resp_cap, "bad_envelope", "'args' is not an array"));
    }
  } else {
    picomesh_msgpack_reader_init(&args_reader, &args_reader_buf, frame,
                                 frame_len);
  ...
  picomesh_msgpack_writer_init(&value_writer, &value_buf_state, value_buf,
                               MSGPACK_FRAME_MAX);

  char err[8192] = {0};
  struct picomesh_void_result invoke_res =
      method_fn(&call.ctx, call.obj, hdrs, &args_reader, argc, &value_writer,
                err, sizeof(err));
  picomesh_service_call_release(&call);

  if (PICOMESH_IS_ERR(invoke_res)) {
    picomesh_error_print(stderr, "msgpack handle_invoke: method dispatch",
                         invoke_res.error);
    picomesh_error_destroy(invoke_res.error);
    return PICOMESH_OK(picomesh_size, pack_error(resp, resp_cap, "call_error",
                                                 err[0] ? err : "call failed"));
  }
  size_t response_len =
      pack_success(resp, resp_cap, value_buf, value_buf_state.offset);
  if (response_len == 0)
    return PICOMESH_OK(picomesh_size,
                       pack_error(resp, resp_cap, "response_too_large",
...
static struct picomesh_size_result
handle_describe(struct picomesh_engine *engine, const char *path, uint8_t *resp,
                size_t resp_cap) {
  if (!path || !*path)
    return PICOMESH_OK(picomesh_size, pack_error(resp, resp_cap, "bad_path",
  ...
  if (PICOMESH_IS_ERR(call_res)) {
    ...
    picomesh_error_print(stderr,
                         "msgpack handle_describe: resolve_service_call",
                         call_res.error);
    picomesh_error_destroy(call_res.error);
    return PICOMESH_OK(picomesh_size, response_len);
  }
  ...
  if (!params) {
    picomesh_service_call_release(&call);
    return PICOMESH_OK(picomesh_size,
                       pack_error(resp, resp_cap, "no_such_method",
  ...
  cmp_ctx_t value_writer;
  struct picomesh_msgpack_buffer value_buf_state;
  picomesh_msgpack_writer_init(&value_writer, &value_buf_state, value,
                               sizeof(value));
  ...
  for (size_t i = 0; i < params->count; ++i) {
    ...
    cmp_write_str(&value_writer, params->items[i].name,
                  (uint32_t)strlen(params->items[i].name));
    cmp_write_str(&value_writer, "type", 4);
    cmp_write_str(&value_writer, params->items[i].type,
                  (uint32_t)strlen(params->items[i].type));
  }
  picomesh_service_call_release(&call);
  size_t response_len =
      pack_success(resp, resp_cap, value, value_buf_state.offset);
  if (response_len == 0)
    return PICOMESH_OK(picomesh_size,
                       pack_error(resp, resp_cap, "response_too_large",
                                  "describe result too large"));
  return PICOMESH_OK(picomesh_size, response_len);
}
 ...
 * a response). */
static struct picomesh_size_result
dispatch_frame(struct picomesh_engine *engine, const uint8_t *frame,
               size_t frame_len, uint8_t *value_buf, uint8_t *resp,
               size_t resp_cap) {
  cmp_ctx_t reader;
  ...
  if (!cmp_read_map(&reader, &top))
    return PICOMESH_OK(picomesh_size,
                       pack_error(resp, resp_cap, "bad_envelope",
                                  "envelope is not a msgpack map"));
  ...
  for (uint32_t i = 0; i < top; ++i) {
    ...
    if (!cmp_read_str(&reader, key, &klen)) {
      ...
      return PICOMESH_OK(
          picomesh_size,
          pack_error(resp, resp_cap, "bad_envelope", "bad envelope key"));
    }
    if (strcmp(key, "op") == 0) {
      ...
      if (!cmp_read_str(&reader, op, &read_len)) {
        ...
        return PICOMESH_OK(
            picomesh_size,
            pack_error(resp, resp_cap, "bad_envelope", "bad 'op'"));
      }
      ...
      if (!cmp_read_str(&reader, path, &read_len)) {
        ...
        return PICOMESH_OK(
            picomesh_size,
            pack_error(resp, resp_cap, "bad_envelope", "bad 'path'"));
      }
      ...
      if (!cmp_skip_object_no_limit(&reader)) {
        ...
        return PICOMESH_OK(
            picomesh_size,
            pack_error(resp, resp_cap, "bad_envelope", "bad 'args'"));
      }
      ...
      for (uint32_t j = 0; j < kwargs_count * 2u; ++j) {
        if (!cmp_skip_object_no_limit(&reader)) {
          ...
          return PICOMESH_OK(
              picomesh_size,
              pack_error(resp, resp_cap, "bad_envelope", "bad 'kwargs'"));
        }
      }
      ...
      if (!cmp_skip_object_no_limit(&reader)) {
        ...
        return PICOMESH_OK(
            picomesh_size,
            pack_error(resp, resp_cap, "bad_envelope", "bad envelope value"));
      }
    }
  }
  ...
  if (kwargs_nonempty) {
    ...
    return PICOMESH_OK(
        picomesh_size,
        pack_error(
            resp, resp_cap, "kwargs_unsupported",
  ...
  if (strcmp(op, "describe") == 0)
    ...
    handler_res = handle_invoke(engine, path, frame, frame_len, args_present,
                                args_offset, hdrs, value_buf, resp, resp_cap);
  yheaders_free(hdrs);
  return handler_res;
}
...
static void serve_one(struct loop *l, struct loop_stream *s, void *ud) {
  ...
  if (!frame || !value_buf || !resp) {
    ...
    yerror("msgpack: per-connection scratch alloc failed (%u bytes ×3)",
           MSGPACK_FRAME_MAX);
    free(frame);
    free(value_buf);
    free(resp);
  ...
  for (;;) {
    ...
    if (PICOMESH_IS_ERR(lenprefix_read)) {
      picomesh_error_destroy(lenprefix_read.error);
      break;
    }
    ...
    if (PICOMESH_IS_ERR(frame_read)) {
      picomesh_error_destroy(frame_read.error);
      break;
    }
    if (frame_read.value != frame_len)
      break;

    struct picomesh_size_result dispatch_res = dispatch_frame(
        engine, frame, frame_len, value_buf, resp, MSGPACK_FRAME_MAX);
    if (PICOMESH_IS_ERR(dispatch_res)) {
      picomesh_error_print(stderr, "msgpack: dispatch_frame",
                           dispatch_res.error);
...
msgpack_start(struct picomesh_engine *e, const struct msgpack_config *cfg) {
  ...
  if (!l)
    return PICOMESH_ERR(msgpack_frontend_ptr,
                        "msgpack_start: engine has no loop");
  ...
  if (PICOMESH_IS_ERR(lr)) {
    free(f);
    return PICOMESH_ERR(msgpack_frontend_ptr,
                        "msgpack_start: loop_listen_tcp failed", lr);
  }
