...
extern "C" void json_doc_free(struct json_doc *doc) { delete doc; }
...
extern "C" int json_is_null(const struct json_value *v) {
  return json_kind(v) == JSON_NULL;
}
extern "C" int json_is_bool(const struct json_value *v) {
  return json_kind(v) == JSON_BOOL;
}
extern "C" int json_is_int(const struct json_value *v) {
  return json_kind(v) == JSON_INT;
}
extern "C" int json_is_float(const struct json_value *v) {
  return json_kind(v) == JSON_FLOAT;
}
extern "C" int json_is_string(const struct json_value *v) {
  return json_kind(v) == JSON_STRING;
}
extern "C" int json_is_array(const struct json_value *v) {
  return json_kind(v) == JSON_ARRAY;
}
extern "C" int json_is_object(const struct json_value *v) {
...
extern "C" int json_as_bool(const struct json_value *v, int fallback) {
  if (!v)
    return fallback;
  bool b;
  if (v->elem.get(b) == sj::SUCCESS)
    return b ? 1 : 0;
  int64_t i;
  if (v->elem.get(i) == sj::SUCCESS)
    return i != 0;
  return fallback;
}
...
extern "C" int64_t json_as_int(const struct json_value *v, int64_t fallback) {
  ...
  if (v->elem.get(i) == sj::SUCCESS)
    return i;
  uint64_t u;
  if (v->elem.get(u) == sj::SUCCESS)
    return (int64_t)u;
  double d;
  if (v->elem.get(d) == sj::SUCCESS)
    return (int64_t)d;
  bool b;
  if (v->elem.get(b) == sj::SUCCESS)
    return b ? 1 : 0;
...
extern "C" const char *json_as_string(const struct json_value *v,
                                      const char *fallback) {
  if (!v)
    return fallback;
  std::string_view sv;
  if (v->elem.get(sv) != sj::SUCCESS)
    return fallback;
  if (!v->str_cached) {
...
extern "C" size_t json_array_size(const struct json_value *v) {
  if (!v)
    return 0;
  sj::dom::array arr;
  if (v->elem.get(arr) != sj::SUCCESS)
    return 0;
  return arr.size();
}

extern "C" const struct json_value *json_array_at(const struct json_value *v,
                                                  size_t idx) {
  if (!v)
    return nullptr;
  sj::dom::array arr;
...
extern "C" const struct json_value *json_object_get(const struct json_value *v,
                                                    const char *key) {
  if (!v || !key)
    return nullptr;
  sj::dom::object obj;
  if (v->elem.get(obj) != sj::SUCCESS)
    return nullptr;
  auto e = obj[key];
  if (e.error())
    return nullptr;
...
extern "C" void json_writer_free(struct json_writer *w) { delete w; }
...
extern "C" const char *json_writer_data(struct json_writer *w,
                                        size_t *len_out) {
  if (!w) {
    if (len_out)
      *len_out = 0;
