/* yexec — sharded key-affine blocking executor. See yexec.h.
 *
 ...
 * then wakes the coroutine on its loop via yloop_post_resume (coroutines are
...
static void shard_main(void *arg)
{
    ...
    for (;;) {
        ...
        yloop_post_resume(w->loop, w->coro);
    }
    /* not reached: process-lifetime worker (matches sharded_storage's shard set) */
}

struct yexec *yexec_create(int n, void *(*shard_init)(void *), void (*shard_free)(void *), void *ud)
{
    if (n < 1) n = 1;
    struct yexec *e = calloc(1, sizeof(*e));
    if (!e) return NULL;
    e->shards = calloc((size_t)n, sizeof(struct yexec_shard));
    ...
    e->ud = ud;
    for (int i = 0; i < n; i++) {
        struct yexec_shard *s = &e->shards[i];
        s->owner = e;
        uv_mutex_init(&s->mu);
        uv_cond_init(&s->cv);
                  ...
                  void (*fn)(void *shard_state, void *arg), void *arg)
{
    ...
    struct yexec_shard *s = &e->shards[key % (uint32_t)e->n];
    struct yexec_work w = {.fn = fn, .arg = arg, .loop = loop, .coro = self, .next = NULL};
    uv_mutex_lock(&s->mu);
