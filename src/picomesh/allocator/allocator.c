...
struct block_header {
  uint32_t
      klass; /* size class 1..MAX, or KLASS_DIRECT for a malloc passthrough */
  uint32_t magic;
...
struct picomesh_allocator {
  struct free_node
      *freelist[PICOMESH_ALLOC_MAX_CLASS + 1]; /* indexed by klass */
...
struct picomesh_allocator *picomesh_allocator_thread(void) {
  ...
  if (!self)
    self = picomesh_allocator_create();
  return self;
}
...
void picomesh_allocator_free(struct picomesh_allocator *pool, void *ptr) {
  ...
  struct block_header *header =
      (struct block_header *)((char *)ptr - HEADER_BYTES);
  if (header->magic != BLOCK_MAGIC) {
    ywarn("picomesh_allocator_free: bad/corrupt block header (double free or "
          "foreign ptr)");
    return;
  }
  if (header->owner != pool) {
    ...
    ywarn("picomesh_allocator_free: block freed on a different pool than it "
          "was allocated on");
    pool = header->owner;
  }
