...
static unsigned ytelemetry_shard_for(const char *trace_id,
  ...
  for (const unsigned char *byte = (const unsigned char *)trace_id;
       byte && *byte; ++byte) {
    hash ^= *byte;
...
static void
ytelemetry_store_setup_locked(struct ytelemetry_store *store,
                              const struct ytelemetry_store_config *config) {
  if (store->inited)
    return;

  struct ytelemetry_store_config cfg =
      config ? *config : (struct ytelemetry_store_config){0};
  ...
  store->bucket_spans =
      cfg.bucket_spans ? cfg.bucket_spans : YTEL_DEFAULT_BUCKET_SPANS;
  store->flush_ns =
      (cfg.flush_ms ? cfg.flush_ms : YTEL_DEFAULT_FLUSH_MS) * 1000000ull;
  store->max_age_ns = cfg.max_age_seconds * 1000000000ull;

  store->shards = calloc(store->shard_count, sizeof(*store->shards));
  if (!store->shards) {
    store->shard_count = 1;
    store->shards = calloc(1, sizeof(*store->shards));
  }
...
void ytelemetry_store_init_config(
    const struct ytelemetry_store_config *config) {
...
void ytelemetry_store_init(size_t max_spans, uint64_t max_age_seconds) {
  struct ytelemetry_store_config cfg = {.max_spans = max_spans,
 ...
 */
static struct ytelemetry_stored_span *
ytelemetry_shard_nth(struct ytel_shard *shard, size_t i) {
  if (i >= shard->count)
    return NULL;
...
static int ytelemetry_store_fresh(uint64_t max_age_ns,
                                  const struct ytelemetry_stored_span *span,
                                  uint64_t now) {
  if (!max_age_ns)
    return 1;
...
static void ytelemetry_fill_span(struct ytelemetry_stored_span *dst,
  ...
  ytelemetry_store_copystr(
      dst->parent_id, sizeof(dst->parent_id),
  ...
  ytelemetry_store_copystr(
      dst->service, sizeof(dst->service),
      json_as_string(json_object_get(root, "service_name"), ""));
  ytelemetry_store_copystr(
      dst->node, sizeof(dst->node),
      json_as_string(json_object_get(root, "node_id"), ""));
  ytelemetry_store_copystr(
      dst->status, sizeof(dst->status),
      json_as_string(json_object_get(root, "status"), "ok"));
  ytelemetry_store_copystr(
      dst->kind, sizeof(dst->kind),
      json_as_string(json_object_get(root, "kind"), "internal"));
  ytelemetry_store_copystr(
      dst->err, sizeof(dst->err),
      json_as_string(json_object_get(root, "error_message"), ""));
  dst->start_unix_ns =
      (uint64_t)json_as_int(json_object_get(root, "start_time_ns"), 0);
  dst->duration_ns =
      (uint64_t)json_as_int(json_object_get(root, "duration_ns"), 0);
...
static void ytelemetry_shard_put(struct ytel_shard *shard, size_t cap,
  ...
  if (shard->count == shard->cap) {
    ...
    if (old->used) {
      ...
      if (entry && entry->live && --entry->live == 0) {
        HASH_DEL(shard->index, entry);
        free(entry);
      }
    }
    shard->evicted++;
  ...
  if (!entry) {
    ...
    if (entry) {
      ytelemetry_store_copystr(entry->trace_id, sizeof(entry->trace_id),
                               src->trace_id);
  ...
  if (entry) {
    entry->live++;
    if (src->start_unix_ns < entry->start)
      entry->start = src->start_unix_ns;
    uint64_t end = src->start_unix_ns + src->duration_ns;
    if (end > entry->end)
      entry->end = end;
    if (strcmp(src->status, "error") == 0)
      entry->error = 1;
    if (!entry->root_name[0] ||
        !src->parent_id[0]) { /* first span, or the real root */
      ytelemetry_store_copystr(entry->root_name, sizeof(entry->root_name),
                               src->name);
      ytelemetry_store_copystr(entry->root_service, sizeof(entry->root_service),
                               src->service);
    }
  }
}
...
static void ytelemetry_arena_flush_shard(struct ytelemetry_store *store,
                                         struct ytel_arena *arena,
                                         unsigned shard_index) {
  if (!arena->fill[shard_index])
    return;
  struct ytel_shard *shard = &store->shards[shard_index];
  struct ytelemetry_stored_span *bucket =
      &arena->spans[(size_t)shard_index * arena->bucket_spans];
...
static void ytelemetry_arena_flush_all(struct ytelemetry_store *store,
                                       struct ytel_arena *arena, uint64_t now) {
  for (unsigned k = 0; k < arena->shard_count; ++k)
    ytelemetry_arena_flush_shard(store, arena, k);
  arena->last_flush_ns = now;
}

void ytelemetry_store_flush_local(void) {
  struct ytel_arena *arena = &ytel_tls_arena;
  if (!arena->spans || !arena->total)
    return;
  ytelemetry_arena_flush_all(ytelemetry_store_state(), arena,
                             ytelemetry_store_now_ns());
}
...
static int ytelemetry_append_parsed(struct ytelemetry_store *store,
                                    const struct json_value *root) {
  const char *trace =
      root ? json_as_string(json_object_get(root, "trace_id"), NULL) : NULL;
  const char *span =
      root ? json_as_string(json_object_get(root, "span_id"), NULL) : NULL;
  if (!trace || !span) {
  ...
  if (!arena->spans) {
    ...
    arena->spans = calloc((size_t)arena->shard_count * arena->bucket_spans,
                          sizeof(*arena->spans));
...
int ytelemetry_store_ingest_json(const char *json, size_t len) {
  ...
  if (!doc) {
    ytelemetry_count_malformed(store);
    return 0;
  }
...
void ytelemetry_store_write_stats(struct json_writer *writer) {
  ...
  json_writer_key(writer, "ingested");
  json_writer_int(writer, (int64_t)ingested);
  json_writer_key(writer, "malformed");
  json_writer_int(writer, (int64_t)malformed);
  json_writer_key(writer, "evicted");
  json_writer_int(writer, (int64_t)evicted);
  json_writer_key(writer, "stored");
  json_writer_int(writer, (int64_t)stored);
  json_writer_key(writer, "capacity");
  json_writer_int(writer, (int64_t)(store->shard_cap * store->shard_count));
  json_writer_key(writer, "max_age_seconds");
  json_writer_int(writer, (int64_t)(store->max_age_ns / 1000000000ull));
  json_writer_key(writer, "shards");
  json_writer_int(writer, (int64_t)store->shard_count);
...
static uint64_t pctl_u64(const uint64_t *sorted, size_t count,
                         double percentile) {
  if (!count)
    return 0;
...
static void emit_span(struct json_writer *writer,
                      const struct ytelemetry_stored_span *span) {
  json_writer_begin_object(writer);
  json_writer_key(writer, "span_id");
  json_writer_string(writer, span->span_id);
  json_writer_key(writer, "parent_span_id");
  json_writer_string(writer, span->parent_id);
  json_writer_key(writer, "trace_id");
  json_writer_string(writer, span->trace_id);
  json_writer_key(writer, "name");
  json_writer_string(writer, span->name);
  json_writer_key(writer, "kind");
  json_writer_string(writer, span->kind);
  json_writer_key(writer, "service_name");
  json_writer_string(writer, span->service);
  json_writer_key(writer, "node_id");
  json_writer_string(writer, span->node);
  json_writer_key(writer, "start_time_ns");
  json_writer_int(writer, (int64_t)span->start_unix_ns);
  json_writer_key(writer, "duration_ns");
  json_writer_int(writer, (int64_t)span->duration_ns);
  json_writer_key(writer, "status");
  json_writer_string(writer, span->status);
  if (span->err[0]) {
    json_writer_key(writer, "error_message");
    json_writer_string(writer, span->err);
  }
  json_writer_key(writer, "attributes");
  json_writer_begin_object(writer);
  json_writer_key(writer, "picomesh.uid");
  json_writer_int(writer, (int64_t)span->uid);
  json_writer_end_object(writer);
  json_writer_end_object(writer);
}
...
void ytelemetry_store_write_trace(struct json_writer *writer,
                                  const char *trace_id) {
  struct ytelemetry_store *store = ytelemetry_store_state();
  ytelemetry_store_ensure(store);
  struct ytel_shard *shard = &store->shards[ytelemetry_shard_for(
      trace_id ? trace_id : "", store->shard_count)];
  pthread_mutex_lock(&shard->mu);
  ...
  if (trace_id)
    HASH_FIND_STR(shard->index, trace_id, trace_idx);
  ...
  for (size_t i = 0;
       i < shard->count && match_count < CAP && match_count < want; ++i) {
    struct ytelemetry_stored_span *span =
        ytelemetry_shard_nth(shard, i); /* newest first */
  ...
  for (size_t i = 0; i < match_count; ++i) {
    ...
    if (i == 0 || match[i]->start_unix_ns < min_start)
      min_start = match[i]->start_unix_ns;
    if (i == 0 || end > max_end)
      max_end = end;
  }
  ...
  for (size_t i = 0; i < match_count && !root_span[0]; ++i)
    if (!match[i]->parent_id[0])
      root_span = match[i]->span_id;
  for (size_t i = 0; i < match_count && !root_span[0]; ++i) {
    int parent_in = 0;
    for (size_t j = 0; j < match_count; ++j)
      if (strcmp(match[j]->span_id, match[i]->parent_id) == 0) {
        parent_in = 1;
        break;
      }
    if (!parent_in)
  ...
  json_writer_begin_object(writer);
  json_writer_key(writer, "trace_id");
  json_writer_string(writer, trace_id ? trace_id : "");
  json_writer_key(writer, "root_span_id");
  json_writer_string(writer, root_span);
  json_writer_key(writer, "span_count");
  json_writer_int(writer, (int64_t)match_count);
  json_writer_key(writer, "duration_ns");
  json_writer_int(writer, (int64_t)(match_count ? max_end - min_start : 0));
  json_writer_key(writer, "spans");
  json_writer_begin_array(writer);
  for (size_t i = 0; i < match_count; ++i)
    emit_span(writer, match[i]);
...
static int cmp_sum_newest(const void *a, const void *b) {
  ...
  return (left->start < right->start) -
         (left->start > right->start); /* newest first */
}

void ytelemetry_store_write_traces(struct json_writer *writer,
                                   const char *service, uint64_t since_ns,
                                   const char *status) {
  struct ytelemetry_store *store = ytelemetry_store_state();
  ytelemetry_store_ensure(store);
  uint64_t now = ytelemetry_store_now_ns();
  ...
  enum { PER_SHARD = 64 };
  struct ytel_trace_sum *cand =
      malloc((size_t)store->shard_count * PER_SHARD * sizeof(*cand));
  size_t ncand = 0;
  ...
  if (cand) {
    for (unsigned k = 0; k < store->shard_count; ++k) {
      ...
      for (size_t i = 0; i < shard->count && nseen < PER_SHARD; ++i) {
        struct ytelemetry_stored_span *span =
            ytelemetry_shard_nth(shard, i); /* newest→oldest */
        if (!span->used ||
            !ytelemetry_store_fresh(store->max_age_ns, span, now))
          continue;
        if (since_ns && span->start_unix_ns < since_ns)
          continue;
        if (service && service[0] && strcmp(span->service, service) != 0)
          continue;

        int dup = 0;
        for (size_t j = 0; j < nseen; ++j)
          if (strcmp(seen[j], span->trace_id) == 0) {
            dup = 1;
            break;
          }
        if (dup)
        ...
        if (!entry)
          continue; /* span racing its own eviction — skip */

        struct ytel_trace_sum sum = {0};
        ytelemetry_store_copystr(sum.trace_id, sizeof(sum.trace_id),
                                 span->trace_id);
        ytelemetry_store_copystr(sum.root_name, sizeof(sum.root_name),
                                 entry->root_name);
        ytelemetry_store_copystr(sum.root_service, sizeof(sum.root_service),
                                 entry->root_service);
        ...
        ytelemetry_store_copystr(seen[nseen], sizeof(seen[nseen]),
                                 span->trace_id);
        nseen++;
        cand[ncand++] = sum;
      }
  ...
  for (size_t i = 0; i < ncand && emitted < 256; ++i) {
    ...
    if (status && status[0] && strcmp(status, tstatus) != 0)
      continue;
    json_writer_begin_object(writer);
    json_writer_key(writer, "trace_id");
    json_writer_string(writer, cand[i].trace_id);
    json_writer_key(writer, "root_name");
    json_writer_string(writer, cand[i].root_name);
    json_writer_key(writer, "service_name");
    json_writer_string(writer, cand[i].root_service);
    json_writer_key(writer, "start_time_ns");
    json_writer_int(writer, (int64_t)cand[i].start);
    json_writer_key(writer, "duration_ns");
    json_writer_int(writer, (int64_t)(cand[i].end - cand[i].start));
    json_writer_key(writer, "span_count");
    json_writer_int(writer, (int64_t)cand[i].span_count);
    json_writer_key(writer, "status");
    json_writer_string(writer, tstatus);
    json_writer_end_object(writer);
    emitted++;
  }
  json_writer_end_array(writer);
...
void ytelemetry_store_write_services(struct json_writer *writer) {
  ...
  for (unsigned k = 0; k < store->shard_count; ++k) {
    ...
    for (size_t i = 0; i < shard->count; ++i) {
      ...
      if (!span->used || !span->service[0])
        continue;
      size_t j = 0;
      for (; j < name_count; ++j)
        if (strcmp(names[j], span->service) == 0)
          break;
      if (j == name_count && name_count < MAX_SVC) {
        ytelemetry_store_copystr(names[name_count], 64, span->service);
        name_count++;
      }
      if (j < MAX_SVC)
        counts[j]++;
  ...
  for (size_t j = 0; j < name_count; ++j) {
    json_writer_begin_object(writer);
    json_writer_key(writer, "service_name");
    json_writer_string(writer, names[j]);
    json_writer_key(writer, "span_count");
    json_writer_int(writer, (int64_t)counts[j]);
    json_writer_end_object(writer);
  }
  json_writer_end_array(writer);
  json_writer_end_object(writer);
...
void ytelemetry_store_write_operations(struct json_writer *writer,
                                       const char *service) {
  struct ytelemetry_store *store = ytelemetry_store_state();
  ytelemetry_store_ensure(store);
  ...
  for (unsigned k = 0; k < store->shard_count; ++k) {
    ...
    for (size_t i = 0; i < shard->count; ++i) {
      struct ytelemetry_stored_span *span = ytelemetry_shard_nth(shard, i);
      if (!span->used || !span->name[0])
        continue;
      if (service && service[0] && strcmp(span->service, service) != 0)
        continue;
      size_t j = 0;
      for (; j < op_count; ++j)
        if (strcmp(ops[j], span->name) == 0)
          break;
      if (j == op_count && op_count < MAX_OPS) {
        ytelemetry_store_copystr(ops[op_count], 64, span->name);
        op_count++;
      }
      if (j < MAX_OPS)
        counts[j]++;
  ...
  json_writer_begin_object(writer);
  json_writer_key(writer, "service");
  json_writer_string(writer, service ? service : "");
  json_writer_key(writer, "operations");
  json_writer_begin_array(writer);
  for (size_t j = 0; j < op_count; ++j) {
    json_writer_begin_object(writer);
    json_writer_key(writer, "name");
    json_writer_string(writer, ops[j]);
    json_writer_key(writer, "count");
    json_writer_int(writer, (int64_t)counts[j]);
    json_writer_end_object(writer);
  }
  json_writer_end_array(writer);
...
void ytelemetry_store_write_latency(struct json_writer *writer,
  ...
  if (durs) {
    for (unsigned k = 0; k < store->shard_count; ++k) {
      ...
      for (size_t i = 0; i < shard->count && count < total; ++i) {
        ...
        if (!span->used)
          continue;
        if (floor && span->start_unix_ns < floor)
          continue;
        if (service && service[0] && strcmp(span->service, service) != 0)
          continue;
        if (operation && operation[0] && strcmp(span->name, operation) != 0)
          continue;
  ...
  json_writer_key(writer, "service");
  json_writer_string(writer, service ? service : "");
  json_writer_key(writer, "operation");
  json_writer_string(writer, operation ? operation : "");
  json_writer_key(writer, "window_ns");
  json_writer_int(writer, (int64_t)window_ns);
  json_writer_key(writer, "count");
  json_writer_int(writer, (int64_t)count);
  json_writer_key(writer, "p50_ns");
  json_writer_int(writer, (int64_t)pctl_u64(durs, count, 50));
  json_writer_key(writer, "p90_ns");
  json_writer_int(writer, (int64_t)pctl_u64(durs, count, 90));
  json_writer_key(writer, "p99_ns");
  json_writer_int(writer, (int64_t)pctl_u64(durs, count, 99));
  json_writer_key(writer, "max_ns");
  json_writer_int(writer, (int64_t)(count ? durs[count - 1] : 0));
  json_writer_end_object(writer);

  free(durs);
}

void ytelemetry_store_write_errors(struct json_writer *writer,
                                   uint64_t since_ns) {
  struct ytelemetry_store *store = ytelemetry_store_state();
  ytelemetry_store_ensure(store);

  enum { PER_SHARD = 64 };
  struct ytelemetry_stored_span *errs =
      malloc((size_t)store->shard_count * PER_SHARD * sizeof(*errs));
  size_t nerr = 0;
  if (errs) {
    for (unsigned k = 0; k < store->shard_count; ++k) {
      struct ytel_shard *shard = &store->shards[k];
      ...
      for (size_t i = 0; i < shard->count && taken < PER_SHARD; ++i) {
        struct ytelemetry_stored_span *span =
            ytelemetry_shard_nth(shard, i); /* newest first */
        if (!span->used || strcmp(span->status, "error") != 0)
          continue;
        if (since_ns && span->start_unix_ns < since_ns)
          continue;
        errs[nerr++] = *span;
  ...
  if (errs) {
    ...
    size_t emitted = 0;
    for (size_t i = 0; i < nerr && emitted < 256; ++i) {
      emit_span(writer, &errs[i]);
      emitted++;
    }
  }
  json_writer_end_array(writer);
