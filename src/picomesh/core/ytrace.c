 ...
 *      for its spare (O(1) under the per-thread lock), k-way merges every
 * drained line by timestamp, and writes the whole batch to the sink in one
 * call. So the shared sink lock is paid once per flush, not once per line, and
 * worker threads never serialize on it. Overflow (collector behind) drops +
 * counts; it never blocks a worker.
 */
...
static const char *color_reset(void) { return g_use_colors ? ANSI_RESET : ""; }
...
static void format_timestamp(char *buf, size_t bufsize,
  ...
  snprintf(buf, bufsize, "%02d:%02d:%02d.%03d", tmv.tm_hour, tmv.tm_min,
           tmv.tm_sec, ms);
}
...
struct ytrace_logbuf {
  ...
  uint64_t
      drop_first_ts; /* timestamp of the first such drop (marker position) */
};
...
static struct ytrace_thread *ytrace_thread_register(void) {
  ...
  if (!thread->buf[0].data || !thread->buf[1].data) {
    free(thread->buf[0].data);
    free(thread->buf[1].data);
    free(thread);
    return NULL;
  }
...
static void ytrace_emit(uint64_t ts_ns, const char *text, size_t len) {
  ...
  if (!thread) {
    ...
    if (!thread) {
      ytrace_write_all(g_sink_fd, text, len);
      return;
    }
  }
  size_t need = YTRACE_REC_HDR + len;
  if (need > thread->buf[0].cap) {
    ytrace_write_all(g_sink_fd, text, len);
    return;
  }
...
/* A line to write, in the merged batch. text!=NULL → a real log line (points
 * into a thread buffer); text==NULL → a synthesized "lines dropped here" marker
 * that the collector formats at write time, positioned at ts_ns (the first
 * drop). */
struct ytrace_desc {
  uint64_t ts_ns;
  const char *text;
...
struct ytrace_snap {
  struct ytrace_thread *t;
  int idx;
};

static struct ytrace_desc *g_desc = NULL;
static size_t g_desc_cap = 0;
static struct ytrace_snap *g_snap = NULL;
...
static void ytrace_flush_all(void) {
   ...
   * writer swapped away, else force-swap the partial active buffer) and read
   * its drop counter. The text bytes stay valid: a non-active buffer with len>0
   * is owned by the collector and the writer won't touch it until we reset its
   * len. So we parse it lock-free below. */
  pthread_mutex_lock(&g_threads_lock);
  for (struct ytrace_thread *thread = g_threads; thread;
       thread = thread->next) {
    pthread_mutex_lock(&thread->lock);
    ...
    if (thread->buf[inactive].len > 0) {
      ...
      thread->active =
          inactive; /* O(1) swap: writer moves to the free buffer */
      take = active;
    }
    ...
    if (grow((void **)&g_snap, &g_snap_cap, nsnap + 1, sizeof(*g_snap))) {
      g_snap[nsnap].t = thread;
      g_snap[nsnap].idx = take;
      nsnap++;
    }
    ...
    while (cursor + YTRACE_REC_HDR <= end) {
      ...
      if (grow((void **)&g_desc, &g_desc_cap, ndesc + 1, sizeof(*g_desc))) {
        g_desc[ndesc] =
            (struct ytrace_desc){.ts_ns = ts, .text = txt, .len = rec_len};
        ndesc++;
      }
      cursor = txt + rec_len;
    }
    ...
    if (drop_count > 0 &&
        grow((void **)&g_desc, &g_desc_cap, ndesc + 1, sizeof(*g_desc))) {
      g_desc[ndesc] = (struct ytrace_desc){
          .ts_ns = drop_ts, .text = NULL, .tid = tid, .drop = drop_count};
      ndesc++;
    }
  }
  pthread_mutex_unlock(&g_threads_lock);
    ...
    if (grow((void **)&g_outbuf, &g_outbuf_cap, reserve, 1)) {
      ...
      for (size_t i = 0; i < ndesc; ++i) {
        if (g_desc[i].text) {
          ...
          int marker_len = format_drop_marker(
              g_outbuf + off, g_outbuf_cap - off, g_desc[i].ts_ns,
              g_desc[i].tid, g_desc[i].drop);
          if (marker_len > 0)
            off += (size_t)marker_len;
        }
      }
...
void ytrace_output(const char *level, const char *file, int line,
  ...
  int rec_len = snprintf(rec, sizeof(rec), "[%s] %s[%-5s]%s %s:%d (%s): %s\n",
                         ...
                         base_name, line, func, msg_buf);
  if (rec_len <= 0)
    return;
  if ((size_t)rec_len >= sizeof(rec))
    rec_len = (int)sizeof(rec) - 1; /* truncated line */
...
void ytrace_set_file_enabled(const char *file, bool enabled) {
  ...
  for (size_t i = 0; i < g_point_count; i++) {
    ...
    if (!base_name)
      base_name = strrchr(g_points[i].file, '\\');
...
void ytrace_list(void) {
  ...
  for (size_t i = 0; i < g_point_count; i++) {
    ...
    fprintf(stderr, "%-4zu %-7s %-6s %-30s %-20s %s\n", i,
            *point->enabled ? "ON" : "off", point->level, loc_buf,
            point->function, msg_buf);
  }
  fprintf(stderr, "\n");
  YTRACE_UNLOCK();
}

void ytime_timer_observe(struct ytime_timer *timer, const char *name,
  ...
  if (elapsed_ms < timer->min_ms)
    timer->min_ms = elapsed_ms;
  if (elapsed_ms > timer->max_ms)
    timer->max_ms = elapsed_ms;
  YTRACE_UNLOCK();
}
...
void ytime_timer_list(void) {
  ...
  for (size_t i = 0; i < g_timer_count; i++) {
    ...
    fprintf(stderr, "%-4zu %-20s %-30s %10llu %10.3f %10.3f %10.3f %10.3f\n", i,
            timer->name ? timer->name : "", loc_buf,
            (unsigned long long)timer->count, timer->last_ms, timer->avg_ms,
            timer->min_ms, timer->max_ms);
  }
  fprintf(stderr, "\n");
  YTRACE_UNLOCK();
}
