...
#define YPERF_EMIT(level, ...)                                                 \
  ytrace_output((level), __FILE__, __LINE__, __func__, __VA_ARGS__)
...
static int perf_child_lookup_cb(const char *key, const struct config_node *val,
                                void *ud) {
...
static const struct config_node *perf_node_child(const struct config_node *map,
                                                 const char *key) {
  if (!map || config_node_kind(map) != CONFIG_MAP)
    return NULL;
...
static int perf_config_enabled(const struct config_node *perf_node) {
  return perf_node &&
         config_node_as_bool(perf_node_child(perf_node, "enabled"), 0);
}
...
static long perf_event_open_syscall(struct perf_event_attr *attr, pid_t pid,
...
static int perf_event_lookup(const char *name, uint32_t *type,
                             uint64_t *config) {
  size_t n = 0;
...
static uint64_t yperf_counter_scaled(struct yperf_counter *counter) {
  ...
  if (n != (ssize_t)sizeof(raw))
    return counter->prev_scaled; /* read failed: no progress */

  counter->multiplexed =
      (raw.time_running > 0 && raw.time_running < raw.time_enabled);
  if (raw.time_running == 0)
    return 0;
  if (raw.time_running < raw.time_enabled)
    return (uint64_t)((double)raw.value * (double)raw.time_enabled /
                      (double)raw.time_running);
  return raw.value;
}
...
static void yperf_report_tick(void *ud) {
  ...
  off += (size_t)snprintf(line + off, sizeof(line) - off,
                          "perf[%s] +%ums:", perf->label, perf->interval_ms);
  for (size_t i = 0; i < perf->count && off < sizeof(line); ++i) {
    ...
    uint64_t delta =
        cur >= counter->prev_scaled ? cur - counter->prev_scaled : 0;
    counter->prev_scaled = cur;
    if (counter->multiplexed)
      any_multiplexed = 1;
...
void yperf_destroy(struct yperf *perf) {
  ...
  for (size_t i = 0; i < perf->count; ++i)
    if (perf->counters[i].fd > 0)
      close(perf->counters[i].fd);
  free(perf);
}
...
struct yperf_ptr_result yperf_create(const struct config_node *perf_node,
  ...
  const char *mode =
      config_node_as_string(perf_node_child(perf_node, "mode"), "counters");
  if (strcmp(mode, "counters") != 0)
    YPERF_EMIT("warn",
               "yperf[%s]: mode '%s' not implemented yet — only 'counters' is "
               "supported; "
  ...
  if (!events || config_node_kind(events) != CONFIG_LIST ||
      config_node_size(events) == 0)
    return PICOMESH_ERR(
        yperf_ptr,
  ...
  int interval_ms =
      (int)config_node_as_int(perf_node_child(perf_node, "interval_ms"), 1000);
  if (interval_ms < 10)
    interval_ms = 10;
  int do_log = config_node_as_bool(perf_node_child(perf_node, "log"), 1);
  int exclude_kernel =
      config_node_as_bool(perf_node_child(perf_node, "exclude_kernel"), 0);

  struct yperf *perf = calloc(1, sizeof(struct yperf));
  if (!perf)
    return PICOMESH_ERR(yperf_ptr, "yperf: calloc failed");
  ...
  for (size_t i = 0; i < want; ++i) {
    if (perf->count >= YPERF_MAX_EVENTS) {
      YPERF_EMIT(
          "warn",
          "yperf[%s]: more than %d events configured — ignoring the rest", tag,
          YPERF_MAX_EVENTS);
      break;
    }
    ...
    if (!perf_event_lookup(ename, &type, &config)) {
      YPERF_EMIT("error",
                 "yperf[%s]: unknown perf event '%s' (see "
                 "docs/perf-counters.md for the "
                 "supported set)",
                 tag, ename);
      yperf_destroy(perf);
      return PICOMESH_ERR(
  ...
  if (perf->count == 0) {
    ...
    return PICOMESH_ERR(yperf_ptr,
                        "yperf: perf.events listed no usable event names");
  }
  ...
  if (perf->log) {
    ...
    if (PICOMESH_IS_ERR(tr)) {
      yperf_destroy(perf);
      return PICOMESH_ERR(yperf_ptr,
                          "yperf: failed to start the reporter timer", tr);
    }
    perf->timer = tr.value;
  }
...
void yperf_destroy(struct yperf *perf) { (void)perf; }
...
struct yperf_ptr_result yperf_create(const struct config_node *perf_node,
  ...
  if (perf_config_enabled(perf_node))
    YPERF_EMIT("warn",
               "yperf[%s]: perf profiling requested in config but "
               "perf_event_open(2) is "
