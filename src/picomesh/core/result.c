...
void picomesh_error_print(FILE *out, const char *headline,
  ...
  fprintf(out, "    at %s:%d (%s)\n", err.file ? err.file : "<unknown>",
          err.line, err.func ? err.func : "<unknown>");
  for (const struct picomesh_error *cause = err.cause; cause;
       cause = cause->cause) {
    fprintf(out, "  caused by: %s\n", cause->msg ? cause->msg : "<no message>");
    fprintf(out, "    at %s:%d (%s)\n", cause->file ? cause->file : "<unknown>",
            cause->line, cause->func ? cause->func : "<unknown>");
  }
}
...
size_t picomesh_error_snprint(char *buf, size_t bufsize,
  ...
  int written = snprintf(buf, bufsize, "%s\n    at %s:%d (%s)",
                         err.msg ? err.msg : "<no message>",
  ...
  for (const struct picomesh_error *cause = err.cause; cause;
    ...
    off += (size_t)cause_len < bufsize - off ? (size_t)cause_len
                                             : bufsize - 1 - off;
  }
  return off;
}
