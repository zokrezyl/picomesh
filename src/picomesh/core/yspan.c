...
size_t yspan_dump(char *buf, size_t cap) {
  ...
  pthread_mutex_lock(yspan_lock());
  size_t off = 0;
  int written = snprintf(buf + off, cap - off, "%-34s %8s %9s %9s %9s %9s\n",
  ...
  for (size_t i = 0; i < state->n; ++i) {
    ...
    for (size_t j = 0; j < op_count; ++j)
      if (strcmp(ops[j], state->e[i].op) == 0) {
        found = 1;
        break;
      }
  ...
  for (size_t j = 0; j < op_count; ++j) {
    ...
    for (size_t i = 0; i < state->n; ++i)
      if (strcmp(state->e[i].op, ops[j]) == 0)
        durs[sample_count++] = state->e[i].dur_us;
    qsort(durs, sample_count, sizeof(double), cmp_double);
    if (off < cap) {
      written =
          snprintf(buf + off, cap - off, "%-34s %8zu %9.0f %9.0f %9.0f %9.0f\n",
                   ops[j], sample_count, pctl(durs, sample_count, 50),
                   pctl(durs, sample_count, 90), pctl(durs, sample_count, 99),
                   durs[sample_count ? sample_count - 1 : 0]);
      if (written > 0)
        off += (size_t)written;
    }
  }
