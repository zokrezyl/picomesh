...
void ytelemetry_mint_trace_id(char out[33]) {
  ...
  if (ytel_bytes_all_zero(bytes, sizeof(bytes)))
    bytes[0] = 1; /* W3C: all-zero trace id invalid */
...
void ytelemetry_mint_span_id(char out[17]) {
  ...
  ytel_fill_random(bytes, sizeof(bytes));
  if (ytel_bytes_all_zero(bytes, sizeof(bytes)))
    bytes[0] = 1; /* W3C: all-zero span id invalid */
  ytel_to_hex(out, bytes, sizeof(bytes));
}
...
int ytelemetry_traceparent_parse(const char *traceparent, char *trace_id_hex,
  ...
  if (traceparent[2] != '-' || traceparent[35] != '-' || traceparent[52] != '-')
    return 0;
  if (!ytel_all_hex(traceparent, 2))
    return 0;
  if (!ytel_all_hex(traceparent + 3, 32))
    return 0;
  if (!ytel_all_hex(traceparent + 36, 16))
    return 0;
  if (!ytel_all_hex(traceparent + 53, 2))
    return 0;
  if (ytel_all_zero(traceparent + 3, 32))
    return 0;
  if (ytel_all_zero(traceparent + 36, 16))
    return 0;
  ...
  if (sampled) {
    int flags =
        ytel_hex_val(traceparent[53]) * 16 + ytel_hex_val(traceparent[54]);
    *sampled = flags & 0x01;
  }
  return 1;
}
...
void ytelemetry_traceparent_format(char *buf, size_t cap,
  ...
  snprintf(buf, cap, "00-%s-%s-%02x", trace_id_hex, span_id_hex,
           sampled ? 1 : 0);
}
...
void ytelemetry_hdrs_seed_root(struct yheaders *hdrs,
  ...
  if (!hdrs || !ytel_enabled())
    return;
  char trace_id[33], span_id[17];
  int sampled = 1;
  if (inbound_traceparent && *inbound_traceparent &&
      ytelemetry_traceparent_parse(inbound_traceparent, trace_id, span_id,
                                   &sampled)) {
...
static void ytel_span_begin(struct ytelemetry_span *span, struct yheaders *hdrs,
  ...
  if (trace_id && strlen(trace_id) == 32) {
    ...
    if (hdrs)
      yheaders_set(hdrs, "trace_id", span->trace_id);
  }

  const char *parent = yheaders_get(hdrs, "parent_span_id");
  if (parent && *parent)
    ytel_copystr(span->parent_id, sizeof(span->parent_id), parent);
...
void ytelemetry_server_span_begin(struct ytelemetry_span *span,
...
void ytelemetry_client_span_begin(struct ytelemetry_span *span,
                                  struct yheaders *hdrs, const char *name) {
  if (!ytel_enabled()) {
    if (span)
      memset(span, 0, sizeof(*span));
...
size_t ytelemetry_client_serialize_headers(const struct ytelemetry_span *span,
                                           ...
                                           size_t cap) {
  if (!hdrs || !ytel_enabled())
    return yheaders_serialize(hdrs, buf, cap);
  char save[17] = {0};
  const char *current = yheaders_get(hdrs, "parent_span_id");
  if (current)
    ytel_copystr(save, sizeof(save), current);
  yheaders_set(hdrs, "parent_span_id", span->span_id);
...
static const char *ytel_service_name(void) {
  ...
  if (!done) {
    struct picomesh_engine *engine = picomesh_active_engine();
    const char *name_arg =
        engine ? argv_get_string(picomesh_engine_cli(engine), "name", NULL)
               : NULL;
    ytel_copystr(name, sizeof(name),
                 name_arg && *name_arg ? name_arg : "picomesh");
    done = 1;
  }
  return name;
}
...
static const char *ytel_kind_str(uint8_t kind) {
  switch (kind) {
  case YTELEMETRY_KIND_SERVER:
    return "server";
  case YTELEMETRY_KIND_CLIENT:
    return "client";
  default:
    return "internal";
  }
  ...
  for (; str && *str; ++str) {
    ...
    if (ch == '"' || ch == '\\') {
      ...
      for (int j = 0; j < escape_len; ++j)
        off = ytel_putc(buf, cap, off, escape[j]);
    } else {
      off = ytel_putc(buf, cap, off, (char)ch);
    }
  }
...
static size_t ytel_build_span(char *span_json, size_t cap,
                              const struct ytelemetry_span *span, int ok,
                              const char *err, uint64_t dur_ns) {
  size_t off = 0;
  char uidbuf[16];
  snprintf(uidbuf, sizeof(uidbuf), "%u", span->uid);
  off = ytel_lit(span_json, cap, off, "{\"trace_id\":");
  off = ytel_jstr(span_json, cap, off, span->trace_id);
  off = ytel_lit(span_json, cap, off, ",\"span_id\":");
  off = ytel_jstr(span_json, cap, off, span->span_id);
  off = ytel_lit(span_json, cap, off, ",\"parent_span_id\":");
  off = ytel_jstr(span_json, cap, off, span->parent_id);
  off = ytel_lit(span_json, cap, off, ",\"name\":");
  off = ytel_jstr(span_json, cap, off, span->name);
  off = ytel_lit(span_json, cap, off, ",\"kind\":");
  off = ytel_jstr(span_json, cap, off, ytel_kind_str(span->kind));
  off = ytel_lit(span_json, cap, off, ",\"service_name\":");
  off = ytel_jstr(span_json, cap, off, ytel_service_name());
  off = ytel_lit(span_json, cap, off, ",\"node_id\":");
  off = ytel_jstr(span_json, cap, off, ytel_node_id());
  {
    char num[48];
    snprintf(num, sizeof(num), ",\"start_time_ns\":%llu",
             (unsigned long long)span->start_unix_ns);
    off = ytel_lit(span_json, cap, off, num);
  }
  {
    char num[48];
    snprintf(num, sizeof(num), ",\"duration_ns\":%llu",
             (unsigned long long)dur_ns);
    off = ytel_lit(span_json, cap, off, num);
  }
  off = ytel_lit(span_json, cap, off, ",\"status\":");
  off = ytel_jstr(span_json, cap, off, ok ? "ok" : "error");
  if (!ok && err && *err) {
    off = ytel_lit(span_json, cap, off, ",\"error_message\":");
    off = ytel_jstr(span_json, cap, off, err);
  }
  off = ytel_lit(span_json, cap, off,
                 ",\"attributes\":{\"rpc.system\":\"yrpc\",\"picomesh.uid\":");
...
static const struct ytel_batch_cfg *ytel_batch_config(void) {
  ...
  if (!loaded) {
    ...
    const struct config *config =
        engine ? picomesh_engine_config(engine) : NULL;
...
static void ytel_flush_batch(void) {
  ...
  struct picomesh_engine *engine = picomesh_active_engine();
  struct ctx ctx = engine
                       ? picomesh_engine_service_ctx(engine, "trace_collector")
                       : (struct ctx){0};
  if (!ctx.peer) {
    batch->count = 0;
    batch->len = 0;
    return;
  } /* collector not wired → drop */
  ...
  if (ingest_slot == METHOD_SLOT_UNDEFINED) {
    ...
    if (PICOMESH_IS_ERR(slot_res)) {
      picomesh_error_print(stderr, "ytelemetry flush: method_slot_by_name",
                           slot_res.error);
  ...
  struct object_ptr_result object_res =
      object_create_in_ctx(&ctx, "trace_collector_trace_collector");
  if (PICOMESH_IS_ERR(object_res)) {
    picomesh_error_print(stderr, "ytelemetry flush: object_create_in_ctx",
                         object_res.error);
  ...
  struct picomesh_uint32_result remote_id_res =
      peer_channel_ensure_remote_id(ctx.peer, ingest_slot);
  if (PICOMESH_IS_ERR(remote_id_res)) {
    picomesh_error_print(stderr, "ytelemetry flush: ensure remote id",
                         remote_id_res.error);
  ...
  if (remote_id == RPC_REMOTE_ID_UNRESOLVED) {
    batch->count = 0;
    batch->len = 0;
    return;
  }
  ...
  memcpy(batch->body + body_off, &handle, 8);
  body_off += 8;
  uint32_t arg_len32 = (uint32_t)arg_len;
  memcpy(batch->body + body_off, &arg_len32, 4);
  body_off += 4;
  batch->body[body_off++] = '[';
  memcpy(batch->body + body_off, batch->json, batch->len);
  body_off += batch->len;
  batch->body[body_off++] = ']';
...
int ytelemetry_pending_local(void) { return ytel_batch.count; }

/* Append one finished span to this worker's batch; flush on size/bytes/time. */
static void ytel_buffer_span(const struct ytelemetry_span *span, int ok,
                             const char *err, uint64_t dur_ns) {
  ...
  if (!batch->json) {
    ...
    if (!batch->json) {
      batch->cap = 0;
      return;
    } /* can't buffer → drop (best-effort) */
    batch->last_flush_ns = ytel_mono_ns();
  }

  char span_json[1024];
  size_t off =
      ytel_build_span(span_json, sizeof(span_json), span, ok, err, dur_ns);
  if (off == 0)
    return; /* overflow → drop this span */

  /* Flush first if appending would exceed the byte budget for one frame. */
  if (batch->count && batch->len + 1 + off > batch_cfg->max_bytes)
    ytel_flush_batch();

  if (batch->len + (batch->count ? 1u : 0u) + off <= batch->cap) {
    if (batch->count)
      batch->json[batch->len++] = ',';
  ...
  if (batch->count >= batch_cfg->max_spans) {
    ytel_flush_batch();
             ...
             ytel_mono_ns() - batch->last_flush_ns > batch_cfg->flush_ns) {
    ytel_flush_batch();
  }
}

void ytelemetry_span_end(struct ytelemetry_span *span, int ok,
                         const char *err) {
  /* Telemetry off (or a span that begin() zeroed): nothing to record or ship.
   */
  if (!ytel_enabled() || !span || span->start_mono_ns == 0)
    return;
