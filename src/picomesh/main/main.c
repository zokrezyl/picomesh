...
static const struct argv_option_def PICOMESH_OPTIONS[] = {
    ...
    {"--config-file", "-f", "config_file", "explicit config file", ARGV_VALUE,
     0},
    {"--config", NULL, "config", "config override (dotted=value)",
     ARGV_KEY_VALUE, 1},
    ...
    {"--app-name", NULL, "app_name", "app name (drives XDG path)", ARGV_VALUE,
     0},
    {"--name", NULL, "name", "instance name", ARGV_VALUE, 0},
    {"--frontend", NULL, "frontend",
     "frontend: yrpc (default), yttp, yhttp, alpine or msgpack", ARGV_VALUE, 0},
    {"--transport", NULL, "transport",
     "client transport: yrpc (default) or msgpack", ARGV_VALUE, 0},
    {"--workers", NULL, "workers", "in-process worker threads (default 1)",
     ARGV_VALUE, 0},
    {"--plugins", NULL, "plugins", "comma-sep plugin list (yaapp compat)",
     ARGV_VALUE, 0},
};
#define PICOMESH_OPTION_COUNT                                                  \
  (sizeof(PICOMESH_OPTIONS) / sizeof(PICOMESH_OPTIONS[0]))
...
struct plugin_reg {
  const char *name;
  struct picomesh_void_result (*reg)(void);
};
...
static const struct plugin_reg *plugin_registry(size_t *count) {
  static const struct plugin_reg ROWS[] = {
      ...
      {"personal_access_tokens",
       picomesh_plugin_personal_access_tokens_register},
...
static struct picomesh_void_result activate_one(const char *name) {
  ...
  for (size_t i = 0; i < done_n; ++i)
    if (strcmp(done[i], name) == 0)
      return PICOMESH_OK_VOID(); /* already active */
  ...
  for (size_t i = 0; i < row_count; ++i) {
    if (strcmp(rows[i].name, name) == 0) {
      ...
      PICOMESH_RETURN_IF_ERR(picomesh_void, reg_res,
                             "activate: plugin register failed");
      if (done_n < sizeof(done) / sizeof(done[0]))
        done[done_n++] = rows[i].name;
...
static int remotes_service_cb(const char *key, const struct config_node *val,
                              void *ud) {
  if (strcmp(key, "service") == 0) {
    const char *str = config_node_as_string(val, NULL);
    if (str) {
      snprintf((char *)ud, 64, "%s", str);
      return 1; /* stop */
    }
  }
  return 0;
 ...
 *   4. nothing                                 — activate nothing (yaapp
 * default)
 */
static struct picomesh_void_result
activate_plugins(struct picomesh_engine *engine) {
  /* 1. --plugins CLI: comma-separated names. */
  const char *csv =
      argv_get_string(picomesh_engine_cli(engine), "plugins", NULL);
  if (csv && *csv) {
    char buf[1024];
    ...
    for (char *tok = strtok(buf, ","); tok; tok = strtok(NULL, ",")) {
      ...
      if (*tok) {
        ...
        PICOMESH_RETURN_IF_ERR(picomesh_void, ar,
  ...
  if (name && *name) {
    ...
    PICOMESH_RETURN_IF_ERR(picomesh_void, list_res,
                           "activate_plugins: reading service plugins config");
  }
  if (!(PICOMESH_IS_OK(list_res) && list_res.value)) {
    list_res = config_get(picomesh_engine_config(engine), "plugins");
    PICOMESH_RETURN_IF_ERR(picomesh_void, list_res,
                           "activate_plugins: reading plugins config");
  }
  ...
  if (PICOMESH_IS_OK(remotes_res) && remotes_res.value &&
    ...
    for (size_t i = 0; i < count; ++i) {
      ...
      if (service_name[0]) {
        ...
        PICOMESH_RETURN_IF_ERR(picomesh_void, ar,
                               "activate_plugins: remote service");
      }
    }
  }
  return PICOMESH_OK_VOID();
 ...
 */
static struct const_char_ptr_result
resolve_host(struct picomesh_engine *engine) {
  const char *cli_host =
      argv_get_string(picomesh_engine_cli(engine), "host", NULL);
  if (cli_host && *cli_host)
    return PICOMESH_OK(const_char_ptr, cli_host);
  ...
  if (name && *name) {
    ...
    PICOMESH_RETURN_IF_ERR(const_char_ptr, host_res,
                           "resolve_host: service host config read failed");
    if (host_res.value) {
      const char *str = config_node_as_string(host_res.value, NULL);
      if (str && *str)
        return PICOMESH_OK(const_char_ptr, str);
    }
  }
  struct config_node_ptr_result host_res =
      config_get(picomesh_engine_config(engine), "yrpc.host");
  PICOMESH_RETURN_IF_ERR(const_char_ptr, host_res,
                         "resolve_host: yrpc.host config read failed");
  if (host_res.value) {
    const char *str = config_node_as_string(host_res.value, NULL);
    if (str && *str)
      return PICOMESH_OK(const_char_ptr, str);
...
static struct picomesh_int_result resolve_port(struct picomesh_engine *engine) {
  ...
  if (port_cli > 0)
    return PICOMESH_OK(picomesh_int, (int)port_cli);
  ...
  if (name && *name) {
    ...
    PICOMESH_RETURN_IF_ERR(picomesh_int, port_res,
                           "resolve_port: service port config read failed");
    if (port_res.value) {
      int64_t port_val = config_node_as_int(port_res.value, -1);
      if (port_val > 0)
        return PICOMESH_OK(picomesh_int, (int)port_val);
    }
  }
  struct config_node_ptr_result port_res =
      config_get(picomesh_engine_config(engine), "yrpc.port");
  PICOMESH_RETURN_IF_ERR(picomesh_int, port_res,
                         "resolve_port: yrpc.port config read failed");
  if (port_res.value) {
    int64_t port_val = config_node_as_int(port_res.value, -1);
    if (port_val > 0)
      return PICOMESH_OK(picomesh_int, (int)port_val);
...
resolve_workers(struct picomesh_engine *engine) {
  int64_t worker_count = -1;
  int64_t cli_workers =
      argv_get_int(picomesh_engine_cli(engine), "workers", -1);
  if (cli_workers > 0) {
    worker_count = cli_workers;
  } else {
    const char *name =
        argv_get_string(picomesh_engine_cli(engine), "name", NULL);
    if (name && *name) {
      char path[256];
      ...
      PICOMESH_RETURN_IF_ERR(
          picomesh_int, workers_res,
          "resolve_workers: service workers config read failed");
      if (workers_res.value)
        worker_count = config_node_as_int(workers_res.value, -1);
    }
    if (worker_count <= 0) {
      struct config_node_ptr_result workers_res =
          config_get(picomesh_engine_config(engine), "workers");
      PICOMESH_RETURN_IF_ERR(picomesh_int, workers_res,
                             "resolve_workers: workers config read failed");
      if (workers_res.value)
        worker_count = config_node_as_int(workers_res.value, -1);
    }
  }
...
static struct picomesh_void_result
serve_worker_setup(struct picomesh_engine *engine, int worker_index, void *ud) {
  struct serve_setup *setup = ud;

  if (setup->name && *setup->name) {
    struct picomesh_size_result remotes_res =
        picomesh_engine_open_remotes(engine, setup->name);
    PICOMESH_RETURN_IF_ERR(picomesh_void, remotes_res,
                           "serve_worker_setup: open remotes failed");
    yinfo("serve[%s w%d]: opened %zu remote(s) from "
          "mesh.services.%s.config.remotes",
  ...
  struct picomesh_void_result perf_res =
      picomesh_engine_perf_start(engine, setup->name);
  ...
  if (strcmp(setup->frontend, "yhttp") == 0) {
    ...
    PICOMESH_RETURN_IF_ERR(picomesh_void, frontend_res,
                           "serve_worker_setup: yhttp_start failed");
    ...
    PICOMESH_RETURN_IF_ERR(picomesh_void, frontend_res,
                           "serve_worker_setup: yttp_start failed");
...
static struct picomesh_void_result cmd_serve(struct picomesh_engine *engine) {
  ...
  PICOMESH_RETURN_IF_ERR(picomesh_void, host_res,
                         "cmd_serve: resolve host failed");
  ...
  int port;
  struct picomesh_int_result is_auto =
      picomesh_serve_port_is_auto(engine, name);
  PICOMESH_RETURN_IF_ERR(picomesh_void, is_auto,
                         "cmd_serve: port-auto check failed");
  if (is_auto.value) {
    struct picomesh_int_result alloc_res =
        picomesh_autoport_allocate(engine, name, host);
    PICOMESH_RETURN_IF_ERR(picomesh_void, alloc_res,
                           "cmd_serve: port:auto allocation failed");
    port = alloc_res.value;
  } else {
    struct picomesh_int_result port_res = resolve_port(engine);
    PICOMESH_RETURN_IF_ERR(picomesh_void, port_res,
                           "cmd_serve: resolve port failed");
    port = port_res.value;
  }
   ...
   * us. */
  struct picomesh_void_result register_res =
      picomesh_autoport_register_self(engine, name, host, port);
  PICOMESH_RETURN_IF_ERR(picomesh_void, register_res,
                         "cmd_serve: register self failed");
  struct picomesh_void_result remotes_res =
      picomesh_autoport_resolve_remotes(engine, name);
  PICOMESH_RETURN_IF_ERR(picomesh_void, remotes_res,
                         "cmd_serve: resolve remotes failed");
  struct picomesh_int_result workers_res = resolve_workers(engine);
  PICOMESH_RETURN_IF_ERR(picomesh_void, workers_res,
                         "cmd_serve: resolve workers failed");
  int workers = workers_res.value;
  const char *frontend =
      argv_get_string(picomesh_engine_cli(engine), "frontend", "yrpc");
  if (!frontend)
    frontend = "yrpc";
  ...
  if (strcmp(frontend, "yrpc") != 0 && strcmp(frontend, "yttp") != 0 &&
    ...
    fprintf(stderr,
            "unknown --frontend '%s' (try yrpc | yttp | yhttp | alpine | "
            "msgpack)\n",
            frontend);
    return PICOMESH_ERR(picomesh_void, "cmd_serve: unknown frontend");
  }

  yinfo("picomesh serve [%s]: %s:%d (%d worker%s)", frontend, host, port,
        workers, workers == 1 ? "" : "s");
  ...
  struct serve_setup setup = {
      .name = name, .host = host, .port = port, .frontend = frontend};
  struct picomesh_void_result run_res = picomesh_engine_run_workers(
      engine, (size_t)workers, serve_worker_setup, &setup);
  PICOMESH_RETURN_IF_ERR(picomesh_void, run_res,
                         "cmd_serve: engine_run_workers failed");
  return PICOMESH_OK_VOID();
}
...
static struct picomesh_void_result cmd_client(struct picomesh_engine *engine) {
  struct const_char_ptr_result host_res = resolve_host(engine);
  PICOMESH_RETURN_IF_ERR(picomesh_void, host_res,
                         "cmd_client: resolve host failed");
  const char *host = host_res.value;
  struct picomesh_int_result port_res = resolve_port(engine);
  PICOMESH_RETURN_IF_ERR(picomesh_void, port_res,
                         "cmd_client: resolve port failed");
  int port = port_res.value;

  int fd = socket(AF_INET, SOCK_STREAM, 0);
  if (fd < 0) {
    perror("socket");
    return PICOMESH_ERR(picomesh_void, "cmd_client: socket() failed");
  }
  ...
  if (connect(fd, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
    perror("connect");
    close(fd);
    return PICOMESH_ERR(picomesh_void, "cmd_client: connect failed");
  }
  ...
  const char *transport =
      argv_get_string(picomesh_engine_cli(engine), "transport", NULL);
  if (transport && strcmp(transport, "msgpack") == 0) {
    ...
    if (!msgpack_peer) {
      ...
      return PICOMESH_ERR(picomesh_void,
                          "cmd_client: msgpack peer alloc failed");
    }
    ...
    if (PICOMESH_IS_ERR(create_res)) {
      ...
      return PICOMESH_ERR(picomesh_void, "cmd_client: calc_create", create_res);
    }
    struct object *calc = create_res.value;
    struct ctx msgpack_ctx = {.peer = msgpack_peer};
    struct picomesh_int64_result add_res =
        calculator_calc_add(&msgpack_ctx, calc, NULL, 6, 7);
    if (PICOMESH_IS_ERR(add_res)) {
      peer_channel_destroy(msgpack_peer);
      return PICOMESH_ERR(picomesh_void, "cmd_client: calc_add(msgpack)",
                          add_res);
    }
    printf("msgpack client: 6 + 7 = %lld\n", (long long)add_res.value);
    struct picomesh_int64_result mul_res =
        calculator_calc_mul(&msgpack_ctx, calc, NULL, 6, 7);
    if (PICOMESH_IS_ERR(mul_res)) {
      peer_channel_destroy(msgpack_peer);
      return PICOMESH_ERR(picomesh_void, "cmd_client: calc_mul(msgpack)",
                          mul_res);
    }
  ...
  if (!channel) {
    close(fd);
    return PICOMESH_ERR(picomesh_void, "cmd_client: peer channel alloc failed");
  }
  ...
  struct object *calc = create_res.value;
  struct picomesh_int64_result add_res =
      calculator_calc_add(&ctx, calc, NULL, 6, 7);
  if (PICOMESH_IS_ERR(add_res)) {
    peer_channel_destroy(channel);
    return PICOMESH_ERR(picomesh_void, "cmd_client: calc_add", add_res);
  }
  yinfo("client: 6 + 7 = %lld", (long long)add_res.value);
  struct picomesh_int64_result mul_res =
      calculator_calc_mul(&ctx, calc, NULL, 6, 7);
  if (PICOMESH_IS_ERR(mul_res)) {
    peer_channel_destroy(channel);
    return PICOMESH_ERR(picomesh_void, "cmd_client: calc_mul", mul_res);
  }
...
int main(int argc, char **argv) {
  ...
  struct argv_chain_ptr_result parse_res =
      argv_parse(PICOMESH_OPTIONS, PICOMESH_OPTION_COUNT, argc, argv);
  if (PICOMESH_IS_ERR(parse_res))
    return die_err("argv_parse", parse_res.error);
  ...
  struct picomesh_engine_ptr_result engine_res =
      picomesh_engine_create(&engine_args);
  if (PICOMESH_IS_ERR(engine_res))
    return die_err("engine_create", engine_res.error);
  ...
  if (strcmp(sub, "serve") == 0) {
    ...
    rc = PICOMESH_IS_ERR(cli_res) ? die_err("invoke", cli_res.error)
                                  : cli_res.value;
  } else {
    usage(stderr, argv[0]);
    rc = 2;
