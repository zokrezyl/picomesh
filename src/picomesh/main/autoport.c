...
autoport_registry_addr(struct picomesh_engine *engine, char *host_out,
                       size_t cap, int *port_out) {
  struct config_node_ptr_result registry_res =
      config_get(picomesh_engine_config(engine), "registry");
  PICOMESH_RETURN_IF_ERR(picomesh_int, registry_res,
                         "autoport_registry_addr: registry config read failed");
  if (!registry_res.value || config_node_kind(registry_res.value) != CONFIG_MAP)
    return PICOMESH_OK(picomesh_int, 0);
  const struct config_node *port_node =
      config_node_get(registry_res.value, "port");
  int64_t port = port_node ? config_node_as_int(port_node, -1) : -1;
  if (port <= 0)
    return PICOMESH_OK(picomesh_int, 0);
  const struct config_node *host_node =
      config_node_get(registry_res.value, "host");
...
static struct picomesh_int_result
autoport_plugin_active(struct picomesh_engine *engine, const char *name,
                       const char *plugin) {
  const struct config_node *plugins = NULL;
  if (name && *name) {
    char path[256];
    snprintf(path, sizeof(path), "mesh.services.%s.plugins", name);
    struct config_node_ptr_result plugins_res =
        config_get(picomesh_engine_config(engine), path);
    PICOMESH_RETURN_IF_ERR(
        picomesh_int, plugins_res,
        "autoport_plugin_active: service plugins config read failed");
    if (plugins_res.value && config_node_kind(plugins_res.value) == CONFIG_LIST)
      plugins = plugins_res.value;
  }
  if (!plugins) {
    struct config_node_ptr_result plugins_res =
        config_get(picomesh_engine_config(engine), "plugins");
    PICOMESH_RETURN_IF_ERR(
        picomesh_int, plugins_res,
        "autoport_plugin_active: plugins config read failed");
    if (plugins_res.value && config_node_kind(plugins_res.value) == CONFIG_LIST)
      plugins = plugins_res.value;
  }
  if (!plugins)
...
static struct picomesh_int_result
reg_session_open(struct picomesh_engine *engine, struct reg_session *session) {
  char host[64];
  int port = 0;
  struct picomesh_int_result addr_res =
      autoport_registry_addr(engine, host, sizeof(host), &port);
  PICOMESH_RETURN_IF_ERR(picomesh_int, addr_res,
                         "reg_session_open: registry addr lookup failed");
  if (!addr_res.value)
    return PICOMESH_OK(picomesh_int, 0);
  int fd = -1;
  for (int attempt = 0; attempt < 100 && fd < 0; ++attempt) {
    ...
    if (fd < 0)
      nap_ms(100); /* registry may still be binding */
  }
  if (fd < 0) {
  ...
  if (PICOMESH_IS_ERR(obj_res)) {
    ...
    return PICOMESH_ERR(picomesh_int,
                        "reg_session_open: registry create failed", obj_res);
  }
...
static struct picomesh_int_result reg_resolve(struct reg_session *session,
                                              const char *service, int wait,
...
picomesh_serve_port_is_auto(struct picomesh_engine *engine, const char *name) {
  if (!engine)
    return PICOMESH_OK(picomesh_int, 0);
  ...
  if (name && *name) {
    ...
    struct config_node_ptr_result port_res =
        config_get(picomesh_engine_config(engine), path);
    PICOMESH_RETURN_IF_ERR(
        picomesh_int, port_res,
        "picomesh_serve_port_is_auto: service port config read failed");
    if (port_res.value)
      node = port_res.value;
  }
  if (!node) {
    struct config_node_ptr_result port_res =
        config_get(picomesh_engine_config(engine), "port");
    PICOMESH_RETURN_IF_ERR(
        picomesh_int, port_res,
        "picomesh_serve_port_is_auto: port config read failed");
    if (port_res.value)
      node = port_res.value;
  }
  if (!node || config_node_kind(node) != CONFIG_STRING)
    return PICOMESH_OK(picomesh_int, 0);
...
struct picomesh_int_result
picomesh_autoport_allocate(struct picomesh_engine *engine, const char *name,
                           const char *host) {
  if (!engine || !name || !*name)
    return PICOMESH_ERR(picomesh_int,
                        "autoport_allocate: engine/name required");
  if (!host || !*host)
    host = "127.0.0.1";
  ...
  struct picomesh_int_result portalloc_active =
      autoport_plugin_active(engine, name, "portalloc");
  PICOMESH_RETURN_IF_ERR(picomesh_int, portalloc_active,
                         "autoport_allocate: plugin active check failed");
  if (portalloc_active.value) {
    ...
    if (PICOMESH_IS_ERR(obj_res)) {
      ...
      return PICOMESH_ERR(picomesh_int,
                          "autoport_allocate: local portalloc create failed",
                          obj_res);
    }
    pobj = obj_res.value;
  }
  ...
  if (!pobj) {
    ...
    PICOMESH_RETURN_IF_ERR(picomesh_int, session_res,
                           "autoport_allocate: registry session failed");
    if (!session_res.value) {
      ywarn("autoport: '%s' wants port:auto but the registry is unreachable",
            name);
      return PICOMESH_ERR(picomesh_int,
                          "autoport_allocate: registry unreachable");
    }
    char portalloc_host[64];
    int portalloc_port = 0;
    struct picomesh_int_result resolve_res =
        reg_resolve(&session, "portalloc", 1, portalloc_host,
                    sizeof(portalloc_host), &portalloc_port);
    reg_session_close(&session);
    PICOMESH_RETURN_IF_ERR(picomesh_int, resolve_res,
                           "autoport_allocate: portalloc resolve failed");
    if (!resolve_res.value) {
      ywarn("autoport: portalloc not discoverable through registry for '%s'",
            name);
      return PICOMESH_ERR(picomesh_int,
                          "autoport_allocate: portalloc not discoverable");
    }
    int fd = -1;
    for (int attempt = 0; attempt < 100 && fd < 0; ++attempt) {
    ...
    if (fd < 0) {
      ywarn("autoport: portalloc %s:%d unreachable for '%s'", portalloc_host,
            portalloc_port, name);
      return PICOMESH_ERR(picomesh_int,
                          "autoport_allocate: portalloc unreachable");
    }
    portalloc_channel = peer_channel_create(fd);
    if (!portalloc_channel) {
      close(fd);
      return PICOMESH_ERR(picomesh_int,
                          "autoport_allocate: peer channel alloc failed");
    }
    ...
    if (PICOMESH_IS_ERR(obj_res)) {
      ...
      return PICOMESH_ERR(picomesh_int,
                          "autoport_allocate: remote portalloc create failed",
                          obj_res);
    }
    pobj = obj_res.value;
  }
  ...
  for (int attempt = 0; attempt < 64; ++attempt) {
    ...
    if (PICOMESH_IS_ERR(release_res))
      picomesh_error_destroy(release_res.error);
    nap_ms(20);
  }
  ...
  return PICOMESH_ERR(picomesh_int,
                      "autoport_allocate: no bindable port from portalloc");
}

struct picomesh_void_result
picomesh_autoport_register_self(struct picomesh_engine *engine,
                                const char *name, const char *host, int port) {
  if (!engine || !name || !*name || port <= 0)
    return PICOMESH_OK_VOID();
  char rhost[64];
  int rport = 0;
  struct picomesh_int_result addr_res =
      autoport_registry_addr(engine, rhost, sizeof(rhost), &rport);
  PICOMESH_RETURN_IF_ERR(picomesh_void, addr_res,
                         "register_self: registry addr lookup failed");
  if (!addr_res.value)
    return PICOMESH_OK_VOID(); /* no registry */
  ...
  struct picomesh_int_result reg_active =
      autoport_plugin_active(engine, name, "registry");
  PICOMESH_RETURN_IF_ERR(picomesh_void, reg_active,
                         "register_self: plugin active check failed");
  if (reg_active.value)
    return PICOMESH_OK_VOID();

  struct reg_session session = {0};
  struct picomesh_int_result session_res = reg_session_open(engine, &session);
  PICOMESH_RETURN_IF_ERR(picomesh_void, session_res,
                         "register_self: registry session failed");
  if (!session_res.value)
    return PICOMESH_OK_VOID();
  const char *reg_host = (strcmp(host, "0.0.0.0") == 0) ? "127.0.0.1" : host;
  struct picomesh_int_result register_res = registry_registry_register_service(
      &session.ctx, session.obj, NULL, name, name, reg_host, (uint32_t)port);
  if (PICOMESH_IS_OK(register_res)) {
    yinfo("autoport: registered '%s' -> %s:%d", name, reg_host, port);
  } else {
    ywarn("autoport: register '%s' failed: %s", name,
          register_res.error.msg ? register_res.error.msg : "?");
...
struct picomesh_void_result
picomesh_autoport_resolve_remotes(struct picomesh_engine *engine,
                                  const char *name) {
  if (!engine || !name || !*name)
    return PICOMESH_OK_VOID();
  char rhost[64];
  ...
  struct picomesh_int_result addr_res =
      autoport_registry_addr(engine, rhost, sizeof(rhost), &rport);
  PICOMESH_RETURN_IF_ERR(picomesh_void, addr_res,
                         "resolve_remotes: registry addr lookup failed");
  if (!addr_res.value)
    return PICOMESH_OK_VOID(); /* no registry */
  ...
  PICOMESH_RETURN_IF_ERR(picomesh_void, list_res,
                         "resolve_remotes: remotes config read failed");
  if (!list_res.value || config_node_kind(list_res.value) != CONFIG_LIST)
    return PICOMESH_OK_VOID();
  ...
  for (size_t i = 0; i < count; ++i) {
    ...
    if (port_val > 0)
      continue;
    if (!opened) {
      struct picomesh_int_result session_res =
          reg_session_open(engine, &session);
      PICOMESH_RETURN_IF_ERR(picomesh_void, session_res,
                             "resolve_remotes: registry session failed");
      if (!session_res.value)
        return PICOMESH_OK_VOID();
      opened = 1;
    }
    ...
    int port = 0;
    struct picomesh_int_result resolve_res =
        reg_resolve(&session, svc, 1, host, sizeof(host), &port);
    if (PICOMESH_IS_ERR(resolve_res)) {
      reg_session_close(&session);
      return PICOMESH_ERR(picomesh_void,
                          "resolve_remotes: registry resolve failed",
                          resolve_res);
    }
    if (resolve_res.value) {
      ...
      ywarn("autoport: remote '%s' (consumed by '%s') not resolvable via "
            "registry",
            svc, name);
    }
  }
