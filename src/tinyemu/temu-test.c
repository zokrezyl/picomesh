  ...
  for (int i = 0; i < 32; i++) {
    ...
    if (SymGetLineFromAddr64(proc, frame.AddrPC.Offset, &col, &line)) {
      fprintf(stderr, "  #%d %p  %s + 0x%llx  (%s:%lu)\n", i,
              (void *)(uintptr_t)frame.AddrPC.Offset, name,
...
int main(int argc, char *argv[]) {
  ...
  CHECKPOINT("load_config_file ok: drives=%d fs=%d eth=%d", p.drive_count,
  ...
  p.eth_count = 0;
  CHECKPOINT("  bios_filename=%s", p.files[VM_FILE_BIOS].filename
                                       ? p.files[VM_FILE_BIOS].filename
                                       : "(null)");
  CHECKPOINT("  bios buf=%p len=%d", (void *)p.files[VM_FILE_BIOS].buf,
             p.files[VM_FILE_BIOS].len);
  CHECKPOINT("  kernel_filename=%s", p.files[VM_FILE_KERNEL].filename
                                         ? p.files[VM_FILE_KERNEL].filename
                                         : "(null)");
  CHECKPOINT("  kernel buf=%p len=%d", (void *)p.files[VM_FILE_KERNEL].buf,
  ...
  for (int i = 0; i < p.drive_count; i++) {
    ...
    CHECKPOINT("drive%d: block_device_init ok (sectors=%lld)", i,
               (long long)bd->get_sector_count(bd));
  }
