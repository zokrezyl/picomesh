...
static int fs_walk(FSDevice *fs, FSFile **pf, FSQID *qids, FSFile *f, int n,
                   char **names) {
  (void)fs;
    ...
    if (h != INVALID_HANDLE_VALUE) {
      ...
      do {
        if (f->dirent.count == cap) {
          cap = cap ? cap * 2 : 16;
          f->dirent.names = realloc(f->dirent.names, cap * sizeof(char *));
        }
...
static int fs_setattr(FSDevice *fs, FSFile *f, uint32_t mask, uint32_t mode,
...
static int fs_readdir(FSDevice *fs, FSFile *f, uint64_t offset, uint8_t *buf,
                      int count) {
  (void)fs;
  if (!f->is_dir || !f->dirent.names)
    return -P9_EINVAL;
