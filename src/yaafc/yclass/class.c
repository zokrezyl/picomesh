...
struct slot_table_ptr_result slot_table_get(const char *domain)
{
    ...
    if (!domain) return YAAFC_ERR(slot_table_ptr, "slot_table_get: NULL domain");
    struct domain_registry *reg = dreg();
    struct slot_table *tbl = NULL;
    HASH_FIND(hh_dom, reg->by_name, domain, strlen(domain), tbl);
    if (tbl) return YAAFC_OK(slot_table_ptr, tbl);
    if (reg->next_id >= METHOD_SLOT_MAX_DOMAINS) {
        return YAAFC_ERR(slot_table_ptr, "slot_table_get: domain id capacity exhausted");
    }
    tbl = calloc(1, sizeof(*tbl));
    if (!tbl) return YAAFC_ERR(slot_table_ptr, "slot_table_get: calloc(slot_table) failed");
    tbl->domain = strdup(domain);
    if (!tbl->domain) {
        free(tbl);
        return YAAFC_ERR(slot_table_ptr, "slot_table_get: strdup(domain) failed");
    }
    tbl->domain_id = reg->next_id++;
    reg->by_id[tbl->domain_id] = tbl;
    HASH_ADD_KEYPTR(hh_dom, reg->by_name, tbl->domain, strlen(tbl->domain), tbl);
    return YAAFC_OK(slot_table_ptr, tbl);
}
                                               ...
                                               method_id_t id)
{
    ydebug("domain=%s name=%s id=%p", domain ? domain : "(null)", name ? name : "(null)",
           (void *)id);
    if (!domain || !name) {
        return YAAFC_ERR(method_slot, "method_slot_register: NULL domain or name");
    }
    struct slot_table_ptr_result tbl_r = slot_table_get(domain);
    YAAFC_RETURN_IF_ERR(method_slot, tbl_r, "method_slot_register: slot_table_get failed");
    struct slot_table *tbl = tbl_r.value;

    struct slot_entry *e = NULL;
    HASH_FIND(hh_lname, tbl->by_local_name, name, strlen(name), e);
    if (e) return YAAFC_OK(method_slot, e->slot_index);

    e = calloc(1, sizeof(*e));
    if (!e) return YAAFC_ERR(method_slot, "method_slot_register: calloc(slot_entry) failed");
    ...
    if (!e->qname) {
        free(e);
        return YAAFC_ERR(method_slot, "method_slot_register: malloc(qname) failed");
    }
    ...
    if (tbl->count >= tbl->cap) {
        ...
        if (!na) {
            free(e->qname);
            free(e);
            return YAAFC_ERR(method_slot, "method_slot_register: realloc(by_index) failed");
        }
    ...
    return YAAFC_OK(method_slot, e->slot_index);
}
...
struct method_slot_result method_slot_get(const char *domain, method_id_t id)
{
    ...
    if (!domain || !id) return YAAFC_ERR(method_slot, "method_slot_get: NULL domain or id");
    struct domain_registry *reg = dreg();
    struct slot_table *tbl = NULL;
    HASH_FIND(hh_dom, reg->by_name, domain, strlen(domain), tbl);
    if (!tbl) return YAAFC_ERR(method_slot, "method_slot_get: domain not registered");
    struct slot_entry *e = NULL;
    HASH_FIND(hh_id, tbl->by_local_id, &id, sizeof(method_id_t), e);
    if (!e) return YAAFC_ERR(method_slot, "method_slot_get: id not in domain's slot table");
    return YAAFC_OK(method_slot, e->slot_index);
}
...
struct method_slot_result method_slot_by_name(const char *domain, const char *name)
{
    ...
    if (!domain || !name) {
        return YAAFC_ERR(method_slot, "method_slot_by_name: NULL domain or name");
    }
    struct domain_registry *reg = dreg();
    struct slot_table *tbl = NULL;
    HASH_FIND(hh_dom, reg->by_name, domain, strlen(domain), tbl);
    if (!tbl) return YAAFC_ERR(method_slot, "method_slot_by_name: domain not registered");
    struct slot_entry *e = NULL;
    HASH_FIND(hh_lname, tbl->by_local_name, name, strlen(name), e);
    if (!e) return YAAFC_ERR(method_slot, "method_slot_by_name: name not in slot table");
    return YAAFC_OK(method_slot, e->slot_index);
}
...
struct method_slot_result method_slot_by_qname(const char *qname)
{
    ...
    if (!qname) return YAAFC_ERR(method_slot, "method_slot_by_qname: NULL qname");
    struct slot_entry *e = NULL;
    HASH_FIND(hh_qname, *global_qname_root(), qname, strlen(qname), e);
    if (!e) return YAAFC_ERR(method_slot, "method_slot_by_qname: qname not registered");
    return YAAFC_OK(method_slot, e->slot_index);
}
...
struct const_char_ptr_result method_slot_name(method_slot slot)
{
    ...
    if (slot == METHOD_SLOT_UNDEFINED) {
        return YAAFC_ERR(const_char_ptr, "method_slot_name: METHOD_SLOT_UNDEFINED");
    }
    uint8_t dom = METHOD_SLOT_DOMAIN_OF(slot);
    uint32_t idx = METHOD_SLOT_INDEX_OF(slot);
    if (dom == 0 || dom >= METHOD_SLOT_MAX_DOMAINS) {
        return YAAFC_ERR(const_char_ptr, "method_slot_name: invalid domain id");
    }
    struct slot_table *tbl = dreg()->by_id[dom];
    if (!tbl || idx >= tbl->count) {
        return YAAFC_ERR(const_char_ptr, "method_slot_name: slot index out of range");
    }
    return YAAFC_OK(const_char_ptr, tbl->by_index[idx]->qname);
}
...
static struct yaafc_void_result class_registry_add(struct class *cls)
{
    ...
    if (reg->count == reg->cap) {
        ...
        if (!na) return YAAFC_ERR(yaafc_void, "class_registry_add: realloc failed");
        reg->by_index = na;
        reg->cap = ncap;
    }
...
static struct yaafc_void_result dispatch_slice_grow(struct dispatch_slice *ds, size_t needed)
{
    if (needed <= ds->count) return YAAFC_OK_VOID();
    impl_t *nd = realloc(ds->impls, needed * sizeof(*nd));
    if (!nd) return YAAFC_ERR(yaafc_void, "dispatch_slice_grow: realloc failed");
...
static struct yaafc_void_result class_inherit_dispatch(struct class *cls, const struct class *src)
{
    for (uint8_t d = 0; d < METHOD_SLOT_MAX_DOMAINS; ++d) {
        ...
        struct yaafc_void_result g = dispatch_slice_grow(dd, sd->count);
        YAAFC_RETURN_IF_ERR(yaafc_void, g, "class_inherit_dispatch: grow failed");
                                       ...
                                       const struct class *const *mixins, size_t mixin_count)
{
    ...
    if (!desc) return YAAFC_ERR(class_ptr, "class_register: NULL descriptor");
    struct class *cls = calloc(1, sizeof(*cls));
    if (!cls) return YAAFC_ERR(class_ptr, "class_register: calloc(class) failed");
    cls->desc = desc;
    cls->parent = parent;
    ...
    if (mixin_count > 0) {
        ...
        if (!cls->mixins) {
            class_destroy(cls);
            return YAAFC_ERR(class_ptr, "class_register: malloc(mixins) failed");
        }
    ...
    if (parent) {
        struct yaafc_void_result inh = class_inherit_dispatch(cls, parent);
        if (YAAFC_IS_ERR(inh)) {
            class_destroy(cls);
            return YAAFC_ERR(class_ptr, "class_register: inherit parent dispatch failed", inh);
        }
    }
    for (size_t m = 0; m < mixin_count; ++m) {
        struct yaafc_void_result inh = class_inherit_dispatch(cls, mixins[m]);
        if (YAAFC_IS_ERR(inh)) {
            class_destroy(cls);
            return YAAFC_ERR(class_ptr, "class_register: inherit mixin dispatch failed", inh);
        }
    }
    ...
    for (size_t i = 0; i < ops_count; ++i) {
        ...
        if (YAAFC_IS_ERR(sr)) {
            class_destroy(cls);
            return YAAFC_ERR(class_ptr, "class_register: method_slot_register failed", sr);
        }
        method_slot slot = sr.value;
        uint8_t dom = METHOD_SLOT_DOMAIN_OF(slot);
        uint32_t idx = METHOD_SLOT_INDEX_OF(slot);
        struct dispatch_slice *ds = &cls->dispatch_by_domain[dom];
        struct yaafc_void_result g = dispatch_slice_grow(ds, (size_t)idx + 1);
        if (YAAFC_IS_ERR(g)) {
            class_destroy(cls);
            return YAAFC_ERR(class_ptr, "class_register: dispatch slice grow failed", g);
        }
        ds->impls[idx] = ops[i].impl;
    }

    struct yaafc_void_result a = class_registry_add(cls);
    if (YAAFC_IS_ERR(a)) {
        class_destroy(cls);
        return YAAFC_ERR(class_ptr, "class_register: class_registry_add failed", a);
    }
    return YAAFC_OK(class_ptr, cls);
}
...
struct yaafc_void_result class_add_accessor_lookup(accessor_lookup_fn fn)
{
    ...
    if (!fn) return YAAFC_ERR(yaafc_void, "class_add_accessor_lookup: NULL fn");
    struct accessor_node *node = calloc(1, sizeof(*node));
    if (!node) return YAAFC_ERR(yaafc_void, "class_add_accessor_lookup: calloc failed");
    node->fn = fn;
...
struct class_ptr_result class_by_name(const char *name)
{
    ...
    if (!name) return YAAFC_ERR(class_ptr, "class_by_name: NULL name");
    struct class_registry *reg = class_registry_get();
    ...
    for (struct accessor_node *n = *accessor_chain_head(); n; n = n->next) {
        ...
        if (YAAFC_IS_ERR(r)) {
            return YAAFC_ERR(class_ptr, "class_by_name: accessor hook failed", r);
        }
        if (r.value) return r;
    }
    return YAAFC_ERR(class_ptr, "class_by_name: class not found in registry or any hook");
}
                         ...
                         void (*cb)(const char *name, method_slot slot, void *ud), void *userdata)
{
    ...
    for (uint8_t d = 0; d < METHOD_SLOT_MAX_DOMAINS; ++d) {
        ...
        for (size_t i = 0; i < ds->count; ++i) {
            ...
            if (YAAFC_IS_OK(nr) && nr.value) {
                cb(nr.value, slot, userdata);
            } else if (YAAFC_IS_ERR(nr)) {
...
struct object_ptr_result object_alloc(const struct class *cls)
{
    ...
    if (!cls) return YAAFC_ERR(object_ptr, "object_alloc: NULL class");
    struct object *obj = calloc(1, cls->instance_size);
    if (!obj) return YAAFC_ERR(object_ptr, "object_alloc: calloc failed");
    obj->klass = cls;
