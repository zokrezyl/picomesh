                                          ...
                                          size_t def_count, int argc, char **argv)
{
    ...
    if (!c) return YAAFC_ERR(yargv_chain_ptr, "yargv_parse: calloc failed");
    c->defs = defs;
    c->def_count = def_count;
    c->slots = calloc(def_count ? def_count : 1, sizeof(*c->slots));
    if (def_count && !c->slots) {
        free(c);
        return YAAFC_ERR(yargv_chain_ptr, "yargv_parse: slots alloc failed");
    }
        ...
        if (!options_ended && tok[0] == '-' && tok[1] != '\0') {
            ...
            case YARGV_VALUE:
                if (i + 1 >= argc) {
                    ...
                    return YAAFC_ERR(yargv_chain_ptr, "yargv_parse: missing value for option");
                }
                s->present = 1;
                free(s->string_val);
                ...
                if (!s->string_val) {
                    yargv_chain_destroy(c);
                    return YAAFC_ERR(yargv_chain_ptr, "yargv_parse: strdup failed");
                }
                i += 2;
                break;
            case YARGV_KEY_VALUE:
                if (i + 1 >= argc) {
                    yargv_chain_destroy(c);
                    return YAAFC_ERR(yargv_chain_ptr, "yargv_parse: missing key=value for option");
                }
                s->present = 1;
                if (kv_push(s, argv[i + 1]) < 0) {
                    yargv_chain_destroy(c);
                    return YAAFC_ERR(yargv_chain_ptr, "yargv_parse: kv push failed");
                }
                i += 2;
                break;
            }
    ...
    return YAAFC_OK(yargv_chain_ptr, c);
}
