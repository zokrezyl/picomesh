...
{
    const char *app = (args && args->app_name) ? args->app_name : "yaafc";

    struct yconfig *c = calloc(1, sizeof(*c));
    if (!c) return YAAFC_ERR(yconfig_ptr, "yconfig_create: calloc failed");
    c->root = node_new(YCONFIG_MAP);
    if (!c->root) {
        free(c);
        return YAAFC_ERR(yconfig_ptr, "yconfig_create: root alloc failed");
    }
    ...
    if (args && args->cli_overrides) {
        for (size_t i = 0; i < args->cli_override_count; ++i) {
            if (apply_cli_override(c->root, args->cli_overrides[i]) < 0) {
                yconfig_destroy(c);
                return YAAFC_ERR(yconfig_ptr, "yconfig_create: cli override merge failed");
            }
        }
    }
...
{
    if (!c || !dot_path) {
        return YAAFC_ERR(yconfig_node_ptr, "yconfig_get: NULL args");
    }
    const struct yconfig_node *direct = walk_path(c->root, dot_path);
    if (direct) return YAAFC_OK(yconfig_node_ptr, direct);
    ...
    if (len >= sizeof(buf)) return YAAFC_OK(yconfig_node_ptr, NULL);
    memcpy(buf, dot_path, len + 1);
    char *dot = strchr(buf, '.');
    while (dot) {
        const struct yconfig_node *hit = walk_path(c->root, dot + 1);
        if (hit) return YAAFC_OK(yconfig_node_ptr, hit);
        dot = strchr(dot + 1, '.');
    }
    return YAAFC_OK(yconfig_node_ptr, NULL);
}
