...
#include <yaafc/issues/methods.gen.h>
...
struct yaafc_uint32_result issues_store_open(struct ctx * ctx, struct object * obj, uint32_t repo_id, uint32_t author_id)
{
    ...
    if (_s && _s->session) {
        ...
        if (_wbuf[0] != 0) return YAAFC_ERR(yaafc_uint32, "issues_store_open: remote impl returned error");
        if (_wn != sizeof(_wbuf)) return YAAFC_ERR(yaafc_uint32, "issues_store_open: truncated RPC payload");
        uint32_t _v;
...
struct yaafc_int_result issues_store_close(struct ctx * ctx, struct object * obj, uint32_t issue_id)
{
    ...
    if (_s && _s->session) {
        ...
        if (_wbuf[0] != 0) return YAAFC_ERR(yaafc_int, "issues_store_close: remote impl returned error");
        if (_wn != sizeof(_wbuf)) return YAAFC_ERR(yaafc_int, "issues_store_close: truncated RPC payload");
        int _v;
        memcpy(&_v, _wbuf + 1, sizeof(_v));
...
struct yaafc_int_result issues_store_status(struct ctx * ctx, struct object * obj, uint32_t issue_id)
{
    ...
    if (_s && _s->session) {
        ...
        if (_wbuf[0] != 0) return YAAFC_ERR(yaafc_int, "issues_store_status: remote impl returned error");
        if (_wn != sizeof(_wbuf)) return YAAFC_ERR(yaafc_int, "issues_store_status: truncated RPC payload");
        int _v;
        memcpy(&_v, _wbuf + 1, sizeof(_v));
        return YAAFC_OK(yaafc_int, _v);
    } else {
...
struct yaafc_size_result issues_store_count_open_in_repo(struct ctx * ctx, struct object * obj, uint32_t repo_id)
{
    ...
    if (_s && _s->session) {
        ...
        if (_wbuf[0] != 0) return YAAFC_ERR(yaafc_size, "issues_store_count_open_in_repo: remote impl returned error");
        if (_wn != sizeof(_wbuf)) return YAAFC_ERR(yaafc_size, "issues_store_count_open_in_repo: truncated RPC payload");
        size_t _v;
        memcpy(&_v, _wbuf + 1, sizeof(_v));
...
struct yaafc_size_result issues_store_count_total(struct ctx * ctx, struct object * obj)
{
    ...
    if (_s && _s->session) {
        ...
        if (_wbuf[0] != 0) return YAAFC_ERR(yaafc_size, "issues_store_count_total: remote impl returned error");
        if (_wn != sizeof(_wbuf)) return YAAFC_ERR(yaafc_size, "issues_store_count_total: truncated RPC payload");
        size_t _v;
        memcpy(&_v, _wbuf + 1, sizeof(_v));
        return YAAFC_OK(yaafc_size, _v);
    } else {
