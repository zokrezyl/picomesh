...
struct yaafc_int_result storage_kv_set(struct ctx * ctx, struct object * obj, uint32_t key_id, int32_t value)
{
    ...
    if (_s && _s->session) {
        ...
        if (_wbuf[0] != 0) return YAAFC_ERR(yaafc_int, "storage_kv_set: remote impl returned error");
        if (_wn != sizeof(_wbuf)) return YAAFC_ERR(yaafc_int, "storage_kv_set: truncated RPC payload");
        int _v;
...
struct yaafc_int_result storage_kv_get(struct ctx * ctx, struct object * obj, uint32_t key_id)
{
    ...
    if (_s && _s->session) {
        ...
        if (_wbuf[0] != 0) return YAAFC_ERR(yaafc_int, "storage_kv_get: remote impl returned error");
        if (_wn != sizeof(_wbuf)) return YAAFC_ERR(yaafc_int, "storage_kv_get: truncated RPC payload");
        int _v;
        memcpy(&_v, _wbuf + 1, sizeof(_v));
        return YAAFC_OK(yaafc_int, _v);
    } else {
...
struct yaafc_size_result storage_kv_count(struct ctx * ctx, struct object * obj)
{
    ...
    if (_s && _s->session) {
        ...
        if (_wbuf[0] != 0) return YAAFC_ERR(yaafc_size, "storage_kv_count: remote impl returned error");
        if (_wn != sizeof(_wbuf)) return YAAFC_ERR(yaafc_size, "storage_kv_count: truncated RPC payload");
        size_t _v;
        memcpy(&_v, _wbuf + 1, sizeof(_v));
...
struct yaafc_int_result storage_sql_set(struct ctx * ctx, struct object * obj, const char * key, int64_t value)
{
    ...
    if (_s && _s->session) {
        ...
        if (_wbuf[0] != 0) return YAAFC_ERR(yaafc_int, "storage_sql_set: remote impl returned error");
        if (_wn != sizeof(_wbuf)) return YAAFC_ERR(yaafc_int, "storage_sql_set: truncated RPC payload");
        int _v;
        memcpy(&_v, _wbuf + 1, sizeof(_v));
        return YAAFC_OK(yaafc_int, _v);
    } else {
...
struct yaafc_int64_result storage_sql_get(struct ctx * ctx, struct object * obj, const char * key)
{
    ...
    if (_s && _s->session) {
        ...
        if (_wbuf[0] != 0) return YAAFC_ERR(yaafc_int64, "storage_sql_get: remote impl returned error");
        if (_wn != sizeof(_wbuf)) return YAAFC_ERR(yaafc_int64, "storage_sql_get: truncated RPC payload");
        int64_t _v;
        memcpy(&_v, _wbuf + 1, sizeof(_v));
...
struct yaafc_int_result storage_sql_exists(struct ctx * ctx, struct object * obj, const char * key)
{
    ...
    if (_s && _s->session) {
        ...
        if (_wbuf[0] != 0) return YAAFC_ERR(yaafc_int, "storage_sql_exists: remote impl returned error");
        if (_wn != sizeof(_wbuf)) return YAAFC_ERR(yaafc_int, "storage_sql_exists: truncated RPC payload");
        int _v;
        memcpy(&_v, _wbuf + 1, sizeof(_v));
        return YAAFC_OK(yaafc_int, _v);
    } else {
...
struct yaafc_int_result storage_sql_del(struct ctx * ctx, struct object * obj, const char * key)
{
    ...
    if (_s && _s->session) {
        ...
        if (_wbuf[0] != 0) return YAAFC_ERR(yaafc_int, "storage_sql_del: remote impl returned error");
        if (_wn != sizeof(_wbuf)) return YAAFC_ERR(yaafc_int, "storage_sql_del: truncated RPC payload");
        int _v;
        memcpy(&_v, _wbuf + 1, sizeof(_v));
        return YAAFC_OK(yaafc_int, _v);
    } else {
...
struct yaafc_size_result storage_sql_count(struct ctx * ctx, struct object * obj)
{
    ...
    if (_s && _s->session) {
        ...
        if (_wbuf[0] != 0) return YAAFC_ERR(yaafc_size, "storage_sql_count: remote impl returned error");
        if (_wn != sizeof(_wbuf)) return YAAFC_ERR(yaafc_size, "storage_sql_count: truncated RPC payload");
        size_t _v;
        memcpy(&_v, _wbuf + 1, sizeof(_v));
        return YAAFC_OK(yaafc_size, _v);
    } else {
