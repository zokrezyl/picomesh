...
struct yaafc_int_result github_authn_store_set_credentials(struct ctx * ctx, struct object * obj, uint32_t client_id, uint32_t secret_id)
{
    ...
    if (_s && _s->session) {
        ...
        if (_wbuf[0] != 0) return YAAFC_ERR(yaafc_int, "github_authn_store_set_credentials: remote impl returned error");
        if (_wn != sizeof(_wbuf)) return YAAFC_ERR(yaafc_int, "github_authn_store_set_credentials: truncated RPC payload");
        int _v;
...
struct yaafc_int_result github_authn_store_register_code(struct ctx * ctx, struct object * obj, uint32_t code, uint32_t user_id)
{
    ...
    if (_s && _s->session) {
        ...
        if (_wbuf[0] != 0) return YAAFC_ERR(yaafc_int, "github_authn_store_register_code: remote impl returned error");
        if (_wn != sizeof(_wbuf)) return YAAFC_ERR(yaafc_int, "github_authn_store_register_code: truncated RPC payload");
        int _v;
        memcpy(&_v, _wbuf + 1, sizeof(_v));
        return YAAFC_OK(yaafc_int, _v);
    } else {
...
struct yaafc_uint32_result github_authn_store_resolve(struct ctx * ctx, struct object * obj, uint32_t code)
{
    ...
    if (_s && _s->session) {
        ...
        if (_wbuf[0] != 0) return YAAFC_ERR(yaafc_uint32, "github_authn_store_resolve: remote impl returned error");
        if (_wn != sizeof(_wbuf)) return YAAFC_ERR(yaafc_uint32, "github_authn_store_resolve: truncated RPC payload");
        uint32_t _v;
        memcpy(&_v, _wbuf + 1, sizeof(_v));
...
struct yaafc_size_result github_authn_store_count_codes(struct ctx * ctx, struct object * obj)
{
    ...
    if (_s && _s->session) {
        ...
        if (_wbuf[0] != 0) return YAAFC_ERR(yaafc_size, "github_authn_store_count_codes: remote impl returned error");
        if (_wn != sizeof(_wbuf)) return YAAFC_ERR(yaafc_size, "github_authn_store_count_codes: truncated RPC payload");
        size_t _v;
        memcpy(&_v, _wbuf + 1, sizeof(_v));
