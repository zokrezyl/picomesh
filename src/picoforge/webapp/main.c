...
static const struct argv_option_def WEBAPP_OPTIONS[] = {
    {"--gateway-url", NULL, "gateway_url", "Gateway base URL (required)",
     ARGV_VALUE, 0},
    {"--host", NULL, "host", "Bind address (default 0.0.0.0)", ARGV_VALUE, 0},
    {"--port", NULL, "port", "Bind port (default 8080)", ARGV_VALUE, 0},
    {"--templates", NULL, "templates",
     "Template directory (default ./templates)", ARGV_VALUE, 0},
    {"--static", NULL, "static_dir", "Static directory (default ./static)",
     ARGV_VALUE, 0},
    {"--console-url", NULL, "console_url",
     "Service console URL linked on /admin/services "
     "(default http://127.0.0.1:8231/_alpine; \"\" to hide)",
     ARGV_VALUE, 0},
    {"--github-client", NULL, "github_client",
     "GitHub OAuth App client id (or $PICOFORGE_GITHUB_CLIENT)", ARGV_VALUE, 0},
    {"--github-url", NULL, "github_url",
     "GitHub web URL (default https://github.com)", ARGV_VALUE, 0},
    {"--public-url", NULL, "public_url",
     "Externally visible webapp URL for OAuth callbacks", ARGV_VALUE, 0},
    {"--verbose", "-v", "verbose", "Enable debug logging", ARGV_BOOL, 0},
    {"--help", "-h", "help", "Show this message", ARGV_BOOL, 0},
};
...
static void usage(const char *prog) {
  fprintf(stderr,
          ...
          "          [--templates <dir>] [--static <dir>] [--console-url "
          "<url>] [-v]\n"
          "\n"
          "Standalone HTML web app. Talks to the gateway via POST /_rpc.\n"
          "Works against either the picomesh C gateway or the yaapp Python "
          "gateway —\n"
...
int main(int argc, char **argv) {
  ...
  const char *console_url =
      argv_get_string(cli, "console_url", "http://127.0.0.1:8231/_alpine");
  const char *github_client =
      argv_get_string(cli, "github_client", getenv("PICOFORGE_GITHUB_CLIENT"));
  const char *github_url =
      argv_get_string(cli, "github_url", "https://github.com");
  const char *public_url =
      argv_get_string(cli, "public_url", getenv("PICOFORGE_PUBLIC_URL"));
  ...
  yinfo("picoforge-webapp: listening on %s:%d (gateway=%s)", host, port,
        gateway_url);
