...
static void send_response(struct loop_stream *s, int status,
  ...
  int n = snprintf(header, sizeof(header),
                   ...
                   "%s"
                   "\r\n",
                   status, http_reason(status), content_type, body_len,
...
static void send_redirect(struct loop_stream *s, const char *location,
  ...
  if (n <= 0 || (size_t)n >= sizeof(hdrs)) {
    send_response(s, 500, "text/plain", "redirect too large",
                  strlen("redirect too large"), NULL, keep_alive);
    return;
  }
  send_response(s, 303, "text/plain", "", 0, hdrs, keep_alive);
}

static void send_text(struct loop_stream *s, int status, const char *body,
                      int keep_alive) {
  send_response(s, status, "text/plain; charset=utf-8", body, strlen(body),
                NULL, keep_alive);
}
...
static int serve_static(struct loop_stream *s, const char *root,
  ...
  if (rlen >= 5 && strcasecmp(rel + rlen - 5, ".html") == 0)
    return 0;
  if (rlen >= 4 && strcasecmp(rel + rlen - 4, ".htm") == 0)
    return 0;
...
static void route_login_get(struct loop_stream *s, const struct serve_ud *sud,
                            int keep_alive) {
  if (!sud || !sud->cfg || !sud->cfg->github_client_id ||
      !*sud->cfg->github_client_id) {
    send_response(s, 200, "text/html; charset=utf-8", LOGIN_HTML,
                  sizeof(LOGIN_HTML) - 1, NULL, keep_alive);
    return;
  }
  struct buf b;
  buf_init(&b);
  buf_puts(&b,
           ...
           "<p><a class=\"oauth-btn btn\" href=\"/-/auth/github\">Sign in with "
           "GitHub</a></p>"
           "<p><a href=\"/-/register\">Create an account</a></p>"
           "</main></body></html>");
  send_response(s, 200, "text/html; charset=utf-8", b.data, b.len, NULL,
                keep_alive);
  buf_free(&b);
}
...
static void route_login_get_with_error(struct loop_stream *s, const char *err,
                                       int keep_alive) {
  char escaped[1024];
  html_escape(escaped, sizeof(escaped), err && *err ? err : "Sign-in failed");
  char body[4096];
  int n = snprintf(
      body, sizeof(body),
      "<!doctype html><html><head>"
  ...
  if (n <= 0)
    return;
  send_response(s, 200, "text/html; charset=utf-8", body, (size_t)n, NULL,
                keep_alive);
}
...
static int read_body(struct loop_stream *s, char *dst, size_t n) {
  size_t got = 0;
  while (got < n) {
    struct picomesh_size_result chunk_res =
        loop_read_some(s, dst + got, n - got);
    if (PICOMESH_IS_ERR(chunk_res)) {
      picomesh_error_destroy(chunk_res.error);
      return -1;
    }
    if (chunk_res.value == 0)
      return -1; /* EOF before the full body arrived */
    got += chunk_res.value;
  }
  return 0;
}
...
static long header_content_length(const struct phr_header *hdrs, size_t n) {
  for (size_t i = 0; i < n; ++i) {
    ...
    if (!eq)
      continue;
    char tmp[32];
    size_t copy_len = hdrs[i].value_len < sizeof(tmp) - 1 ? hdrs[i].value_len
                                                          : sizeof(tmp) - 1;
...
static void build_auth_cookies(char *out, size_t cap, const char *relayed_sid,
                               const char *username) {
  char uname_v[40];
...
route_login_post(struct loop *loop, struct loop_stream *s,
                 const struct serve_ud *sud, const char *body, size_t body_len,
                 int keep_alive) {
  ...
  struct picomesh_int_result post =
      http_post(loop, &sud->gw, "/login", "application/x-www-form-urlencoded",
...
static struct picomesh_void_result resolve_claims(struct loop *loop,
                                                  const struct serve_ud *sud,
                                                  const char *sid,
                                                  struct claims *out);

static void public_base_url(const struct serve_ud *sud,
                            const struct phr_header *hdrs, size_t n, char *out,
                            size_t cap) {
...
static void github_redirect_uri(const struct serve_ud *sud,
                                const struct phr_header *hdrs, size_t n,
                                char *out, size_t cap) {
  char base[512];
...
static int oauth_state(char *out, size_t cap) {
  ...
  close(fd);
  if (n != (ssize_t)sizeof(rnd) || cap < sizeof(rnd) * 2 + 1)
    return 0;
  static const char hex[] = "0123456789abcdef";
...
static void route_github_start(struct loop_stream *s,
                               const struct serve_ud *sud,
                               const struct phr_header *hdrs, size_t n,
                               int keep_alive) {
  const char *client = sud && sud->cfg ? sud->cfg->github_client_id : NULL;
  const char *ghurl =
      (sud && sud->cfg && sud->cfg->github_url && *sud->cfg->github_url)
          ? sud->cfg->github_url
          : "https://github.com";
  if (!client || !*client) {
    route_login_get_with_error(s, "GitHub login is not configured", keep_alive);
    return;
  }
  char state[33];
  if (!oauth_state(state, sizeof(state))) {
    route_login_get_with_error(s, "GitHub login is temporarily unavailable",
                               keep_alive);
    return;
  }
  ...
  int ln = snprintf(
      ...
      "%s/login/oauth/"
      "authorize?client_id=%s&redirect_uri=%s&scope=repo,user:email&state=%s",
      ghurl, enc_client, enc_redirect, enc_state);
  if (ln <= 0 || (size_t)ln >= sizeof(location)) {
    route_login_get_with_error(s, "GitHub redirect URL is too large",
                               keep_alive);
    return;
  }
  char cookie[128];
  snprintf(cookie, sizeof(cookie),
           "Set-Cookie: picoforge-oauth-state=%s; Path=/-/auth/github; "
...
static struct picomesh_void_result
route_github_callback(struct loop *loop, struct loop_stream *s,
                      const struct serve_ud *sud, const struct phr_header *hdrs,
                      size_t hn, const char *full_path, int keep_alive) {
  ...
  if (!state || !state_cookie || strcmp(state, state_cookie) != 0) {
    ...
    free(state);
    free(state_cookie);
    route_login_get_with_error(s, "GitHub OAuth state did not match",
                               keep_alive);
    return PICOMESH_OK_VOID();
  }
  char enc_state[160];
  ...
  if (!code || !*code) {
    free(code);
    route_login_get_with_error(s, "GitHub did not return an OAuth code",
                               keep_alive);
    return PICOMESH_OK_VOID();
  }
  ...
  if (bn <= 0 || (size_t)bn >= sizeof(body)) {
    route_login_get_with_error(s, "GitHub callback is too large", keep_alive);
    return PICOMESH_OK_VOID();
  }
  ...
  struct http_response resp;
  struct picomesh_int_result post =
      http_post(loop, &sud->gw, "/auth/github/callback",
                "application/x-picoforge-oauth-relay", NULL, NULL, body,
                (size_t)bn, &resp);
  if (PICOMESH_IS_ERR(post)) {
    picomesh_error_print(stderr, "route_github_callback: /auth/github/callback",
                         post.error);
    picomesh_error_destroy(post.error);
    http_response_free(&resp);
    route_login_get_with_error(s, "gateway unreachable", keep_alive);
  ...
  if (resp.status == 303 && resp.set_cookie[0]) {
    ...
    char hdrs_out[1024];
    build_auth_cookies(hdrs_out, sizeof(hdrs_out), resp.set_cookie,
                       claims.username);
    strncat(hdrs_out,
            "Set-Cookie: picoforge-oauth-state=; Path=/-/auth/github; "
...
static void route_register_get(struct loop_stream *s, int keep_alive) {
  send_response(s, 200, "text/html; charset=utf-8", REGISTER_HTML,
                sizeof(REGISTER_HTML) - 1, NULL, keep_alive);
}
...
static struct picomesh_void_result
route_register_post(struct loop *loop, struct loop_stream *s,
                    const struct serve_ud *sud, const char *body,
  ...
  if (!have_both) {
    free(username);
    route_register_get_with_error(s, "username and password are required",
                                  keep_alive);
    return PICOMESH_OK_VOID();
  }

  struct http_response resp;
  struct picomesh_int_result post = http_post(
      loop, &sud->gw, "/register", "application/x-www-form-urlencoded", NULL,
      NULL, body, body_len, &resp);
  if (PICOMESH_IS_ERR(post)) {
  ...
  http_response_free(&resp);
  free(username);
  route_register_get_with_error(
      s, "could not create account (the username may already be taken)",
      keep_alive);
  return PICOMESH_OK_VOID();
}
...
static struct picomesh_int64_result
rpc_result_int(struct loop *loop, const struct serve_ud *sud, const char *sid,
               const char *rpc_path, const char *args_json, long fallback);
static struct picomesh_string_result
rpc_result_str(struct loop *loop, const struct serve_ud *sud, const char *sid,
               const char *rpc_path, const char *args_json, int *was_error);
...
static void render_topbar(struct buf *b, const char *uname,
  ...
  if (uname && *uname) {
    buf_puts(
        b, "<form class=\"global-search\" method=\"get\" action=\"/-/search\">"
    ...
    buf_puts(
        b, "<nav class=\"top-actions\"><a href=\"/-/login\">Sign in</a></nav>");
  }
  buf_puts(b, "</header>");
}
...
static void render_sidebar(struct buf *b, const char *active) {
  ...
  size_t n =
      admin ? sizeof(ADMIN) / sizeof(ADMIN[0]) : sizeof(APP) / sizeof(APP[0]);

  buf_puts(b, "<aside class=\"sidebar\">");
  if (admin)
    buf_puts(b, "<div class=\"sidebar-title\">Admin area</div>");
  buf_puts(b, "<nav class=\"sidebar-nav\">");
  for (size_t i = 0; i < n; ++i) {
    int on = active && strcmp(items[i].key, active) == 0;
    buf_printf(b, "<a%s href=\"%s\">%s</a>", on ? " class=\"active\"" : "",
               items[i].href, items[i].label);
  }
  buf_puts(b, "</nav>");
  if (admin)
...
static void render_shell_open(struct buf *b, const char *title,
                              const char *uname, const char *active_nav,
                              int is_admin) {
  buf_puts(
      b,
...
static void render_shell_close(struct loop_stream *s, struct buf *b,
                               int keep_alive) {
  buf_puts(b,
           "</main></div>"
           ...
           "</body></html>");
  send_response(s, 200, "text/html; charset=utf-8", b->data ? b->data : "",
                b->len, NULL, keep_alive);
  buf_free(b);
}
...
static void send_forbidden(struct loop_stream *s, const char *uname,
                           int keep_alive) {
  struct buf b;
  buf_init(&b);
  ...
  buf_puts(&b,
           "<p class=\"muted\">This area is restricted to site administrators. "
...
static int require_admin(struct loop_stream *s, const struct claims *claims,
                         int keep_alive) {
  if (!claims->uid) {
  ...
  if (!claims->is_admin) {
    send_forbidden(s, claims->username[0] ? claims->username : NULL,
                   keep_alive);
    return 0;
  }
  return 1;
...
static void render_project_header(struct buf *b, const char *acct,
  ...
  buf_puts(b, "/");
  buf_esc(b, repo);
  buf_puts(b, "/-/runs/new\"><button class=\"btn\" type=\"submit\">Run "
              "pipeline</button>"
              "</form></div></header>");
}
...
static void render_project_tabs(struct buf *b, const char *acct,
                                const char *repo, const char *active,
                                long issue_count, long run_count) {
  struct tab {
    const char *label;
    const char *suffix;
    const char *key;
  ...
  for (size_t i = 0; i < sizeof(TABS) / sizeof(TABS[0]); ++i) {
    buf_puts(b, strcmp(TABS[i].key, active) == 0
                    ? "<a class=\"active\" href=\"/"
                    : "<a href=\"/");
    buf_esc(b, acct);
    buf_puts(b, "/");
    ...
    buf_puts(b, "\">");
    buf_puts(b, TABS[i].label);
    if (TABS[i].count >= 0)
      buf_printf(b, " <span class=\"count\">%ld</span>", TABS[i].count);
    buf_puts(b, "</a>");
  }
  buf_puts(b, "</nav>");
}
...
static struct picomesh_void_result
repo_tab_counts(struct loop *loop, const struct serve_ud *sud, const char *sid,
                uint32_t rid, long *issues_out, long *runs_out) {
  char args[48];
  snprintf(args, sizeof(args), "[%u]", rid);
  if (service_active(sud, "issues")) {
    struct picomesh_int64_result issues_r = rpc_result_int(
        loop, sud, sid, "issues.issues.count_open_in_repo", args, -1);
    PICOMESH_RETURN_IF_ERR(picomesh_void, issues_r,
                           "repo_tab_counts: open-issue count");
    *issues_out = issues_r.value;
  } else {
    *issues_out = -1;
  }
  if (service_active(sud, "git_pipeline")) {
    struct picomesh_int64_result pending = rpc_result_int(
        loop, sud, sid, "git_pipeline.git_pipeline.count_pending", "[]", 0);
    PICOMESH_RETURN_IF_ERR(picomesh_void, pending,
                           "repo_tab_counts: pending count");
    struct picomesh_int64_result running = rpc_result_int(
        loop, sud, sid, "git_pipeline.git_pipeline.count_running", "[]", 0);
    PICOMESH_RETURN_IF_ERR(picomesh_void, running,
                           "repo_tab_counts: running count");
...
static struct json_doc_ptr_result
whoami_doc(struct loop *loop, const struct serve_ud *sud, const char *sid);
static int role_at_least(const char *role, const char *floor);
static int json_escape(char *dst, size_t cap, const char *src);
static struct json_doc_ptr_result
rpc_result_doc(struct loop *loop, const struct serve_ud *sud, const char *sid,
               const char *rpc_path, const char *args_json,
               const struct json_value **result_out);

/* True iff `full` already appears as a "\n<full>\n" token in the
 * newline-wrapped `seen` buffer — the dedup test for the /repos discovery
 * merge. */
 ...
 * (accounts.ns_subtree — the role inherits down the whole subtree) and list
 * each namespace once (git_repo.list_for_namespace), deduplicating namespaces
 * and repos. The creator index (list_for_owner) is intentionally NOT consulted
 * — it misses RBAC-visible group/subgroup repos AND leaks repos the caller
 * created but has since lost access to. Returns a heap, newline-separated,
...
static struct picomesh_string_result
collect_accessible_repos(struct loop *loop, const struct serve_ud *sud,
                         const char *sid) {
  struct json_doc_ptr_result who_r = whoami_doc(loop, sud, sid);
  if (PICOMESH_IS_ERR(who_r)) {
    picomesh_error_print(stderr, "webapp: collect_accessible_repos whoami",
                         who_r.error);
    picomesh_error_destroy(who_r.error);
  }
  struct json_doc *who = PICOMESH_IS_OK(who_r) ? who_r.value : NULL;
  const struct json_value *nss =
      who ? json_object_get(json_doc_root(who), "namespaces") : NULL;
  ...
  for (size_t i = 0; i < ns_count; ++i) {
    ...
    if (!root || !*root || !role || !role_at_least(role, "reporter"))
      continue;

    char esc[256], sargs[300];
    if (!json_escape(esc, sizeof(esc), root))
      continue;
    snprintf(sargs, sizeof(sargs), "[\"%s\"]", esc);
    const struct json_value *paths = NULL;
    struct json_doc_ptr_result sub_r = rpc_result_doc(
        loop, sud, sid, "accounts.accounts.ns_subtree", sargs, &paths);
    if (PICOMESH_IS_ERR(sub_r)) {
      picomesh_error_print(stderr, "webapp: ns_subtree", sub_r.error);
      picomesh_error_destroy(sub_r.error);
    }
    struct json_doc *sub = PICOMESH_IS_OK(sub_r) ? sub_r.value : NULL;
    size_t path_count =
        (sub && paths && json_is_array(paths)) ? json_array_size(paths) : 0;
    for (size_t j = 0; j < path_count; ++j) {
      const char *nspath = json_as_string(
          json_object_get(json_array_at(paths, j), "path"), NULL);
      if (!nspath || !*nspath)
        continue;
      if (repo_seen(ns_seen.data, nspath))
        continue; /* already listed via another membership */
      buf_puts(&ns_seen, "\n");
      buf_puts(&ns_seen, nspath);
      buf_puts(&ns_seen, "\n");

      char nesc[256], nargs[300];
      if (!json_escape(nesc, sizeof(nesc), nspath))
        continue;
      snprintf(nargs, sizeof(nargs), "[\"%s\"]", nesc);
      struct picomesh_string_result names_r = rpc_result_str(
          loop, sud, sid, "git_repo.git_repo.list_for_namespace", nargs, NULL);
      if (PICOMESH_IS_ERR(names_r)) {
        picomesh_error_print(stderr, "webapp: list_for_namespace",
                             names_r.error);
        picomesh_error_destroy(names_r.error);
      }
  ...
  return PICOMESH_OK(
      picomesh_string,
      out.data); /* ownership transferred to caller (NULL when empty) */
}
...
static struct picomesh_void_result
route_repos_get(struct loop *loop, struct loop_stream *s,
                const struct serve_ud *sud, const char *sid, const char *uname,
                uint32_t owner_uid, int is_admin, int keep_alive) {
  if (!sid || !*sid || !owner_uid) {
    send_redirect(s, "/-/login", NULL, keep_alive);
    return PICOMESH_OK_VOID();
  }

  struct picomesh_int64_result total_r =
      rpc_result_int(loop, sud, sid, "git_repo.git_repo.count_total", "[]", -1);
  if (PICOMESH_IS_ERR(total_r)) {
    picomesh_error_print(stderr, "webapp: count_total", total_r.error);
    picomesh_error_destroy(total_r.error);
  }
  ...
  struct picomesh_string_result repos_r =
      collect_accessible_repos(loop, sud, sid);
  if (PICOMESH_IS_ERR(repos_r)) {
    picomesh_error_print(stderr, "webapp: collect repos", repos_r.error);
    picomesh_error_destroy(repos_r.error);
  }
  ...
  render_page_header(
      &b, "Repositories", "Repositories you can access across the mesh.",
      "<a class=\"btn primary\" href=\"/-/repos/new\">New repository</a>");

  char meta[64];
  if (total >= 0)
    snprintf(meta, sizeof(meta), "%ld total in the mesh", total);
  else
    meta[0] = 0;
  ...
  if (repos && *repos) {
    ...
    for (char *full = strtok_r(repos, "\n", &save); full;
      ...
      if (!*full)
        continue;
      any = 1;
      buf_puts(&b, "<tr><td class=\"ic\">\xf0\x9f\x93\x81</td><td><a "
                   "class=\"file-name dir\" href=\"/");
      buf_esc(&b, full);
      buf_puts(&b, "/-/tree\">");
      buf_esc(&b, full);
      buf_puts(&b, "</a></td></tr>");
...
rpc_result_str(struct loop *loop, const struct serve_ud *sud, const char *sid,
  ...
  int n = snprintf(body, need, "{\"path\":\"%s\",\"args\":%s}", rpc_path,
                   args_json);
  ...
  struct http_response resp;
  struct picomesh_int_result post = http_post_json(
      loop, &sud->gw, "/_rpc", NULL, sid, body, (size_t)n, &resp);
  free(body);
  if (PICOMESH_IS_ERR(post)) {
    http_response_free(&resp);
    if (was_error)
      *was_error = 1;
    return PICOMESH_ERR(picomesh_string, "rpc_result_str: gateway POST failed",
                        post);
  }
...
rpc_result_doc(struct loop *loop, const struct serve_ud *sud, const char *sid,
  ...
  char *body = malloc(need);
  if (!body)
    return PICOMESH_OK(json_doc_ptr, NULL);
  int n = snprintf(body, need, "{\"path\":\"%s\",\"args\":%s}", rpc_path,
                   args_json);
  if (n <= 0 || (size_t)n >= need) {
    free(body);
  ...
  if (PICOMESH_IS_ERR(post)) {
    http_response_free(&resp);
    return PICOMESH_ERR(json_doc_ptr, "rpc_result_doc: gateway POST failed",
                        post);
  }
...
static int split_namespace_repo(const char *resource, size_t rlen, char *acct,
                                size_t acct_cap, char *repo, size_t repo_cap) {
  char tmp[1024];
  if (rlen == 0 || rlen >= sizeof(tmp))
    return 0;
  memcpy(tmp, resource, rlen);
  tmp[rlen] = 0;
  char *segs[16] = {0};
  int ns = 0;
  ...
  for (int i = 0; i < ns - 1; ++i)
    al += (size_t)snprintf(acct + al, al < acct_cap ? acct_cap - al : 0, "%s%s",
                           i ? "/" : "", segs[i]);
  if (al >= acct_cap)
    return 0;
...
static int parse_repo_path(const char *path, size_t path_len, char *acct,
                           size_t acct_cap, char *repo, size_t repo_cap,
  ...
  if (d <= 0)
    return 0; /* 0 = top-level command, -1 = bare namespace path */
  if (!split_namespace_repo(path, (size_t)d, acct, acct_cap, repo, repo_cap))
    return 0;
  ...
  size_t vlen = 0;
  while (vlen < tlen && tail[vlen] != '/')
    ++vlen;
  if (vlen == 0 || vlen >= verb_cap)
    return 0;
  memcpy(verb, tail, vlen);
  verb[vlen] = 0;
  return 1;
}
...
static struct picomesh_void_result
route_repo_browse(struct loop *loop, struct loop_stream *s,
  ...
  char dir_esc[1024];
  if (!json_escape(dir_esc, sizeof(dir_esc), dirv)) {
    free(dir);
    send_text(s, 400, "bad dir\n", keep_alive);
    return PICOMESH_OK_VOID();
  }
  char args[1200];
  snprintf(args, sizeof(args), "[%u,\"\",\"%s\"]", rid, dir_esc);
  int err = 0;
  struct picomesh_string_result tree_r =
      rpc_result_str(loop, sud, sid, "git_repo.git_repo.read_tree", args, &err);
  if (PICOMESH_IS_ERR(tree_r)) {
    picomesh_error_print(stderr, "webapp: read_tree", tree_r.error);
    picomesh_error_destroy(tree_r.error);
  }
  char *tree = PICOMESH_IS_OK(tree_r) ? tree_r.value : NULL;

  long issue_count = -1, run_count = -1;
  struct picomesh_void_result counts_r =
      repo_tab_counts(loop, sud, sid, rid, &issue_count, &run_count);
  if (PICOMESH_IS_ERR(counts_r)) {
    picomesh_error_print(stderr, "webapp: repo_tab_counts", counts_r.error);
    picomesh_error_destroy(counts_r.error);
  }
  ...
  buf_puts(&b, "<section class=\"panel repo-browser\"><header "
  ...
  if (*dirv) {
    ...
    buf_puts(&b, "<span class=\"branch\">main</span>");
  }
  buf_puts(&b, "</div><div class=\"panel-actions\">");
  {
    char href[1200];
    if (*dirv)
      snprintf(href, sizeof(href), "/%s/%s/-/new?dir=%s", acct, repo, dirv);
    else
      snprintf(href, sizeof(href), "/%s/%s/-/new", acct, repo);
  ...
  if (!tree || !*tree) {
    ...
    if (tree == NULL && err)
      ...
      buf_puts(&b,
               "<p class=\"muted\">Empty repository. Use "
               "\xe2\x80\x9cNew file\xe2\x80\x9d to add the first file.</p>");
    buf_puts(&b, "</div>");
  } else {
    buf_puts(&b, "<table class=\"file-table\"><thead><tr>"
                 "<th></th><th>Name</th><th>Last "
                 "commit</th><th>Updated</th></tr></thead><tbody>");
    if (*dirv) {
      char up[1024];
      snprintf(up, sizeof(up), "%s", dirv);
      ...
      if (up[0])
        buf_printf(&b,
                   "<tr><td class=\"ic\">\xf0\x9f\x93\x81</td>"
                   "<td><a class=\"file-name dir\" "
                   "href=\"/%s/%s/-/tree?dir=%s\">..</a></td>"
    ...
    for (char *line = strtok_r(tree, "\n", &save); line;
      ...
      *tab = 0;
      ...
      char child[1024];
      if (*dirv)
        snprintf(child, sizeof(child), "%s/%s", dirv, entry_name);
      else
        snprintf(child, sizeof(child), "%s", entry_name);
      ...
      if (is_dir) {
        ...
        buf_puts(&b, "/-/edit?path=");
        buf_esc(&b, child);
      }
      buf_puts(&b, "\">");
      buf_esc(&b, entry_name);
      buf_puts(
          &b,
          "</a></td><td class=\"muted\">-</td><td class=\"muted\">-</td></tr>");
    }
    buf_puts(&b, "</tbody></table>");
  }
  buf_puts(&b, "</section>");
...
static void render_editor_page(struct loop_stream *s, const char *uname,
                               ...
                               int is_new, int is_admin, int keep_alive) {
  struct buf b;
  buf_init(&b);
  char title[200];
  snprintf(title, sizeof(title), "%s %s/%s", is_new ? "New file" : "Edit", acct,
           repo);
  /* render_shell_open emits the topbar + sidebar + <main>; then the
   * Monaco stylesheet (vendored under /static — no CDN, the in-browser
   * VM is offline). */
  render_shell_open(&b, title, uname, "projects", is_admin);
  buf_puts(&b, "<link rel=\"stylesheet\" "
               "href=\"/static/vendor/monaco/vs/editor/editor.main.css\">");

  /* Editor header: breadcrumb to the file + a Cancel back to the repo. */
  buf_puts(&b, "<header class=\"editor-header\"><div><div "
  ...
  buf_puts(&b, "/");
  buf_esc(&b, repo);
  buf_puts(&b, "/-/tree\">Cancel</a></header>");

  buf_puts(&b,
           "<form id=\"f\" class=\"editor-form\" method=\"post\" action=\"/");
  buf_esc(&b, acct);
  buf_puts(&b, "/");
  buf_esc(&b, repo);
  buf_puts(&b, "/-/edit\">");
  ...
  buf_puts(&b, "<div class=\"editor-toolbar\">"
               "<input class=\"path\" name=\"path\" required "
               "placeholder=\"path/to/file.ext\" value=\"");
  buf_esc(&b, path ? path : "");
  buf_puts(&b, "\"");
  if (!is_new)
    buf_puts(&b, " readonly");
  buf_puts(&b, "><input class=\"message\" name=\"message\" "
               "placeholder=\"Commit message\" value=\"");
  buf_esc(&b, is_new ? "create file" : "update file");
  buf_puts(&b, "\"><button type=\"submit\" class=\"primary\">Commit "
               "changes</button></div>");
  ...
  buf_puts(&b,
           ...
           "function uriFor(p){try{return "
           "monaco.Uri.file(p||'untitled.txt');}catch(e){return undefined;}}"
           "var model=monaco.editor.createModel(ta.value, undefined, "
           "uriFor(pathField.value));"
           ...
           "return (l.extensions||[]).some(function(e){return "
           "nm.endsWith(e);});})||{}).id;"
...
static struct picomesh_void_result route_repo_edit_get(
    struct loop *loop, struct loop_stream *s, const struct serve_ud *sud,
    const char *sid, const char *uname, const char *acct, const char *repo,
    const char *verb, const char *full_path, int is_admin, int keep_alive) {
  if (!sid || !*sid) {
    send_redirect(s, "/-/login", NULL, keep_alive);
    return PICOMESH_OK_VOID();
  }
  ...
  if (strcmp(verb, "new") == 0) {
    ...
    char seed[1024] = {0};
    if (dir && *dir)
      snprintf(seed, sizeof(seed), "%s/", dir);
    render_editor_page(s, uname, acct, repo, seed, "", /*is_new=*/1, is_admin,
                       keep_alive);
    free(dir);
    return PICOMESH_OK_VOID();
  }
  ...
  if (!path || !*path) {
    free(path);
    render_editor_page(s, uname, acct, repo, "", "", /*is_new=*/1, is_admin,
                       keep_alive);
    return PICOMESH_OK_VOID();
  }
  char path_esc[1024];
  if (!json_escape(path_esc, sizeof(path_esc), path)) {
    free(path);
    send_text(s, 400, "bad path\n", keep_alive);
    return PICOMESH_OK_VOID();
  }
  char args[1200];
  snprintf(args, sizeof(args), "[%u,\"\",\"%s\"]", rid, path_esc);
  int err = 0;
  struct picomesh_string_result content_r =
      rpc_result_str(loop, sud, sid, "git_repo.git_repo.read_file", args, &err);
  if (PICOMESH_IS_ERR(content_r)) {
    picomesh_error_print(stderr, "webapp: read_file", content_r.error);
    picomesh_error_destroy(content_r.error);
  }
...
static struct picomesh_void_result
route_repo_edit_post(struct loop *loop, struct loop_stream *s,
                     const struct serve_ud *sud, const char *sid,
                     const char *acct, const char *repo, const char *body,
                     size_t body_len, int keep_alive) {
  if (!sid || !*sid) {
    send_redirect(s, "/-/login", NULL, keep_alive);
    return PICOMESH_OK_VOID();
  ...
  int ok = pe && ce && me && args && json_escape(pe, 2048, path) &&
           json_escape(ce, CAP, content ? content : "") &&
           json_escape(me, 2048, message ? message : "");
  if (ok) {
    snprintf(args, (size_t)CAP + 4096, "[%u,\"%s\",\"%s\",\"%s\",\"\",\"\"]",
             rid, pe, ce, me);
    int err = 0;
    struct picomesh_string_result oid_r = rpc_result_str(
        loop, sud, sid, "git_repo.git_repo.put_file", args, &err);
    if (PICOMESH_IS_ERR(oid_r)) {
      picomesh_error_print(stderr, "webapp: put_file", oid_r.error);
      picomesh_error_destroy(oid_r.error);
    }
  ...
  if (slash) {
    *slash = 0;
    snprintf(where, sizeof(where), "/%s/%s/-/tree?dir=%s", acct, repo, dir);
  } else
    snprintf(where, sizeof(where), "/%s/%s/-/tree", acct, repo);
  free(path);
  free(content);
  free(message);
...
rpc_result_int(struct loop *loop, const struct serve_ud *sud, const char *sid,
  ...
  if (PICOMESH_IS_ERR(post)) {
    http_response_free(&resp);
    return PICOMESH_ERR(picomesh_int64, "rpc_result_int: gateway POST failed",
                        post);
  }
...
static struct picomesh_void_result resolve_claims(struct loop *loop,
  ...
  struct http_response resp;
  struct picomesh_int_result post = http_post(
      loop, &sud->gw, "/_whoami", "application/json", NULL, sid, "", 0, &resp);
  if (PICOMESH_IS_ERR(post)) {
    http_response_free(&resp);
    return PICOMESH_ERR(picomesh_void, "resolve_claims: /_whoami POST failed",
                        post);
  }
  if (resp.body) {
    struct json_doc *doc = json_parse(resp.body, resp.body_len);
...
whoami_doc(struct loop *loop, const struct serve_ud *sud, const char *sid) {
  ...
  if (PICOMESH_IS_ERR(post)) {
    http_response_free(&resp);
    return PICOMESH_ERR(json_doc_ptr, "whoami_doc: /_whoami POST failed", post);
  }
  struct json_doc *doc =
      resp.body ? json_parse(resp.body, resp.body_len) : NULL;
  http_response_free(&resp);
  return PICOMESH_OK(json_doc_ptr, doc);
}
...
rpc_invoke(struct loop *loop, const struct serve_ud *sud, const char *sid,
           const char *rpc_path, const char *args_json, char *errbuf,
           size_t errcap) {
  if (errbuf && errcap)
    snprintf(errbuf, errcap, "the action could not be completed");
  size_t need = strlen(rpc_path) + strlen(args_json) + 32;
  char *body = malloc(need);
  if (!body)
    return PICOMESH_ERR(picomesh_int, "rpc_invoke: out of memory");
  int n = snprintf(body, need, "{\"path\":\"%s\",\"args\":%s}", rpc_path,
                   args_json);
  if (n <= 0 || (size_t)n >= need) {
    free(body);
    return PICOMESH_ERR(picomesh_int, "rpc_invoke: request body format failed");
  }
  struct http_response resp;
  struct picomesh_int_result post = http_post_json(
      loop, &sud->gw, "/_rpc", NULL, sid, body, (size_t)n, &resp);
  ...
  if (PICOMESH_IS_ERR(post)) {
    http_response_free(&resp);
    return PICOMESH_ERR(picomesh_int, "rpc_invoke: gateway POST failed", post);
  }
  int ok = 0;
  if (resp.body) {
    struct json_doc *doc = json_parse(resp.body, resp.body_len);
...
static void render_action_error(struct loop_stream *s, const char *uname,
                                int is_admin, const char *msg,
                                const char *back_url, int keep_alive) {
  struct buf b;
  buf_init(&b);
  render_shell_open(&b, "Action failed", uname,
                    is_admin ? "admin-namespaces" : "projects", is_admin);
  render_page_header(&b, "Action failed",
                     "The requested change was not applied.", NULL);
...
static int role_at_least(const char *role, const char *floor) {
  static const char *ladder[] = {"guest", "reporter", "developer", "maintainer",
                                 "owner"};
  int role_index = -1, floor_index = -1;
  for (int i = 0; i < 5; ++i) {
    if (role && strcmp(role, ladder[i]) == 0)
      role_index = i;
    if (floor && strcmp(floor, ladder[i]) == 0)
      floor_index = i;
  }
...
static struct picomesh_void_result services_ensure(struct loop *loop,
                                                   struct serve_ud *sud) {
  ...
  struct http_response resp;
  struct picomesh_int_result post =
      http_post(loop, &sud->gw, "/_describe", "application/json", NULL, NULL,
                "", 0, &resp);
  ...
  if (resp.body) {
    ...
    if (doc) {
      ...
      sud->services.n = 0;
      for (size_t i = 0;
           i < cnt && sud->services.n < sizeof(sud->services.names) /
                                            sizeof(sud->services.names[0]);
           ++i) {
        const struct json_value *svc_entry = json_array_at(svcs, i);
        const char *svc_name =
            json_as_string(json_object_get(svc_entry, "service"), NULL);
        const char *svc_source =
            json_as_string(json_object_get(svc_entry, "source"), NULL);
        if (svc_name && *svc_name) {
          size_t slot = sud->services.n++;
          snprintf(sud->services.names[slot], 64, "%s", svc_name);
          snprintf(sud->services.sources[slot], 32, "%s",
                   svc_source && *svc_source ? svc_source : "-");
        }
      }
...
static int service_active(const struct serve_ud *sud, const char *name) {
  for (size_t i = 0; i < sud->services.n; ++i)
    if (strcmp(sud->services.names[i], name) == 0)
      return 1;
  return 0;
}
...
static void page_close_and_send(struct loop_stream *s, struct buf *b,
                                int keep_alive) {
  render_shell_close(s, b, keep_alive);
}
...
static struct picomesh_void_result
page_account_landing(struct loop *loop, struct loop_stream *s,
                     const struct serve_ud *sud, const char *sid,
  ...
  if (!json_escape(acct_esc, sizeof(acct_esc), acct))
    acct_esc[0] = 0;
  snprintf(args, sizeof(args), "[\"%s\"]", acct_esc);
  struct picomesh_int64_result repos_r = rpc_result_int(
      loop, sud, sid, "git_repo.git_repo.count_for_namespace", args, -1);
  if (PICOMESH_IS_ERR(repos_r)) {
    picomesh_error_print(stderr, "webapp: count_for_namespace", repos_r.error);
    picomesh_error_destroy(repos_r.error);
  }
  long repos = PICOMESH_IS_OK(repos_r) ? repos_r.value : -1;
  struct picomesh_string_result names_r = rpc_result_str(
      loop, sud, sid, "git_repo.git_repo.list_for_namespace", args, NULL);
  if (PICOMESH_IS_ERR(names_r)) {
    picomesh_error_print(stderr, "webapp: list_for_namespace", names_r.error);
    picomesh_error_destroy(names_r.error);
  }
  char *names = PICOMESH_IS_OK(names_r) ? names_r.value : NULL;
  ...
  char meta[48];
  if (repos >= 0)
    snprintf(meta, sizeof(meta), "%ld owned", repos);
  else
    meta[0] = 0;
  ...
  if (!any)
    buf_puts(&b, "<p class=\"muted\">No repositories.</p>");
  panel_close(&b);
  free(names);
 ...
 * the repo, plus the new/close action forms. Data:
 * issues.issues.count_open_in_repo. */
static struct picomesh_void_result
page_repo_issues(struct loop *loop, struct loop_stream *s,
                 const struct serve_ud *sud, const char *sid, const char *uname,
                 const char *acct, const char *repo, const char *full_path,
                 int is_admin, int keep_alive) {
  (void)full_path;
  uint32_t rid = repo_hash(acct, repo);
  char args[48];
  snprintf(args, sizeof(args), "[%u]", rid);
  struct picomesh_int64_result open_n_rr = rpc_result_int(
      loop, sud, sid, "issues.issues.count_open_in_repo", args, -1);
  if (PICOMESH_IS_ERR(open_n_rr)) {
    picomesh_error_print(stderr, "webapp: rpc_result_int", open_n_rr.error);
    picomesh_error_destroy(open_n_rr.error);
  }
  long open_n = PICOMESH_IS_OK(open_n_rr) ? open_n_rr.value : (-1);

  long run_count = -1;
  if (service_active(sud, "git_pipeline")) {
    struct picomesh_int64_result pending_rr = rpc_result_int(
        loop, sud, sid, "git_pipeline.git_pipeline.count_pending", "[]", 0);
    if (PICOMESH_IS_ERR(pending_rr)) {
      picomesh_error_print(stderr, "webapp: rpc_result_int", pending_rr.error);
      picomesh_error_destroy(pending_rr.error);
    }
    long pending = PICOMESH_IS_OK(pending_rr) ? pending_rr.value : (0);
    struct picomesh_int64_result running_rr = rpc_result_int(
        loop, sud, sid, "git_pipeline.git_pipeline.count_running", "[]", 0);
    if (PICOMESH_IS_ERR(running_rr)) {
      picomesh_error_print(stderr, "webapp: rpc_result_int", running_rr.error);
      picomesh_error_destroy(running_rr.error);
    }
  ...
  buf_puts(&b, "<section class=\"panel\"><header "
               "class=\"panel-header\"><div><strong>Issues</strong> ");
  if (open_n >= 0)
    buf_printf(&b, "<span class=\"muted\">%ld open</span>", open_n);
  else
    buf_puts(&b, "<span class=\"muted\">service unreachable</span>");
  buf_puts(
      &b,
  ...
  buf_puts(&b, "/");
  buf_esc(&b, repo);
  buf_puts(&b, "/-/issues/new\"><button class=\"primary\" type=\"submit\">New "
               "issue</button></form></div></header>");

  buf_puts(&b, "<div class=\"filters\"><a class=\"active\" "
  ...
  if (open_n > 0) {
    buf_printf(&b,
               "<li class=\"issue-row\"><div class=\"issue-main\">"
               "<span class=\"issue-title\">%ld open issue%s</span>"
               "<div class=\"issue-meta\">per-issue rows arrive with the list "
               "API</div>"
  ...
  buf_puts(&b, "<div class=\"panel-body\"><form class=\"inline-form\" "
               "method=\"post\" action=\"/");
  buf_esc(&b, acct);
  buf_puts(&b, "/");
  buf_esc(&b, repo);
  buf_puts(&b, "/-/issues/close\"><label>Close issue <input type=\"number\" "
               "name=\"issue_id\" required></label>"
 ...
 * Data: git_pipeline.git_pipeline.count_pending/running/done (global counts
 * today). */
static struct picomesh_void_result
page_repo_runs(struct loop *loop, struct loop_stream *s,
               const struct serve_ud *sud, const char *sid, const char *uname,
               const char *acct, const char *repo, const char *full_path,
               int is_admin, int keep_alive) {
  (void)full_path;
  ...
  long running = PICOMESH_IS_OK(running_rr) ? running_rr.value : (0);
  struct picomesh_int64_result done_rr = rpc_result_int(
      loop, sud, sid, "git_pipeline.git_pipeline.count_done", "[]", 0);
  if (PICOMESH_IS_ERR(done_rr)) {
    picomesh_error_print(stderr, "webapp: rpc_result_int", done_rr.error);
    picomesh_error_destroy(done_rr.error);
  }
  ...
  if (service_active(sud, "issues")) {
    char issue_args[48];
    snprintf(issue_args, sizeof(issue_args), "[%u]", rid);
    struct picomesh_int64_result issue_rr = rpc_result_int(
        loop, sud, sid, "issues.issues.count_open_in_repo", issue_args, -1);
    if (PICOMESH_IS_ERR(issue_rr)) {
      picomesh_error_print(stderr, "webapp: count_open_in_repo",
                           issue_rr.error);
      picomesh_error_destroy(issue_rr.error);
    } else
      issue_count = issue_rr.value;
  }
  ...
  buf_puts(
      &b,
      "<section class=\"panel\"><header class=\"panel-header\"><div>"
      "<strong>Pipeline runs</strong> <span class=\"muted\">queued, running, "
      "finished</span>"
      "</div><div class=\"panel-actions\"><form method=\"post\" action=\"/");
  buf_esc(&b, acct);
  buf_puts(&b, "/");
  buf_esc(&b, repo);
  buf_puts(&b, "/-/runs/new\"><button class=\"primary\" type=\"submit\">Run "
               "pipeline</button></form></div></header>");
  buf_printf(
      &b,
      "<table class=\"pipeline-table\"><thead><tr>"
      "<th>Status</th><th>Run</th><th>Ref</th><th>Runner</th><th>Count</th></"
      "tr></thead><tbody>"
      "<tr><td><span class=\"badge queued\">queued</span></td><td "
      "class=\"muted\">-</td>"
      "<td><code>main</code></td><td class=\"muted\">-</td><td>%ld</td></tr>"
      "<tr><td><span class=\"badge running\">running</span></td><td "
      "class=\"muted\">-</td>"
      "<td><code>main</code></td><td class=\"muted\">-</td><td>%ld</td></tr>"
      "<tr><td><span class=\"badge succeeded\">finished</span></td><td "
      "class=\"muted\">-</td>"
      "<td><code>main</code></td><td class=\"muted\">-</td><td>%ld</td></tr>"
      "</tbody></table>",
...
static void stat_tile(struct buf *b, long value, const char *label) {
  ...
  if (value >= 0)
    buf_printf(b, "%ld", value);
  else
    buf_puts(b, "\xe2\x80\x94");
...
static struct picomesh_void_result
page_admin_overview(struct loop *loop, struct loop_stream *s,
                    const struct serve_ud *sud, const char *sid,
                    const char *uname, int keep_alive) {
  long users = -1;
  if (service_active(sud, "accounts")) {
    struct picomesh_int64_result users_rr =
        rpc_result_int(loop, sud, sid, "accounts.accounts.count", "[]", -1);
    if (PICOMESH_IS_ERR(users_rr)) {
      picomesh_error_print(stderr, "webapp: accounts.count", users_rr.error);
      picomesh_error_destroy(users_rr.error);
    } else
      users = users_rr.value;
  }
  long repos = -1;
  if (service_active(sud, "git_repo")) {
    struct picomesh_int64_result repos_rr = rpc_result_int(
        loop, sud, sid, "git_repo.git_repo.count_total", "[]", -1);
    if (PICOMESH_IS_ERR(repos_rr)) {
      picomesh_error_print(stderr, "webapp: count_total", repos_rr.error);
      picomesh_error_destroy(repos_rr.error);
    } else
      repos = repos_rr.value;
  }
  long token_count = -1;
  if (service_active(sud, "personal_access_tokens")) {
    struct picomesh_int64_result tok_rr = rpc_result_int(
        loop, sud, sid,
        "personal_access_tokens.personal_access_tokens.count_active", "[]", -1);
    if (PICOMESH_IS_ERR(tok_rr)) {
      picomesh_error_print(stderr, "webapp: pat.count_active", tok_rr.error);
      picomesh_error_destroy(tok_rr.error);
    } else
      token_count = tok_rr.value;
  }
  long runs = -1;
  if (service_active(sud, "git_pipeline")) {
    struct picomesh_int64_result pending_rr = rpc_result_int(
  ...
  render_page_header(
      &b, "Admin", "Operational overview of this picoforge deployment.", NULL);
  ...
  buf_puts(
      ...
      "<tr><td><a class=\"file-name\" "
      "href=\"/-/admin/repos\">Repositories</a></td>"
      ...
      "<tr><td><a class=\"file-name\" "
      "href=\"/-/admin/services\">Services</a></td>"
...
static struct picomesh_void_result
page_admin_users(struct loop *loop, struct loop_stream *s,
                 const struct serve_ud *sud, const char *sid, const char *uname,
                 int keep_alive) {
  struct picomesh_int64_result users_rr =
      rpc_result_int(loop, sud, sid, "accounts.accounts.count", "[]", -1);
  if (PICOMESH_IS_ERR(users_rr)) {
    picomesh_error_print(stderr, "webapp: rpc_result_int", users_rr.error);
    picomesh_error_destroy(users_rr.error);
  }
  ...
  struct json_doc_ptr_result roster_doc_rr =
      rpc_result_doc(loop, sud, sid, "accounts.accounts.list", "[]", &roster);
  if (PICOMESH_IS_ERR(roster_doc_rr)) {
    picomesh_error_print(stderr, "webapp: rpc_result_doc", roster_doc_rr.error);
    picomesh_error_destroy(roster_doc_rr.error);
  }
  struct json_doc *roster_doc =
      PICOMESH_IS_OK(roster_doc_rr) ? roster_doc_rr.value : NULL;

  struct buf b;
  buf_init(&b);
  page_open(&b, "Users", uname, "admin-users", /*is_admin=*/1);
  render_page_header(&b, "Users", "Registered accounts on this deployment.",
                     NULL);
  buf_puts(&b, "<section class=\"stats-grid\">");
  stat_tile(&b, users, "users");
  buf_puts(&b, "</section>");
  ...
  if (users < 0) {
    ...
    for (size_t i = 0; i < roster_n; ++i) {
      ...
      const char *name =
          json_as_string(json_object_get(roster_entry, "username"), "");
      char uid_buf[24];
    ...
    if (!any)
      buf_puts(&b, "<tr><td colspan=\"2\" class=\"muted\">No users registered "
                   "yet.</td></tr>");
    buf_puts(&b, "</tbody></table>");
  }
  buf_puts(
      &b,
      "<p class=\"muted small\">Users are created by signing up at "
...
static struct picomesh_void_result
page_admin_repos(struct loop *loop, struct loop_stream *s,
                 const struct serve_ud *sud, const char *sid, const char *uname,
                 int keep_alive) {
  struct picomesh_int64_result total_rr =
      rpc_result_int(loop, sud, sid, "git_repo.git_repo.count_total", "[]", -1);
  if (PICOMESH_IS_ERR(total_rr)) {
    picomesh_error_print(stderr, "webapp: rpc_result_int", total_rr.error);
    picomesh_error_destroy(total_rr.error);
  }
  ...
  render_page_header(&b, "Repositories", "All repositories across the mesh.",
                     NULL);
  buf_puts(&b, "<section class=\"stats-grid\">");
  ...
  buf_puts(
      &b,
      "<p class=\"muted\">A mesh-wide repository listing arrives with the "
...
static struct picomesh_void_result
page_admin_tokens(struct loop *loop, struct loop_stream *s,
                  const struct serve_ud *sud, const char *sid,
                  const char *uname, int keep_alive) {
  ...
  render_page_header(&b, "Tokens",
                     "Personal access tokens issued by the gateway.", NULL);
  buf_puts(&b, "<section class=\"stats-grid\">");
  stat_tile(&b, token_count, "active tokens");
  buf_puts(&b, "</section>");
  panel_open(&b, "Mint personal access token", NULL);
  if (!active)
    buf_puts(&b, "<p class=\"muted\">PAT service not active.</p>");
  buf_puts(
      &b,
      "<form class=\"form-grid\" method=\"post\" "
      "action=\"/-/admin/tokens/mint_pat\">"
...
static struct picomesh_void_result
page_admin_namespaces(struct loop *loop, struct loop_stream *s,
                      const struct serve_ud *sud, const char *sid,
                      const char *uname, int keep_alive) {
  const struct json_value *list = NULL;
  struct json_doc_ptr_result doc_rr =
      rpc_result_doc(loop, sud, sid, "accounts.accounts.ns_list", "[]", &list);
  if (PICOMESH_IS_ERR(doc_rr)) {
    picomesh_error_print(stderr, "webapp: rpc_result_doc", doc_rr.error);
    picomesh_error_destroy(doc_rr.error);
  }
  ...
  render_page_header(
      &b, "Namespaces",
      "RBAC namespaces — personal and group — and their role memberships.",
      NULL);
  buf_puts(&b, "<section class=\"stats-grid\">");
  ...
  if (!doc) {
    ...
    buf_puts(&b,
             "<table class=\"file-table\"><thead><tr><th>Path</th><th>Kind</th>"
             "<th>Owner uid</th><th></th></tr></thead><tbody>");
    for (size_t i = 0; i < n; ++i) {
      const struct json_value *ns_entry = json_array_at(list, i);
      const char *ns_path =
          json_as_string(json_object_get(ns_entry, "path"), "");
      ...
      buf_puts(&b,
               "<tr><td><a class=\"file-name\" href=\"/-/admin/namespaces/");
      buf_esc(&b, ns_path);
      buf_puts(&b, "\">");
      buf_esc(&b, ns_path);
      buf_puts(&b, "</a></td><td>");
    ...
    if (n == 0)
      buf_puts(
          &b,
          "<tr><td colspan=\"4\" class=\"muted\">No namespaces yet.</td></tr>");
    buf_puts(&b, "</tbody></table>");
  }
  panel_close(&b);
  ...
  buf_puts(
      &b,
      "<form class=\"form-grid\" method=\"post\" "
      "action=\"/-/admin/namespaces/create\">"
      "<label>Slug <input type=\"text\" name=\"slug\" "
      "placeholder=\"acme\"></label>"
      "<label>Parent path <input type=\"text\" name=\"parent\" "
      "placeholder=\"(empty = root group)\"></label>"
      "<button type=\"submit\" class=\"primary\">Create group</button></form>"
      "<p class=\"muted small\">A root group has no parent; a subgroup names "
      "its "
      "parent path (e.g. parent <code>acme</code> &rarr; "
      "<code>acme/platform</code>). "
      "You become its owner. Personal namespaces are created automatically at "
      "sign-up.</p>");
  panel_close(&b);
  json_doc_free(doc);
  page_close_and_send(s, &b, keep_alive);
 ...
 * surfaces. */
static struct picomesh_void_result
render_namespace_members(struct buf *b, struct loop *loop,
                         const struct serve_ud *sud, const char *sid,
                         const char *nspath, const char *base) {
  char esc[256], args[300];
  if (!json_escape(esc, sizeof(esc), nspath))
    esc[0] = 0;
  ...
  struct json_doc_ptr_result mdoc_rr = rpc_result_doc(
      loop, sud, sid, "accounts.accounts.ns_members", args, &members);
  if (PICOMESH_IS_ERR(mdoc_rr)) {
    picomesh_error_print(stderr, "webapp: rpc_result_doc", mdoc_rr.error);
    picomesh_error_destroy(mdoc_rr.error);
  }
  struct json_doc *mdoc = PICOMESH_IS_OK(mdoc_rr) ? mdoc_rr.value : NULL;

  panel_open(b, "Members", NULL);
  if (!mdoc || !members) {
    buf_puts(b, "<p class=\"muted\">members are not available (you may not "
                "manage this namespace).</p>");
  } else {
    buf_puts(b,
             "<table class=\"file-table\"><thead><tr><th>User</th><th>uid</th>"
    ...
    for (size_t i = 0; i < member_count; ++i) {
      ...
      long member_uid =
          (long)json_as_int(json_object_get(member_entry, "uid"), 0);
      const char *role =
          json_as_string(json_object_get(member_entry, "role"), "");
      const char *username =
          json_as_string(json_object_get(member_entry, "username"), NULL);
      buf_puts(b, "<tr><td>");
      ...
      buf_esc(b, base);
      buf_puts(b, "/remove_member\" style=\"display:inline\"><input "
    ...
    if (member_count == 0)
      buf_puts(b,
               "<tr><td colspan=\"4\" class=\"muted\">No members.</td></tr>");
    buf_puts(b, "</tbody></table>");
  }
  panel_close(b);
  ...
  buf_puts(b,
           ...
           "placeholder=\"alice\" required></label>"
  ...
  buf_esc(b, nspath);
  buf_puts(b, "\"><label>Slug <input type=\"text\" name=\"slug\" "
...
/* GET /admin/namespaces/<path> — a namespace's members + grant/revoke forms. */
static struct picomesh_void_result
page_admin_namespace(struct loop *loop, struct loop_stream *s,
                     const struct serve_ud *sud, const char *sid,
                     const char *uname, const char *nspath, int keep_alive) {
  struct buf b;
  buf_init(&b);
  char title[160];
  snprintf(title, sizeof(title), "Namespace \xc2\xb7 %s", nspath);
  page_open(&b, title, uname, "admin-namespaces", /*is_admin=*/1);
  render_page_header(&b, nspath,
                     "Role memberships for this namespace. Roles inherit to "
                     "child namespaces.",
                     NULL);
  buf_puts(&b,
           "<p class=\"muted small\"><a "
           "href=\"/-/admin/namespaces\">\xe2\x86\x90 All namespaces</a></p>");
...
static const char *whoami_role_on(const struct json_value *nss,
                                  const char *nspath) {
  ...
  for (size_t plen = strlen(nspath); plen > 0;) {
    if (plen < sizeof(prefix)) {
      memcpy(prefix, nspath, plen);
      prefix[plen] = 0;
      for (size_t i = 0; i < n; ++i) {
        const struct json_value *ns_entry = json_array_at(nss, i);
        const char *ns_path =
            json_as_string(json_object_get(ns_entry, "path"), NULL);
        const char *role =
            json_as_string(json_object_get(ns_entry, "role"), NULL);
 ...
 * into per-group member management. Non-admin surface for the GitLab-like
 * model. */
static struct picomesh_void_result
page_groups(struct loop *loop, struct loop_stream *s,
            const struct serve_ud *sud, const char *sid, const char *uname,
            int is_admin, int keep_alive) {
  if (!uname || !*uname) {
    send_redirect(s, "/-/login", NULL, keep_alive);
  ...
  if (PICOMESH_IS_ERR(who_rr)) {
    picomesh_error_print(stderr, "webapp: whoami_doc", who_rr.error);
    picomesh_error_destroy(who_rr.error);
  }
  ...
  render_page_header(&b, "Groups",
                     "Namespaces you belong to. Maintainers and owners can "
                     "also manage members and create subgroups.",
                     NULL);
  panel_open(&b, "Your namespaces", NULL);
  buf_puts(&b,
           "<table class=\"file-table\"><thead><tr><th>Namespace</th><th>Your "
           "role</th>"
  ...
  if (!any)
    buf_puts(
        &b, "<tr><td colspan=\"3\" class=\"muted\">You are not a member of any "
            "namespace yet.</td></tr>");
  buf_puts(&b, "</tbody></table>");
  panel_close(&b);
  ...
  buf_puts(
      &b,
      "<form class=\"form-grid\" method=\"post\" action=\"/-/groups/create\">"
      ...
      "subgroup, open the parent group and use its \xe2\x80\x9c"
      "Create subgroup"
      "\xe2\x80\x9d form.</p>");
  panel_close(&b);
  ...
  buf_puts(&b,
           "<form class=\"form-grid\" method=\"post\" action=\"/-/groups/go\">"
           "<label>Namespace path <input type=\"text\" name=\"path\" "
           "placeholder=\"acme/platform\" pattern=\"[A-Za-z0-9._/-]{1,128}\" "
           "required></label>"
...
static struct picomesh_void_result
page_group_detail(struct loop *loop, struct loop_stream *s,
                  const struct serve_ud *sud, const char *sid,
                  const char *uname, const char *nspath, int is_admin,
                  int keep_alive) {
  if (!uname || !*uname) {
    send_redirect(s, "/-/login", NULL, keep_alive);
    return PICOMESH_OK_VOID();
  ...
  buf_init(&b);
  char title[160];
  snprintf(title, sizeof(title), "Group \xc2\xb7 %s", nspath);
  page_open(&b, title, uname, "groups", is_admin);
  render_page_header(&b, nspath, "Members and subgroups of this namespace.",
                     NULL);
  buf_puts(&b, "<p class=\"muted small\"><a href=\"/-/groups\">\xe2\x86\x90 "
               "Your groups</a></p>");
  ...
  if (!json_escape(esc, sizeof(esc), nspath))
    esc[0] = 0;
  snprintf(args, sizeof(args), "[\"%s\"]", esc);
  struct picomesh_string_result names_rr = rpc_result_str(
      loop, sud, sid, "git_repo.git_repo.list_for_namespace", args, NULL);
  if (PICOMESH_IS_ERR(names_rr)) {
    picomesh_error_print(stderr, "webapp: rpc_result_str", names_rr.error);
    picomesh_error_destroy(names_rr.error);
  }
...
/* POST <base>/create — create a group namespace. `base` is
 * "/-/admin/namespaces" or "/-/groups", so the same handler serves the
 * site-admin and the group-owner surfaces; the backend enforces the real
 * authority (root creation = site admin; subgroup = maintainer on parent).
 * Failures are shown, not swallowed. */
static struct picomesh_void_result
webapp_ns_create(struct loop *loop, struct loop_stream *s,
  ...
  if (slug && *slug) {
    ...
    if (json_escape(eslug, sizeof(eslug), slug) &&
      ...
      snprintf(args, sizeof(args), "[0,\"group\",\"%s\",\"%s\"]", eslug,
               eparent);
      struct picomesh_int_result rpc_inv_1 =
          rpc_invoke(loop, sud, sid, "accounts.accounts.ns_create", args, err,
                     sizeof(err));
      if (PICOMESH_IS_ERR(rpc_inv_1)) {
        picomesh_error_print(stderr, "webapp: rpc_invoke", rpc_inv_1.error);
        picomesh_error_destroy(rpc_inv_1.error);
      }
      if (PICOMESH_IS_ERR(rpc_inv_1) || !rpc_inv_1.value) {
        free(slug);
        free(parent);
        render_action_error(s, claims.username, claims.is_admin, err, base,
                            keep_alive);
        return PICOMESH_OK_VOID();
      }
    }
...
static struct picomesh_void_result
webapp_ns_add_member(struct loop *loop, struct loop_stream *s,
                     const struct serve_ud *sud, const char *sid,
                     const char *base, const char *body, size_t body_len,
                     int keep_alive) {
  struct claims claims;
  resolve_claims(loop, sud, sid, &claims);
  char *path = form_get(body, body_len, "path");
  ...
  char redirect[320];
  snprintf(redirect, sizeof(redirect), "%s", base);
  if (path && *path)
    snprintf(redirect, sizeof(redirect), "%s/%s", base, path);
  if (path && *path && username && *username && role && *role) {
    ...
    if (json_escape(euname, sizeof(euname), username)) {
      ...
      struct picomesh_int64_result uid_res = rpc_result_int(
          loop, sud, sid, "accounts.accounts.uid_for_username", uargs, 0);
    ...
    if (!target) {
      ...
      free(username);
      free(role);
      render_action_error(s, claims.username, claims.is_admin, "no such user",
                          redirect, keep_alive);
      return PICOMESH_OK_VOID();
    }
    char epath[256], erole[32], args[360], err[256];
    if (json_escape(epath, sizeof(epath), path) &&
        json_escape(erole, sizeof(erole), role)) {
      snprintf(args, sizeof(args), "[\"%s\",%u,\"%s\"]", epath, target, erole);
      struct picomesh_int_result rpc_inv_2 =
          rpc_invoke(loop, sud, sid, "accounts.accounts.ns_add_member", args,
                     err, sizeof(err));
      if (PICOMESH_IS_ERR(rpc_inv_2)) {
        picomesh_error_print(stderr, "webapp: rpc_invoke", rpc_inv_2.error);
        picomesh_error_destroy(rpc_inv_2.error);
      }
      if (PICOMESH_IS_ERR(rpc_inv_2) || !rpc_inv_2.value) {
        free(path);
        free(username);
        free(role);
        render_action_error(s, claims.username, claims.is_admin, err, redirect,
                            keep_alive);
        return PICOMESH_OK_VOID();
      }
    }
...
static struct picomesh_void_result
webapp_ns_remove_member(struct loop *loop, struct loop_stream *s,
                        const struct serve_ud *sud, const char *sid,
                        const char *base, const char *body, size_t body_len,
                        int keep_alive) {
  struct claims claims;
  ...
  if (path && *path && uid && *uid) {
    ...
    if (json_escape(epath, sizeof(epath), path)) {
      snprintf(args, sizeof(args), "[\"%s\",%ld]", epath,
               strtol(uid, NULL, 10));
      struct picomesh_int_result rpc_inv_3 =
          rpc_invoke(loop, sud, sid, "accounts.accounts.ns_remove_member", args,
                     err, sizeof(err));
      if (PICOMESH_IS_ERR(rpc_inv_3)) {
        picomesh_error_print(stderr, "webapp: rpc_invoke", rpc_inv_3.error);
        picomesh_error_destroy(rpc_inv_3.error);
      }
...
static struct picomesh_void_result
page_repo_settings(struct loop *loop, struct loop_stream *s,
                   const struct serve_ud *sud, const char *sid,
                   const char *uname, const char *acct, const char *repo,
                   int is_admin, int keep_alive) {
  uint32_t rid = repo_hash(acct, repo);
  ...
  buf_esc(&b, repo);
  buf_puts(&b, "</td></tr><tr><th>Default branch</th><td><span "
               "class=\"branch\">main</span></td></tr>"
  ...
  buf_esc(&b, repo);
  buf_puts(
      &b, "</a>.</p><p class=\"muted small\">Files are served over the gateway "
          "<code>/_rpc</code> surface; direct git clone is not wired in this "
          "build.</p>");
  panel_close(&b);
 ...
 * by name substring (case-insensitive). Uses the SAME RBAC discovery as
 * /-/repos (collect_accessible_repos: whoami → ns_subtree → list_for_namespace)
 * so group/subgroup repos visible through inherited membership are searchable,
 * then matches in-process; no backend search method is assumed. Not signed in →
 * bounce to /login. */
static struct picomesh_void_result
page_search(struct loop *loop, struct loop_stream *s,
            const struct serve_ud *sud, const char *sid, const char *uname,
            const char *full_path, int is_admin, int keep_alive) {
  ...
  size_t query_len = 0;
  for (const char *p = query ? query : "";
       *p && query_len + 1 < sizeof(query_lower); ++p) {
    char c = *p;
    if (c >= 'A' && c <= 'Z')
      c = (char)(c - 'A' + 'a');
  ...
  struct picomesh_string_result names_rr =
      collect_accessible_repos(loop, sud, sid);
  if (PICOMESH_IS_ERR(names_rr)) {
    picomesh_error_print(stderr, "webapp: collect_accessible_repos",
                         names_rr.error);
    picomesh_error_destroy(names_rr.error);
  }
  char *names = PICOMESH_IS_OK(names_rr) ? names_rr.value : NULL;
  ...
  if (query && *query)
    ...
    snprintf(metabuf, sizeof(metabuf), "Your repositories");
  ...
  if (names && *names) {
    ...
    for (char *line = strtok_r(names, "\n", &save); line;
      ...
      if (query_len) {
        char line_lower[256];
        size_t line_len = 0;
        for (const char *p = line; *p && line_len + 1 < sizeof(line_lower);
             ++p) {
          char c = *p;
          if (c >= 'A' && c <= 'Z')
            c = (char)(c - 'A' + 'a');
  ...
  if (!any)
    buf_puts(&b, "<p class=\"muted\">No matching repositories.</p>");
  panel_close(&b);
  free(names);
  free(query);
  page_close_and_send(s, &b, keep_alive);
...
static struct picomesh_void_result
page_repos_new_form(struct loop *loop, struct loop_stream *s,
                    const struct serve_ud *sud, const char *sid,
  ...
  render_page_header(&b, "New repository",
                     "Create a repository in a namespace you can push to.",
                     NULL);
   ...
   * personal namespace. */
  buf_puts(&b,
           "<form class=\"form-grid\" method=\"post\" action=\"/-/repos/new\">"
           "<label>Namespace <input name=\"namespace\" list=\"ns-options\" "
           "value=\"");
  buf_esc(&b, uname);
  ...
  for (size_t i = 0; i < ns_count; ++i) {
    ...
    const char *ns_path =
        json_as_string(json_object_get(ns_entry, "path"), NULL);
    const char *role = json_as_string(json_object_get(ns_entry, "role"), NULL);
    if (!ns_path || !*ns_path || !role || strcmp(ns_path, "site") == 0)
      continue;
    if (!role_at_least(role, "developer"))
      continue;
  ...
  buf_puts(&b, "</datalist>"
               ...
               "<p class=\"muted small\">Pick a group you can push to "
               "(developer+), or type "
               "a subgroup path you have inherited access to, e.g. "
               "<code>acme/platform</code>.</p>");
  panel_close(&b);
  json_doc_free(who);
  render_shell_close(s, &b, keep_alive);
  return PICOMESH_OK_VOID();
...
static struct picomesh_void_result
page_admin_services(struct loop *loop, struct loop_stream *s,
                    struct serve_ud *sud, const char *uname, int keep_alive) {
  ...
  render_page_header(&b, "Services",
                     "Active services discovered from the gateway /_describe.",
                     NULL);
  ...
  if (console_url && *console_url) {
    buf_puts(&b,
             "<p class=\"muted\">Inspect &amp; invoke these services in the "
  ...
  buf_puts(
      &b, "<table class=\"service-table\"><thead><tr>"
          "<th>Service</th><th>Source</th><th>Status</th></tr></thead><tbody>");
  for (size_t i = 0; i < sud->services.n; ++i) {
    buf_puts(&b, "<tr><td><code>");
    buf_esc(&b, sud->services.names[i]);
    buf_puts(&b, "</code></td><td>");
    buf_esc(&b, sud->services.sources[i]);
    buf_puts(
        &b, "</td><td><span class=\"badge succeeded\">active</span></td></tr>");
  }
  if (sud->services.n == 0)
    buf_puts(&b, "<tr><td colspan=\"3\" class=\"muted\">No services "
                 "discovered.</td></tr>");
  buf_puts(&b, "</tbody></table>");
  panel_close(&b);
  render_shell_close(s, &b, keep_alive);
...
static struct picomesh_void_result
page_dashboard_runs(struct loop *loop, struct loop_stream *s,
                    const struct serve_ud *sud, const char *sid,
                    const char *uname, int is_admin, int keep_alive) {
  struct picomesh_int64_result pending_rr = rpc_result_int(
      loop, sud, sid, "git_pipeline.git_pipeline.count_pending", "[]", 0);
  ...
  render_page_header(&b, "Pipelines", "Pipeline activity across the mesh.",
                     NULL);
  ...
  buf_printf(
      ...
      "queued\">queued</span></td><td><code>main</code></td><td>%ld</td></tr>"
      "<tr><td><span class=\"badge "
      "running\">running</span></td><td><code>main</code></td><td>%ld</td></tr>"
      "<tr><td><span class=\"badge "
      "succeeded\">finished</span></td><td><code>main</code></td><td>%ld</td></"
      "tr>"
      "</tbody></table>",
      pending, running, done);
  panel_close(&b);
 ...
 * access (no global list API yet; aggregate per-repo counts). Uses the SAME
 * RBAC discovery as /-/repos (collect_accessible_repos) so group-managed and
 * inherited-subgroup repos are included, not just the creator index. */
static struct picomesh_void_result
page_dashboard_issues(struct loop *loop, struct loop_stream *s,
                      const struct serve_ud *sud, const char *sid,
                      const char *uname, int is_admin, int keep_alive) {
  struct picomesh_string_result names_rr =
      collect_accessible_repos(loop, sud, sid);
  ...
  render_page_header(&b, "Issues", "Open issues across your repositories.",
                     NULL);
  ...
  if (names && *names) {
    ...
    for (char *line = strtok_r(names, "\n", &save); line;
      ...
      if (service_active(sud, "issues")) {
        char issue_args[48];
        snprintf(issue_args, sizeof(issue_args), "[%u]", rid);
        struct picomesh_int64_result open_rr = rpc_result_int(
            loop, sud, sid, "issues.issues.count_open_in_repo", issue_args, -1);
        if (PICOMESH_IS_ERR(open_rr)) {
          picomesh_error_print(stderr, "webapp: count_open_in_repo",
                               open_rr.error);
          picomesh_error_destroy(open_rr.error);
        } else
          open_n = open_rr.value;
      }
      buf_puts(&b, "<tr><td><a class=\"file-name\" href=\"/");
      buf_esc(&b, line);
      buf_puts(&b, "/-/issues\">");
      ...
      buf_puts(&b, "</a></td><td>");
      if (open_n >= 0)
        buf_printf(&b, "<span class=\"badge open\">%ld</span>", open_n);
      else
        buf_puts(&b, "<span class=\"muted\">-</span>");
      buf_puts(&b, "</td></tr>");
    }
    buf_puts(&b, "</tbody></table>");
  }
...
static struct picomesh_void_result
post_issue_new(struct loop *loop, struct loop_stream *s,
               const struct serve_ud *sud, const char *sid, const char *acct,
               const char *repo, int keep_alive) {
  /* No valid session → no mutation. Never attribute an issue to a
   * fallback uid; an unresolved actor is an auth failure. */
  struct claims claims;
  struct picomesh_void_result claims_res =
      resolve_claims(loop, sud, sid, &claims);
  if (PICOMESH_IS_ERR(claims_res)) {
    picomesh_error_print(stderr, "webapp: resolve_claims", claims_res.error);
    picomesh_error_destroy(claims_res.error);
  }
  if (!claims.uid) {
    send_redirect(s, "/-/login", NULL, keep_alive);
    return PICOMESH_OK_VOID();
  ...
  char errbuf[256];
  struct picomesh_int_result open_res = rpc_invoke(
      loop, sud, sid, "issues.issues.open", args, errbuf, sizeof(errbuf));
  if (PICOMESH_IS_ERR(open_res)) {
    yerror("webapp: issues.open transport failure");
    render_action_error(s, claims.username, claims.is_admin, errbuf, where,
                        keep_alive);
...
static struct picomesh_void_result
post_issue_close(struct loop *loop, struct loop_stream *s,
                 const struct serve_ud *sud, const char *sid, const char *acct,
                 const char *repo, const char *body, size_t body_len,
                 int keep_alive) {
  struct claims claims;
  struct picomesh_void_result claims_res =
      resolve_claims(loop, sud, sid, &claims);
  ...
  if (issue_id_str && *issue_id_str) {
    ...
    char errbuf[256];
    struct picomesh_int_result close_res = rpc_invoke(
        loop, sud, sid, "issues.issues.close", args, errbuf, sizeof(errbuf));
    free(issue_id_str);
...
static struct picomesh_void_result
post_run_new(struct loop *loop, struct loop_stream *s,
             const struct serve_ud *sud, const char *sid, const char *acct,
             const char *repo, int keep_alive) {
  struct claims claims;
  struct picomesh_void_result claims_res =
  ...
  char errbuf[256];
  struct picomesh_int_result enqueue_res =
      rpc_invoke(loop, sud, sid, "git_pipeline.git_pipeline.enqueue", args,
...
static struct picomesh_void_result
post_run_lease(struct loop *loop, struct loop_stream *s,
               const struct serve_ud *sud, const char *sid, const char *acct,
               const char *repo, const char *body, size_t body_len,
               int keep_alive) {
  struct claims claims;
  ...
  unsigned long runner =
      runner_str && *runner_str ? strtoul(runner_str, NULL, 10) : 1;
  if (!runner)
    runner = 1;
  free(runner_str);
  char args[48];
  ...
  snprintf(where, sizeof(where), "/%s/%s/-/runs", acct, repo);
  char errbuf[256];
  struct picomesh_int_result lease_res =
      rpc_invoke(loop, sud, sid, "git_pipeline.git_pipeline.lease", args,
                 errbuf, sizeof(errbuf));
...
static struct picomesh_void_result
relay_post(struct loop *loop, struct loop_stream *s, const struct serve_ud *sud,
           const char *sid, const char *gw_path, const char *body,
           size_t body_len, const char *redirect_to, int keep_alive) {
  struct http_response resp;
  struct picomesh_int_result post =
      http_post(loop, &sud->gw, gw_path, "application/x-www-form-urlencoded",
                NULL, sid, body, body_len, &resp);
...
static struct picomesh_void_result
webapp_repos_new(struct loop *loop, struct loop_stream *s,
                 const struct serve_ud *sud, const char *sid, const char *body,
                 size_t body_len, int keep_alive) {
  ...
  const char *owner_name =
      (namespace && *namespace) ? namespace : claims.username;
  char redirect[160];
  snprintf(redirect, sizeof(redirect), "/%s", owner_name);
  if (name && *name) {
    ...
    if (json_escape(name_esc, sizeof(name_esc), name) &&
      ...
      snprintf(args, sizeof(args), "[%u,\"%s\",\"%s\"]", claims.uid, owner_esc,
               name_esc);
      struct picomesh_int_result rpc_inv_4 = rpc_invoke(
          loop, sud, sid, "git_repo.git_repo.make", args, err, sizeof(err));
      if (PICOMESH_IS_ERR(rpc_inv_4)) {
        picomesh_error_print(stderr, "webapp: rpc_invoke", rpc_inv_4.error);
        picomesh_error_destroy(rpc_inv_4.error);
      }
      if (PICOMESH_IS_ERR(rpc_inv_4) || !rpc_inv_4.value) {
        free(name);
        free(namespace);
        render_action_error(s, claims.username, claims.is_admin, err,
                            "/-/repos/new", keep_alive);
        return PICOMESH_OK_VOID();
      }
 ...
 * opaque sid + uname. */
static struct picomesh_void_result
route_logout(struct loop *loop, struct loop_stream *s,
             const struct serve_ud *sud, const char *sid, int keep_alive) {
  struct http_response resp;
  struct picomesh_int_result post =
      http_post(loop, &sud->gw, "/logout", "application/x-www-form-urlencoded",
                NULL, sid, "", 0, &resp);
...
static int parse_action_path(const char *path, size_t path_len, char *acct,
                             size_t acct_cap, char *repo, size_t repo_cap,
                             char *sect, size_t sect_cap, char *act,
                             size_t act_cap) {
  size_t plen = path_len;
  const char *q = memchr(path, '?', path_len);
  if (q)
  ...
  char tail[128];
  size_t tlen = plen - (size_t)d - 3;
  if (tlen == 0 || tlen >= sizeof(tail))
    return 0;
  memcpy(tail, path + d + 3, tlen);
  tail[tlen] = 0;
  if (strstr(tail, ".."))
    return 0;
  ...
  if (slash) {
    *slash = 0;
    snprintf(act, act_cap, "%s", slash + 1);
  } else
    act[0] = 0;
...
static const struct repo_route *repo_routes(size_t *count) {
  static const struct repo_route ROUTES[] = {
      {"tree", "git_repo", RP_BROWSE},   {"issues", "issues", RP_ISSUES},
      {"runs", "git_pipeline", RP_RUNS}, {"edit", "git_repo", RP_EDIT},
      {"new", "git_repo", RP_EDIT},      {"settings", "git_repo", RP_SETTINGS},
  };
...
static const struct repo_route *repo_route_for(const char *verb) {
  ...
  for (size_t i = 0; i < count; ++i)
    if (strcmp(routes[i].verb, verb) == 0)
      return &routes[i];
  return NULL;
}
...
static void serve_one(struct loop *l, struct loop_stream *s, void *ud) {
  ...
  if (!buf) {
    yerror(
        "[webapp] serve_one: out of memory allocating %d-byte request buffer",
        WEBAPP_REQ_BUF);
    loop_close(s);
    return;
  }
  ...
  while (total < WEBAPP_REQ_BUF) {
    struct picomesh_size_result chunk_res =
        loop_read_some(s, buf + total, WEBAPP_REQ_BUF - total);
    if (PICOMESH_IS_ERR(chunk_res)) {
      ywarn("[webapp] serve_one: request read error: %s",
            chunk_res.error.msg ? chunk_res.error.msg : "?");
      picomesh_error_destroy(chunk_res.error);
      free(buf);
      loop_close(s);
      return;
    ...
    if (chunk_res.value == 0) {
      ...
      loop_close(s);
      return;
    } /* clean client close / EOF — normal */
    total += chunk_res.value;
    num_headers = WEBAPP_MAX_HEADERS;
    int parse_result =
        phr_parse_request(buf, total, &method, &method_len, &path, &path_len,
                          &minor_version, headers, &num_headers, last);
    if (parse_result > 0) {
      parsed = 1;
      break;
    }
  ...
  if (!parsed) {
    ...
    ywarn("[webapp] serve_one: request headers exceed %d bytes, rejecting with "
          "431",
          WEBAPP_REQ_BUF);
  ...
  if (header_match(headers, num_headers, "connection", ka_hdr,
                   sizeof(ka_hdr))) {
    if (strstr(ka_hdr, "close"))
      keep_alive = 0;
    if (strstr(ka_hdr, "keep-alive"))
      keep_alive = 1;
  }
  ...
  if (method_len == 3 && memcmp(method, "GET", 3) == 0) {
    if (path_equals(path, path_len, "/")) {
      ...
      const char *who =
          claims.uid ? (claims.username[0] ? claims.username
                                           : (uname_cookie ? uname_cookie : ""))
                     : "";
      ...
      if (path_equals(path, path_len, "/-/repos")) {
        if (service_active(sud, "git_repo"))
          route_repos_get(l, s, sud, sid, who, claims.uid, claims.is_admin,
                          keep_alive);
        else
          send_text(s, 404, "no such page (git_repo not active)\n", keep_alive);
      } else if (path_equals(path, path_len, "/-/search") ||
                 starts_with(path, path_len, "/-/search?")) {
        if (service_active(sud, "git_repo"))
          page_search(l, s, sud, sid, who, fp, claims.is_admin, keep_alive);
        else
          send_text(s, 404, "no such page (git_repo not active)\n", keep_alive);
      } else if (path_equals(path, path_len, "/-/repos/new")) {
        if (service_active(sud, "git_repo"))
          page_repos_new_form(l, s, sud, sid, who, claims.is_admin, keep_alive);
        else
          send_text(s, 404, "no such page (git_repo not active)\n", keep_alive);
        ...
        if (require_admin(s, &claims, keep_alive)) {
          if (path_equals(path, path_len, "/-/admin")) {
            ...
            if (service_active(sud, "accounts"))
              page_admin_users(l, s, sud, sid, who, keep_alive);
            else
              send_text(s, 404, "no such page (accounts not active)\n",
                        keep_alive);
          } else if (path_equals(path, path_len, "/-/admin/repos")) {
            if (service_active(sud, "git_repo"))
              page_admin_repos(l, s, sud, sid, who, keep_alive);
            else
              send_text(s, 404, "no such page (git_repo not active)\n",
                        keep_alive);
            ...
            if (service_active(sud, "accounts"))
              page_admin_namespaces(l, s, sud, sid, who, keep_alive);
            else
              send_text(s, 404, "no such page (accounts not active)\n",
                        keep_alive);
            ...
            if (rest >= sizeof(nspath))
              rest = sizeof(nspath) - 1;
        ...
        if (service_active(sud, "git_repo"))
          page_dashboard_issues(l, s, sud, sid, who, claims.is_admin,
                                keep_alive);
        else
          send_text(s, 404, "no such page (git_repo not active)\n", keep_alive);
      } else if (path_equals(path, path_len, "/-/dashboard/runs")) {
        if (service_active(sud, "git_pipeline"))
          page_dashboard_runs(l, s, sud, sid, who, claims.is_admin, keep_alive);
        else
          send_text(s, 404, "no such page (git_pipeline not active)\n",
                    keep_alive);
      } else if (path_equals(path, path_len, "/-/groups")) {
        if (service_active(sud, "accounts"))
          page_groups(l, s, sud, sid, who, claims.is_admin, keep_alive);
        else
          send_text(s, 404, "no such page (accounts not active)\n", keep_alive);
        ...
        size_t rest = path_len > pfx ? path_len - pfx : 0;
        if (rest >= sizeof(gpath))
          rest = sizeof(gpath) - 1;
        memcpy(gpath, path + pfx, rest);
        gpath[rest] = 0;
        url_decode(gpath);
        page_group_detail(l, s, sud, sid, who, gpath, claims.is_admin,
                          keep_alive);
      } else if (starts_with(path, path_len, "/-/")) {
        /* Unknown top-level command. */
        send_text(s, 404, "not found\n", keep_alive);
      } else if (parse_repo_path(path, path_len, acct, sizeof(acct), repo,
                                 sizeof(repo), verb, sizeof(verb))) {
        ...
        if (!route) {
          ...
          switch (route->page) {
          case RP_BROWSE:
            route_repo_browse(l, s, sud, sid, who, acct, repo, fp,
                              claims.is_admin, keep_alive);
            break;
          case RP_ISSUES:
            page_repo_issues(l, s, sud, sid, who, acct, repo, fp,
                             claims.is_admin, keep_alive);
            break;
          case RP_RUNS:
            page_repo_runs(l, s, sud, sid, who, acct, repo, fp, claims.is_admin,
                           keep_alive);
            break;
          case RP_EDIT:
            route_repo_edit_get(l, s, sud, sid, who, acct, repo, verb, fp,
                                claims.is_admin, keep_alive);
            break;
          case RP_SETTINGS:
            page_repo_settings(l, s, sud, sid, who, acct, repo, claims.is_admin,
                               keep_alive);
            break;
          }
      } else if (!claims.uid) {
        ...
        /* Any other path (no "/-/") is a NAMESPACE path:
         * /<group>[/<subgroup>…]. Render the namespace landing — its repos
         * (each linking to the project under /-/tree) and, for a member, its
         * subgroups. */
        char nspath[256];
        ...
        if (plen > 1 && path[0] == '/' && plen - 1 < sizeof(nspath) &&
            !memchr(path, '\\', plen)) {
          memcpy(nspath, path + 1, plen - 1);
          nspath[plen - 1] = 0;
          url_decode(nspath);
          if (!strstr(nspath, "..") && service_active(sud, "git_repo"))
            page_account_landing(l, s, sud, sid, who, nspath, claims.is_admin,
                                 keep_alive);
          else
            send_text(s, 404, "not found\n", keep_alive);
        } else {
    ...
    if (content_length > 1 << 20) {
      ...
      if (!body_buf) {
        ...
        size_t copy = buffered < (size_t)content_length
                          ? buffered
                          : (size_t)content_length;
        memcpy(body_buf, buf + hdr_end, copy);
        if (copy < (size_t)content_length) {
          if (read_body(s, body_buf + copy, (size_t)content_length - copy) <
              0) {
            free(body_buf);
        ...
        if (path_equals(path, path_len, "/-/login")) {
          route_login_post(l, s, sud, body_buf, (size_t)content_length,
                           keep_alive);
        } else if (path_equals(path, path_len, "/-/register")) {
          route_register_post(l, s, sud, body_buf, (size_t)content_length,
                              keep_alive);
          ...
          char *sid = cookie_get(headers, num_headers, "picomesh-sid");
          webapp_repos_new(l, s, sud, sid, body_buf, (size_t)content_length,
                           keep_alive);
          free(sid);
          ...
          if (!admin_claims.is_admin) {
            if (admin_claims.uid)
              send_forbidden(
                  s, admin_claims.username[0] ? admin_claims.username : NULL,
                  keep_alive);
            else
              send_redirect(s, "/-/login", NULL, keep_alive);
            ...
            relay_post(l, s, sud, sid, "/admin/tokens/mint_pat", body_buf,
                       (size_t)content_length, "/-/admin/tokens", keep_alive);
          } else if (path_equals(path, path_len,
                                 "/-/admin/namespaces/create")) {
            webapp_ns_create(l, s, sud, sid, "/-/admin/namespaces", body_buf,
                             (size_t)content_length, keep_alive);
          } else if (path_equals(path, path_len,
                                 "/-/admin/namespaces/add_member")) {
            webapp_ns_add_member(l, s, sud, sid, "/-/admin/namespaces",
                                 body_buf, (size_t)content_length, keep_alive);
          } else if (path_equals(path, path_len,
                                 "/-/admin/namespaces/remove_member")) {
            webapp_ns_remove_member(l, s, sud, sid, "/-/admin/namespaces",
                                    body_buf, (size_t)content_length,
                                    keep_alive);
          } else {
            send_text(s, 404, "not found\n", keep_alive);
          }
          ...
          if (!group_claims.uid) {
            ...
            char *group_path =
                form_get(body_buf, (size_t)content_length, "path");
            char to[300] = "/-/groups";
            if (group_path && *group_path)
              snprintf(to, sizeof(to), "/-/groups/%s", group_path);
            free(group_path);
            send_redirect(s, to, NULL, keep_alive);
          } else if (path_equals(path, path_len, "/-/groups/create")) {
            webapp_ns_create(l, s, sud, sid, "/-/groups", body_buf,
                             (size_t)content_length, keep_alive);
          } else if (path_equals(path, path_len, "/-/groups/add_member")) {
            webapp_ns_add_member(l, s, sud, sid, "/-/groups", body_buf,
                                 (size_t)content_length, keep_alive);
          } else if (path_equals(path, path_len, "/-/groups/remove_member")) {
            webapp_ns_remove_member(l, s, sud, sid, "/-/groups", body_buf,
                                    (size_t)content_length, keep_alive);
          } else {
            send_text(s, 404, "not found\n", keep_alive);
          }
          ...
          if (act_ok && strcmp(sect, "edit") == 0) {
            route_repo_edit_post(l, s, sud, sid, acct, repo, body_buf,
                                 (size_t)content_length, keep_alive);
          } else if (act_ok && strcmp(sect, "issues") == 0 &&
                     strcmp(act, "new") == 0) {
            post_issue_new(l, s, sud, sid, acct, repo, keep_alive);
          } else if (act_ok && strcmp(sect, "issues") == 0 &&
                     strcmp(act, "close") == 0) {
            post_issue_close(l, s, sud, sid, acct, repo, body_buf,
                             (size_t)content_length, keep_alive);
          } else if (act_ok && strcmp(sect, "runs") == 0 &&
                     strcmp(act, "new") == 0) {
            post_run_new(l, s, sud, sid, acct, repo, keep_alive);
          } else if (act_ok && strcmp(sect, "runs") == 0 &&
                     strcmp(act, "lease") == 0) {
            post_run_lease(l, s, sud, sid, acct, repo, body_buf,
                           (size_t)content_length, keep_alive);
          } else {
            send_text(s, 404, "not found\n", keep_alive);
          }
...
struct picomesh_void_result webapp_start(struct loop *loop, const char *host,
                                         int port,
                                         const struct webapp_config *cfg) {
  ...
  if (!ud)
    return PICOMESH_ERR(picomesh_void, "webapp_start: calloc failed");
  ud->loop = loop;
  ud->cfg = cfg;
  ...
  PICOMESH_RETURN_IF_ERR(picomesh_void, listen_res,
                         "webapp_start: loop_listen_tcp");
  return PICOMESH_OK_VOID();
}
