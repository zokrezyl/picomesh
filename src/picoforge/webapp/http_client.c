...
static int read_full_response(struct loop_stream *stream, char **out_body,
                              size_t *out_body_len, int *out_status,
  ...
  while (total < HC_RESP_BUF) {
    struct picomesh_size_result chunk_res =
        loop_read_some(stream, buf + total, HC_RESP_BUF - total);
    ...
    int parse_result =
        phr_parse_response(buf, total, &minor, &status_code, &msg, &msg_len,
                           headers, &num_headers, last);
    if (parse_result > 0) {
      header_end = parse_result;
      break;
    }
    if (parse_result == -1) {
      free(buf);
  ...
  if (out_set_cookie && out_set_cookie_cap)
    out_set_cookie[0] = 0;
  for (size_t i = 0; i < num_headers; ++i) {
    ...
    } else if (headers[i].name_len == 10 && out_set_cookie &&
               out_set_cookie_cap) {
      int eq = 1;
  ...
  while (body_need > 0) {
    struct picomesh_size_result chunk_res =
        loop_read_some(stream, buf + total, body_need);
    if (PICOMESH_IS_ERR(chunk_res)) {
      picomesh_error_destroy(chunk_res.error);
      break;
    }
    if (chunk_res.value == 0)
      break; /* peer closed mid-body — caught by body_need check below */
    total += chunk_res.value;
...
http_post(struct loop *loop, const struct gateway_url *gw, const char *path,
  ...
  if (!loop || !gw || !path || !body || !resp)
    return PICOMESH_ERR(picomesh_int, "http_post: invalid arguments");
  if (!content_type)
  ...
  if (header_len <= 0 || (size_t)header_len >= sizeof(hdr)) {
    loop_close(stream);
    return PICOMESH_ERR(picomesh_int, "http_post: request header too large");
  }
  if (bearer && *bearer) {
    int written = snprintf(hdr + header_len, sizeof(hdr) - header_len,
                           "Authorization: Bearer %s\r\n", bearer);
    if (written <= 0 || header_len + written >= (int)sizeof(hdr)) {
      loop_close(stream);
      return PICOMESH_ERR(picomesh_int, "http_post: request header too large");
    }
    header_len += written;
  ...
  if (written <= 0) {
    loop_close(stream);
    return PICOMESH_ERR(picomesh_int,
                        "http_post: request header format failed");
  }
  header_len += written;

  struct picomesh_size_result header_write =
      loop_write(stream, hdr, (size_t)header_len);
  if (PICOMESH_IS_ERR(header_write)) {
    loop_close(stream);
    return PICOMESH_ERR(picomesh_int, "http_post: request header write failed",
                        header_write);
  }
  struct picomesh_size_result body_write = loop_write(stream, body, body_len);
  if (PICOMESH_IS_ERR(body_write)) {
    loop_close(stream);
    return PICOMESH_ERR(picomesh_int, "http_post: request body write failed",
                        body_write);
  }

  int read_res =
      read_full_response(stream, &resp->body, &resp->body_len, &resp->status,
  ...
  if (read_res != 0)
    return PICOMESH_ERR(picomesh_int,
                        "http_post: reading gateway response failed");
...
http_post_json(struct loop *loop, const struct gateway_url *gw,
               ...
               const char *body, size_t body_len, struct http_response *resp) {
  return http_post(loop, gw, path, "application/json", bearer, sid, body,
                   body_len, resp);
}
