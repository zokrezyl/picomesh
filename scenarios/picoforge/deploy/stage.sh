...
#   - copied into the riscv64 VM rootfs by scenarios/picoforge/yemu/
#     build-image.sh (which swaps the host x86_64 picomesh for the
#     cross-compiled riscv64 one).
#
# Source inputs (in repo):
#   scenarios/picoforge/picoforge.yaml         mesh services config
#   scenarios/picoforge/webapp/static/   CSS + html + js
#   scenarios/picoforge/deploy/run.sh.tmpl portable launcher template
#
# Build inputs:
...
# This script lives at scenarios/picoforge/deploy/stage.sh; the repo
...
YAML_SRC="$REPO_ROOT/scenarios/picoforge/picoforge-webasm.yaml"
STATIC_SRC="$REPO_ROOT/scenarios/picoforge/webapp/static"
RUNSH_SRC="$REPO_ROOT/scenarios/picoforge/deploy/run.sh.tmpl"
...
# yaml uses an in-repo path (scenarios/picoforge/webapp/static) so
...
sed -i 's|static_dir: scenarios/picoforge/webapp/static|static_dir: ./frontend/static|' \
