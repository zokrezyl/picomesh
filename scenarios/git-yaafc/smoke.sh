#!/usr/bin/env bash
# git-yaafc smoke: launch yaafc with the scenario config, drive every
...
# Start the yttp server. The yaafc.yaml binds yttp on 0.0.0.0:8801 (see
# `mesh.services.yttp` in the scenario file).
"$YAAFC" --config-file "$CONFIG" --frontend yttp --host 127.0.0.1 --port "$PORT" serve \
    > tmp/git-yaafc-srv.log 2>&1 &
SRV=$!
