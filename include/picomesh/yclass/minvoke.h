...
#ifndef PICOMESH_YCLASS_MINVOKE_H
#define PICOMESH_YCLASS_MINVOKE_H
 ...
 * of `err_cap` bytes), writes nothing to `result`, and returns non-zero. */
typedef int (*minvoke_fn)(struct ctx *ctx, struct object *obj, struct yheaders *hdrs,
                          cmp_ctx_t *args, uint32_t args_count, cmp_ctx_t *result,
...
#endif /* PICOMESH_YCLASS_MINVOKE_H */
