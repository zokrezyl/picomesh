#ifndef YAAFC_YCORE_YTRACE_H
#define YAAFC_YCORE_YTRACE_H
...
/* Re-exported from yplatform so legacy ytrace_*() macros don't need
 * to learn about yplatform headers. New code should call
 * yaafc_yplatform_time_monotonic_sec directly. */
...
#define YAAFC_TRACE_EMIT(level_str, fmt, ...)                                                      \
    do {                                                                                           \
...
#define ytrace(fmt, ...) YAAFC_TRACE_EMIT("trace", fmt, ##__VA_ARGS__)
#else
#define ytrace(fmt, ...) ((void)0)
#endif

#if YTRACE_C_ENABLE_DEBUG
#define ydebug(fmt, ...) YAAFC_TRACE_EMIT("debug", fmt, ##__VA_ARGS__)
#else
#define ydebug(fmt, ...) ((void)0)
#endif

#if YTRACE_C_ENABLE_INFO
#define yinfo(fmt, ...) YAAFC_TRACE_EMIT("info", fmt, ##__VA_ARGS__)
#else
#define yinfo(fmt, ...) ((void)0)
#endif

#if YTRACE_C_ENABLE_WARN
#define ywarn(fmt, ...) YAAFC_TRACE_EMIT("warn", fmt, ##__VA_ARGS__)
#else
#define ywarn(fmt, ...) ((void)0)
#endif

#if YTRACE_C_ENABLE_ERROR
#define yerror(fmt, ...) YAAFC_TRACE_EMIT("error", fmt, ##__VA_ARGS__)
#else
#define yerror(fmt, ...) ((void)0)
#endif

#define ylog(lvl, fmt, ...) YAAFC_TRACE_EMIT(lvl, fmt, ##__VA_ARGS__)

#define ytime_start(name) double ytime_##name = yaafc_ytime_monotonic_sec()

#define ytime_report(name)                                                                         \
    do {                                                                                           \
        static struct ytime_timer _ytime_timer_##name;                                             \
        double _ytime_elapsed_ms_ =                                                                \
            (yaafc_ytime_monotonic_sec() - ytime_##name) * 1000.0;                                 \
